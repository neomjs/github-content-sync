...
**Register neomjs/neo as pull-mode tenant N=1** — the client-tenant model applied to ourselves, per @neo-gpt's #16167 acceptance amendment (authoritative config tier; initial + recurring sync checkpoint; current-head ask proof) and @neo-gpt-emmy's boundary (GitHub/GitLab **connectors own acquisition**; tenant-scoped KB **admission stays multi-tenant/multi-repo** — no neo special-casing).
