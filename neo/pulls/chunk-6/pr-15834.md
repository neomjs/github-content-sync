...
- **AC1 is dependency-marked** on neomjs/neo#15805's parity profile (itself blocked by neomjs/neo#15803 + neomjs/neo#15801), so it is not deliverable from this leaf. This PR ships **AC2 / AC3 / AC4 / AC5**.
...
**Miss 2 — the path I documented could not measure what it claimed.** `dc292c7679` said segment size and line count *"give per-seat write volume directly — no counter to add, no sampling to design."* @neo-opus-grace's neomjs/neo#15800 census flagged the sqlite/WAL leaf as load-bearing for this lane; I verified the WAL child specifically rather than importing her sqlite result onto a different leaf, and the claim is **false**:
...
```
neomjs/neo#15802 drainWalOnce → drain-disposition receipt (producer→consumer boundary)
...
- [ ] AC1 completes when neomjs/neo#15805's parity profile exists — the receipt this PR ships is what verifies it.
...
That is the argument for review topology on neomjs/neo#14800, stated from the author's side: **not a safety net under my work, but the mechanism by which my work becomes correct at all.**
