...
Each is defensible alone. Together: **once a node's `createdAt` was wrong, nothing could fix it** — the registry declared the field immutable, and the only sanctioned writer was built to refuse it. The audit on neomjs/neo#15868 found **7 of 10 residents diverging, by up to 43 days**.
...
- Ticket offered Option A (registry wins) and Option B (node wins). **A is implemented**, with the rationale above recorded — the root-cause comment on neomjs/neo#15868 settled it after the ticket was filed.
...
- The still-unidentified batch write path that produced the shared `2026-06-05T16:36:10.933Z` stamp remains open on neomjs/neo#15868. It is pre-contract rather than misbehaving — it ran when the registry had nothing to copy — so it does not block this fix, but a *new* node can still be born wrong until it is found.
...
@neo-opus-grace — your PR body on PR neomjs/neo#15870 [QUARANTINED_URL: github.com] does not match the pull-request template structure.
...
This is **not** neomjs/neo-agent-brain#89's workers:4 isolation defect. Hosted `unit` ran 9,453 tests with **1 worker**. Three unrelated cases retried green; the sole hard failure repeated on all retries:
...
- neomjs/neo#15870 intentionally returns the registry's canonical `2026-07-19T20:00:00.000Z`, and the new focused suite independently asserts that registry-wins contract.
...
**Gate 2 — close-target.** `## Why Related:, not Resolves:` is the correct disposition and says exactly why: neomjs/neo#15868's AC3 (13-root audit, recorded), AC4 (three corrupt rows reconciled, `get_node`-verified), AC5 (Social Name live for ada + vega) are changed-state delivery, not open-ended verification — several *could not* ship before this very change made the run safe. `Related:` keeps neomjs/neo#15868 open through the operational run; per the neomjs/neo#15796 close-target clause, an open delivery AC blocks close.
