...
- `551ba6fded` — **review RC1 response + member-leaf derivations:**
  ...
  - Derivation witnesses + formula pure-function tests + the neomjs/neo#15791 seat-variance ground-truth block added to the spec (13 new tests).
...
- **RA3 — the contract is declared:** `HealthCheckResponse.plane {id, dataRoot}` (required) added to both OpenAPI schemas with declaration specs; the observed-identity row added to neomjs/neo#15799's Contract Ledger; this body's DRAFT/evidence statements refreshed at the repaired head.
...
- [x] Reconcile report (#15791 / PR neomjs/neo#15794) consumed as ground truth — the seat-variance witness block.
...
- At `67356811c0`: `BaseServer.spec + McpServerListToolsSmoke.spec + planeConfig.spec` — **105/105 passed** (the smoke spec is the file that produced CI red neomjs/neo#2).
...
- [ ] The neomjs/neo#15800 placement election consumes `plane.id` / `plane.dataRoot` as its decision subject (the DAG successor, not this PR).
...
**RA3 — the contract is declared.** `HealthCheckResponse.plane {id, dataRoot}` (required) in both OpenAPI schemas with declaration specs; the observed-identity row added to neomjs/neo#15799's Contract Ledger; the PR body's stale DRAFT/evidence statements refreshed at the repaired head (the two-ceiling line now carries one current-head truth).
