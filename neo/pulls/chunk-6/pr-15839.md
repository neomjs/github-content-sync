...
contentTrust:
  ...
  signals: []
---
`check-aiconfig-test-mutation` is the **fail-build** guard for ADR-0019 §3 **B4** — runtime writes to the `AiConfig` singleton, which §4 names as how test data bleeds into live DBs (the neomjs/neo#12335 orphan incident, ~1,281 orphans reclaimed). It anchored on the **literal identifiers** `aiConfig|Memory_Config`, case-sensitive, so any binding of the same singleton under a different name was invisible to it.
...
- **Deliberately NOT grandfathered: my own spec on PR neomjs/neo#15824**, which discloses its own B4 mutation. Pre-emptively exempting it would quietly grant an exemption its reviewer was explicitly asked to rule on. When that PR merges, this gate fails on it — which is what makes the ruling load-bearing instead of optional. Sequencing note for whoever merges: if neomjs/neo#15824 lands after this, expect that failure and treat it as the ruling coming due, not a regression.
...
- Before neomjs/neo-agent-brain#139's sweep lands, amend its AC3 from *"must stay green"* to *"must **fire** on a seeded PascalCase mutation"*; a green inert lint must not satisfy it.
- ADR-0019 §4 footnotes ticket neomjs/neo#12435 for this cleanup — it is **CLOSED**, so the ADR points at a finished ticket while the class stays reachable. Correcting that pointer is tracked on neomjs/neo#15838.
...
`Resolves neomjs/neo#15838` would have closed the tracker for four residuals. Each now has a live home: the production `ai/**` enforcement gap and the stale ADR-0019 neomjs/neo#12435 pointer → **#15843**; neomjs/neo-agent-brain#139's "must FIRE on a seeded mutation" AC amendment → **#13532** (its owner); the un-exempted `MailboxService.ReceiptDurability.spec.mjs` stays with **#15824**'s reviewer by design. A **Contract Ledger** for the exported matcher/allowlist/CLI is authored on neomjs/neo#15838.

Resolves neomjs/neo#15838

Related: neomjs/neo-agent-brain#139 — the rename this unblocks safely; neomjs/neo#15824 — the PR whose disclosed B4 mutation is deliberately left un-exempted.
...
- production `ai/**` scope gap + ADR-0019 §4's stale neomjs/neo#12435 pointer → **#15843** (filed).
- neomjs/neo-agent-brain#139's "must FIRE on a seeded PascalCase mutation" AC → **#13532** (its owner @neo-opus-vega; requested via A2A).
