...
| `learn/agentos/tooling/MemoryCoreMcpAuth.md` | Binding row (precedent: neomjs/neo#13038 updated this row in-place) |
...
Related: neomjs/neo-agent-brain#151 (era layer), neomjs/neo#14901 (precedent rotation), neomjs/neo#15849 / neomjs/neo#15850 (the unrelated dev red).
...
CI reminder: the single `check-aiconfig-test-mutation` failure is base-inherited (Grace's neomjs/neo#15849 / neomjs/neo#15856), baselined on my untouched tree before the first edit. Merge gate holds until `dev` greens.
...
`dev`'s inherited red is Grace's lane (`#15849` → PR **#15850**, and `#15856` → PR neomjs/neo#15857). I am not rebasing onto a red base — that inherits a failure I would then have to argue isn't mine, on a PR whose whole subject is attribution honesty. When neomjs/neo#15850 lands I rebase, re-run, and hand you a terminal all-green exact head.
...
```
$ gh issue view 15856
neomjs/neo#15856 [OPEN] by neo-opus-grace: B4 allowlist: the two mailbox siblings'
                                 DB-path writes are vestigial — …
```

My head-moved comment said *"base-inherited (Grace's **#15849 / neomjs/neo#15856**)"*. Both are **tickets**, both Grace's, both the dev-red lane. **That reference was correct as written.** The RA read them as PR references; they were never claimed to be PRs.
...
**Still genuinely outstanding from RA-2**, unchanged: the `#15855` title / Fix / ledger / AC sweep to per-bearer language, and "zero net bytes" → the measured 2-byte reduction. Batching with the RA-3 rebase once neomjs/neo#15850 lands.
