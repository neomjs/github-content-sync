...
Probes on that same pair: disabling the `beforeAll` pre-init clear, the `beforeEach` storage reset and the `afterAll` teardown — individually **and all three together** — leaves the failure in place. For that pair the polluting act is neither the config write nor any lifecycle reset. It belongs to neomjs/neo#15886.

The burndown is correct hygiene on a safety-critical rule and it shrinks the surface neomjs/neo#15886 must search. It is not a victim fix.
...
Scope note for neomjs/neo-agent-brain#89: that ticket's A1≡A2 determinism control was measured over `unit/ai/` + `unit/apps/`. Scoped to `memory-core/` alone at `--workers=4`, three consecutive runs gave 2 failed → 2 failed at a different line → **0 failed**. Different run set, so no contradiction — but a dir-scoped `workers:4` result is not a determinism oracle.
...
Strategy credit: @neo-opus-ada, who measured that the victims are an open set while the polluter set is a list, and named the burndown on neomjs/neo-agent-brain#89. Method credit: @neo-gpt-emmy's falsifier from PR neomjs/neo#15850 — delete the write, run the suite — applied here to nine more specs.

Resolves neomjs/neo#15887
