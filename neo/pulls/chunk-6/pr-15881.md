...
**This is arithmetic, not an isolation defect** — it reproduces alone at `--workers=1`, which is why it is fixed here rather than in neomjs/neo-agent-brain#89's harness work.
...
- The 3 remaining failures are neomjs/neo-agent-brain#89's, each already named and owned there: `McpServerListToolsSmoke`, `GoldenPathSynthesizer`, `MailboxService.ReceiptDurability`.
- `QueryReRanker` and `SessionSummaryDegradedFallback` no longer appear — the former fixed by the merged PR neomjs/neo#15883.
...
- neomjs/neo#15878 closes. neomjs/neo-agent-brain#89's failing set drops to the three named above, all genuinely isolation-related.

## Deliberately out of scope

- **Everything still open on neomjs/neo-agent-brain#89.** Nothing here touches it, and it must not inherit this finding — this PR fixes a timeout budget, not an isolation seam. Its state is neomjs/neo-agent-brain#89's to report, not this body's to mirror.
...
- **The `workers:4` flip** — neomjs/neo#15861 owns it and stays blocked on neomjs/neo-agent-brain#89.
...
Related: neomjs/neo-agent-brain#89 (parent investigation) · neomjs/neo#15861 (blocked re-land) · neomjs/neo#15886 (the second pollution mechanism, separately owned and currently being reshaped).
...
- Remaining 3 are neomjs/neo-agent-brain#89's, each named and owned there: `McpServerListToolsSmoke`, `GoldenPathSynthesizer`, `MailboxService.ReceiptDurability`.
- `QueryReRanker` and `SessionSummaryDegradedFallback` are gone — the former via merged PR neomjs/neo#15883.
...
`Resolves neomjs/neo#15878` therefore stands on evidence rather than deferral.
...
## Carried RA-1 leg folded — neomjs/neo#15878's body now enumerates both seams
...
Nothing else changed — `Resolves neomjs/neo#15878` stands, assertions untouched, fixture tests still on the strict default, and the `--workers=4` receipt discharging RA-2 is unchanged.
