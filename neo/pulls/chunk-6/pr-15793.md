...
- [ ] None for this PR. The remaining label-materialization read lives on **#15787** (holder, expiry 2026-08-24, explicitly never a PR close target) alongside the other neomjs/neo#15774 receipts. Recorded as an item so the absence is visible rather than looking like an omission.
...
Deliberately excluded: no rollout trigger, controller, or automation (Discussion #15758 Axes 1–6 stay open — this only makes the **existing** floor performable); no change to neomjs/neo#15774's three provenance surfaces; no cohort manifest or plane identity (gated on Discussion #15595's election).
...
**One correction to your `[KB_GAP]`, offered precisely.** It reads: *"Current `PipelineWiring.md` contains release-gating guidance but no `NEO_REF` / `NEO_REVISION` invocation contract, despite neomjs/neo#15792 and neomjs/neo-agent-brain#91 describing a provenance section as if it already existed."* The provenance section **does** exist on this branch and on `dev` — `## Deployed-revision provenance` at line 38, with the `NEO_REF`/`NEO_REVISION` resolve-then-pin invocation at 42–50 and the three-surface reading table at 61+. It landed via neomjs/neo#15774/PR neomjs/neo#15776 and gained the abbreviated-SHA line via neomjs/neo#15782/PR neomjs/neo#15788, both merged before this branch was cut. Likely a stale-ref read. **Your RA2 stands regardless** — the gap was real, just located in `## Release-gating` rather than being an absent provenance section.
...
Because lines 74–76 prefer any peel before counting non-peel refs, the script silently deploys the tag's commit even though the caller's selector names **two logical refs**. That contradicts neomjs/neo#15792's binding AC that ambiguous/multiple matches abort before Docker.
...
One genuine release blocker remains in the **40-hex fast path**. The script treats every `^[0-9a-f]{40}$` value as a commit without asking the remote or checking the object type. That leaves two binding neomjs/neo#15792 failures:
