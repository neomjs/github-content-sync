...
Related: neomjs/neo#15691
Related: neomjs/neo-agent-brain#92
Related: neomjs/neo#15695

## Deltas from ticket

None substantive from the rewritten live ticket. Intake first removed durable checkpoint, retry, shadow/fence/promotion, and orchestrator receipt work from this leaf; those contracts now live exclusively in neomjs/neo-agent-brain#92. This PR therefore adds no cursor, callback, checkpoint store, scheduler, or re-embedding path.
...
- Disposable preserved-vector controls, 4,096 dimensions:
  ...
  The pre-change 20k mock-store controls recorded in neomjs/neo#15695 were 921.5 MiB MC RSS and 934.2 MiB KB RSS. The new controls scale request count while holding row/ID state to the configured batch.

## Post-Merge Validation

- [ ] On the next off-host restore after merge, append operational RSS and observed batch maxima to neomjs/neo#15695 or the consuming neomjs/neo-agent-brain#92 run. This is corroboration, not a neomjs/neo#15692 close blocker; the leaf's deterministic L2 evidence is complete.

## Evolution

The original ticket combined a measured importer-memory problem with a not-yet-owned durable resume contract. Live intake found that ADR-0027 and neomjs/neo-agent-brain#92 already own recovery-run persistence, retry reconciliation, heavy-maintenance fencing, resumable shadows, and validation-clean promotion. Narrowing before code kept this diff data-plane-only and made the public importer contracts smaller rather than larger.
