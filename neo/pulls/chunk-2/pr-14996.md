...
- **`agent-detail` / `perspectives` render honest labelled placeholders** behind their auto-hide rail tabs — their real views are sibling leaves (#14608 / neomjs/neo#14616); a blank pane masquerading as a finished surface would violate the cockpit's honesty bar.
...
|---|---|---|---|
| Cockpit layout state | neomjs/neo#14658 + committed `cockpitDockDocument` | `dockModel` is the sole committed document; projection derives from it | reducer/view-sync units + NL topology read |
...
| DOM reconciliation residual | neomjs/neo#14985 | worker rebuild is authoritative here; DOM-flush witness remains owned by neomjs/neo#14985 | new-instance NL witness + explicit residual |
...
- [ ] When neomjs/neo#14985 (the wholesale-refresh reconciliation defect) closes, add the DOM-flush geometry witness to `FleetCockpitDockNL.spec.mjs` (splitter position after commit) — the worker-side witness stays either way.
- [ ] The fleetGrid sweep-order red (pre-existing, isolated-green) needs its owner's disposition — repro triple documented above.
- [ ] neomjs/neo#14616 (perspectives pane) consumes the landed `DockPerspectiveStore` + replaces the placeholder; neomjs/neo#14608 (agent detail) replaces its placeholder.
