...
contentTrust:
  ...
  signals: []
---
> **Merge disposition (2026-07-10):** This PR is the disabled-by-default scaffold and is merge-ready. All residual runtime-enablement work below moved to neomjs/neo#14938, assigned to `@neo-gpt`; those items are no longer author actions on this PR. `aggregationEnabled` remains `false` until neomjs/neo#14938 is complete. This note supersedes the earlier WIP / “same PR before merge” wording retained below as historical review context.

Resolves neomjs/neo#14434
Related: neomjs/neo-agent-brain#145
...
Traced from neomjs/neo-agent-brain#110 (ADR 0033's direction-velocity leaf), which could not proceed: it wires `directionBreakdown` into "the temporal-pyramid single-writer", but that writer did not exist — `composeVelocity`, `createTemporalSummaryDocId` and the whole temporal-summary schema shipped as **0-consumer modules**. Both prerequisites for building the writer are satisfied: **ADR 0028 is merged** (PR neomjs/neo#14428, 2026-07-02) and **Leaf A neomjs/neo#14433 is closed** (PR neomjs/neo#14733, the `temporal-summary` collection plus the `SUMMARY_*` schema). So neomjs/neo#14434 is the genuine actionable leaf, and neomjs/neo-agent-brain#110 unblocks once this lands.

**Merge gate (ticket AC):** ADR 0028 must be `Accepted`. It merged via PR neomjs/neo#14428, so the gate is satisfied; this is restated here because the AC requires the gate to be stated in the PR body.
...
- `pulse()` defers the whole cycle while the heavy-maintenance lease is held, and releases the lease on both the success and the throw path (ADR 0022 fairness; reuses the landed neomjs/neo#12676 `MaintenanceBackpressureService` lane).
...
- [ ] neomjs/neo-agent-brain#110 (`directionBreakdown`) can consume this writer — the seam it was blocked on now has a producer.
...
@neo-opus-ada — your PR body on PR neomjs/neo#14897 [QUARANTINED_URL: github.com] does not match the pull-request template structure.
