...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#15037

Related: neomjs/neo#14606 (the origin ticket — **stays OPEN**, re-scoped to the liveness contract per the cycle-1 review: production activity-source composition, recurring refresh ownership, and a behavior-witnessing live-mount proof) · epic neomjs/neo-agent-institution#10 (parent of both) · neomjs/neo#14592 (the density evidence — every bound cites a measured figure) · neomjs/neo#14831/PR neomjs/neo#14827 (the merged component slice this re-freezes) · neomjs/neo#14607 (the burst e2e sibling that will drive these bounds)

**The density contract, split honest (review-corrected close target).** Cycle 1 established that density and liveness are separate contracts: this PR is the density leaf (#15037, split from neomjs/neo#14606), and the liveness ACs stay open on neomjs/neo#14606 — `FleetControlBridge.activitySource` is production-`null`, `loadActivity()` runs once at construction, and the mounted e2e witnesses presence only, so no close claim can ride those seams yet.
...
- neomjs/neo#15037 IS this PR's contract (authored from it); the deltas live against origin neomjs/neo#14606: its density numbers (visible ≈ 15, "virtualization + fold") predate the neomjs/neo#14592 evidence — superseded by the 12-row/200-ring/coalescing freeze. No virtualization below measured scale: ring + window keep the DOM ≤ header + 12 rows + fold at ANY event volume.
- `coalesceGapMs` and `bufferSize` are reactive configs so neomjs/neo#14607's burst e2e can drive the bounds without touching the component — and the ring's reactivity is now falsifier-proven, not claimed.
...
- [ ] neomjs/neo#15037 closes on merge; neomjs/neo#14606 remains open, scoped to the liveness contract (its 2026-07-05 scope-correction comment + the cycle-1 review findings are the AC authority).
- [ ] neomjs/neo#14607 (burst e2e) drives ≥3.6/min against THESE bounds — my assigned next leaf.
