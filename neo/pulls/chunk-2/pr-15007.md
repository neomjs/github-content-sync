...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#15015

Related: neomjs/neo-agent-institution#11 — the harness PROGRAM stays open for the remainder (per-state AgentCard goldens behind a harness-scoped fixture seam, the token-demo surface, the post-preset-bar shell refresh); this PR closes its delivered-scope child, converged in review.
...
**The four goldens** (scope floor): the default cockpit shell (the committed document projected), the fleet grid at the density-ranked bar, the activity stream's chip-row vocabulary, and the Accounts surface — the latter **inheriting the design-gate golden orphaned by PR neomjs/neo#14998's intentional spec reshaping** (dispositioned there: functional specs stay side-effect-free; baseline semantics live here).

**Known first refresh, by design**: the shell golden predates PR neomjs/neo#15004's preset bar (in review as this opens); its merge triggers the harness's first reviewed refresh — demonstrating the semantics working rather than undermining them.
...
- The inherited Accounts golden (not in the original scope floor — routed here by the PR neomjs/neo#14998 design-gate disposition).
...
- [ ] PR neomjs/neo#15004's merge triggers the shell golden's first reviewed refresh (`--update-snapshots`, diff in that PR or a follow-up).
