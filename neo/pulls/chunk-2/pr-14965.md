...
- **Phase A** (`define` → `repo` → `roster`): reads and writes the Fleet registry through the one long-lived HTTP owner, refuses occupied-definition drift, creates or reconciles exact `metadata.repo` coordinates through `setRepo`, then prints the neomjs/neo#14950 roster-generator invocation with a concrete family (`gpt` or `claude`). Clone credentials, HTTP(S) query/fragment payloads, and terminal-control characters are rejected before persistence, and dry-run output never prints the clone URL.
...
- [ ] Start the canonical loopback owner with `npm run ai:fleet-server`, then onboard the second GPT-family peer end-to-end: Phase A `--commit`, roster PR via neomjs/neo#14950, merge + refresh `origin/dev` + Memory Core restart, Phase B `--commit`, one login using the printed resolved-binary command, then first boot self-registers its wake route.
...
Related: parent neomjs/neo#13015 · consumed contracts: neomjs/neo#14918 (launch, merged) · neomjs/neo#14950 (roster generator, merged) · closed-by-composition: neomjs/neo#14915 · UI twin: neomjs/neo#14807 · v13.2 cornerstone-1 (#14560 / neomjs/neo-agent-institution#9).
