...
- **`FleetCockpitController.onStartFleet` (recomposed)** — reads the roster STORE (the full fleet truth: a folded idle card is still a member; rendered-cards fallback preserved), partitions, cascades `start` over the eligible through the landed per-verb C2 adapter (each record drives its own honest round-trip — the landed neomjs/neo#14611 machine at roster scale), renders the summary into the chrome, and keeps the settle semantics: roster re-polled ONCE when anything really started, never on all-reject (the honest reasons must stand). Ordering/backoff stays SERVICE-side per the seam discipline — the cockpit renders progression and outcome only.
- **`FleetAgent` + `mapRosterRow`** — the launch-seam truths (`launchable`, `authMode`) now ride the record tri-state (the named neomjs/neo#14991 consumer moment: eligibility reads the wire, never a cockpit guess). Both fields typeless by design so `null` (not read back) survives without coercion.
...
- Existing `onStartFleet` contracts hold unchanged (fan-out fail-closed per card, EXACTLY-ONCE re-poll, the neomjs/neo#14978 composition-root witness).
- Pre-commit gates green (whitespace, shorthand, jsdoc-types, ticket-archaeology, block-alignment).

## Post-Merge Validation

- [ ] NL-verifiable staged bring-up against the real seam with a mixed-outcome fleet (rides neomjs/neo#14646, the walkthrough leaf)
...
Related: parent neomjs/neo-agent-institution#10 (OPERATE spine, operator-directed split) · composes neomjs/neo#14611 (per-verb machine, closed) · consumes neomjs/neo#14991 (`launchable`/`authMode` roster stamps — the named consumer moment) · seam authority: Lane C bring-up semantics stay service-side · next spine leaves: neomjs/neo#14641 → neomjs/neo#14620 → neomjs/neo#14646.
