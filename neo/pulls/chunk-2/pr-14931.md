...
1. **Identity node**: `GraphService.initAsync` seeds every `identityRoots.mjs` roster entry into the graph at boot (idempotent, createdAt-preserving). The merged R3b generator (PR neomjs/neo#14950) emits the roster entry **including its `subscriptionTemplate`**.
...
**What survives:** the three-key correction from the neomjs/neo#14919 Drop+Supersede (resident anchor ÷ instance linkage ÷ observed engine) — it lives on in the roster/registry split. The cycle-1 repairs at `11e19b969` remain on the branch for reference. If any probe finding here reflects a real gap in `WakeSubscriptionService` itself, it becomes one small ticket at the owning service.
