...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#14598

Related: epic neomjs/neo-agent-institution#10 (parent — 18/35 after the neomjs/neo#14577 steward close) · neomjs/neo#14592 (the density evidence this executes — the badge is its named AgentCard consequence) · neomjs/neo#14774 (the merged card composition this completes) · neomjs/neo#14899 (the record-driven data path the badge rides) · neomjs/neo#15036 (the Brain-side enricher that stamps live counts — the seam's named successor)

**The leaf's one remaining delta made real: the open-lane count badge.** Every other neomjs/neo#14598 AC already shipped and is spec'd through the merged arc — the SSOT anatomy composition (#14774), the ADR-0032 display-state fixtures (name/engine/family re-render in place, never a re-key), the live wire binding with stale-as-stale degrade (`loadRoster` + the record-driven rebuild, neomjs/neo#14899), and the NL-verified card wall (FleetGridScaleNL, merged). What the density evidence added on top was concrete: **7–17 measured open lanes per active agent cannot read as one truncated lane line.** This PR lands exactly that consequence.
...
- Review convergence (Euclid, cycle 1): the original OMIT design left the field seed-only — erased by the first authoritative load. Flipped to roster-DTO ownership (the `launchable` pattern) with regressions on both hemispheres; the live stamping producer is split to neomjs/neo#15036 (the field degrades to null-no-badge until it lands, per the contract's tri-state rule).
...
- [ ] neomjs/neo#14598 closes on merge; the epic ticks 18→19/35.
...
- [ ] neomjs/neo#15036 stamps live counts Brain-side; the wired cockpit then renders live badges with zero further Body diff.
...
- d25b52570 — model field + lane row/badge + factory mapping + measured seed + token-layer styling + specs (8 files, +127/−23; rebased onto the neomjs/neo#15022 merge — `FleetAgent.mjs` now carries `launchable`/`openLaneCount`/`participationStatus` in field order, and the `mapRosterRow` docblock unions the eligibility prose with the producer-ownership OMIT rule for `laneLine` + `openLaneCount`).
...
**Rebased onto the neomjs/neo#15022 merge — conflict cleared, head `d25b52570`.** The fleet morning start landed on `dev` minutes after this branch was cut and extends the same two files. Union resolution: `FleetAgent.mjs` now carries `launchable` / `openLaneCount` / `participationStatus` in field order (all three nullable, each with its producer's honesty comment intact), and the `mapRosterRow` docblock keeps the morning-start eligibility prose while extending the producer-ownership OMIT sentence to `laneLine` + `openLaneCount`.
...
- **Ownership flipped to the roster DTO end-to-end** — the exact `launchable` tri-state precedent from the neomjs/neo#15022 merge: `createFleetCockpitStatus` hoists `openLaneCount` from the assembler rows (`null` = not stamped; the pure map derives nothing), and `mapRosterRow` carries it onto the shared record. The OMIT rule now covers `laneLine` only.
...
- **The stamping producer is real and named**: neomjs/neo#15036 (Brain-side enricher counting open assigned lanes per resident, the `resolveIdentityDisplay` precedent — source decision recorded at claim). Until it lands, every live row carries `null` and the wired cockpit honestly renders **no badges** — which is your own degrade clause ("missing truth degrading to null"), not a mock path. `Resolves neomjs/neo#14598` stands on that basis: the live wire exists and is regression-proven; the enricher turns the value on.
...
If you read the enricher as load-bearing for the close (rather than the seam + degrade rule), say so and I flip to non-closing + bind neomjs/neo#15036 as the closer — one message, no cycle.
