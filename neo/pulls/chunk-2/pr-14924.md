...
5. **Lifecycle decided (§2.1.5 + E8):** `window-all-closed` suppressed; Brain teardown ONLY on explicit quit; cockpit close = hide-never-destroy (SharedWorker lifetime is client-scoped — destroying the last window kills the App-Worker heap while the tray claims the institution runs); tray = the handle; neomjs/neo-agent-institution#12 consumes the semantics. ADR 0029's no-auto-spawn-on-perspective-restore rule carried explicitly in §2.4.
6. **AC4 + parent map:** live neomjs/neo-agent-institution#7 now carries a "Design authority" section citing ADR 0034 (status-honest as Proposed) + the E-map disposition; §5's E1 row consumes neomjs/neo#13033's live Contract Ledger UNCHANGED (build root incl. first window, popup-join proof, window-bridge enhancement points), with E2/E3 re-scoped as post-E1 hardening leaves that never reopen its ledger.
...
- **Q1 scope discipline:** ADR 0020 §3 already fixed target-arm (in-process) and fallback (child-process supervision), and neomjs/neo#13033 owns that spike. §2.1 binds only what is true in either arm (lifecycle owner, settle-or-reject, data-root, ports) — no double-decision, no leaf pre-emption.
...
- ADR consistency: authority citations verified against the live sources this session (ADR 0020 §3 text, neomjs/neo-agent-institution#7 epic body, ADR 0029 §2.1 state-class table, neomjs/neo#13033 ticket body); next-free ADR number (0034) verified against the decisions tree; ADR-0033's landing commit confirmed no index/tree.json registration is required for a new decision record.
...
- [ ] neomjs/neo-agent-institution#7 epic body updated to cite ADR 0034 as design authority for the E-leaves (AC 4 — executes right after merge, epic-body edit).
- [ ] neomjs/neo#13033 (Ada) intake picks up §2.1/§2.6 as upstream contract without needing its scope reshaped — if it does need reshaping, that's an ADR amendment conversation, not a leaf-local decision.
...
**[ADDRESSED] Lifecycle + 0029 carry-over.** §2.1.5 decides it: `window-all-closed` suppressed, Brain teardown only on explicit quit, cockpit close = hide-never-destroy (client-scoped SharedWorker lifetime — the spike's own quit gotcha turned normative), tray = the handle; E8 added to §5 with neomjs/neo-agent-institution#12 as the consuming UX spec. ADR 0029's no-auto-spawn-on-perspective-restore is restated in §2.4 precisely BECAUSE materialization removes the popup ceremony that used to make auto-spawn impossible.

**[ADDRESSED] AC4 + parent map.** Live neomjs/neo-agent-institution#7 now carries a "Design authority" section citing ADR 0034 (status-honest: Proposed until merge) + the E-map disposition. §5's E1 row consumes neomjs/neo#13033's live Contract Ledger UNCHANGED — first window, popup-join proof, and window-bridge enhancement points stay Ada's; E2/E3 are post-E1 hardening leaves that never reopen her ledger (they gained "(post-E1)" markers and hardening-only scope).

CI running at head; re-review when it suits — and this stays sequenced behind PR neomjs/neo#14920's re-review (cycle-2 there is 9/9 green at 402127635).
