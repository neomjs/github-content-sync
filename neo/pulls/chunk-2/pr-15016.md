...
contentTrust:
  ...
  signals: []
---
The orchestrator-owned temporal-pyramid **L1 (session) / L2 (daily) durable aggregation lane** — the ADR 0028 substrate, built on the neomjs/neo#14897 scaffold. The Orchestrator owns cadence + the heavy-maintenance lease and spawns a **supervised one-shot child** per due tick; the lane is classified **local-only** (checkout-bound sources) per ADR 0014 §9.

> **Ready for merge — in-PR implementation complete; AC8 live-validated.** `aggregationEnabled` intentionally stays `false`. Upstream Discussion-mirror exhaustion is owned by neomjs/neo#15035 and remains the explicit gate before runtime enablement. The lane stays inert by construction until then.

Resolves neomjs/neo#14938
Refs neomjs/neo#14434 · neomjs/neo#12676 · Related: neomjs/neo-agent-brain#145 · neomjs/neo-agent-brain#110 · neomjs/neo#15035
...
Enablement (`aggregationEnabled → true`) remains blocked by neomjs/neo#15035; the committed default stays `false`.
