...
**So the guaranteed trigger exists. It is also global while the damage is partial**, and that is why neomjs/neo#17428 escalates rather than decides:
...
Which cost the deployment bears is not a leaf's call, so neomjs/neo#17428's first AC is the operator resolving it, and its RED-PROOF is written per branch — including that the targeted branch is *refuted* rather than deferred if the detector cannot be built.
...
## [ADDRESSED] P3 falsifier accepted · body + neomjs/neo#17428 corrected · code untouched at `d8e5416081`
...
**PR body** corrected in the same pass, and the residual still points at neomjs/neo#17428 — which is now an accurate owner rather than one resting on a mechanism that does not exist.
