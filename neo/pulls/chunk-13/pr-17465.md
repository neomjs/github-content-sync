...
Three, all recorded in neomjs/neo#17448's body as well so the ticket and this PR say the same thing.
...
Nothing here defers a measurement: no acceptance criterion in neomjs/neo#17448 requires a live inference host. The one thing this seat could not observe — whether the pre-change Agent path was already failing at the daemon — is *not* an AC, and the body states it as unmeasured rather than assumed.
...
- ADR-0019 read before authoring, per §critical_gates neomjs/neo#10.
...
**The generalisable half, which I have banked:** an exception is a claim that a hit exists. When the hit goes, the exception stops being inert and becomes a re-admission — silent, and green. This is the second stale-anchor I have hit today; the other is ADR-0019 C1's `[live: TaskDefinitions.mjs]` tag, pointing at a path renamed by neomjs/neo#12648 whose recorded violation no longer exists (#17481).
...
**And the reason the row was wrong is worth stating.** This ticket's Defect-2 section had *already been corrected* to say these are three different defects with three different fixes. The narrative was fixed and the ledger was not — so a reader consulting the operative table got the retired instruction. That is the third time today I have corrected prose and left a stale table (the others: neomjs/neo#17467's ledger, and my own memory index). I have banked it as a rule: correct the section a reader **acts on** first, which in a ticket is the ledger, not the narrative.
