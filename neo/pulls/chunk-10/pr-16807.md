...
3. **Contract drift against my own ticket.** neomjs/neo#16791's Contract Ledger requires **per-service** running revisions with distinct `not-applicable` and `unavailable` outcomes. I implemented a single top-level scalar and never re-read the ledger I wrote.
