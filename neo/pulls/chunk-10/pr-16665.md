...
*(Corrected in place per this PR's own rule: the figure read `+168` before cycle 2's fix, and `— below this branch's own base of +224, so it nets negative` was made obsolete when neomjs/neo#16656 merged and the stacking dissolved. Old→new disclosed in the cycle-2 response comment. A stale number is a fact, not a position — see §6.)*

~~⚠️ **Stacked on PR neomjs/neo#16656**~~ — **resolved.** neomjs/neo#16656 merged at 09:43Z; this branch was rebased onto `dev`, dropping its two foreign commits, and the stacked-PR guard no longer fires.
...
Specimen: PR neomjs/neo#16656's RA-3, not a hypothetical.
...
## 🚨 Stacked-PR Guard: foreign commits in PR neomjs/neo#16665

@neo-opus-vega — this PR's commit list contains 2 commit(s) for ticket(s) its body does not declare.
The body declares neomjs/neo#16661. The commits below claim other tickets:
...
§10's row now names the axis explicitly (`fact-vs-position, not answered-vs-unanswered`) so the anti-pattern cannot be read as forbidding accuracy — which was the exact misreading that produced neomjs/neo#16661.
...
- **old → new:** `Net +168 bytes against dev` → **`+209`**, and `below this branch's own base of +224, so it nets negative` is obsolete — neomjs/neo#16656 merged at 09:43Z and the stacking dissolved, so there is no longer a base to net against.
...
The stacking warning is also resolved rather than still standing: this branch was rebased onto `dev`, dropping the two foreign neomjs/neo#16655 commits, and the stacked-PR guard no longer fires.
