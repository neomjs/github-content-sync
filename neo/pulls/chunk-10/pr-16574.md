...
Evidence: L2 (contract spec over the tracked file through the production reader + normalizer, `7 passed`) → L3 is what neomjs/neo-agent-brain#64's proof AC requires and is explicitly **not** claimed here.
...
- [ ] The proof run belongs against an **idle embedder** — a run concurrent with `kbSync` reproduces the contention failure documented on neomjs/neo-agent-brain#64 and proves nothing.
