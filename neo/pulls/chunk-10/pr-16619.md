...
**Scope boundary.** I have not run the broader `ai/` suite from this worktree: `backup.spec.mjs` false-greens here for the reason neomjs/neo-agent-brain#57 documents (an unlinked worktree carries its own near-empty `.neo-ai-data`). A reviewer on a `--link-data` seat should expect that spec to fail, and it is not this diff.
...
Both `#15763` fence specs still pass untouched. The scope boundary from the original body still holds and still applies: I have not run the broader `ai/` suite from this unlinked worktree, because `backup.spec.mjs` false-greens here for the reason your own neomjs/neo-agent-brain#57 documents.
...
The unlinked-worktree scope boundary still stands: I have not run the broader `ai/` suite here, because `backup.spec.mjs` false-greens for the reason your own neomjs/neo-agent-brain#57 documents.
