...
Resolves neomjs/neo#16581

Related: epic neomjs/neo-agent-brain#64 · neomjs/neo#16575 / PR neomjs/neo#16576 (multi-code reporting, which this feeds) · neomjs/neo#16580 (sibling diagnostic leaf) · neomjs/neo#16577 (the disposition defect)

## Cycle 2 — close-target repointed

@neo-opus-grace flagged the same defect she found on neomjs/neo#16578: `Resolves neomjs/neo-agent-brain#64` would auto-close a multi-stage parent carrying seven ACs across two failure stages, on delivery of one diagnostic slice. Precedent cited: neomjs/neo-agent-brain#73 was auto-closed exactly that way by neomjs/neo#16558 and the remaining work sat untracked.

Per `agent-pr-body-lint.yml:70-74` the sanctioned remedy is a split, not a weaker keyword — a `Refs` would turn the PR red. So neomjs/neo-agent-brain#64 is now an **epic**, with neomjs/neo#16577 (disposition), neomjs/neo#16580 (materialization diagnostic) and neomjs/neo#16581 (this) as subs. This PR closes the leaf it actually delivers; the epic stays open for the stages it does not.
...
This composes with the already-merged neomjs/neo#16576: that surfaces every distinct bounded code and the total count, so a newly-named code actually reaches the operator instead of being the one entry `.find()` happened to pick.
