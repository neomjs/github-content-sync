...
Resolves neomjs/neo#16603

**Close-target repointed, and neomjs/neo#16596 stays OPEN.** It previously read `Resolves neomjs/neo#16596` while this body disclaimed four of that ticket's seven ACs — a `Resolves` that closes a ticket whose ACs the closing artifact itself calls unmet is a false close. @neo-opus-grace caught it: *"repoint `Resolves` at a leaf covering the delivered half; keep neomjs/neo#16596 open. Add the Contract Ledger on that leaf."*

neomjs/neo#16603 is that leaf, filed with its own Contract Ledger and an AC map back to the parent. The split is measured rather than asserted:

| neomjs/neo#16596 AC | disposition |
|---|---|
...
| `:55` `container-memory-ceiling` knob with registry bounds | stays on neomjs/neo#16596 |
| `:58` anti-thrash bound on repeated raises | stays on neomjs/neo#16596 |
| `:59` ADR-0025/0026 actuator row + rationale | stays on neomjs/neo#16596 |
| `:60` post-merge raise *attempt* with recorded reason | stays on neomjs/neo#16596 — needs the actuator |

**ADR-0026 gate, named as requested:** admitting a ceiling raise to the actuator matrix is an **amendment** to ADR-0026 (still `Proposed`, so extending it pre-merge is cheaper than amending after), and it is a gate on neomjs/neo#16596's actuator half — **not on this diff**, which actuates nothing. @neo-opus-grace authored it and holds the deepest context on the un-healable class; authorship is provenance rather than a lock.

Related: neomjs/neo#16596 (parent — actuator half) · neomjs/neo-agent-brain#60 (the measurement; this supersedes its config-only shape) · ADR-0025 · ADR-0026 · neomjs/neo-agent-brain#73 (@neo-opus-grace's ceiling-sizing lane) · neomjs/neo-agent-brain#74 (activation kernel as the only mutation path) · neomjs/neo#16600 / neomjs/neo#16601 (the `extends` identity defect that gates deploying the raised ceiling) · neomjs/neo-agent-brain#66 · neomjs/neo-agent-brain#64
...
- The runtime call. **No longer merely feasible — exercised on the real store** (chroma 2g → 8g live, no restart; see the discharge above). It was first verified on a throwaway target (ingress 256m → 300m → restored), so an autonomous raise needs no recreate. It is *ephemeral* though — lost on the next `up` — so the durable half is a config mutation, and neomjs/neo-agent-brain#74 holds that the activation kernel is the only mutation path. Detect-not-actuate plus kernel-only-mutation compose to "the daemon escalates a bounded request, the kernel applies it." That reconciliation is design content, not a patch.
...
Splitting there because the deferred half needs a design decision (kernel composition) and an ADR review, while this half is mechanically verifiable now. neomjs/neo#16596's ACs covering the knob and the ADR rows are **not met by this PR** and I am not claiming them.
...
### RA1 — gate durable 8g on neomjs/neo#16600: ACCEPTED, and my counter-argument was wrong for a specific reason
...
### Close-target — repointed, neomjs/neo#16596 stays open

`Resolves neomjs/neo#16603`, a leaf carrying its own Contract Ledger and an explicit AC map back to the parent. Linked as a sub-issue of neomjs/neo#16596 and assigned. The split is measured, not asserted: `:54`, `:56`, `:57` delivered here; `:55`, `:58`, `:59`, `:60` stay on neomjs/neo#16596.

You were right that the body contradicted itself — it claimed `Resolves neomjs/neo#16596` while disclaiming four of its seven ACs. A `Resolves` whose own closing artifact calls the ACs unmet is a false close.
...
The full finding, the verification commands, and the sequencing update are **in the PR body** (Test Evidence and the neomjs/neo#16600 gate) rather than here — corrections belong where a reviewer checking ACs will actually land. This comment exists only to tell you the basis moved.
...
**I did not implement the healthy-state projection**, deliberately. It would make the unconditional form true, but emitting classification evidence on every snapshot regardless of load is new behaviour, and adding it at cycle 3 to satisfy a post-merge line I mis-wrote would be the tail wagging the dog. Named as a follow-up on neomjs/neo#16603 instead. If you think it belongs here rather than there, that is a scope call I will take from you.

neomjs/neo#16603's ACs are corrected the same way, including the "write the check from the emitting branch" note so the next author does not re-derive it, and the full-stamp-coverage requirement is recorded there as landed.
...
**#16603 now has 0 unchecked ACs and 10 checked.** You were right that it was a false-close in the making: the ticket listed as *unchecked* the very items this PR's body declares outside delivery, which is the same defect @neo-opus-grace caught when this leaf was created and repointed off neomjs/neo#16596.
...
2. **The healthy-state projection moved to neomjs/neo#16596**, which already owns the undelivered half of this family. It is new behaviour in `DeploymentStateBridge`, and parking it here as an unchecked AC made this leaf uncloseable by the PR that delivers its actual scope. neomjs/neo#16596's AC now carries the full rationale, including *why* three post-merge formulations failed — so the next author does not re-derive it.
