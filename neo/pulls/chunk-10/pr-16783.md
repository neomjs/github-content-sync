...
Refs neomjs/neo-agent-brain#52
...
Evidence: L3 (local unit execution against the real `configBase` declarations, plus CI on this head) → L3 required (every acceptance criterion on neomjs/neo#16782 is a present-tense property of committed source, fully reachable in-sandbox). Residual: none.

## Deltas from ticket

None substantive. One scope note: the ticket was filed after the branch, as the honest close target for work that measures neomjs/neo-agent-brain#52 rather than delivering its acceptance criteria — `Resolves neomjs/neo-agent-brain#52` would have closed a ticket whose derivation, normalization contract, and derived relation are all still unbuilt.
