...
Resolves neomjs/neo#16600

Related: neomjs/neo-agent-brain#66 (the incident + the measurement chain) · neomjs/neo#16599 (merge keyed on a content digest — would have *detected* this) · neomjs/neo#16597 / neomjs/neo-agent-brain#60 (the chroma ceiling this **gates**, see below) · neomjs/neo#16556 (the plane move that exposed it)
...
## Merge order — this PR gates neomjs/neo#16597

**#16600 must land before neomjs/neo#16597.** Raising chroma's ceiling durably lets a full kbSync pass *finish* for the first time, which bakes `extends: ''` into ~64k rows; fixing it afterwards re-churns every src id and arms a second mass stale-deletion. The dying store was an accidental circuit breaker (@neo-opus-grace), and neomjs/neo#16597 removes it permanently while ids are wrong.
...
- **Reverting neomjs/neo#16556.** The plane move was correct — the artifact's plane-presence was simply never declared, so no plane move could have known it was carrying it.
...
**Corrected — the cross-family mandate IS satisfiable, and my original note here was wrong.** It claimed @neo-gpt could not submit a formal review at 0% quota. He then did: `pullrequestreview-4877846070` on neomjs/neo#16597, submitted through the MCP path (`manage_pr_review`) at a UI-reported 0%. What is blocked is the Codex-side GitHub surface; the MCP surface is not. None of us had tested that, so the constraint I recorded was an assumption wearing a measurement's clothes — the same shape as the "operator-owned recreate" precondition that stalled a restore for hours earlier today.
