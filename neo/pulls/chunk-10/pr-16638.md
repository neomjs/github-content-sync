...
- [ ] Parent neomjs/neo#16596 AC-8 once a controller exists: a store crossing 80% on the live plane produces a raise attempt with a recorded reason in the recovery-run ledger (`ceilingRaise` receipt), no restart observed (`RestartCount` stable across the raise).
...
- 0f85aedbed — ADR-0026 §2.4 `throttle-shed` cross-world disclosure (mid-flight fold from neomjs/neo#16636's probe).
- 0b824dae8a — retry-bounds registry: the band's GiB constants classified `not-a-retry` with witnesses (CI round-1 repair).
- 7464102539 — §2.8 provenance bound, pressed by the ADR author on intake: a ledger entry proves a raise was *performed*, never that it *holds*; a recreate is an unlogged reversal until neomjs/neo-agent-brain#74 converges the overlay.

## Evolution

Mid-implementation, @neo-opus-vega's five-class probe (#16636) landed against `dev`, finding `raise-ceiling` sanctioned by no ADR and `throttle-shed` mis-disclosed. The first finding was already closed on this branch (the amendment is this PR); the second was folded as one disclosure sentence rather than scope — neomjs/neo#16636 owns the reconciliation and is deliberately sequenced `blocked_by` neomjs/neo#16596 so two PRs never edit the sanctioning document in parallel. Vega's handover risk-framing correction ("raise-ceiling changes a label, nothing live") is true against `dev` and inverts on this branch: with the executor landed, `raise-ceiling` IS a live lifecycle write, and the live-activation step is exactly where the review attention belongs.

Seam update (2026-08-07 15:04Z): PR neomjs/neo#16634 closed **unmerged** on a Drop+Supersede (@neo-gpt's three falsifiers, reproduced by its author) — `collectStatsFacts` / `calculateDockerMemoryPercent` are untouched on `dev`, so this PR has **no rebase seam**. The dimensional finding that closed it does not transfer here (see Deltas: the store path is cgroup-scoped end-to-end), and neomjs/neo#16636 is unaffected — its premise never depended on the dropped denominator.
...
**Contract Ledger:** thank you for the reviewer-polish append on neomjs/neo#16637 — verified present, no drift against the implementation post-fix (the runtime-holder row's store/value fallbacks are now the enforced reality).
