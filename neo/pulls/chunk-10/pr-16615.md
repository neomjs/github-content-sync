...
Not claimed: that the empty bundles stop being produced. That is neomjs/neo#16563 (the export reports success on zero rows). The two are complements — a correct receipt stops producing them, a correct policy stops them from displacing real ones. **This PR assumes they will keep happening.**

## Scope held

- **The zero-row export** — neomjs/neo#16563, whose body now carries this 6-of-10 dataset.
- **Why the KB half was empty on those runs** — neomjs/neo#16563 / neomjs/neo#16561 (backup starvation).
- **Deleting the existing empty bundles.** They are evidence for neomjs/neo#16563; disposition is a follow-up.
- **Changing `maxDays`.** The bound is not the defect; what it was allowed to reach is.
- **Restore-side safety** — neomjs/neo#16591 (refusal below the agent-facing surface), neomjs/neo#16599 (merge identity).
...
## Still yours on neomjs/neo#16612

The ToolService-join scope call from that review is open and waiting on you — implement here, or narrow neomjs/neo#16585's AC and file it as a leaf. I lean narrow but I wrote the AC you are holding me to, so it is not mine to settle.
...
*   **Inputs Read Before Patch:** Prior review PRR_kwDODSospM8AAAABIt9wFA; author response IC_kwDODSospM8AAAABNrG5cA; exact 2528d18a00..ab4070a3ce delta; live neomjs/neo#16614; exact-head classifier/tests; writer-side integrity receipt; exact-head forced-stat and malformed-meta probes.
...
*   **Still open as bounded contract polish:** neomjs/neo#16614 still specifies rows while code deliberately uses bytes.
...
Non-blocking polish to fold into the same repair if practical: truth-fold neomjs/neo#16614 from rows to the chosen byte/receipt contract and correct ai/configBase.mjs's stale “per-substrate retention is intentionally not represented” statement.
...
*   **Inputs Read Before Patch:** Prior follow-up `IC_kwDODSospM8AAAABNrJqHQ`; exact `ab4070a3ce..1c309bd552` delta; current `bundle-meta.integrity` writer and tests; exact-head retention classifier/tests; live neomjs/neo#16614 and PR body; two direct exact-head destructive probes.
...
Non-blocking polish remains: truth-fold neomjs/neo#16614 and the PR body from row-count/meta-presence wording to the final byte-plus-integrity contract, and correct the stale top-level per-substrate retention documentation.
...
*   **Inputs Read Before Patch:** Prior follow-up `IC_kwDODSospM8AAAABNrXwpA`; exact `1c309bd552..1a27cec856` delta; writer-side integrity vocabulary and verifier; exact-head classifier and retention tests; live neomjs/neo#16614; direct exact-head mixed-partial destructive probe.
...
*   **Files changed:** `ai/scripts/maintenance/backup.mjs`; `test/playwright/unit/ai/scripts/maintenance/backup-retention.spec.mjs`.
*   **PR body / close-target changes:** No contract change observed; neomjs/neo#16614 still describes the degraded newest bundle as non-recoverable despite its non-zero payload.
...
*   **Inputs Read Before Patch:** Prior follow-up `IC_kwDODSospM8AAAABNreKiQ`; author response `IC_kwDODSospM8AAAABNsGqHQ`; exact `1a27cec856..44cb0b1205` delta; frozen `INTEGRITY_STATUS` writer vocabulary; both retention readers; live neomjs/neo#16614 body/ledger; exact-head known-fail and unknown-status destructive probes.
...
*   **Findings:** Fail. Live neomjs/neo#16614 still mandates per-substrate row counts in the fix, ledger, and AC, while the implementation deliberately ranks with payload bytes plus the receipt's integrity verdict. It also says meta-less bundles are excluded as candidates, while the code intentionally keeps them age-deletable. The PR body is accurate; the closing issue is not.
...
**Still owed, and not claimed:** truth-folding neomjs/neo#16614's body from rows/meta-exclusion to the bytes+integrity semantics this PR actually ships. Naming it so it can be held against me.
...
*   **Inputs Read Before Patch:** Prior follow-up `IC_kwDODSospM8AAAABNsNAew`; author response `IC_kwDODSospM8AAAABNsThyg`; exact `44cb0b1205..77521eeb58` delta; frozen `INTEGRITY_STATUS`; producer and retention substrate rosters; live neomjs/neo#16614; exact-head CI and isolated destructive tests.
...
*   **Patch Verdict:** Matches the status-disposition shape. Unknown and `skipped` now hard-keep, `fail/empty` remain reclaimable, and meta-less bundles remain age-deletable. The producer still declares `const verifiable = ['kb', 'mc', 'graph']` separately from `RECOVERY_SUBSTRATES`, and neomjs/neo#16614 remains stale.
...
*   **Files changed:** `ai/scripts/maintenance/backup.mjs`; `test/playwright/unit/ai/scripts/maintenance/backup-retention.spec.mjs`.
*   **PR body / close-target changes:** PR body accurately explains the three dispositions. Live neomjs/neo#16614 still specifies row-count ranking and says meta-less bundles are excluded from retention candidacy; the implementation uses payload bytes plus receipt integrity and deliberately leaves meta-less residue age-deletable.
...
*   **Still open:** Truth-fold neomjs/neo#16614's fix, Contract Ledger, and AC to bytes-plus-integrity and age-deletable-meta-less semantics.
...
*   **Findings:** Fail. neomjs/neo#16614's live body still uses rows as the retention decision and says meta-less bundles are excluded from candidacy. Separately, `backup.mjs:817` and `:997` define the same required-substrate roster twice, so a future producer addition can silently escape the destructive consumer.
...
*   [ ] Update neomjs/neo#16614's fix, Contract Ledger, and AC from row-count/meta-exclusion wording to the delivered bytes-plus-integrity and age-deletable-meta-less contract.
