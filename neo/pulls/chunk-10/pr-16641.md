...
The interleaving point is named as **two boundaries**, not as actors: *the second ensure returns strictly between call 1's client assignment and call 1's `connect()` resolving.* Six of my neomjs/neo#16619 race witnesses passed against broken code by naming the actors instead.
...
3. **The ticket's premise had decayed** — its title claimed Data Sync *"still cannot run"*; the pipeline had been green for days behind the neomjs/neo#16495 exception.
...
7. **All of the above are now folded into neomjs/neo#16488** as a marked fourth correction, retaining the wrong text. That was the second Required Action and it is complete.
...
9. **One out-of-scope one-line repair:** `lifecycleGuardPath` was called at four sites in `heavyMaintenanceLeasePrimitives.mjs` and never imported — pre-existing on `dev`, fixed because it hard-blocked neomjs/neo#16619's required witness. Splittable on request.
...
| full Brain-only set denied → barrel | fails on `better-sqlite3` (**KNOWN STATE**, asserted as such, now neomjs/neo#16649) |
...
**Scope boundary.** `backup.spec.mjs` false-greens in this worktree for the reason neomjs/neo-agent-brain#57 documents, so no broad `ai/` claim is made from here.
...
The async `registerNeoChromaEmbeddingFunctions(): Promise<String[]>` signature is now in the ledger. neomjs/neo#16488's own ledger and ACs are being folded in the same pass.
...
The interleaving point is stated as **two boundaries** rather than as actors: *the second ensure returns strictly between call 1's client assignment and call 1's `connect()` resolving.* Six of my neomjs/neo#16619 race witnesses passed against broken code because they named the actors instead.
...
This is the second design error he has caught on this PR and the ninth real defect across it and neomjs/neo#16619 — on a benched budget. His `CHANGES_REQUESTED` remains the sole formal gate. Merging is @tobiu's call, never mine.
...
| item | disposition |
|---|---|
| `lifecycleGuardPath` import repair | **Already on `dev`** via `5ce07c4236` (#16619). I claimed repeatedly that it needed salvaging from here — **that was wrong.** I fixed it while building neomjs/neo#16619 because it blocked that PR's witness, then misremembered its provenance. Nothing to move. |
...
Nine real defects across this and neomjs/neo#16619, on a benched budget at 0% flatrate — including two here that CI could not have caught: a static-vs-runtime deferral error, and a concurrent-first-use race where a public entry executed against a half-initialized client.
