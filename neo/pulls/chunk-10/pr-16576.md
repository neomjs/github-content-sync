...
Resolves neomjs/neo#16575

Related: neomjs/neo-agent-brain#64 (the multi-cause failure this makes legible) · neomjs/neo#16551 (adjacent reporting defect on the same lane)
...
My first framing of this (on neomjs/neo-agent-brain#64) was *"surface the underlying embed error instead of wrapping it"*. That would have breached the boundary. Reading the docblock before implementing is what turned it into a safe change.

## Prior art: `.find()` was designed, and I endorsed it — what changes is the proxy, not the property

**This is not fixing an oversight.** neomjs/neo#15748's Contract Ledger specifies the single-code behavior twice — *"first safe `KB_*` summary code is retained as source provenance"* — and I reviewed PR **#15752** approvingly on exactly that basis, calling it the standout: *"projecting only the first bounded `KB_*` code and deliberately dropping messages/details… exactly the right instinct on a tenant path."* I scored its architecture 97 on the strength of it.

Credit to @neo-opus-grace for surfacing this; without it the next reader finding neomjs/neo#15748 would reasonably conclude this PR contradicts a settled decision, and re-litigate it.
...
So neomjs/neo#15748's decision stands on its stated rationale; only its cardinality moves, and the reason it can move is that the guard is a regex rather than a discipline.
...
- [ ] **Deliberately not claimed:** this fixes no ingest. neomjs/neo-agent-brain#64's embed failure is untouched — this makes its cause set readable, which is what the "error-bearing summary" wrapper currently hides.
...
- **Substrate accretion:** two additive fields on an existing error object and two conditional log suffixes. No new module, config leaf, dependency, or consumed surface. Sunset: if the new fields reach the deployment-state snapshot per-repo entries (named Out of Scope on neomjs/neo#16575 because that assembly path needs tracing first), the log suffixes become a redundant second surface and should be reconsidered then.
