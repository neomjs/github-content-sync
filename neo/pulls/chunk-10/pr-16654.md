...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#16653
Refs neomjs/neo-agent-brain#59
...
**So the guarantee is what neomjs/neo#15807 named when it made this mandatory — *"topology + mock-embedding contract"* — with isolation as ONE dimension.**
...
| job name / required check context | neomjs/neo-agent-brain#59 | **UNCHANGED** | old context stays required; nothing to fall back from | no diff to job name |
...
1. **The ticket was split out of neomjs/neo-agent-brain#59 during this work**, so implementation matches its own prescription exactly. neomjs/neo-agent-brain#59 keeps the rename and its operator-gated ordering; nothing was silently rescoped.
...
- [ ] neomjs/neo-agent-brain#59 re-evaluates whether a rename is warranted at all. `parity` read as **dev-profile parity** is defensible, and the proposed `plane-isolation` would name 4 of 8 test cases as the whole guarantee.
- [ ] If a rename does proceed, branch protection is updated **first**, and these comments are re-checked against the new name.

## Evolution

I opened this as the fifth in a family of names meaning something other than what they say — `localOnly`, the `ai` commit scope, `package.brain.json`'s "Body", neomjs/neo#16488's "Body install tier". **Review demoted it out of that family.** `parity` is underspecified, not false; dev-profile parity is a live concern and the word is defensible. The four others were genuinely wrong; this one is merely incomplete.
...
## 🚨 Stacked-PR Guard: foreign commits in PR neomjs/neo#16654

@neo-opus-ada — this PR's commit list contains 10 commit(s) for ticket(s) its body does not declare.
The body declares neomjs/neo#16653, neomjs/neo-agent-brain#59. The commits below claim other tickets:
