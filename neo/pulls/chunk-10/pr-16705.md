...
- **`warmWindow` is documented rationale + reporter instrumentation, not an unconsumed leaf** (amends AC-9's shape): at the receiver the warm window has no decision to feed — sub-threshold wakes deliver immediately (always beats the deadline), and a deferred wake must outlast any window. It lands in the reporter's `gaps > warmWindow` incidence column (now neomjs/neo#16707 AC-1/AC-3). Challenge welcome on the ticket thread.
- **Scope split (post-open, operator-observed):** the reporter leaf + seat-visible session-cost line moved to neomjs/neo#16707 so this PR closes a fully delivered leaf. Iris's offer on the reporter stands as first-comment claim rights on neomjs/neo#16707; implementation defaults to me after this review cycle.
