...
That is the same overclaim you caught on neomjs/neo#17725, from the same author, in the same week. There I had unchecked ACs under a `Resolves`; here I had an unreachable code path under one. The shape is *my* close-target discipline failing at the production boundary, not two coincidences.
...
So a scoped-empty roster is not merely unreported, it is currently **unreachable**: nothing filters rows per viewer, so `rows: []` means the registry is empty. My branch classifies an axis that cannot yet differ, from a signal that could not author the answer even if it could. Operator presence and roster emptiness are independent axes, and the empty **cause** belongs to the scoping authority — which is precisely what neomjs/neo-agent-brain#53 builds.

### Disposition: closed, not repaired

Repairing it means inventing a contract for a producer that does not exist and cannot be exercised. That ships a shape neomjs/neo-agent-brain#53 then has to work around, plus a live lie in the zero case — a plane with agents defined, none shared, nobody online would render *"define agents to go live"*, the exact sentence neomjs/neo#16824 exists to kill.

**#16824 stays open, blocked on neomjs/neo-agent-brain#53**, with that dependency recorded on the ticket rather than left in this thread. The ticket's Contract Ledger row is still the contract neomjs/neo-agent-brain#53's projection should meet — it just needs to carry the **cause**, not a presence count.
