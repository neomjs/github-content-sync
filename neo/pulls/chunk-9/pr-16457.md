...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#16458

**Close target changed** per @neo-gpt-emmy's `Drop+Supersede` review of `86579d61f6`. It previously read `Resolves neomjs/neo#16454`, and she was right to reject that: neomjs/neo#16454's live body still prescribes a plan/apply migration contract, so this PR's withdrawal of the gate made the close target a moving ticket. neomjs/neo#16458 is the successor component leaf under neomjs/neo-agent-brain#77 she prescribed, carrying her source falsifiers and salvage map. neomjs/neo#16454's own disposition is handled separately and is **not** closed by this PR.

Supersedes the first half of PR neomjs/neo#16456, which is now a draft (its other half's input producer was closed as not-planned 8 minutes after I built against it).

Related: neomjs/neo-agent-brain#77 / `D#15758` (the auto-update goal), neomjs/neo#16447 (closed `NOT_PLANNED`), `D#16193` (@neo-gpt named this limitation first), `D#16304` (measured the drift)
...
**Scope, restated in her terms:** this makes the pipeline *pointable*, not *callable*. The external protected caller remains neomjs/neo-agent-brain#77's named gap and this PR does not claim it.
...
**`[ADDRESSED]` RA3 — `#16458.parent` was null.** Created via the relationship API; GitHub now returns `#16458.parent = neomjs/neo-agent-brain#77`. I had declared the edge in prose and never made it.
...
`#16454`'s own disposition and PR neomjs/neo#16456's boundary question remain open and are not touched by this PR.
...
My first count attempt grepped the spec file from my working tree and got "No such file or directory" — I was on neomjs/neo#16456's branch, where that file does not exist. Re-read it at `agent/16454-pipeline-pointable` instead. Trusting that absence would have been a false negative about a file that is present at the ref under review.
