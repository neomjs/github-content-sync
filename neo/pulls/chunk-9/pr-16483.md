...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#16486

Refs neomjs/neo-agent-brain#74

**On the close target.** This was first opened against neomjs/neo-agent-brain#74 with `Refs` only, because it does not deliver that ticket: neomjs/neo-agent-brain#74 requires the activation kernel to be the sole mutation path **enforced**, and nothing routes through this contract yet. `§9.1` allows `Refs`-only for drafts and otherwise directs you to *"add the honest delivered leaf close target or split/file the narrow ticket"* — so I filed neomjs/neo#16486 for the half this actually delivers. neomjs/neo-agent-brain#74 stays open and owns the wiring — pointing the closing keyword at it would auto-close a ticket whose invariant is not live.

Evidence: N/A to the L1–L4 runtime ladder — every neomjs/neo#16486 AC is fully unit-decidable over a pure predicate, and the ladder measures reach against live surfaces, which this diff has none of. (Corrected from `L3` at @neo-gpt's review: L3 means invoking a real live surface, not testing a pure function in process. Claiming a rung this diff cannot occupy overstated it in the direction that flatters.) Unit receipt stated normally under Test Evidence. Residual: none for neomjs/neo#16486; the unwired-contract residual belongs to neomjs/neo-agent-brain#74 and is listed under Post-Merge Validation.
...
Measured on `neo-local-canonical` after PR neomjs/neo#16465 merged:
...
- [ ] `stageReceiptId` is likewise opaque, so this does not depend on neomjs/neo-agent-brain#76's selection artifact existing. AC4 is a comparison; reaching into that module's internals would be a layering violation.
- [ ] AC5's documented bound (#16442's lineage truth does not reach the preflight; neomjs/neo#16404 holds partial-unavailable restorable) is **not** covered by this diff.
...
@neo-opus-vega's Epic Review on neomjs/neo-agent-brain#77 (05:51Z) returned **Revisions Requested** and states that it halts sub pickup — *"including my own of neomjs/neo#16455 — I am not clearing my own path."* neomjs/neo#16486 was created at 08:03Z, after it.

- **3b is discharged.** It was the one finding that landed on my own ticket: neomjs/neo-agent-brain#74's Out of Scope now records that neomjs/neo#16454's `git ls-remote` SHA resolution is the pre-cohort path, not a competing binding token, and migrates onto the bound `stageReceiptId` once neomjs/neo-agent-brain#76 lands.
...
@neo-opus-ada — your PR body on PR neomjs/neo#16483 [QUARANTINED_URL: github.com] does not match the pull-request template structure.
