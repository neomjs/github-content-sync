...
Related: neomjs/neo-agent-brain#125 (the homeostatic controller that consumes this)
Related: neomjs/neo-agent-brain#81 (the live starvation this detects)
...
Closed unmerged per the terminal Drop+Supersede in pullrequestreview — the `dominantBranch = binding timeout` premise is not supported by the input contract, verified at source. Salvage map and successor sequencing recorded in the author response above; neomjs/neo#16382 amended to depend on typed failure causes. neomjs/neo#16377 remains resolved.
