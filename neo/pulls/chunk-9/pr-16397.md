...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#16388

Related: neomjs/neo#16382 (the detector this unblocks), neomjs/neo-agent-brain#81 (the live starvation the chain serves)
...
That is what closed PR neomjs/neo#16383 unmerged: I read `failedInner` as "the inner timeout is binding". A branch counter cannot carry that fact. This adds the missing dimension at the source. `failedInner` / `failedOuter` keep their exact meaning and values.

Evidence: L2 (unit coverage at exact head; every cause is now asserted through the real producer or a real wrapper rejection) → L2 required. Residual: which cause actually dominates on the CPU-only plane of neomjs/neo-agent-brain#81 is a live-plane observation, listed under Post-Merge.
...
**The completion log carries the cause tally, on failing runs only.** Not in the ticket. A starved plane is diagnosed from captured stderr — nobody on the live plane calls `backfillMiniSummaries()` and inspects its return, so a cause that exists only in a return value is invisible where neomjs/neo-agent-brain#81 gets read. Healthy runs keep their existing line.
...
- [ ] On the CPU-only deployment behind neomjs/neo-agent-brain#81, a real starved sweep names its dominant cause in the completion log — and the plane says whether `timeout-inner` actually dominates or whether the truth is `provider-error`, which no instrument could distinguish before this.
- [ ] neomjs/neo#16382's detector consumes `failureCauses` rather than `failedInner`, with the tie-safe policy re-based on causes.
...
- [x] **`[ADDRESSED]`** neomjs/neo#16388 Contract Ledger line 46 still says truthiness readers unaffected
