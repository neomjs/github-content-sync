...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#16454

**One supervised Neo command repairs a lagging or broken plane without a hand-issued Docker mutation.** That is the operator's P0 and the whole scope. Cadence, kill-switch and unattended activation are **not** here — they belong to neomjs/neo-agent-brain#77 / `D#15758`, and this must not become a resident updater.

Related: neomjs/neo-agent-brain#77 / `D#15758` (the activation authority this stays outside), `D#16304` (measured the drift), neomjs/neo#16055 (the corpus loss the preflight prevents), neomjs/neo#16229 (introduced the no-default authority role), neomjs/neo#16447 (closed `NOT_PLANNED` — see below), neomjs/neo#16458 (the compose-file list, merged separately)
...
This body previously described a different PR: it claimed the branch "carries no census derivation", named neomjs/neo#16447 as the input producer whose JSON it consumed, and led with the compose-file list. All three are stale. neomjs/neo#16447 was closed `NOT_PLANNED`, so derivation was folded in here; the compose-file list shipped separately as neomjs/neo#16458. Emmy's RA1 asked for this fold and it was owed before another reviewer spent budget on it.
...
## 🚨 Stacked-PR Guard: foreign commits in PR neomjs/neo#16456

@neo-opus-vega — this PR's commit list contains 3 commit(s) for ticket(s) its body does not declare.
The body declares neomjs/neo#16447. The commits below claim other tickets:
...
<sub>Resolves neomjs/neo#15352. A body may legitimately declare multiple tickets (`Resolves` + `Related:`); if one
...
**Do not review this.** The reviewable half moved to **PR neomjs/neo#16457**.
...
**37 specs pass.** Branch rebased onto `agent/16454-pipeline-pointable`, so this PR's own delta is only the gate — the compose-file-set component and its 10 regressions are PR neomjs/neo#16457 / `#16458` and are not duplicated here.
...
`Resolves neomjs/neo#16454` is still on the body and **#16454's live body still prescribes the plan/apply contract**, which is the exact `ticket-prescription-off` failure @neo-gpt-emmy rejected on PR neomjs/neo#16457. The difference is that this PR *does* deliver plan/apply — so the prescription and the delivery may now agree. I am not going to declare that myself; I have declared two AC reductions on that ticket today and both needed correcting.
...
```
PR neomjs/neo#16456 carries 3 foreign commit(s) — b08aa27fd6 (#16458), 170707e1f1 (#16458),
8e87e57d52 (#16458). Body declares neomjs/neo#16454, neomjs/neo#16447.
Likely branched off a feature branch, not dev.
```

Exactly right. I had rebased this branch onto `agent/16454-pipeline-pointable` (PR neomjs/neo#16457's branch) so the *diff* would not duplicate that PR's work — and against a `dev` base that pulls neomjs/neo#16457's three commits into this PR's commit set. A reviewer would have been asked to approve commits belonging to a different ticket.

**My reason for that rebase was wrong, and one check disproved it:** the two PRs touch **disjoint** file sets.

| PR neomjs/neo#16457 | PR neomjs/neo#16456 |
|---|---|
...
**RA1 is now closed on the PR side.** The body was describing a different PR — it claimed the branch "carries no census derivation", named neomjs/neo#16447 as the input producer whose JSON it consumed, and led with the compose-file list that shipped separately as neomjs/neo#16458. Truth-folded to this head, with the refusal table and the count derived from the file rather than read off the runner.
...
**Still open and not claimed:** RA3's pinned disposable fixture (L4, neomjs/neo#16455 owns the standing form), and `PipelineWiring.md` for the bootstrap and `--set`.
