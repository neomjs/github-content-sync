...
**RA-1 [ADDRESSED]** — Close-target basis changed to `Related: neomjs/neo#17668`; the issue stays OPEN to receive its AC-1 live in-container receipt post-deploy, and AC-1 is annotated `[L3-deferred — operator handoff needed]` on the ticket. No merge-time claim of an unobserved live effect remains.
...
Initial attempt used `Related:` — which your review offered as an option but which the `#12367` operator rule forbids for non-draft agent PR bodies (verified against neomjs/neo#16829/#16865 archaeology; two fresh lint failures confirmed it live). Switched to the **deferred-close route**, all three conditions met:

- `Resolves neomjs/neo#17668` restored → lint-pr-body **SUCCESS** at current body
- neomjs/neo#17668 AC-1 annotated `[L3-deferred — operator handoff needed]`
...
**RA-1 [ADDRESSED]** — The Deltas bullet now states the coherent deferred-close basis: `Resolves neomjs/neo#17668`, AC-1 annotated L3-deferred, live receipt owned by OPEN leaf neomjs/neo-agent-brain#15. The contradictory `Related` sentence is gone; line 1 and the narrative agree.

**RA-3 [ADDRESSED]** — Live neomjs/neo#17668 no longer carries the contradicting AC. The old unset-case line is struck through with the amendment recorded in place (dated, review-round-attributed), matching the clarified contract and ledger: UNSET = silent unchanged (zero token-related warns, asserted via observed sink); CONFIGURED-but-unreadable = named WARN.
