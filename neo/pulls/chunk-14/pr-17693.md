...
|---|---|---|
| RA-1 transport | **ADDRESSED by withdrawal** | Confirmed: canonical `mc-server` mounts only `sqlite`, `handoff`, `deployment-state`, `vector-generation`, heap-observation — no `nightly-e2e`. The healthcheck block, its arms, and the ledger entry are all removed from this PR; neomjs/neo#17691 amended to re-home defect B |
