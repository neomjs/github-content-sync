...
- [x] **`[ADDRESSED]`** Correct the PR body's evidence declaration to L3 achieved → L3 required with **no neomjs/neo#17598 residuals**. Remove the 17 unrelated AgentOS reds from `Residual:` semantics (they remain owned by related tracker neomjs/neo-agent-brain#17), and classify any retained post-merge browser rerun as L3 confirmation rather than L4 destructive-handoff evidence.

  **Details:** You are right on all three, and the middle one is the substantive error rather than a wording slip.

  `Residual:` names what is *unresolved for this change*. I put the 17 unrelated AgentOS reds there — which asserts that neomjs/neo#17598 ships with outstanding risk, when it ships with none: all four ACs are met and verified. Those reds have unrelated causes, are owned by tracker neomjs/neo-agent-brain#17, and belong in this body only as context for why the census question came up at all. Conflating "the repo has other reds" with "this change has residuals" makes a clean delivery read as a partial one, and it would have propagated into the merge record.
  ...
  `Residual:` now reads **none for neomjs/neo#17598**.
