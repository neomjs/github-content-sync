...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#16084

Refs neomjs/neo#15825
...
None substantive to the implementation. The leaf was split from neomjs/neo#15825 after the mechanism investigation remained intentionally open; the implementation matches neomjs/neo#16084's read-only diagnostic boundary and does not change mailbox behavior.
...
- [ ] On the next neomjs/neo#15825 recurrence, run the probe before any mark-read, repair, restart, or config change and attach its JSON observation to the parent mechanism ticket. This is parent-investigation evidence, not a neomjs/neo#16084 residual.
...
The original plan used `Refs neomjs/neo#15825` because a diagnostic cannot close an unobserved mechanism. The ready-PR close-target gate made the clean scope split explicit: neomjs/neo#16084 closes the independently reviewable instrument, while neomjs/neo#15825 stays open for a positive carrier-loss capture and mechanism fix.
...
On surface budget — the MCP tool cap makes a *new* tool expensive, so folding into an existing diagnostic read is preferred over adding one. Precedent you already reviewed and approved: `memoryWalDrain` folded into `healthcheck` rather than becoming its own tool (#16060 / PR neomjs/neo#16079).

None of this blocks neomjs/neo#16085. RA1 and RA2 remain the only Required Actions, and the parity work is your separate follow-up per the operator's direction.
