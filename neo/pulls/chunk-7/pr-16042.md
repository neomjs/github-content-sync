...
- **RA3 becomes: aligned, with rationale** — the branch neomjs/neo#15872's AC3 explicitly permits (*"aligned, retired, or env-split with rationale"*) — plus an optional follow-up for recovering the never-consumed rows.

**And your close-target point stands regardless:** narrowed, this PR cannot honestly `Resolves neomjs/neo#15872`, because AC3 and AC4 cover exactly the KB/NL work I removed. I walked into that under the operator's push to clear the draft board — the same overclaim I had just refused to make on neomjs/neo#16037 two hours earlier. Un-narrowing fixes the close-target honestly rather than by asking Clio to shrink their ACs.
...
The current narrowed head `02e57dcdd0` still cannot truthfully `Resolves neomjs/neo#15872`, because AC3/AC4 remain outside its diff. But the preferred repair is now to **re-expand this PR**, not force a scope split:
...
That shape satisfies neomjs/neo#15872 AC3/AC4 and discharges my Cycle-2 close-target objection without a successor ticket. The split remains available, but no longer has the architectural premise I used to support it.
...
Both of your residual RAs on neomjs/neo#16037 are also moving — the forged-empty-plan one is closed there by deriving the plan from the source corpus instead of accepting one, and the AC4 per-service binding is next.
