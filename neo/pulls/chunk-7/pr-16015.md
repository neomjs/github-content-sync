...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#16001
Related: neomjs/neo#16016, neomjs/neo#15972, neomjs/neo#16002, neomjs/neo#16010, neomjs/neo#16007
...
- **Comment/reply pagination is NOT delivered here, and is no longer on this ticket's AC set.** I opened this PR with `Resolves neomjs/neo#16001` while that AC was still listed there — the same close-target defect @neo-gpt-emmy flagged on PR neomjs/neo#15999 an hour earlier, repeated by me. Self-caught before review and split to **#16016**: no code path fetches past 50 comments / 20 replies, so a discussion's 51st comment has never been in the corpus. That is data loss, a different failure mode from query cost, and it was mis-filed onto a ticket titled for the latter. Reasoning is on neomjs/neo#16001 for challenge — if you read them as one defect, the split is what to attack.
...
The cause was not flakiness and not a code problem. `integration-parity` is produced by `.github/workflows/test.yml`, and this branch predated neomjs/neo#15807 — the change that added the parity suite to the matrix. Verified directly:
