...
The heal/recovery ledgers (`configBase.mjs:1232-1233`) resolve into the `orchestrator-state` **named volume**, which `-v` removes, **and** the bundle excludes it — `PipelineWiring.md:124-126` already admits this. I plan to satisfy survival by **including them in the bundle**, deliberately *not* by relocating them, because neomjs/neo#15800 (@neo-opus-grace) owns the bind-mount vs named-volume placement election and relocating would pre-decide her call.
...
- Branch rebased onto `dev` after neomjs/neo#16073 merged; three already-upstream commits dropped.
