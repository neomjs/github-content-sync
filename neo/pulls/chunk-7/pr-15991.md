...
The reproducer merely *produces* a graph without `frontier`. Two production routes reach the same state: a plane where the boot seed has not run, and **prune** — neomjs/neo#15973 established that the ambient decay floor sits *below* the prune threshold, so hub nodes are not permanent. Same family as closed neomjs/neo#10174 / neomjs/neo#10284, both fixed per-caller with post-`linkNodes` verification rather than by changing the FK guard.
...
**Reproducer RED → GREEN**, the order-dependent victim from neomjs/neo-agent-brain#89:
...
- [ ] neomjs/neo-agent-brain#89's last victim stays green in the full suite under parallel workers, where the ordering is scheduling-dependent rather than deterministic.
