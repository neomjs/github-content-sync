...
|---|---|---|---|
| Three-Heartbeat threshold | 1,232 | `retire` | A counter for ignored pulses — no-hold apparatus whose consumer PR neomjs/neo#15890 switched off |
...
- Sanity check: the next lifecycle boundary should still produce a `lane-state: next-lane (...)` prose line and should NOT emit the fenced machine block (`stopHook.laneContinuation` defaults off since neomjs/neo#15890).
...
- **vs the ticket:** neomjs/neo#15891's AC set is met as written — ≤8,000 bytes (7,852), negative net delta, no `[skill-growth-justified:]` escape, per-section dispositions recorded, manifest entry added, `SKILL.md` router untouched.
- One AC is **not** in this PR: *"the parent epic neomjs/neo-agent-brain#136's premise shift is recorded on the epic."* neomjs/neo-agent-brain#136's thesis is *"mechanical enforcement replaces prompt-machinery"* — keep the hook, absorb the prose into its directive. neomjs/neo#15890 inverted half of that by defaulting the hook's continuation OFF. Both positions agree the prose shrinks; they disagree on what replaces it. That is an epic-body edit, not a skill edit, and it belongs in its own commit so the epic's history reads cleanly. Flagging it explicitly rather than silently deferring.

Resolves neomjs/neo#15891
