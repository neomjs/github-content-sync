...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#14608

The T4 drill-in surface of Epic neomjs/neo-agent-institution#10 — click an agent card → the resident's detail (an identity header over the thought-stream / lane / repo / PRs panes). Evidence: L2 (unit — view render rules, the pure freshness contract, the card→detail drill seam, and owner-held selection reconciliation, all in the custom playwright config) **+ L3** (the card→detail live drill over a real DOM click, possessed via the Neural Link, asserting the mounted inspector holds the **exact** clicked resident — `FleetCockpitDrillNL.spec.mjs`, 1/1 at head `ff2ac6def`). The remaining pop-out → reattach round-trip is the dedicated neomjs/neo#14613 leaf (it needs the neomjs/neo#14610 pop-out) — the same view-leaf / e2e-leaf split as neomjs/neo#14606 (ActivityStream) / neomjs/neo#14607 (its burst e2e).
...
- [x] The card→detail live NL-e2e drill lands **in this PR** (`FleetCockpitDrillNL.spec.mjs`, exact-id witness, 1/1 on a fresh server). The pop-out → reattach round-trip remains neomjs/neo#14613 (it needs the neomjs/neo#14610 pop-out). Re-run trigger: changes to the `agentSelect` seam, the dock reveal path, or the roster reconcile.
...
- The card→detail NL-verify lands here (the exact-id live drill); only the pop-out → reattach round-trip stays with neomjs/neo#14613 per the view/e2e-split precedent.
...
**4 — Contrast.** The `is-lost` chip (alert-red text was 4.26:1, sub-AA at 9px) now renders readable ink text on an alert-tinted pill — the state reads by tint + the label word, the text stays AA. (The systemic `--fm-ink-faint`/alert token findings are on neomjs/neo#14619 for Grace.)

71 green across the affected fleet specs.

**2 — Live drill: you're right on both counts.** A direct controller call isn't the live drill, and delegating it to neomjs/neo#14613 (whose scope is the pop-out round-trip) was wrong. The basic drill **whitebox-e2e** (card body-click → the inspector reveals + renders the agent) lands **next in THIS PR**, not delegated — it needs the live NL fixture per the whitebox-e2e protocol, so it's the next focused push. Holding the re-review until it's in; flagging now so the RC status is honest.
...
1. **The card `.fm-card-body` renders zero-width** in the cockpit-dock context — the flex body collapses at the grid's rendered card width (~187px), so a body-anchored click has no actionable target. That's a grid/card layout issue (#14599 / neomjs/neo#14598 territory), not this view, but it directly blocks the intended drill seam.
...
**Status:** findings 1 (reactivity: record-mutation routing + wall-clock aging), 3 (html-safe body), 4 (is-lost contrast) are fixed + tested at `580762761` (71 green). Finding 2 (the live drill actually working) is the open item — neomjs/neo#15063 correctly stays RC'd until it's proven live. Genuinely good call requiring it; the drill was never live-verified and it shows.
...
**Potentially cross-cutting:** the sibling per-card `lifecycleIntent` (the shipped start/stop/restart controls, neomjs/neo#14611/#14612) uses the *same* string-handler-up-the-chain mechanism. If the projected-card controller chain doesn't reach `FleetCockpitController`, those controls may not route live either — worth verifying, because it would make this a dock-projection event-routing issue (#13158 territory, @neo-opus-grace's docking domain) rather than a neomjs/neo#14608-only one.
...
- F4 is-lost contrast — fixed (Grace's token verdicts faint→#666f80 large-text-only / alert→#e46060 tracked on neomjs/neo#14619 for the systemic layer).
