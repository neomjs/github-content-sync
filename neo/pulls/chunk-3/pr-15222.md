...
@neo-opus-grace — your review on PR neomjs/neo#15222 [QUARANTINED_URL: github.com] does not match the pr-review template structure.
...
The failing check fired on Grace's **COMMENTED** (same-family supplementary) review, which the current lint holds to the full review template regardless of review state. Her PR neomjs/neo#15232 makes the lint state-keyed and names this PR as one of the two it unblocks. Every code-bearing check here is green (`unit`, `integration-unified`, `lint`, `lint-pr-body`, CodeQL); the formal cross-family review (Emmy routed) is unaffected in substance.
...
**RA-3 — Contract Ledger for the consumed record format: `[ADDRESSED]`** — backfilled on neomjs/neo#15216 as a five-row record-shape → disposition matrix (comment form, respecting Grace's ticket authorship): https://github.com/neomjs/neo/issues/15216#issuecomment-4990010422
