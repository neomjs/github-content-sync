...
The **AC3 producer** for the computed-Golden-Path type-gate — the split-out evidence-window half of neomjs/neo#15057 (its AC3a), the direct analog of neomjs/neo#15068/PR neomjs/neo#15060 (the AC1+AC2 guard-filter record-seam). The GP scoring pipeline has TWO filter points; neomjs/neo#15060 recorded the routing-**contradiction guard**, this records the **actionability type-gate** (`isActionableComputedRecommendation` false — the visibility-only `epic`/`not-code-ready`/… candidates, a measured 42.2% residual per neomjs/neo#14503) that previously recorded nothing. Now the 42.2%-type-gate disposition has a *live* evidence window instead of a one-shot snapshot.
...
- **`routeAttributionLedgerStore.mjs`** — parameterized the shape-agnostic I/O (`getRouteAttributionLedgerFilePath` / `appendRouteAttribution` / `readRouteAttributionLedger` / `pruneRouteAttributionLedger`) with an optional `filename` (default = the route-attribution ledger, so neomjs/neo#15060's guard-filter callers are byte-for-byte unchanged). This lets a SIBLING record shape share the same dir + retention machinery + prune, without duplicating the I/O.
...
- `summarizeTypeGateRejectionLedger` over the accumulated window surfaces the real `byRejectionLabel` distribution — the evidence the neomjs/neo#14503 42.2%-type-gate disposition (the follow-up on neomjs/neo#15057 AC3b) decides against.

## Notes

- **Scope boundary (honest close-target):** this ships neomjs/neo#15057's AC3a (the evidence-window producer). AC3b — the disposition-DECISION (keep / narrow / rank-with-discount) — needs an *accumulated* production window (nothing to decide against until synthesis populates it) and stays on neomjs/neo#15057, naturally a later PR (per neomjs/neo#15057's own note). So this Resolves the split leaf neomjs/neo#15076, not neomjs/neo#15057.
