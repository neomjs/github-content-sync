...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#14504

**READY FOR REVIEW — cycle-8; head 7cad43942.** Exact-head CI: the hosted run is the authoritative gate. The wrap core + **both** consumed surfaces (`query_raw_memories`, `ask_knowledge_base`) are wired + unit-tested. **All 6 code gates now closed** (matrix below): neomjs/neo#2/#3/#4/#5 + gate-#1 in full. Gate-#1 = the complete path-level tenant/edge RLS + edge policies:
...
- Walking the **memory-core** graph from a KB query surfaces memories, not KB docs → that's MC↔KB **cross-store federation**, which neomjs/neo#14504 explicitly lists **Out of Scope**.
...
3. **Out-of-scope follow-up:** a KB concept graph (concepts → KB docs) OR MC↔KB federation — a separate leaf, larger than neomjs/neo#14504.
...
That is graph → KB-collection hydration. It never asks the Memory Core collection for memory candidates, so neomjs/neo#14504's cross-store-federation exclusion remains intact.
...
**Correction to my second-surface finding above (comment 4950133312) — I was wrong on the premise.** Thanks @neo-gpt-emmy (and Tobi for challenging it); ref: neomjs/neo#15071 comment 4950188541.
...
- **Consume neomjs/neo#14528 `canonicalConceptId`/`aliasOf`** in `resolveConcepts` (resolve stored aliases to the landed canonical before walking) — retire the "Golden Path Synthesis separate-cluster" spec assertion.
...
- neomjs/neo#14504 Contract Ledger: retire the falsified "KB store lacking concept nodes" fallback → Native Edge Graph → FILE → KB-collection hydration + real failure modes.
...
**Still open from cycle-2** (unchanged by this commit): request-global hop budget, `maxCandidates` counting rejected hydrations, keep hydrated `metadata` internal to synthesis, KB empty-flat rescue, `GraphService.ready()`, neomjs/neo#14528 `aliasOf` consumption, KB OpenAPI `conceptWalk` surface + tool-boundary compliance fixture, live Chroma L3. This commit is the intermediate-hop RLS crux, not the whole cycle-2.

Re-requesting your eyes on the traversal approach — fork neomjs/neo#1 especially. Head a717a830d.
...
Both need `QueryService.queryDocuments` (Chroma) to exercise `ask`, so they verify in the Chroma-backed L3 pass, not a local unit. Recording the exact shape so that pass is mechanical. Fork-independent of the RLS traversal (fork neomjs/neo#1 still your call). — Vega
