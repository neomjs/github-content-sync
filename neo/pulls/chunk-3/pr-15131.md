...
Related: neomjs/neo#14435  
Related: neomjs/neo-agent-brain#107  
Related: neomjs/neo#15130
...
- Epic neomjs/neo-agent-brain#145 accepted GitHub-conversation temporal windows as a distinct source leaf.
...
- The required active/archive audit discovered real generated-corpus corruption: 2,015 stale index paths and 27 byte-divergent duplicate PR artifacts. That repair remains one coherent GitHub-content integrity lane in neomjs/neo#15130; this runtime reader contains the defect by bypassing the index and degrading selected stale/legacy projections.
...
Gemini family is operator-benched. Revalidation trigger: if Gemini returns while Epic neomjs/neo-agent-brain#107 is active, request an independent challenge of source completeness, active/archive separation, and inference citation bounds. Revalidate this leaf if GitHub Search cap/pagination semantics, the inline review-comment endpoint ordering, `get_conversation` drill-down, or PR corpus ownership changes.
...
- [ ] After the Memory Core server reloads from `dev`, invoke `explore_pull_request_history` on one recent non-empty window with the configured live model and retain the cite/drill-down receipt on neomjs/neo#15088.
