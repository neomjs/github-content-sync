...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#15115 (FM cockpit keyboard operability — the keyboard/focus-order leaf of neomjs/neo#14619). Refs neomjs/neo#14619 (the parent accessibility pass keeps its contrast + reduced-motion + token/contract-docs axes; this PR does NOT close it).
...
Evidence: unit 26/26 (agentCard 14 + fleetGrid 12) + a mounted whitebox-e2e green, head 8c8932f62. Supersedes neomjs/neo#15081 (aria-live + landmark regions), folded in here.
...
- **aria-live activity log + named landmark regions** (folded in from neomjs/neo#15081).
...
- The whitebox-e2e IS in this PR and passing — the mount-authority the unit layer guards off (it catches the vdom-flush class of bug a `.vdom` assertion cannot see). Locally verified (`NEO_E2E_PORT=<fresh> npm run test-e2e`). Per-PR CI does not currently run the e2e suite (a known coverage gap under a design discussion respecting neomjs/neo#14685) — so this PR does NOT claim CI e2e coverage; the e2e is the local mount-authority.
...
- Close-target split (gate-5): this PR Resolves the new keyboard leaf neomjs/neo#15115; neomjs/neo#14619 keeps its contrast + reduced-motion + docs axes and is NOT closed by this PR.
- neomjs/neo#15081 superseded (aria-live + landmarks folded in) → closed.
