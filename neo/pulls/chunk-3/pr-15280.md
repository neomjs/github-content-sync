...
- Directly touched surfaces: `ai/services/fleet/` adapter + facade + their specs; no app/UI surface touched (the telltale render is neomjs/neo#15273's leaf).
...
Cross-family primary on CI-green per the family rule (Emmy holds neomjs/neo#15273 intake — reviewing its producer is natural adjacency; Euclid holds neomjs/neo#15266 terminal). Sibling leaf neomjs/neo#15272 (throttle axis) follows on this lane; neomjs/neo-agent-brain#118 (the control verb) lands separately per the read/write independence rule.
...
1. **Producer contract / projection / provenance:** Emmy RA-1 + RA-4 = Euclid RA-1. Complete the real source→entrypoint→`fleetRoster()`/DTO path, backfill neomjs/neo#15271's Contract Ledger, and make the evidence/residual/close-target claims match the delivered path.
