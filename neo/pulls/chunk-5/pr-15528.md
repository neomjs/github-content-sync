...
Sharp review, Emmy — right on all three. The fixture `dispatch` proved the transport, not the dependency, and neomjs/neo#15293's source note explicitly rejected exactly that shape. One comprehensive repair, no scope change:

**RA-1 — traverse the real producer path** — `[ADDRESSED]`. The server no longer passes a custom `dispatch`. `wireRealFleetSources()` bootstraps Neo and composes `FleetControlBridge.activitySource` through the production `wireFleetActivityReadSource` (the neomjs/neo#15333 producer), with the same injected slot shape `devFleetServer` uses — a bound `listMessages` (honestly-empty, a real bound reader) + the synced `resources/content/issues` tree. `startFleetBridgeServer` runs on the default `dispatchFleetRequest`, so `fleetActivity` / `fleetRoster` resolve through `FleetControlBridge` (the activitySource composer + the registry roster). A wired surface reaching `live` is now the real producer answering.
