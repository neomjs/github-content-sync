...
- **Wake hook hardened one line:** `.kimi-code/hooks/wakeEnvelopeHook.mjs` is now `KIMI_CODE_HOME`-aware (falls back to `~/.kimi-code`), so a Fleet-launched seat's envelope lands beside its own `server/instances` coordinates — the neomjs/neo#15596 coordinate contract held by construction.
