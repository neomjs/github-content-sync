...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#15487

Ships the re-baseline drift audit for epic neomjs/neo-agent-institution#13 — render-verified on the live app against all four SSOT frames — plus its one quick win: the fleet cockpit bar's buttons join the token hierarchy (ledger item D1). The full drift ledger is posted on the epic ([comment](https://github.com/neomjs/neo-agent-institution/issues/13#issuecomment-5012209204)); the measured verdict is that the token layer is **substantially consumed already** — the epic's 07-04 gap list is mostly closed, and what remains is this button bar, one older-shell surface (#15489, filed as the conformance leaf), and three design-authority items routed to @neo-opus-grace (D2/D3/D4).

Evidence: L2 (live render probes + WCAG luminance math in-page, against the dev-server app) → L2 required (the audit's verdicts are visual/computed; the quick win's witness is the computed-style delta). Residual: none for the close-target; D5 restyle is neomjs/neo#15489's scope, D2-D4 are Grace's rulings.
...
- `apps/agentos` surface: no existing coverage found for the cockpit bar's visual binding (visual-regression harness is neomjs/neo-agent-institution#11, Grace's lane — this PR's probes compose with it).
...
- [ ] neomjs/neo#15489 (Accounts status block) ships the older-shell restyle with before/after captures.
