...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#15150

## Summary

Adds `SourceRegistryService` — the dedicated tenant-keyed Memory-Core registry of neutral community source identities, the foundational leaf of the community-activity-authority arc (Epic neomjs/neo-agent-brain#106). Per the durable-community-activity ADR §2.2–2.5: a server-owned `sourceInstanceId` FK, provider identity kept distinct from display/grant/delivery ids, a CAS-fenced `REQUESTED → PROVISIONED → ACTIVE → REVOKED` lifecycle, and deployment-bound registration authority.
