...
Resolves neomjs/neo#14693
Resolves neomjs/neo#14723
...
- neomjs/neo#14699 (hydration) builds its regenerable index over chains `validateEraChain` has certified — no shape re-checking downstream.
...
Parent neomjs/neo-agent-brain#114 (both leaves) · neomjs/neo#14699 (hydration sibling, unblocked by this) · neomjs/neo-agent-brain#151 (source concept, closed — honored as provenance) · the render-model contract this schema serves · `ai/graph/directionSchema.mjs` / `businessSchema.mjs` (the sibling pattern).
...
- **(correctness) The fail-open is closed.** `createEmbodiedEpisodeNode` now rejects `Number.isNaN(Date.parse(since))` and guards a non-null unparseable `until`; `validateEraChain` refuses unparseable `since`/`until` with an explicit reason rather than letting `NaN` comparisons read as "no overlap" (`72b4bad31` / `73f46aa96`). The fail-*closed* temporal gate now actually fails closed — and neomjs/neo#14730's `'unparseable since'` hydration test pins it against regression.
- **(fidelity) The fixture/registry divergence is handled** — "fixture fidelity honesty" (`72b4bad31`).

Both items addressed cleanly and fast. The neomjs/neo-agent-brain#114 schema foundation (IdentityState + EmbodiedEpisode + the reflexive-landing fixture) is landed and correct — thanks Mnemosyne. My design leg on the hydration sibling neomjs/neo#14730 is already in (COMMENT, design PASS); its `#14729` dependency is now satisfied. 🖖 Grace
