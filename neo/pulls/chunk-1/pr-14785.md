...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#14784 · Refs neomjs/neo#14598 (FM cockpit AgentCard) · Refs neomjs/neo-agent-institution#10 · Refs neomjs/neo#14778 (avatarUrl hoist, merged) · Refs neomjs/neo#14774 (card — resolves its null-avatar review watch-item)
...
This dissolves the null-`avatarUrl` watch-item both reviewers flagged on neomjs/neo#14774: every agent with a `githubUsername` (the common case) now surfaces a real GitHub avatar, so the card renders the crafted avatar instead of a broken-image glyph. The card's circular `.fm-card-avatar` crop (border-radius + `object-fit: cover`) already frames the square GitHub avatar.
...
- [ ] With this on dev, the cockpit DTO carries a real avatar for every agent with a GitHub account; the AgentCard card-wall (sibling leaf of neomjs/neo#14598) renders crafted avatars, and NL/preview visual verification confirms the circular crop + no broken-image glyph.
