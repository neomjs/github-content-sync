...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#14760

The **lifecycle-write** side of the #14501 R3 boundary and neomjs/neo-agent-brain#121 Leaf-2 — the third and final control-plane graduation leaf, now unblocked by the merged ADR-0026 amendment (#14758). The daemon-core restart actuator the epic's Option-D solution names.
...
- **Physically absent from every client Bridge / readiness surface** — a structural firewall test scans `src/ai/fleet` + the AgentOS pane + the container healthcheck and asserts **none import** the actuator (mirrors the neomjs/neo#14636 render-ledger firewall). Distinct from the client-reachable FM `restartAgent` (out of scope).
...
- Consensus-gate archive is **pre-seeded** here (Signal Ledger / Dissent / Liveness / Criteria Mapping), per the neomjs/neo#14761/#14775 review cycle — no RC needed for it this time.
...
## Unresolved Dissent
None. @neo-gpt's prior `[GRADUATION_DEFERRED]` (`discussioncomment-17517114`) was **RESOLVED** by author fold neomjs/neo#2 (the daemon-core actuator named distinct from the client FM `restartAgent`) and discharged to approval.
...
- **§6.7 artifact** (ADR amendment neomjs/neo#14758 + AC-2 read-projection neomjs/neo#14759 + this Leaf-2 actuator) → this PR is the neomjs/neo-agent-brain#121 Leaf-2 control-plane actuator sub.

## Deltas
See "Deltas from ticket" above.

Related: `Refs` neomjs/neo-agent-brain#121 (parent, Leaf-2) · graduates Discussion #14501 · neomjs/neo#14758 (the ADR-0026 amendment, merged) · neomjs/neo#14759 (the read-observe sibling) · `DeploymentRuntimeAccessService` (the L0 envelope) · neomjs/neo#14611 (the existing FM client restart — distinct, out of scope).
