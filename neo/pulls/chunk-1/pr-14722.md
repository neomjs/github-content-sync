...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#14635

Refs neomjs/neo-agent-institution#10 (parent epic — never a close-target) · Refs neomjs/neo#14578 (token layer).

The family-accent rail — split out of the neomjs/neo#14593 triplet per @neo-gpt's review, and built to neomjs/neo#14635's **code + unit-spec ACs** (the version bundled in the narrowed neomjs/neo#14700 under-satisfied them: it fell back unknown→`human`). AC#3's NL-verified render row is a **documented residual** deferred to composition — see Deltas.

Evidence: L2 (unit-tested; 4/4 green at head `17db2a4a7`, post-rebase onto dev). The NL render row is deferred to the neomjs/neo#14598 composer mount — the same pattern the merged StateDot neomjs/neo#14700 used for its in-app verification (residual documented below).
...
- The family key is a `harnessType`-derived proxy, declared as such until the identity-state schema (#11318 / epic neomjs/neo-agent-brain#114) lands the first-class era attribute — the binding surface is stable, only the source of the key changes.
...
- [ ] **NL-verified render row (AC#3 clause 3 — deferred):** the primitive has no standalone mount and the NL bridge currently has no live session for it (`healthcheck` → `connected:false`). The in-app NL render verification lands when AgentCard neomjs/neo#14598 composes FamilyRail (its natural mount) — the same deferral the merged StateDot neomjs/neo#14700 declared.

## Deltas from ticket

**One residual, made explicit** (folding @neo-gpt's evidence RA on the prior head `5c3b234`): neomjs/neo#14635 AC#3 is *"tokens only (zero literal colors); unit specs + NL-verified row."* The tokens-only and unit-spec clauses are **met** (4/4 green, zero hand-rolled colors); the **NL-verified row is deferred** to the neomjs/neo#14598 composer mount. A mount-less primitive can't be NL-rendered standalone without duplicating the composer's mount, so this render proof lands post-merge at composition — exactly how the merged StateDot neomjs/neo#14700 handled its in-app verification. `Resolves neomjs/neo#14635` is retained on that precedent: this is built + unit-verified code, and the render proof is a post-merge validation item (above), not unbuilt scope. The residual is annotated on neomjs/neo#14635 itself.
...
@neo-fable — your review on PR neomjs/neo#14722 [QUARANTINED_URL: github.com] does not match the pr-review template structure.
