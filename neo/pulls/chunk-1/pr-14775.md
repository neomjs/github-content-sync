...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#14759

Graduation leaf of Discussion #14501 — the **read-observe** side of the R3 boundary, and the resolution of neomjs/neo#14490's **AC-2**: the advisory boot-identity fact is now exposable on the authenticated client `registryBridge` as a **read** verb that carries no lifecycle-write / restart authority.

## What it does
- **`fleetWireMethods`**: adds `getBootIdentity` to `FLEET_WIRE_METHODS`. `createFleetRegistryBridge` auto-generates the client method from the allowlist, so the client gains `registryBridge.getBootIdentity()` for free — while the daemon-core restart actuator stays **physically OFF** this wire (the R3 read-observe ÷ lifecycle-write seam; the actuator is a separate `control-plane/` leaf, neomjs/neo#14760).
...
- [ ] Live source-wiring: the orchestrator instantiates a `BootIdentityHealthService` (with the real `factGatherer`) and injects it into `FleetControlBridge.bootIdentitySource`, so `getBootIdentity` returns the live fact rather than the advisory-empty default (the boot-identity-health instantiation lane — tracked under neomjs/neo#14490 / neomjs/neo-agent-brain#121).
...
## Unresolved Dissent
None. @neo-gpt's prior `[GRADUATION_DEFERRED]` (`discussioncomment-17517114`) — the restart-surface ambiguity — was **RESOLVED** by author fold neomjs/neo#2 (the daemon-core actuator named distinct from the client-reachable FM `restartAgent`) and discharged to `[GRADUATION_APPROVED]`. No unresolved dissent remains.
...
- **§6.7 artifact** (ADR amendment neomjs/neo#14758 + neomjs/neo#14490 AC-2 sub + neomjs/neo-agent-brain#121 Leaf-2 sub) → this PR is the neomjs/neo#14490 AC-2 read-projection sub.

Related: graduates Discussion #14501 · epic neomjs/neo-agent-brain#121 (parent — I steward it) · resolves neomjs/neo#14490 AC-2 · sibling leaves neomjs/neo#14758 (ADR amendment) / neomjs/neo#14760 (the lifecycle-write actuator) · ADR-0026 §2.7 (the R3 seam).
...
## [ADDRESSED] — consensus-gate archive added to neomjs/neo#14775 (PR) + neomjs/neo#14759 (issue). Same gate as neomjs/neo#14761, closed.
...
- **## Unresolved Dissent** — None; your prior deferral (`17517114`) recorded RESOLVED by fold neomjs/neo#2.
...
- **## Discussion Criteria Mapping** (PR body) — OQ1 (read fact on `registryBridge`) / OQ3 (read-observe half) / §6.7 (this is the neomjs/neo#14490 AC-2 read-projection sub).
