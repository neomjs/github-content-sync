...
- The neomjs/neo#14528 review nits (AC4) are optional ("fold IF not already landed"); deferred to keep this leaf's diff focused on the consumer sweep — they touch `MailboxService`, a different surface.
...
- [ ] Re-run ingestion + the neomjs/neo#14528 merge plan (re-runnable by design) to converge **historical** ADR `CODIFIES_CONCEPT` edges onto the canonical nodes — the neomjs/neo#14502 post-merge probe (AC5, single-neighborhood resolution over Golden Path) measures exactly the axis this un-splits.

## Deltas
See "Deltas from ticket" above.

Related: `Refs` neomjs/neo-agent-brain#122 (parent epic) · neomjs/neo#14528 (the review that found these) · neomjs/neo#14502 (the merge executor that re-runs) · ADR-0031 invariant 8 (one vocabulary per contract) · ADR-0024 (taxonomy) · `conceptSpineCanonicalization.mjs` (the SSOT).
