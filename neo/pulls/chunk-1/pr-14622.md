...
- The app scaffold shipped in THIS leaf rather than a separate one: the neomjs/neo#14577 decision landed fresh-app minutes before, making the namespace's first files this leaf's natural placement executor. `neo-config.json` carries `useAiClient: true` + `useSharedWorkers: true` per the epic's NL-e2e spine + multi-window story.
...
- [ ] neomjs/neo-agent-institution#11 baselines this verification surface as its first fixture.
