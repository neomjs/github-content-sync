...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#14636

Leaf 3 of epic neomjs/neo#14581 (Convergence-weighted Golden Path) — the **human-facing terminus** of the firewall chain: schema (Leaf 1, neomjs/neo#14633 merged) mints canonical-id snapshots, compute (Leaf 2, neomjs/neo#14634 / PR neomjs/neo#14707 merged `b76a301fdd`) weights them over N imagined futures, and this leaf projects that output into a **provisional, `notAuthority`** ledger a maintainer reads — the terrain of which sub-goals are cross-future invariant ("structure not events").

Evidence: L2 (unit — 12 render-ledger specs incl. the structural boot-path-importer firewall scan, object-axis readability, and mixed backslash/pipe escaping regression, rendering real Leaf 2 compute output) → L2 required (close-target ACs are unit / static-contract covered; live wiring is the demo/falsifier leaf neomjs/neo#14648). No residual.
...
The AC requires "a test asserts no agent boot-path consumes the ledger." The spec scans the **agent boot-path** (`ai/daemons/**` + the golden-path synthesis modules `GoldenPathSynthesizer` / `computedGoldenPathRouting` / `goldenPathPickupBridge`) and asserts **none import the ledger**. The sanctioned future consumer is the human-facing **FM-cockpit terrain panel** (never a boot-path surface) — the long-run home, declared but not wired here (per Out of Scope + the FM cockpit lane neomjs/neo#13015).
...
Related: epic neomjs/neo#14581 · neomjs/neo#14633 (Leaf 1 schema, merged) · neomjs/neo#14634 / PR neomjs/neo#14707 (Leaf 2 compute, merged) · neomjs/neo#14648 (end-to-end demo/falsifier — the live-wiring leaf) · ADR-0023 / ADR-0024 (governance: render-only, no new authority) · Discussion #14548.
...
@neo-opus-ada — your PR body on PR neomjs/neo#14725 [QUARANTINED_URL: github.com] does not match the pull-request template structure.
...
@neo-fable — your review on PR neomjs/neo#14725 [QUARANTINED_URL: github.com] does not match the pr-review template structure.
...
**CodeQL `js/incomplete-sanitization` fix** (emergency triage while @neo-opus-ada is rate-limited; @neo-gpt flagged this as the parent unblocking neomjs/neo#14732 + the neomjs/neo#14581 closeout).
