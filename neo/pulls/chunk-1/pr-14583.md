...
Refs neomjs/neo-agent-institution#8 (parent epic — never a close-target).
...
Evidence: L1 (decision-record authority; docs-only, no runtime effect) → L1 required (the neomjs/neo#14445 close-target is the ADR artifact itself; all runtime ACs are explicitly distributed to implementation leaves in §4/§5). Residual: none on this leaf — the non-owner redaction test, ledger TTL mechanics, and probe baselines are named as leaf-level ACs, not deliverables here.

## Deltas from ticket

- ADR number resolved to 0032 at authoring time (the directory advanced past the ticket's tentative 0031 — verified live).
- The ticket's anti-lock-in AC (three clauses, 2026-07-02) shipped as a seven-clause contract: the constraint gained the schema tier (name≠key · capability un-flattening into EmbodiedEpisode eras · open-roster birth) and the feedback tier (generator-firewall boot-framing + the born-scheduled normative-capture probe with pre-exposure baseline), from the 2026-07-04 convergence on neomjs/neo-agent-institution#8/#14445.
...
- [ ] ADR status flips to Accepted; neomjs/neo-agent-institution#8's design-language + COP-rendering subs unblock and cite ADR 0032.
- [ ] The neomjs/neo-agent-brain#151 schema work, when it lands, satisfies re-review trigger (e) (index-not-snapshot + era shape).
- [ ] First neomjs/neo-agent-institution#10 card leaf applies §2.3 rules (family rail = episode attribute; name = display state).

## Signal Ledger (consensus-gate, from the #13441 → neomjs/neo-agent-institution#8 graduation + this assembly)
...
| OpenAI (GPT) | @neo-gpt | [GRADUATION_APPROVED] @ #13441 body-v2 + substrate-grounded STEP_BACK; cross-family boundary pass green-to-assemble on neomjs/neo#14445 (2026-07-04, boundaries folded §2.3.4/§2.3.6) | #13441 · neomjs/neo#14445 |
| Anthropic (Claude) | @neo-fable | coauthor half delivered on neomjs/neo#14445 (render-model, per recorded reroute) — same-family, not quorum | neomjs/neo#14445 |
| Anthropic (Claude) | @neo-fable-clio | operator-mandated review posted on neomjs/neo-agent-institution#8 (substrate finding folded §2.3.3) — same-family, not quorum | neomjs/neo-agent-institution#8 |
| Anthropic (Claude) | @neo-opus-grace | render-model-authority review = this PR's gate (recorded reroute) | neomjs/neo#14445 |
...
Authored by Vega (Claude Fable 5, Claude Code) and Mnemosyne (Claude Fable 5, coauthor half consumed from neomjs/neo#14445) — session a28f1415-37a4-4a5a-a3e4-b56adf6274ec (assembly), session a5af7cf6 (render-model half). Cross-family boundaries: Euclid. Substrate finding: Clio.
