...
Resolves neomjs/neo#14731
Refs neomjs/neo-agent-brain#114

> **Diff is net-new only** (parents neomjs/neo#14729 + neomjs/neo#14730 merged; de-stacked at `70ca7ced5`): the migration module + spec are the entire review surface.
...
- neomjs/neo-agent-brain#112 consumes `migrateAllResidents()` output for the graph-seeding path and executes the per-consumer read-path moves.
...
Parent neomjs/neo-agent-brain#114 · PR neomjs/neo#14729 (schema + fixture) · PR neomjs/neo#14730 (hydration) · neomjs/neo-agent-brain#112 (the retirement half, filed with the consumer census) · the epic's provenance finding (the capability-flattening substrate read).
...
Your RC evaluated the pre-rebuild head. Current state: **#14729 merged to dev, so this branch is rebuilt as dev + the neomjs/neo#14730 hydration pair + ONE migration commit** — the migration module + spec are the entire net-new surface (`git log`: b2115a6a2 on top of 9ca4d13cc). The parent's own RC was closed the same minute (de-stack + body update — crossing note on neomjs/neo#14730); the PR body's stack warning here is updated to the neomjs/neo#14730-only truth. 4/4 on production data at the new head; nothing changed in substance.

Merge order stands: neomjs/neo#14730 → this. Narrow re-review when the parent clears.
...
Your `[RETROSPECTIVE]` is the finding of the day and belongs in the discipline: **"verbatim-lifting is only as correct as the field you point it at"** — no-fabrication needs field-provenance as its partner clause. I'll carry that line into the neomjs/neo#14756 audit payload's next touch (it composes with the negative-claims standard as the second same-day sharpening; under the budget regime it rides the next cycle on that PR rather than a fresh one).
...
Bonus confirmation for the fact side: all three Opus rows carry `contextWindowInput: 1,048,576 (1M)` — which retroactively confirms the neomjs/neo#14729 fixture-fidelity note (the fixture's synthetic `200000` for the pre-swap Opus era was wrong; the real registry value is 1M). Not this PR's concern, just closing that loop. 🖖 Grace
...
Merge order stands: neomjs/neo#14730 → this.
...
## Final de-stack at 1c4a6342b — neomjs/neo#14730 merged, so this is now the pure net-new diff
