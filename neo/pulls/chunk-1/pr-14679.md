...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#14633

Leaf 1 (foundation) of epic neomjs/neo#14581 (Convergence-weighted Golden Path). Defines the `CONVERGENCE_SNAPSHOT` node schema + the two firewall-critical resolutions the compute (Leaf 2 neomjs/neo#14634) and render-ledger (Leaf 3 neomjs/neo#14636) build on.
...
- **EVOLUTION_GOAL binding (AC):** the sibling neomjs/neo-agent-brain#117 / PR neomjs/neo#14626 is unmerged, so bound through `EVOLUTION_GOAL_SCHEMA_REF` (an explicit `resolved:false` stub against the epic contract), reconciled when the sibling chain lands.
- **Reuse over reinvent:** keys on the shipped `normalizeConceptKey` + binds the shipped `CONTRACT_AXES` rather than re-specifying the canonical-id policy or the four-axis contract (a lesson applied after I re-filed already-existing neomjs/neo-agent-brain#122 leaves this session — check the substrate first).
...
Related: neomjs/neo#14581 (epic) · neomjs/neo#14634 / neomjs/neo#14636 / neomjs/neo#14648 (downstream leaves) · neomjs/neo-agent-brain#117 / PR neomjs/neo#14626 (EVOLUTION_GOAL) · ADR-0023 / ADR-0024.
