...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#14728

**Depends-on neomjs/neo#14726** (merge that first — it carries neomjs/neo#14728's state-axis half; see Deltas). Refs neomjs/neo-agent-institution#10 (parent epic) · Refs neomjs/neo#14701 (kindRegistry, merged — the file this hardens) · Refs neomjs/neo#14722 (sibling resolver on the same pattern).
...
- `familyToken` — FamilyRail neomjs/neo#14722
- `stateToken` + `stateLabel` — HealthSwatch neomjs/neo#14726 (fix-at-source)
...
**Close-target correction** (per @neo-gpt's neomjs/neo#14737 RC): neomjs/neo#14728 named all four resolvers; its scope is delivered across **two** PRs — the state-axis pair (`stateToken`/`stateLabel`) folded into neomjs/neo#14726 fix-at-source, and this PR's kind-axis pair. So **#14728 is fully delivered only when BOTH neomjs/neo#14726 and this PR are on dev** — hence `Depends-on neomjs/neo#14726` and the merge-order (land neomjs/neo#14726 first). `Resolves neomjs/neo#14728` is retained on that ordering: once neomjs/neo#14726 is on dev, this PR's merge completes the ticket accurately. If merge-order can't be guaranteed, downgrade this to `Refs` and I'll close neomjs/neo#14728 once both land.
