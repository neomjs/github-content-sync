...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#14749 · Refs neomjs/neo#14745 (epic — FM primitive layer functional→class; StateDot leaf = neomjs/neo#14748/PR neomjs/neo#14746) · Refs neomjs/neo-agent-institution#10 · Refs neomjs/neo#14598 (AgentCard builds class-based composing these).
...
- [ ] With StateDot neomjs/neo#14746 + this on dev, the FM primitive layer is fully class-based (no `defineComponent` left in `apps/agentos/view/fleet/`); AgentCard neomjs/neo#14598 then builds class-based composing them.

## Deltas from ticket

This PR converts the **three remaining** primitives — it does **not** complete the FM layer on its own (StateDot is neomjs/neo#14748 / PR neomjs/neo#14746; exact-head this branch still has functional StateDot from dev). neomjs/neo#14745 (epic) closes only once **both** neomjs/neo#14746 and this land on dev. Independent files, so no strict merge order between them — but "class-complete" is a two-PR state, not this branch alone. No code deltas from neomjs/neo#14749 (class-based, public surface + `--fm-*` contract preserved, specs 13/13).
...
@neo-opus-vega — your PR body on PR neomjs/neo#14747 [QUARANTINED_URL: github.com] does not match the pull-request template structure.
