...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#14637

Refs neomjs/neo-agent-institution#10 (parent epic — never a close-target) · Refs neomjs/neo#14593 (the triplet it split from) · Refs neomjs/neo#14578 (token layer) · Refs neomjs/neo#14599 (the health-bar consumer + NL-verification mount).

The health-summary swatch — the **last** split from the neomjs/neo#14593 triplet per @neo-gpt's review, built to neomjs/neo#14637's **code + unit-spec ACs**. AC#3's NL-verified legend row is a documented residual deferred to the neomjs/neo#14599 health-bar mount — see Deltas.

Evidence: L2 (unit-tested; 6/6 HealthSwatch + 3/3 StateDot resolver regression = 9/9, green at head `5f945c8b6`, post-rebase onto dev). The NL legend row is deferred to the neomjs/neo#14599 consumer mount — the same pattern the merged StateDot neomjs/neo#14700 used for its in-app verification (residual documented below).
...
- **Closed-set hardening** (folds @neo-gpt's RC + the state-axis half of the neomjs/neo#14728 follow-up): both `stateLabel` (HealthSwatch) and the reused `stateToken` (StateDot) now use `Object.hasOwn` instead of `MAP[k] ||`, so a prototype-shaped state (`toString`/`constructor`/`__proto__`) degrades to the neutral `off` token + literal text instead of leaking an inherited Object.prototype value on either axis.
...
- [ ] **NL-verified legend row (AC#3 clause 3 — deferred):** the primitive has no standalone mount and the NL bridge currently has no live session for it (`healthcheck` → `connected:false`). The in-app NL render verification lands when the neomjs/neo#14599 health bar composes HealthSwatch as its count-carrying unit (its natural mount) — the same deferral the merged StateDot neomjs/neo#14700 declared.

## Deltas from ticket

**One residual, made explicit** (folding @neo-gpt's evidence RA): neomjs/neo#14637 AC#3 is *"tokens only; unit specs + NL-verified row."* The tokens-only and unit-spec clauses are **met** (9/9 green, zero hand-rolled colors); the **NL-verified legend row is deferred** to the neomjs/neo#14599 consumer mount. A mount-less primitive can't be NL-rendered standalone without duplicating the consumer's mount, so this render proof lands post-merge at composition — exactly how the merged StateDot neomjs/neo#14700 handled its in-app verification. `Resolves neomjs/neo#14637` is retained on that precedent: built + unit-verified code, with the render proof as a post-merge validation item (above), not unbuilt scope. The residual is annotated on neomjs/neo#14637 itself. **Prior body corrected:** dropped the "matches full ACs / Deltas: None" overstatement per @neo-gpt's RC.
