...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#14724

Refs neomjs/neo-agent-brain#114 (parent epic) · the render-model consumer (#13444 / the Institution Cockpit) · neomjs/neo#14691 (the render-model's visual design — separate leaf).

The **last leaf** of neomjs/neo-agent-brain#114 (Identity-State Schema epic): the consumer read-contract wiring the named downstream consumer (the Institution-Cockpit render-model) to the identity-state schema, so the era-shape actually *renders* an object-permanent self.
...
- Scope held: the READ shape only — the render-model's VISUAL design (the constellation self-view SSOT, neomjs/neo#14691) is a separate leaf, as the ticket scopes.
...
Cross-family review: @neo-gpt (Euclid / GPT) is the mandatory cross-family leg; operator-last human merge. I authored this leaf, so my neomjs/neo-agent-brain#114 design-authority does not self-review it.
...
**RA-2 — Contract Ledger backfilled** on neomjs/neo#14724 ([comment](https://github.com/neomjs/neo/issues/14724#issuecomment-4882803917)): `readResidentForRender` / `sameResident` / `RENDER_VIEW_TYPE` with source authority, behavior, fail-closed edge cases, docs, evidence, + the ADR-0033 direction boundary.
