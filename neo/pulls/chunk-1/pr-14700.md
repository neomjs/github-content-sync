...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#14593

Refs neomjs/neo-agent-institution#10 (parent epic — never a close-target) · Refs neomjs/neo#14578 (the --fm-* token layer this consumes).

**Narrowed per @neo-gpt's review** (was the full triplet): neomjs/neo#14593 is StateDot-only; FamilyRail and HealthSwatch are open sibling leaves (#14635 / neomjs/neo#14637) with stricter ACs — built separately against those, not bundled here.

StateDot — the atomic session-state indicator every fleet surface composes — a Neo functional component under `apps/agentos/view/fleet/`, consuming the merged neomjs/neo#14578 `--fm-state-*` tokens with **zero hand-rolled colors**.
...
- [ ] The AgentCard tranche (#14598) composes StateDot (+ FamilyRail from neomjs/neo#14635); the in-app visual verification lands there (this primitive has no standalone mount yet).
...
Narrowed to neomjs/neo#14593's true scope (StateDot-only) after review surfaced that the bundled FamilyRail/HealthSwatch under-satisfied their sibling tickets' ACs. No duplicate claimable work remains — neomjs/neo#14635/#14637 stay open for their proper implementations.
