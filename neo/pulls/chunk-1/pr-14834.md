...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#14833 (component slice) · Refs neomjs/neo#14599 (parent — live-wire binding neomjs/neo#14595 + NL-verifiable mount remain OPEN, host-gated) · Refs neomjs/neo-agent-institution#10 (epic) · Refs neomjs/neo#14592 (density evidence) · Refs neomjs/neo#14598 / neomjs/neo#14593 (composed primitives)

The cockpit's default view (SSOT §01 fleet zone, `apps/agentos/design/fleet-manager-cockpit-plan.html`): a scale-to-a-glance health bar over a density-ranked grid of AgentCards. Composes the already-built primitives and honors the cockpit-plan design + the measured neomjs/neo#14592 density evidence — ranked-fold at scale, not the mock's six-agent / two-column assumption.

**Close-target scope:** neomjs/neo#14599 bundles this component with the live-wire binding (#14595) + the NL-verifiable live mount — both host-gated (no cockpit shell mounts the fleet components yet; neomjs/neo#14615 is `dropped`/`not-code-ready`). This PR Resolves the unit-provable **component slice** (#14833); neomjs/neo#14599 stays OPEN for the live wire + mount. Same split as neomjs/neo#14831 ⊂ neomjs/neo#14606.

Evidence: L2 — `fleetGrid.spec.mjs` **6/6 green** (pure `rankFleet` / `healthCounts` cores + component DOM: fold thresholds at 6/12/20, deterministic tiering, in-place health counts, honest degrade). L2 fully covers neomjs/neo#14833's component-only ACs.
...
- [ ] With this on dev, FleetGrid + HealthBar render a density-ranked cockpit fleet zone from a roster fixture. The remaining neomjs/neo#14599 ACs — the live wire binding for counts (#14595) and the NL-verifiable render at live roster scale — stay OPEN on neomjs/neo#14599, gated on the cockpit shell / Lane-C wiring.

## Deltas from ticket (#14833)

The FleetGrid + HealthBar components + the pure cores + the CSS anatomy + the unit spec — the full neomjs/neo#14833 slice. Live-wire binding and the NL mount remain on neomjs/neo#14599 (host-gated). The literal per-digit count pulse is a HealthSwatch-level refinement (its own concern); this delivers the stable-instance smooth-update foundation + the `animateCounts` config + the reduced-motion gate. Full-rebuild of the card set on roster change (correctness); incremental card diffing is a perf refinement, not needed for the bound the ACs test.
...
**RA1 — stale/conflicting branch.** Rebased onto current `dev`, dropping the un-squashed neomjs/neo#14827 ancestor (`b703be9c`). The PR diff is now exactly the four neomjs/neo#14833 files (FleetGrid / HealthBar / CSS append / spec) — no more `ActivityStream.mjs` / `activityStream.spec.mjs`, the CSS resolves cleanly against dev's squashed neomjs/neo#14827, and GitHub now reports **MERGEABLE** with checks re-running.
