...
Leaf A of the ADR 0028 temporal-pyramid decomposition (Epic neomjs/neo-agent-brain#145): the **storage substrate** the durable aggregation lane (Leaf B, neomjs/neo#14434) writes into and the dynamic synthesis path (Leaf C, neomjs/neo#14435) reads from — the `temporal-summary` collection inside the `unified` store, the `SUMMARY_*` label schema as a pure module, and the Memory-Core-realm maintenance registration, with the ADR 0024 §2.2 node-type table updated in the same PR per ADR 0028 §2.7's pre-declared obligation. No aggregation logic, no data written beyond schema — sequencing exactly per the ticket.

**Merge gate state: OPEN** — ADR 0028 flipped to `Accepted` on PR neomjs/neo#14428's merge (2026-07-02), so this leaf is merge-eligible per the epic's #11376 §6 contract.

Resolves neomjs/neo#14433

Refs neomjs/neo-agent-brain#145
...
- The ADR-0033 `directionBreakdown` fields ride Leaf B's records on this substrate (ADR 0028 §2.4 amendment) → then neomjs/neo#14568 ({v,s,r} composition) unblocks.

## Related

Parent Epic neomjs/neo-agent-brain#145 · authority ADR 0028 (Accepted; §2.3 storage / §2.6 partitioning / §2.7 obligation discharged here) · amends ADR 0024 §2.2 (this PR) · aligned-with ADR 0017 (within-posture collection add) · ADR 0019 (config leaves read at use site; read-gate honored) · consumed by neomjs/neo#14434 → neomjs/neo#14435 → neomjs/neo#14568.
...
**RA-1 (schema addenda reconciliation)** — dispositioned owner-side rather than folded: [OWNER-DISPOSITION on neomjs/neo#14433](https://github.com/neomjs/neo/issues/14433#issuecomment-4881971733) maps every live addendum → shipped-here (the five-field storage contract) or re-sited→#14434 with [binding content-contract lines posted on Leaf B](https://github.com/neomjs/neo/issues/14434#issuecomment-4881972018) (demands 1/2/6 + the staged direction slot). Rationale in the disposition: the five fields are Chroma *metadata* (addressing); the specimen demands are *document content* — writer-owned per this ticket's own Avoided-Traps boundary, with `version` as the evolution carrier. The sunset fold-queue instruction is explicitly superseded in the same comment, so `Resolves neomjs/neo#14433` closes truthfully on the storage substrate.
...
**RA-3 (config spec coverage)** — new `config.template.spec.mjs` test pins `temporalSummaryProd === 'neo-temporal-summary'`, the per-worker `temporalSummaryTest` leaf, and the resolved `collections.temporalSummary` formula under the toggle — same by-construction shape as the neomjs/neo#12499 memory/session pins.
