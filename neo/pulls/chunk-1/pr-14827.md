...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#14831 (the unit-provable component slice) · Refs neomjs/neo#14606 (parent — live binding to neomjs/neo#14572/#14573 + NL-verifiable live mount remain OPEN, host-gated) · Refs neomjs/neo-agent-institution#10 (Lane B epic) · Refs neomjs/neo#14594 (EventChip, merged — composed here) · Refs neomjs/neo#14592 (density numbers)

Lane B2 of the cockpit plan: the activity feed — where "real-time is the spine" is proven or falsified. The load-bearing requirement (per neomjs/neo#14592: ~9 events/min bursts overflow a small window in ~35s) is a bounded window with honest overflow — never an unbounded feed (frame-freeze) or a silent drop (falsified feed).

**Close-target scope (cross-family review, @neo-gpt on PR neomjs/neo#14827):** neomjs/neo#14606 bundled the component with host-gated live wiring, so `Resolves neomjs/neo#14606` overclaimed. This PR now Resolves the unit-provable **component slice** (#14831); the live binding to the neomjs/neo#14572/#14573 adapters + the NL-verifiable live mount stay OPEN on neomjs/neo#14606. V-B-A confirmed the split is structural, not cosmetic: the adapters are server-side services (`ai/services/fleet/fleet*ActivityAdapter.mjs`) and **no cockpit host view mounts the fleet components yet** — so the binding/mount genuinely cannot land in this slice.

Evidence: L2 — `activityStream.spec.mjs` **4/4 green** (pure bound under a 100-event burst; component DOM bound + fold; EventChip delegation incl. unknown→neutral; honest degrade). L2 fully covers neomjs/neo#14831's component-only ACs.
...
- [ ] With this on dev, the ActivityStream renders a bounded feed from an event array. The remaining neomjs/neo#14606 ACs — live binding to the A2A/PR adapters (#14572/#14573), the NL-verified live mount, and the burst e2e (T3.13) — stay open on neomjs/neo#14606, gated on the cockpit host view.

## Deltas from ticket (#14831)

The component + the bounded-buffer contract + the spec + the CSS anatomy — the full neomjs/neo#14831 scope. Live-adapter binding, the NL mount, and the burst e2e remain on neomjs/neo#14606 (host-gated), out of this slice. Full-rebuild on events change (correctness); incremental/virtualized application is a perf refinement, not needed for the bound the AC tests.
