...
**On this plane a timeout does not stop the work.** That is not a hypothesis — it is this incident's own C2 finding, which I contributed: `ollama/ollama#11889`, abandoned native-Ollama requests run to completion, and neomjs/neo-agent-brain#48's controlled reproduction measured a runner still at ~400% CPU for 60+ seconds after every client had disconnected.
...
Emmy named the cheaper path and it needs no PR from me: **merged neomjs/neo#16978 plus an explicit small batch ceiling.** That ceiling already exists as a config leaf —
