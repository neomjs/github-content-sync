...
- The `#16931`/PR neomjs/neo#16934 horizons work is cited at D5 as the landed producer half of the banded-presence frontier.
...
- **D4** teaches the `INVALID` partition as landed truth with its receipt (PR neomjs/neo#16926 merged 2026-08-11T01:11Z — 40 minutes after the cycle-1 review correctly flagged it as then-open) and anchors the enforcing source at head (`apps/agentos/view/fleet/sourceHealth.mjs`).
- **D5** demotes poll-digest consumption to the open S7 frontier it is (server-side action landed as PR neomjs/neo#16796; production consumer open, neomjs/neo-agent-brain#50) and states the measured failure surfaces the right way around: today's push lane is host-local and survives a dead ingress; everything polling plane truth dies with it.
- **Identity claims** now carry bearer anchors per `identity-claim-audit.md`: Grace's rename cites her own commit (`1e3a0c1e97`, neomjs/neo#13410); the pricing is homed to its author (Ada, D#16764) including the sweep-was-not-sufficient routing fact; the Ada anecdote cites her falsifier + the endorsement-correction + her review re-verification; neomjs/neo-agent-brain#83 is cited on its author's own record; the one named-agent claim without a bearer record is de-named (cite-or-cut → cut).
...
Residual-Owner: neomjs/neo-agent-brain#37
