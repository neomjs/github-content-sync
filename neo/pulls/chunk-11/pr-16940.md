...
> *"`ownerMatch = body.match(RESIDUAL_OWNER_PATTERN)` searches the whole body, so `Residual-Owner: neomjs/neo#200` in an unrelated section satisfies live PMV work."*
...
The operator challenged my **#16959** approval as rubberstamping today and he was right. I named a real residual — nothing bounds a concurrency-slot *holder* — wrote that it *"belongs in neomjs/neo-agent-brain#54's open set"*, and never put it there; `grep` on that body returned **absent**, and the PR merged **59 seconds** after my approval. With A+FU off since 2026-08-10, the correct disposition was **Request Changes**, not a prose promise. It is now homed as **O6** in neomjs/neo-agent-brain#54.
...
C  inline-CODE `Residual-Owner: neomjs/neo#200` in owing PMV    valid:true   valid:false
D  colonless `Residual-Owner neomjs/neo#200`                    valid:true   valid:false
...
1 prose: "We considered Residual-Owner: neomjs/neo#200 but   valid:true   valid:false
  rejected that owner."
...
<!--\nResidual-Owner: neomjs/neo#200\n-->              valid:true   valid:false
<!-- Residual-Owner: neomjs/neo#200 -->                valid:true   valid:false
...
The single-line `<!-- Residual-Owner: neomjs/neo#200 -->` form is pinned beside it at `:295`, and `:300` is the **positive control** — a comment sitting next to a *real* owner must still pass, so blanking cannot silently blank the document around it. Without that arm, a gate that rejected every commented body would have gone green on the same assertion.
...
BYPASS 11 (`:308`) pins the multiline form and the single-line `<!-- Residual-Owner: neomjs/neo#200` with no trailing newline. It carries **two over-reach controls**, because blanking-to-EOF fails toward destroying documents:
