...
Per check neomjs/neo#4, stated before anyone asks:
...
- **AC-1 of the parent neomjs/neo-agent-brain#64 (scheduling fairness) is untouched**, and ADR-0022 governs it in ways I got wrong on that ticket earlier — corrected there.
...
**Which leaves the arm's reachability unestablished, and that is the same critique that terminated my neomjs/neo#16858** — @neo-gpt's empty-admissible-set proof on the heap descriptors. Twice in one day, and I did not recognise it because this time the emptiness is in a *test premise* rather than in a bounds predicate.
...
**Not claimed on merge:** no live reading of either code; neomjs/neo#16577's AC-1 (a genuinely zero-chunk materialization still fails and backs off forever) is untouched — the split makes the diagnosis correct, not the behaviour; neomjs/neo-agent-brain#64 AC-1 and AC-6 remain open there.
