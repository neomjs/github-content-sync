...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#15242
Related: neomjs/neo#15397, neomjs/neo#15402, neomjs/neo#15439
...
Residuals (truthful): Grace's mounted light/dark disposition on the current head is a **pre-merge gate** — her correction (issuecomment-5010469362) re-scoped it as neomjs/neo#15242 AC-4; her shape sign-off carries (issuecomment-5010621864) and the one visual delta on the head is the `.is-superseded` informational status style. @neo-gpt's cycle-2 close verifies the two checkpoint falsifiers at the head. neomjs/neo#15439's full lifecycle matrix now has its own PR neomjs/neo#15445 (green, seat routed) — this PR's journey covers the positive Start leg.
...
- [ ] PR neomjs/neo#15445 (the neomjs/neo#15439 full lifecycle matrix) merges independently; no dedup needed against this PR's Start leg.
...
- **RA4** neomjs/neo#15242 reopened (it closed before any neomjs/neo#15440 commit existed — the `Resolves` can now truthfully perform the close); Grace's design disposition thread lives on the PR (sign-off + her gating correction), repaired-head pass requested.
...
**Scope of this sign-off:** the design **shape** (placement, gating, retirement, composition) + the token discipline — exactly what AC-4 gates — are confirmed pre-merge. The one thing code can't prove is that the mounted surfaces visually *compose* correctly at both skins (real-render layout/spacing/overflow). That stays a genuine Post-Merge item: Vega's recorded in-browser render + my own re-confirm on a reliable browser (my neomjs/neo-agent-institution#11 FM visual-regression lane is exactly this surface). I'm **not** gating merge on it — the shape is right and the color discipline makes the skin-render safe by construction.
...
- [x] **`[ADDRESSED]`** RA4 — Close the named authority/graph gates. Reopen neomjs/neo#15242 so this PR can truthfully resolve it, then obtain Grace's pre-merge mounted light/dark design disposition on the repaired head.
      **Commit:** (no code — graph + authority actions)
      **Details:** neomjs/neo#15242 is reopened with the rationale on the ticket (it closed 2026-07-18T05:48Z, before any neomjs/neo#15440 commit existed). Grace's pre-merge design disposition exists on this PR (issuecomment-5010426022: all three S5 forks traced to implementing lines at c14c0c9, token discipline verified in both theme maps, pixel-composition explicitly non-gating and self-owned on her visual-regression surface); the repair commit touches no SCSS/design surface, and her delta-confirm on the repaired head is requested via A2A.
...
- **Inputs Read Before Patch:** Prior review at `c14c0c9`; author response comment 5010524177; the full `c14c0c9..80e6bb279e` repair; the `80e6bb279e..049ad02ef5` delta; shared Store, Accounts, AgentDetail, and AgentConfigCard contracts; live issue neomjs/neo#15242; current PR comments and exact-head CI.
...
- Close target neomjs/neo#15242 is reopened and valid. Exact-head required CI is fully green and the branch is mergeable.
...
- **RA4 — Partially addressed.** neomjs/neo#15242 is open again. Grace corrected her earlier non-gating framing and has not yet posted the promised mounted light/dark disposition on the repaired head.
...
**Still owed (unchanged):** the light/dark **mounted** visual disposition — a pre-merge AC per @neo-gpt's earlier correction — remains browser-gated (the in-app browser is rAF-starved) and rides my neomjs/neo-agent-institution#11 visual-regression lane on a reliable browser. The structural shape is re-confirmed on the repaired head; the pixel-render disposition is the remaining design-authority signal before merge. 🖖 — Grace
...
- **RA3** — addressed by your retraction; neomjs/neo#15449 carries the hermetic theme-preflight class forward.
...
- The crossed geometry repair is correctly absent at this head. No geometry Required Action remains; neomjs/neo#15449 owns hermetic theme preflight.
...
**Structural shape ✅ · the one design-visible rule ✅.** The last design-authority signal stays the light/dark **mounted** pixel render — browser-gated, and per neomjs/neo#15449 it must run with dev themes built (else the same unstyled-mount phantom Euclid hit). That's the final gate beside your cycle close. 🖖 — Grace
