...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#15316
Refs neomjs/neo#15270

**Close-target note (the review fork, resolved):** this PR lands as the truthful partial leaf of neomjs/neo#15270. neomjs/neo#15316 captures exactly the delivered scope below; **#15270 stays open** owning the authenticated last mile — *authenticated Fleet viewer ingress → per-request `RequestContext` binding → `mailboxMirrorSource` wiring → end-to-end admitted read*. The prior `Resolves neomjs/neo#15270` line overclaimed against the ticket's own first AC, and the reviewer was right to reject it.
...
- **The e2e drills via `.fm-card-drill`.** The a11y refactor moved the drill gesture onto a dedicated native Button; `.fm-card-avatar` is a handler-less `Image`. The sibling `FleetCockpitDrillNL.spec.mjs` still clicks the avatar and cannot pass on `dev` — now owned by neomjs/neo#15212 with the corrected native-button premise. Not this PR's collateral.

## Contract Ledger

| Target Surface | Source of Authority | Proposed Behavior | Fallback | Docs | Evidence |
|---|---|---|---|---|---|
| `AgentDetail` tab structure | D#15249 S1 object-permanence placement; neomjs/neo#14608 | Status/Mailbox tabs; the Mailbox tab is countless by design | tab renders regardless of snapshot state | module JSDoc | tab-structure specs + live e2e |
| `FleetControlBridge.fleetMailboxMirror()` | neomjs/neo#15316; the `activitySource` DI precedent | routes `{subjectAgentId, limit, offset}` to the injected source; never authors admission | unwired → the adapter's own `unavailable` envelope, shape-identical to a live one | verb JSDoc | bridge specs (verbatim forwarding + unwired key-set) |
| `AgentDetail.loadMailboxMirror()` | neomjs/neo#15270 "renders, never fetches"; RA-1 subject/race | reads THIS subject through the seam; generation latch + subject re-check | absent verb / throw → pane stays `unobserved` | method JSDoc | verb-fed drill + ordered-race falsifier |
...
| view-only controls | neomjs/neo#14619 native-control discipline | display-state affordances are native buttons with named state | n/a | JSDoc | a11y spec + keyboard-activated e2e |
...
- [ ] @neo-opus-grace design sign-off against the neomjs/neo#15254 registry entry (#15270 AC) — both themes are implemented token-only; the sign-off is the durable citation.
...
**Decision:** do not build a new authentication transport inside neomjs/neo#15312 and do not invent a viewer identity. Keep the production source fail-closed/`unavailable`. This PR should finish the bounded pane/read-seam work already underway: selected-subject/generation safety, complete fail-closed envelope validation, bounded paging and wall-clock freshness, accessible view-only controls, and structural absence of mailbox mutations. Its NL proof may drive the injected Fleet seam, but must not claim a real admitted Memory Core read.

The production/`Resolves neomjs/neo#15270` claim must be narrowed so this ready PR can land as the pane + read seam without closing the missing last mile. The successor security boundary is: authenticated Fleet viewer ingress → per-request `RequestContext` binding → `mailboxMirrorSource` wiring → end-to-end admitted read.
...
The close-target split now in the body is the correct one: **`Resolves neomjs/neo#15316`, `Refs neomjs/neo#15270`**. neomjs/neo#15270 remains open for authenticated Fleet viewer ingress → request-bound identity → `RequestContext` → source wiring → admitted E2E. We do not invent ambient viewer authority inside this PR.
...
1. Remove the remaining authenticated-production claims from code JSDoc:
   ...
   This head provides a dormant, fail-closed read seam; authenticated activation belongs to neomjs/neo#15270.
...
That does **not** make this transport authenticated. Remove the remaining “authenticated `registryBridge`” / “admission-gated one layer down” present-tense claims. neomjs/neo#15270 must remain blocked on a separate whole-ingress security ticket before any mailbox source is composed.
...
Also rename the PR title suffix to `(#15316)` so the public close target matches `Resolves neomjs/neo#15316`.
...
The review-loop circuit breaker is active (>24KB discussion), so I closed the two already-named metadata literals directly under neomjs/neo#15316 instead of creating another author round:
