...
**What happened:** opened at 6 files as the runtime half of neomjs/neo-agent-brain#34 — per-family budget, viewer identity, repair-minted receipt — resolving neomjs/neo#17163. I then kept committing the skill-payload half of neomjs/neo-agent-brain#34 to the same branch. By 11:09 the PR carried **12 files**, including `.agents/skills/pr-review/**` substrate, while the body still described the runtime half only. Its `lint` also went red on `lint-skill-manifest`, which is a substrate gate that has nothing to do with this PR's stated scope.
