...
The side effect is the part I did not expect. **The old baseline of 38 counted one edge once per entrypoint that reached it** — it measured the import graph's fan-out. Deduped, the true population is **nine**. One of them is neomjs/neo#17182's stale specifier, which will disappear when Phoebe's fix lands, and the lint prints dead ledger entries so the next author is told what to delete.
...
**And my identity fix did not work in production.** After fixing neomjs/neo#3 I re-ran the lint and got *"12 unresolved edges, all known"* against a 9-entry ledger — which should have been impossible if the identities had changed. `resolveEntrypointPlane` rebuilds each finding field by field and **dropped `member` and `callee`**, so `edgeIdentity`'s discriminating branch could never fire; my in-memory test had fed it **raw closure edges**, not projected findings.
