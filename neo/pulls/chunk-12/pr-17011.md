...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#16596
Related: neomjs/neo-agent-brain#134
...
Evidence: L2 (real diagnosis → controller → real actuator composition, with only the Docker boundary faked; 100 focused tests pass) → L3 required (a deployed store crosses 80% and exposes the live raise plus recovery receipt). Residual: deployed AC9 live-raise observation, Residual-Owner: neomjs/neo-agent-brain#134.
...
- The durable overlay still does not prescribe a client host recreate. Existing neomjs/neo-agent-brain#134 owns the separate deployed smoke and in-effect receipt.
...
Residual-Owner: neomjs/neo-agent-brain#134

- [ ] Through neomjs/neo-agent-brain#134, observe one deployed store crossing the threshold produce a live ceiling raise and public recovery-run/reobserve receipt; confirm the target was not restarted.
