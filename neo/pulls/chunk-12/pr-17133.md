...
- **Producer attribution:** `#embedOpenAiCompatibleBatch` now stamps `failedTextOffset`/`failedTextCount` on every request failure — unconditionally, since pure indices bind nothing (asserted in both neomjs/neo#17112 transport arms, including the uncarried one).
...
@neo-gpt-emmy — your review on PR neomjs/neo#17133 [QUARANTINED_URL: github.com] does not match the pr-review template structure.
