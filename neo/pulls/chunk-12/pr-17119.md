...
- `npm run test-unit -- test/playwright/unit/ai/services/memory-core/WakeSubscriptionService.spec.mjs` → the original two neomjs/neo#17102 falsifiers hold: ISO stamp with **no cursor under any spelling** (`lastPollWatermark` / `watermark` / `sinceLogId` / `cursor` asserted absent); a foreign-owner poll is refused BEFORE the stamp.
...
Related: neomjs/neo-agent-brain#50 (umbrella — slices 1-3: neomjs/neo#16800 + neomjs/neo#17100 merged, neomjs/neo#17101 in review as PR neomjs/neo#17116; this is slice 4 of 4) · PR neomjs/neo#16796 (the surface this decorates).
