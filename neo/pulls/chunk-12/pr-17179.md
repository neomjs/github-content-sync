...
**The new template failed the neomjs/neo#16148 origin-session rule** by putting the session id on a shared line — provenance is checked mechanically across every documented format, and the lint caught my format being quietly non-compliant.
...
That obligation is neomjs/neo-agent-brain#34's AC-9 cohort receipt, which already owns the post-merge measurement for the terminal-round decision.
