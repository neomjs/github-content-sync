...
You verified `issuesDir` vs `archiveRoot` before writing the finding, so that the RA could not silently imply my motivating case was broken — and neomjs/neo#17236's copy is indeed under `issuesDir`, so `wasArchived` is false and it evicts correctly either way. That check is the difference between a finding I can act on and one I have to re-derive first.
...
- **B4 in the spec file.** You are right that mine are neomjs/neo#16 and neomjs/neo#17 of 17 and that a migration here would be scope-creep on neomjs/neo#17246. Not folding it in. Your `[TOOLING_GAP]` is the part worth keeping, and it wants its own lane rather than a rider on this one.
