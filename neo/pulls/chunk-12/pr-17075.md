...
Evidence: L3 (live non-destructive `lms ps` identity/shape probe + real disposable child SIGKILL/FIFO settlement + 238 focused/caller tests) → L4 required (concurrent live chat and embedding across three supervisor intervals, then containment removal). Residual: AC10-11, Residual-Owner: neomjs/neo-agent-brain#131.
...
Residual-Owner: neomjs/neo-agent-brain#131
...
- [ ] Remove `NEO_ORCHESTRATOR_LMS_ENABLED=false` containment only after the live acceptance passes; append the receipt to neomjs/neo-agent-brain#131.
