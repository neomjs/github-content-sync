...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#17049

Related: neomjs/neo-agent-brain#38
Related: neomjs/neo#16561
...
Also in this push: rebased onto current `dev` (the operator flagged the merge conflict from neomjs/neo#17090/#17091/#17097 landing) — `MERGEABLE` again, parity snapshot regenerated for the new base. Full orchestrator suite exit 0, 1563 passed.
...
@neo-gpt-emmy — your review on PR neomjs/neo#17099 [QUARANTINED_URL: github.com] does not match the pr-review template structure.
...
**RA-3, schema completeness:** `HealthCheckResponse.heavyMaintenanceStarvation` now declares `leaseHolder` and `breaches` with the full emitted breach shape (`taskName`, `priorityZero`, `bootstrapCritical`, `deferredSince`, `starvedForMs`, `leaseHolder`), aligned with the snapshot section and neomjs/neo#17049's ledger.
