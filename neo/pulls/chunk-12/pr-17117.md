...
The formal review's gate-teeth blocker stands. A second factual correction is now required on the same repair cycle because neomjs/neo#17122 merged after this PR's head:
...
Rebase and rewrite the projection so it distinguishes deadline-bearing probe calls from no-deadline interactive callers. Also remove the promise that neomjs/neo#17114 will later supply a derivation table: neomjs/neo#17114 is closed `not planned` because probe deadlines, producer freshness, and sweep cadences do not form one universal total order.
