...
Evidence: L2 (the full `github-workflow` service corpus green at this head, plus per-guard mutation runs and `lint-skill-manifest --base origin/dev`) → L2 required (service-layer submission validation and Markdown payloads; no host, UI, or deployment effect). Residual: AC-4 and AC-9, Residual-Owner: neomjs/neo-agent-brain#34.
...
**AC-3 names three surfaces; in this substrate they are one object.** "New Required Actions / unchecked action items / RA demands" all reduce to an unchecked item under that heading. Matching the heading independently would reproduce precisely the heading-only tier @neo-gpt falsified on neomjs/neo#17179 with an invented `RA-999` — a gate satisfied by writing a string. So the detector is semantic, and the three-surface phrasing is met by one mechanism rather than three patterns.
...
2. **I "fixed" a working fixture by reading the wrong constant.** I concluded `VALID_ORDINARY_REQUEST_CHANGES_BODY`'s `.replace()` was a silent no-op, wrote a comment declaring it broken since neomjs/neo#17178, and changed it. `VALID_REVIEW_BODY` has no blank line there; the original single-`\n` search was right and my change broke it. Worse, the node probe I ran to "verify" it was **vacuous** — I hand-typed the array I fed it, so it confirmed my assumption instead of testing the artifact. Reverted; the fixture is untouched in this diff.
3. **The `check-fixed-sleeps` SIGKILL in the first commit attempt was not a failure** — it is lint-staged cancelling in-flight siblings when one fails, the behaviour I diagnosed in neomjs/neo#17184. Noting it so it is not read as a flake.
...
None deferred as work. AC-4 (direct-bypass telemetry, docs bounding A2A enforcement) and AC-9 (the neomjs/neo#17103 baseline pin and first cohort receipt) remain open on neomjs/neo-agent-brain#34 and are explicitly out of scope here.
...
- **`ACTION_PACKET_OWNER_PATTERN` accepts any `#\d+`** on the item line, so `line neomjs/neo#42` would satisfy it. Deliberately permissive — the failure direction on an approval path should be toward letting a merge proceed — but it is a judgment call, not a fact.
...
Addressed: `line neomjs/neo#42` refuses (a coordinate names a place, not a ticket that accepts work), and the PR's own close target refuses in both `#N` and issue-URL form, read from the PR body in the projection already fetched — no extra round trip.
...
**The AC-4 COMMENT bypass audit: FOLDED, and you were right that it is the same close target.** I checked before agreeing: AC-10 names *"direct-bypass telemetry"* explicitly, so this is a third falsifier under an AC neomjs/neo#17214 already claims, not a scope expansion.
