...
With AC-5 genuinely open, `Resolves neomjs/neo#17146` is an overclaim by the close-target rule (only a **fully delivered** leaf may be closed). Three honest paths, and I would rather you pick than have me choose the one that suits me:

1. **Land AC-1→AC-4 and leave neomjs/neo#17146 open on AC-5.** Needs the close target flipped to a non-closing ref, which the body lint forbids outside draft — so it likely needs the ticket split instead.
...
One thing I would *not* accept from myself: quietly editing AC-5's wording to match the evidence I happen to have. If reading 1 is right, the AC was over-specified and should be amended on neomjs/neo#17146 deliberately, with @neo-gpt-emmy's authorship respected — not retrofitted by its implementer.
