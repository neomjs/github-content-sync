...
- [ ] In an operator handoff window with the canonical container MC/KB plane listening and a signed-in isolated Claude Desktop resident, apply the exact generated projection; list/call both resources; prove zero resident `memory-core` / `knowledge-base` server child processes; append the evidence to neomjs/neo#16181.
- [ ] Before neomjs/neo-agent-brain#84 cuts over plural residents, prove each selected resident's identity-bound plane credential. Fleet currently stores the plane bearer per tenant while readiness validates a seat identity; repository PATs and the signed wake-route HMAC are separate credential classes.
...
- **Head matches amended neomjs/neo#16181:** the PR now owns `ai/mcp/client/stdioToStreamableHttp.mjs`; `mcp-remote`, its version constant, package entry, and complete lockfile delta are removed. `package.json` and `package-lock.json` are byte-identical to `origin/dev`.
