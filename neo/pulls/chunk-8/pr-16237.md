...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#16229

Successor to the closed PR neomjs/neo#16232, per its terminal Drop+Supersede review: https://github.com/neomjs/neo/pull/16232#pullrequestreview-4831204658
...
Evidence: L3 (live non-destructive probe — `npm run ai:host-edge` executed against an isolated state root, startup line and authority receipt both read `host-edge`, state written outside the checkout, process terminated cleanly) → L3 required (#16229's ACs ask that a documented command start a correctly-roled host edge on a clean checkout and report its role; no destructive or operator-gated step is in scope). Residual: the macOS `launchctl bootstrap` install remains unperformed on the canonical machine — operator-gated, tracked at neomjs/neo#16210, and NOT required by this PR's close target since the portable command is sufficient for correctness.
...
| Finding (PR neomjs/neo#16232) | Disposition here |
|---|---|
...
Falsifier-first, per the method Vega recommended from neomjs/neo#16236: the reviewer's probes were written as failing specs **before** implementation. That order caught two instrument defects a fix-first order would have shipped as false green:
...
**Red-proof (the specimen is validated).** With one harness role declaration removed, the census fails with `Expected value: undefined` — *despite* the Playwright resolver injecting a valid role into the ambient environment. That is precisely the defect PR neomjs/neo#16232 would have shipped, and this suite sees it.
...
## Deltas from PR neomjs/neo#16232
...
@neo-opus-grace — your PR body on PR neomjs/neo#16237 [QUARANTINED_URL: github.com] does not match the pull-request template structure.
