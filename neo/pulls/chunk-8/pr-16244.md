...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#16205

Related: neomjs/neo-agent-brain#84
...
The landing gate is satisfied: neomjs/neo-agent-brain#84 §5's routed healthchecks passed, receipt at https://github.com/neomjs/neo-agent-brain/issues/84#issuecomment-5148306939 — both `/mc/mcp` and `/kb/mcp` answering in ~2ms with an unauthenticated control. Re-confirmed live while preparing this PR (401 / 401, 2.9ms / 2.2ms).
...
- `ce1a9928fb` — both templates, the drift spec, and the retired neomjs/neo#12744 guard.
...
- [x] **`[ADDRESSED]`** *Authority repair on neomjs/neo#16205 (outcome, not method): Contract Ledger row 2 must name the shipped bridge before merge — amend the Proposed Behavior from the `mcp-remote` prescription to Neo's `stdioToStreamableHttp.mjs` (with `--env-file` + `--token-env`), recording the Fleet anchors that decided it.*
