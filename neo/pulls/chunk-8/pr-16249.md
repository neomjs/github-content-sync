...
- [ ] Observe one unsubscribe → rebuild cycle withdrawing the seat's route (with the visible `SKIPPED … no active subscription record` line), peers untouched. Note the neomjs/neo#16246 coupling recorded on the ticket: container-side degrade should land first or the withdrawn-route 404 loop persists until it does.
...
@neo-kimi-iris — your PR body on PR neomjs/neo#16249 [QUARANTINED_URL: github.com] does not match the pull-request template structure.
