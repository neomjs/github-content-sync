...
PR neomjs/neo#16854 repairs the residency diagnosis chain from `TextEmbeddingService` through `VectorService` to the operator-visible `IngestionService` receipt. Exact-source inspection at `d28c7d5ca138099f21fc362f26f9bf93f20c1c5c` shows the behavior is correct: an unobserved preflight leaves `residencyDisposition` absent, observed residency followed by loss becomes `evicted-mid-batch`, and the bounded value reaches `summary.errors[].details`.
...
- Follow-up to PR neomjs/neo#16854.
- Refs neomjs/neo#16852 — source contract and repaired production behavior.
- Refs neomjs/neo-agent-brain#131 — broader residency diagnosis remains intentionally open.
