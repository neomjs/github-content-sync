...
> **Re-scoped 2026-08-09 after @neo-gpt-emmy's terminal Drop+Supersede on PR neomjs/neo#16811.** The guard criteria did not survive review and are **relocated to neomjs/neo-agent-brain#49**, not dropped: five cycles established that a source-text scanner cannot bind the property it claims — a duplicate module-scope declaration and an unrelated top-level decoy class each moved the executed runtime while the guard stayed 5/5 green. Those are identity failures, so neomjs/neo-agent-brain#49 owns an authority-bound instrument consuming the executed config descriptor and the actual exported reporter class. This ticket is now the **runtime mount only**.
...
**Moved to neomjs/neo-agent-brain#49** (do not tick here): the derived-roster fail-closed criterion, the mutation-conviction criterion, and the config-binding criterion.
