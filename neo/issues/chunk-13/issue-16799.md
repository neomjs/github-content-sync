...
Sub of neomjs/neo-agent-brain#64. Closes its open AC *"`KB_REVISION_BOUNDARY_UNAVAILABLE` is root-caused, or explicitly moved to a sibling ticket with its evidence"* on the **disposition** half; @neo-opus-vega owns the successor that wires a real resolver.
...
### Why neomjs/neo#16717 did not cover it, and why that generalizes
...
2. **Capability absent** ⇒ push to `summary.notices` under a **distinct code** `KB_REVISION_BOUNDARY_CAPABILITY_UNWIRED`, with a message that states the capability is not wired and **stops citing closed neomjs/neo#11637**.
...
- [ ] No emitted string cites neomjs/neo#11637 as pending.
...
- **Inferring a cause from a signature.** The premise of this ticket was itself wrong once: a prior revision of the neomjs/neo-agent-brain#54 runbook attributed this exact `consecutiveFailures` pattern to neomjs/neo#16717 and prescribed a rebuild. A rebuild at `dev` head reproduced it. The signature is shared; only `lastSourceErrorCode` separates the causes.

## Related

- Parent: neomjs/neo-agent-brain#64 · successor (wire a real resolver): @neo-opus-vega, to be filed
- neomjs/neo#16717 — fixed the *other* cause of this signature · neomjs/neo#11637 — CLOSED, cited by the message as pending
- neomjs/neo-agent-brain#54 — operator runbook Step 2.5 (the discriminator this must not break) · neomjs/neo#16795 — sibling swallowed-signal defect
