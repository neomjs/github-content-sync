...
- neomjs/neo#16630, neomjs/neo#16763 — where the stale Golden Path was noticed while working an unrelated lane.
- neomjs/neo-agent-brain#149 — tenant-source inventory; the durable ingestion target this must not complicate.
...
**The freeze resolved.** The banner's condition was *"D#16794 Decision A folds and names the projection owner."* It did: `[GRADUATED_TO_TICKET: neomjs/neo#17627]` (@neo-opus-vega, 2026-08-23), composing A6 + B5 + **C1 truthful per-facet receipts** + Decision D.

**The salvaged invariant was re-homed, exactly as the freeze promised.** neomjs/neo#17627's own scope section says it outright:

> *"#16795's non-fatal-but-observable invariant salvages into the same contract."*

**And neomjs/neo#17627 is CLOSED.** So the surviving content of this ticket shipped under its successor.
...
- `:136` goes further than the original ACs: *"A facet that never RAN is still a facet that did not advance, and it has to appear in the [verdict]"* — which closes the adjacent gap neomjs/neo#17627 named, where a disabled task is indistinguishable in every log surface from one that ran and found nothing.
...
**Disposition: closed as superseded by neomjs/neo#17627.** No follow-up ticket — there is no residual. Implementing this as written would have mutation-pinned the coupling that D#16794 existed to remove, which is what the freeze correctly prevented; the freeze did its job and the ticket can now retire rather than be re-litigated.
