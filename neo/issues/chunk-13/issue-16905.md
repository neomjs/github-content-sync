...
A filtered unit order reproduced one orchestrator spec leaking its `deploymentStateBridge.snapshotPath` value into an unrelated MCP smoke assertion. The exact failing arm was `1 failed / 2080 passed`; replacing this spec's ineffective cleanup with the shipped resolved-value capture made the same arm `2097 passed` in PR neomjs/neo#16901.

This leaf records only that delivered repair. Broad worker-local isolation and mutation-shape enforcement remain owned by neomjs/neo-agent-brain#46 and neomjs/neo-agent-brain#89.
...
- Eliminating every shared `AiConfig` mutation across the test tree; neomjs/neo-agent-brain#89 owns that population and its mechanical guard.
- Closing neomjs/neo-agent-brain#46, which retains the broader worker-isolation and unresolved mechanism work.
...
- Do not widen this one-file repair into subtree-replacement cleanup; those destructive shapes remain neomjs/neo-agent-brain#89 scope.

## Related

- neomjs/neo-agent-brain#46 — broader filtered-run isolation carrier; remains open
- neomjs/neo-agent-brain#89 — assignment-shape enforcement and remaining AiConfig test pollution
- PR neomjs/neo#16901 — delivered implementation
- ADR-0019 — reactive Provider single-source-of-truth authority

Origin Session ID: ff601885-0018-437a-af95-6fea47a186b9

Retrieval Hint: `PR neomjs/neo#16901 snapshotAiConfig DeploymentStateBridgeService filtered unit pollution`
