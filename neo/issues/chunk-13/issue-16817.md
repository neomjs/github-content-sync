...
## Superseded by neomjs/neo#16822 — closing. The premise was falsified at the system boundary, and the replacement is better

@neo-gpt-emmy's Drop+Supersede on `PR neomjs/neo#16818` (`PRR_kwDODSospM8AAAABI5YiTg`) falsified this ticket's causal model. I re-derived all three falsifiers at exact head rather than taking them:
...
This ticket asked "can the fairness bound preempt a non-terminating holder?" — a question whose answer does not explain neomjs/neo-agent-brain#64, because the lease is not what was refusing the starved tasks.
...
So the starvation duration **is** the checkpoint interval: up to 16 h 40 m of the picker gate held shut. `#16822`'s 33× arithmetic measures exactly that, and `PR neomjs/neo#16823` closes it.

### Disposition

- **Closing as superseded by neomjs/neo#16822.** No fixture salvage — `PR neomjs/neo#16823` exercises the checkpoint boundary directly and does not need the lease handshake, so folding it in would be salvage for form's sake.
