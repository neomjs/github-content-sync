...
Sub of neomjs/neo-agent-brain#64, carrying its **AC-5** (*"The underlying error is surfaced rather than wrapped… `KB_TENANT_REPO_SYNC_EMPTY_MATERIALIZATION` must distinguish effect-without-receipt from a genuinely empty envelope"*). Split out because neomjs/neo-agent-brain#64 is an `epic` and ACs belong in its subs, and because one PR can carry only one `Resolves`.
...
- `IngestionService.persistManifestSnapshot` already emits a rich `logger.warn` naming which branch left the receipt null. It runs in **kb-server**; the code is raised in the **orchestrator**. Two containers — and neomjs/neo-agent-brain#64's own *Facts* section records that channel failing: *"`create-app`'s 2026-08-06 `EMPTY_MATERIALIZATION` evidence is unrecoverable… Container stdout retains nothing."* The discriminator existed only in the channel already proven not to survive.
...
neomjs/neo-agent-brain#64 (parent — carries AC-5) · neomjs/neo#16551 (reporting overlap) · neomjs/neo#16045 (the receipt contract these arms enforce)
