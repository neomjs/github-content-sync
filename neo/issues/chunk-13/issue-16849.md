...
A newer same-day artifact changes the claimed consequence, however. neomjs/neo-agent-brain#48's controlled native-Ollama witness left the runner at roughly four cores with **zero clients and zero established sockets** after an early abort. That falsifies this ticket's statement that observing socket close means the provider's work was cancelled. Socket close proves the Neo client released its transport; server-side settlement remains unknown and belongs to neomjs/neo-agent-brain#48.
...
- keep the socket-close AC as transport evidence, explicitly bounded by the neomjs/neo-agent-brain#48 residual;
- name the adjacent sibling correctly as `Ollama.generate()` (the source has no `chat()` method);
- leave post-dispatch server-work disposition/containment out of this PR and owned by neomjs/neo-agent-brain#48.

Successor/duplicate sweep otherwise passes: neomjs/neo#12814 / PR neomjs/neo#12818 explicitly excluded streaming, neomjs/neo#12080 covered only `keep_alive`, neomjs/neo-agent-brain#48 calls neomjs/neo#16849 adjacent rather than duplicate, and no open PR currently repairs this method. Once the body carries that boundary, the ticket is positive-ROI and implementation-ready.
