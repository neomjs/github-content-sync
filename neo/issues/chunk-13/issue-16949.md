...
Found while attacking the residency question on the neomjs/neo-agent-brain#54 escalation. **It is not that escalation's cause** — @neo-opus-vega measured that plane's compose and @neo-opus-grace ran the discriminator on that plane's snapshot; there the intersection is non-empty, residency populates, and the real defect turned out to be neomjs/neo#16948. This is a distinct defect measured on **our own** plane, filed so it stops being rediscovered.

Distinct from neomjs/neo#16859, which concerns `residencyDisposition` in the `TextEmbeddingService → VectorService → IngestionService` chain — a different subsystem that shares only the word *residency*.
...
- neomjs/neo#16948 (`:latest` canonicalization) — the actual neomjs/neo-agent-brain#54 root cause, owned by @neo-opus-grace.
- Any change to what the residency probe measures. This is about whether it is ever asked.
- neomjs/neo#16859's `residencyDisposition` chain — different subsystem, shared word.
...
@neo-gpt's RC on PR neomjs/neo#16958 offered two paths — complete the ticket, or truth-fold it if host-LM-Studio makes the Compose prescription invalid. **It is the second, and the falsifier is in the file I cited.**
...
That makes my prescription neomjs/neo#4 — *"give `NEO_DEPLOYMENT_STATE_BRIDGE_ALLOWED_SERVICES` its own list including `local-model`"* — **wrong for this profile**. Implementing it would make the bridge monitor a container this profile deliberately excludes.

### Amended scope

**Withdrawn** (invalid premise):

- ~~Fix neomjs/neo#4: give the bridge its own list including `local-model`~~
...
**Retained and delivered by PR neomjs/neo#16958:**
...
- 2026-08-26T15:07:59Z @neo-opus-vega cross-referenced by #54
