...
- `ai/daemons/shared/fileLease.mjs` (merged neomjs/neo#16242) solves the same namespace-blindness for the authority lease with heartbeat-TTL. Option (b) below names why this ticket does **not** start there: the heavy-maintenance lease's `staleAfterMs` semantics deliberately allow long work under one owner (renew-or-lose), which is a different liveness contract from the authority lease's poll-cadence heartbeat — unification is a deeper alignment, not this fix.
- Precedent shape: the epoch guard (`CURRENT_PROCESS_STARTED_AT`) was added to the same module by PR neomjs/neo#16220; this extends that exact pattern one field over.

## The Fix

**(a) Boot-identity discriminator in the lease payload (recommended, this ticket).**

1. `buildLeasePayload` gains `bootId` (default `os.hostname()`; injectable for specs). Additive field — no schemaVersion, matching the receipt-side precedent of neomjs/neo#16241's additive integrity field: old payloads simply lack the field.
...
| same-boot discrimination | `CURRENT_PROCESS_STARTED_AT` epoch guard (PR neomjs/neo#16220) | unchanged | — | — | existing epoch specs stay green |
...
- **schemaVersion bump on the payload** — condemns every pre-field lease on disk; the additive field is the neomjs/neo#16241 receipt-side precedent.

## Related

- neomjs/neo#16210 — the window specimen + this intake's receipts ([issuecomment-5151446195](https://github.com/neomjs/neo/issues/16210#issuecomment-5151446195))
- neomjs/neo#16242 / PR neomjs/neo#16242 — the merged authority-lease TTL family (compose-not-duplicate boundary)
- neomjs/neo#16208 — the gap-0 window that produced the specimen
- PR neomjs/neo#16220 — the epoch guard this extends
- neomjs/neo-agent-brain#84 — the migration's cleanup series (rebuild lane owner)
...
- 2026-08-01T13:07:01Z @neo-opus-vega cross-referenced by #84
