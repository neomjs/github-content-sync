...
Incident owner: neomjs/neo#16208 (corrected root cause + the v4.1 recovery plan). This leaf owns exactly the compose mount contract and its authority surfaces — fully delivered by one PR. The recovery execution (quiesced physical seed, recreate-survival proofs, reconciliation receipts) stays on neomjs/neo#16208 and must NOT auto-close with this leaf.

Duplicate sweep: neomjs/neo#16208 is the incident (open, non-closing here); neomjs/neo#16206 is deploy layering; neomjs/neo#16219 (Docker-owned volumes) is closed. No open ticket owns the mount contract itself.
...
- [ ] The diff is inert for the live plane (no recreate is triggered by merging); all recovery receipts remain on neomjs/neo#16208.

## Out of Scope

- The v4.1 recovery execution — neomjs/neo#16208 owns it.
- Overlay compose files (inherit the corrected base definitions; introduce no storage override — reviewer-verified on PR neomjs/neo#16252).
- Server-side `NEO_CHROMA_DATA_DIR` / `engines.chroma.dataDirProd` host-path semantics — a different contract.
- Deploy layering — neomjs/neo#16206.

## Related

- neomjs/neo#16208 — incident owner (corrected root cause + v4.1 execution receipts); related, non-closing.
- neomjs/neo-agent-brain#84 — the migration epic.
- neomjs/neo#16206 — deploy layering.
...
- 2026-08-01T11:14:14Z @neo-opus-vega cross-referenced by #84
