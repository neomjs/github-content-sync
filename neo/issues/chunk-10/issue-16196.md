...
| Existing `belief.stillOpen` and `belief.falsified` buckets | Parent neomjs/neo-agent-brain#85 safety contract | Classify a state only when the aliased row's numeric `number` exactly matches the submitted number; only exact `CLOSED` or `MERGED` rows enter `falsified` | Missing, malformed, wrong-number, contradictory, unknown, or provider-error rows remain `unverifiable` | No bucket-shape change | Exhaustive partition test across open, terminal, missing, wrong-number, unknown, contradictory, and unmapped-error rows |
...
- The OpenAPI array-constraint compiler repair tracked separately in neomjs/neo#16194.
...
Parent: neomjs/neo-agent-brain#85

Successor to neomjs/neo#16191

Related: neomjs/neo#16192, neomjs/neo#16194, neomjs/neo#16195
...
**Age / successor risk:** created `2026-07-30T21:44:37Z`, updated `2026-07-30T21:55:32Z`; `pre-stale` under the repository's 90-day stale / 14-day close workflow, with neither `stale` nor `no auto close`. The self-authored drift probe intersected three declared surfaces, so the full intake ran. Merged PR neomjs/neo#16195 is the adjacent schema-constraint/compiler repair for neomjs/neo#16194; current source proves it did not deliver this error classifier, and no merged/open PR closes neomjs/neo#16196.

**Contract / duplication:** every Contract Ledger anchor still exists with the prescribed old behavior; the parent neomjs/neo-agent-brain#85 relationship is live and is not `epic`-labeled. Ticket-focused KB retrieval found neomjs/neo#16196 but no competing owner; three Memory Core framings were a clear miss; live issue/PR search found no duplicate or active implementation.
