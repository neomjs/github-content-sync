...
Issue neomjs/neo#16184 and PR neomjs/neo#16190 hard-cut Fleet's public target selection to canonical `mcpTarget` intent. The first landing included a deliberately temporary read-path translator in `FleetRegistryService.mjs` for a hypothetical persisted `mcpTransport` row.
...
| `FleetRegistryService.readRegistry()` | Canonical persisted definition shape from neomjs/neo#16184 | Hydrate current definitions directly; no retired-row translator or rewrite branch | Existing missing/corrupt-file behavior remains unchanged | Existing source JSDoc | Focused registry hydration tests |
| Public define/configure boundary | Registry + Control Bridge | Retired target-as-transport input remains rejected | Controlled domain rejection; no silent resident degradation | neomjs/neo#16184 | Direct registry + real-wire tests |
...
- Reworking the `mcpTarget` shape, resolved plan, harness adapters, or AgentOS UI delivered by neomjs/neo#16184.
...
None. This completes the deletion boundary already prescribed by neomjs/neo#16184 and remains aligned with the canonical `stdio` / `streamable-http` vocabulary established by neomjs/neo#15188 and PR neomjs/neo#15191.

## Related

Successor to neomjs/neo#16184

Related: neomjs/neo#16190, neomjs/neo#15188, neomjs/neo#15191
...
- 2026-08-25T15:40:42Z @dawesi referenced in commit `7178993` - "fix(fleet): remove unexercised target migration (#16189) (#16198)"
