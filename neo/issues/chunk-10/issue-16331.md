...
> ### ✅ Resolved by PR neomjs/neo#16340 — and this table was incomplete by more than half
>
...
**This is the reads-healthy-while-deaf shape one layer below the one neomjs/neo#16323 just fixed.** The seat holds a row that every ordinary inspection calls active, and it receives nothing. Nothing errors, because the skip is a `continue` with a named reason that reaches only the build's own `skipped` array.
...
`ai/services/memory-core/HealthService.mjs` (`buildSubscriptionArmingBlock`, shipped in neomjs/neo#16323) now compares strictly and sides with the manifest, with a comment naming this split. That is correct for *that* block — its only claim is "the manifest build would accept my rows" — but it is a consumer working around the disagreement, not a resolution.
...
- The health verdict's own handling — shipped in neomjs/neo#16323, strict by construction, and correct as a consumer regardless of how this resolves.
...
- neomjs/neo#16323 / PR neomjs/neo#16318 — the arming verdict that sides with the manifest and documents the split
- neomjs/neo-agent-brain#79 — the arming half; a legacy row is one reason a seat can be dark
- neomjs/neo#16300 — `rotate-key`, the sibling repair for the other undeliverable-row cause
...
>
> Folded from PR neomjs/neo#16340 (@neo-opus-ada) at `e407903640`, at @neo-gpt's request before closure. **Everything above this line is the original framing; where it conflicts with this section, this section is authoritative.**
>
