...
Parent neomjs/neo-agent-brain#85 was created after a stale board claim said four pull requests remained merge-eligible when three had already merged. Its first code-ready leaf, neomjs/neo#16165 / PR neomjs/neo#16170, now gives every `list_pull_requests` row the current merge, review-request, head, and merge-state fields.

The remaining parent prescription is still too broad. Live source and Euclid's intake comment on neomjs/neo-agent-brain#85 prove that a timestamp cannot reconstruct historical aggregate `reviewDecision` or `headRefOid`, and a bounded open page cannot truthfully derive exhaustive `unlisted` membership. Neither `updatedAt` nor an empty delta may substitute for unavailable history.
...
- live latest-20 open issues and GitHub title/body search found neomjs/neo-agent-brain#85 only;
- open-PR search found no competing branch;
- repository issue/discussion archive and Knowledge Base search confirmed neomjs/neo-agent-brain#85 is the broad parent, while neomjs/neo#16165 explicitly leaves this leaf out of scope;
...
- The current default call shape delivered by neomjs/neo#16170 must remain unchanged when no belief is supplied.
...
| Default list response | neomjs/neo#16165 / PR neomjs/neo#16170 | unchanged when `believedOpen` is absent | existing structured GraphQL error | existing docs | exact regression assertion |
...
- [ ] With `believedOpen` absent, the captured query, variables, and returned response remain unchanged from PR neomjs/neo#16170.
...
- Rewriting the freshness skills; parent neomjs/neo-agent-brain#85 retains that final alignment after the instrument shape converges.
...
None. This is an additive, stateless projection on the existing GitHub Workflow read operation, aligned with neomjs/neo#16126's derive-from-caller-coordinate precedent and the no-new-tool constraint in neomjs/neo-agent-brain#85.

## Related

Narrow leaf of neomjs/neo-agent-brain#85 · field-parity predecessor neomjs/neo#16165 / PR neomjs/neo#16170 · source-owned projection precedent neomjs/neo#16029 / PR neomjs/neo#16126.
...
- 2026-08-21T00:31:55Z @tobiu referenced in commit `7432fb7` - "feat(github): classify believed-open pull requests (#16191)"
- 2026-08-21T00:31:55Z @tobiu referenced in commit `ca05dbc` - "fix(github): mechanize believed-open schema constraints (#16191)"
