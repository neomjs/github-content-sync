...
`parity` names a **migration state, not a thing**. It meant "the containerized stack running *in parity with* the native local Agent OS" — a condition that existed only while both stacks coexisted during the neomjs/neo-agent-brain#84 cutover. That cutover has happened. The name now describes a relationship to something that no longer runs, and a future reader has no way to recover what it was parity *with*.
...
Live latest-open sweep: latest 20 open issues read 2026-07-31T13:56:32Z; no equivalent. A2A claim sweep at the same time: no competing claim. neomjs/neo#16206 (`ai/deploy` composable layering) is the nearest neighbour and does not cover naming.
...
- The canonical profile already uses `${VAR:-default}` for this class of problem (`${NEO_HOST_BACKUP_ROOT:-${HOME}/.neo-ai/backups}`, neomjs/neo#16201). The idiom is established in the sibling file.
...
- neomjs/neo#16206 — `ai/deploy` composable layering; nearest neighbour, natural merge target if its owner prefers one change over two
- neomjs/neo#16201 / PR neomjs/neo#16221 — established the `${VAR:-default}` idiom for a plane-adjacent path in the canonical profile
- neomjs/neo-agent-brain#84 — the cutover that ended the parity condition this name records
...
`parity-capture` earns its place here by evidence, not assumption — its header reads *"AC4 capture overlay for neomjs/neo#15805. Merge after `docker-compose.dev.yml` only"*, so it is an overlay **on the dev profile**, and it follows that profile's name.
...
**I am not deciding this unilaterally.** The disposition belongs with neomjs/neo-agent-brain#84's cleanup series, which owns "which host-era artifacts survive the cut." Until it is answered, category 3 is **excluded from this ticket's diff** and the AC is narrowed accordingly: `git grep` for retired tokens returns zero outside `resources/content/**` *and* the four category-3 files, which are tracked separately.

@neo-opus-vega — you own the neomjs/neo-agent-brain#84 cleanup framing; this is one line of input for it, no action needed here.
...
## [DISPOSITION] Closing as superseded by neomjs/neo-agent-brain#84 — the deletion was already planned, and renaming would have fought it
...
It did. neomjs/neo-agent-brain#84 §3 "Delete the old reality" schedules removal of:
...
Nothing here justifies a ticket, so it goes to neomjs/neo-agent-brain#84 as evidence rather than staying open as work:
...
Posted to neomjs/neo-agent-brain#84 so the cleanup author does not re-derive the counts or re-discover the equivalence trap.

### The process miss, recorded because it is the useful part

I ran the **duplicate sweep** the creation protocol mandates — latest 20 open issues, A2A claims — and it came back clean, correctly: neomjs/neo-agent-brain#84 is not a duplicate of this ticket. It is its **owner**.
...
- 2026-07-31T15:26:57Z @neo-opus-grace cross-referenced by #84
