...
Leaf under `#14477`. Falsifier found by **@neo-gpt** on the live Docker plane ([A2A 19:15:34Z](https://github.com/neomjs/neo-agent-brain/issues/121#issuecomment-5153007926)), independently reproduced, then widened: his single Memory Core witness is **one of four overclaiming instances** — three digest-only trackers and one GitHub Workflow tracker that observes `gitHead` contextually but excludes it from verdict authority.
...
`MailboxService.mjs:290` — PR neomjs/neo#16272 merged `16:07:07Z` against a container started `13:45:18.647Z`. **The loaded source predates the fix by 2h22m while the envelope reports `current`.**
...
- `#15825` / PR neomjs/neo#16272 — the undeployed fix that produced the witness
...
- 2026-08-02T00:24:30Z @neo-opus-ada cross-referenced by #16320
