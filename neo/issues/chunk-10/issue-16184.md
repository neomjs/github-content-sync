...
blockedBy: []
blocking:
  - '[x] 16189 Remove the one-shot Fleet MCP target migration'
...
During neomjs/neo#16181, the Fleet vocabulary exposed a semantic collision:
...
- Neo's actual MCP wire vocabulary is `stdio` or `streamable-http`. PR neomjs/neo#15191 / neomjs/neo#15188 deliberately established `streamable-http` as the canonical server-side name and rejects the old ambiguous server value.
...
- GitHub title/body search for Fleet MCP placement/target/transport naming: neomjs/neo#16181 only, where this cleanup is explicitly out of scope;
- Knowledge Base concept walk and team Memory Core sweep: recovered the existing Fleet implementation and neomjs/neo#15191 naming decision, but no successor ticket;
...
- Neo's MCP client/server transport vocabulary is `stdio` / `streamable-http`, established by neomjs/neo#15188 / PR neomjs/neo#15191.
...
| Documentation | neomjs/neo#15191 canonical transport decision + Fleet target model | distinguish target, transport, and vendor spelling | no `remote-http` protocol claim | source/docs sweep |
...
- Reworking the neomjs/neo#16181 Claude Desktop bridge implementation or its lifecycle/security tests.
...
No ADR amendment is expected. This enforces the canonical transport vocabulary already established by neomjs/neo#15188 / PR neomjs/neo#15191 and clarifies the existing Fleet ownership layers. Revalidate if implementation changes public credential ownership or introduces a new transport protocol.

## Related

Naming authority neomjs/neo#15188 / merged PR neomjs/neo#15191 · original Fleet remote target neomjs/neo#15805 / merged PR neomjs/neo#16053 · bridge consumer neomjs/neo#16181 / PR neomjs/neo#16182 · hard cut neomjs/neo-agent-brain#84.
