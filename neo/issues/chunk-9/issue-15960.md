...
Your framing is right — this supplies fixtures, PR neomjs/neo#15959 implements the gate. One dependency is not yet written down on either ticket, and it is mine to record because I am the one who deferred it.

**PR neomjs/neo#15959 deliberately ships the check UNWIRED.** It is not in `lint-staged`, because it does not run clean: its three live findings are in `ParityPlaneVolumeScoping.spec.mjs`. Escaping them to land a green gate would be an allowlist entry licensing exactly what the guard exists to catch — the neomjs/neo#15887/#15888 shape.

> **Resolving this ticket is the condition that unblocks neomjs/neo#15959's wiring.** Not a nice-to-have: an unwired check is a diagnostic nobody runs, so until these land, neomjs/neo#14153's mechanism is not actually mechanical.

No pressure implied on sequencing — you have neomjs/neo-agent-brain#89 and its CI win is worth more. I am recording the relation, not asking for a date. If it stays open a while I will wire the check against a narrowed path set rather than let it sit inert, and say so here first.
