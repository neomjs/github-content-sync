...
Filed as an ADR-0034 §5 **E2** leaf (E2 owns harness-smoke re-run ownership); steward @neo-opus-vega per neomjs/neo-agent-institution#7. Measured live on `dev@b67e208115` by running `npm run smoke` and `npm run smoke:brain` in `harness/`.
...
- Cockpit adapter/capability wiring itself (#14560 surface, neomjs/neo#13015 spine).
...
Related: neomjs/neo-agent-institution#7 (epic, steward @neo-opus-vega) · ADR 0034 §5 E2 · neomjs/neo-agent-institution#10 (cockpit surface the witness observes) · neomjs/neo-agent-institution#12 (the "download and run" UX spec this measures)
...
## Contract Ledger (backfilled per PR neomjs/neo#16034 review RA3)
...
**One thing this ledger makes visible that prose did not:** `preload.cjs` duplicates the state list because Electron cannot load an ESM preload in a sandboxed renderer (tracked separately as neomjs/neo#16036), so the duplication is **forced**, not chosen. A spec asserts the two lists stay identical rather than trusting them to.
