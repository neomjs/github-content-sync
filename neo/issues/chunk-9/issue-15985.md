...
Surfaced as the last remaining victim of neomjs/neo-agent-brain#89's order-dependent pollution sweep, but it is **not** a test-isolation defect — the test failure is the symptom of a product bug, which is why it lands here rather than riding neomjs/neo-agent-brain#89's frame.

After PR neomjs/neo#15980 closed the receipt-durability fixture class, one victim survived. Re-verified at post-#15980 `dev` (`e3bd3243c8`) rather than assumed:
...
This is the same family as the closed precedents neomjs/neo#10174 (*Mailbox SENT_TO edges silently culled by linkNodes FK check*) and neomjs/neo#10284 (*MailboxService.addMessage silently succeeds when routing edges are culled*), both resolved by adding post-`linkNodes` verification rather than by changing the guard.
...
- [ ] `GoldenPathSynthesizer.spec.mjs:1482` passes in the polluted ordering, closing neomjs/neo-agent-brain#89's last victim.

## Out of Scope

- Changing `linkNodes`' FK-guard semantics globally — the guard is correct; silent culling at *call sites* is the defect, and the precedent chain (#10174 / neomjs/neo#10284) fixes it per-caller. A global throw would have blast radius across every caller and belongs in its own ticket if ever wanted.
- Seeding `'frontier'` unconditionally from `GraphService` init — that trades a silent-loss bug for a boot-order coupling.
- neomjs/neo-agent-brain#89's config-mutation and destroy-before-`initAsync` axes, which PR neomjs/neo#15980 and its parent already own.
- Prune policy for hub nodes — neomjs/neo#15973's territory; this ticket only notes prune as a reachability route.
...
- neomjs/neo-agent-brain#89 (parent sweep; this closes its last victim) · PR neomjs/neo#15980 (the fixture-class half, merged)
- neomjs/neo#10174 · neomjs/neo#10284 (closed precedents, same silent-cull family, per-caller remedy)
- neomjs/neo#15973 (ambient decay floor below prune threshold — the prune reachability route)

Live latest-open sweep: checked latest 20 open issues (created-descending) at 2026-07-26T12:12:05Z; no equivalent found. A2A in-flight claim sweep: 30 most recent messages across all read-states; active claims in the herd window are neomjs/neo#15906, neomjs/neo#15807 and neomjs/neo#15984 — none overlapping this scope.
