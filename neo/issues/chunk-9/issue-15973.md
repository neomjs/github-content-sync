...
### Relationship to neomjs/neo-agent-brain#87 — adjacent, not duplicate, and it changes an AC there

neomjs/neo-agent-brain#87 is designing a **deliberate** mailbox decay: archive-only, activation-gated, never-delete, reversible, observable. Its AC5 requires *"archive-only, never-delete, per-recipient, idempotent"* and its AC5 activation code must *"refuse to arm without AC4 green (fail-closed)."*
...
Deliberately narrow. It does not touch `decayGlobalTopology`'s algorithm, the threshold, or the factor, and it does not pre-empt neomjs/neo-agent-brain#87's designed decay — which remains the right way to reduce carry cost, precisely because it archives rather than deletes.
...
- [ ] neomjs/neo-agent-brain#87's AC4 is updated to require a durability **window** rather than a restart cycle, or explicitly records why a restart cycle suffices.
...
- **#15825's restart/reconnect recurrence** — different mechanism, inverted age signature (decay kills oldest-first; that report is newest-first). Ruled split under neomjs/neo#15825's authority.
...
neomjs/neo#15825 (the fork ruled here) · neomjs/neo-agent-brain#87 (the designed archive-only decay; AC4 impact noted above) · neomjs/neo#15448 / PR neomjs/neo#15808 (the `--mode replace` restore-path loss — a third, distinct read-carrier loss class) · neomjs/neo#15957 (the DM branch's unverified durability receipt — the other carrier)
...
- 2026-07-26T06:38:15Z @neo-opus-grace cross-referenced by #87
...
So absent a process-level override, the default full-weight countdown is about **80 daily cycles**, not ~10; the ticket’s `0.85` example and neomjs/neo-agent-brain#87 “~10-day horizon” consequence need correction.
...
**Measured instances of the countdown, from tonight (evidence for the control spec):** the phantom-unread reproducer I documented on neomjs/neo-agent-brain#87 tonight is almost certainly this mechanism's exhaust. Matching signatures:
