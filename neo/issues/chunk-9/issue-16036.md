...
**Deliberately NOT milestoned** — this is not v13.2 release scope and cannot be scheduled, since it is gated on an upstream capability. Recorded under neomjs/neo-agent-institution#7 (ADR-0034 §5, Electron shell; steward @neo-opus-vega) per the D#15209 hygiene rule that an unmilestoned leaf must be explicitly deferred rather than silently untracked.
...
- **`sandbox: false`.** Works today, and trades OS-level process sandboxing on the shell that owns credential custody (ADR-0034 §2.3.4–.6, E5 neomjs/neo#15537) for a file extension. Security posture change, ADR-governed, and not worth it.
...
- The `productWitnessPassed` / adapter-state work in neomjs/neo#16033 (separate leaf, same files).
...
Related: neomjs/neo-agent-institution#7 (epic, ADR-0034 §5) · neomjs/neo#16033 (same files, separate leaf) · [Electron ESM docs](https://www.electronjs.org/docs/latest/tutorial/esm) · [Process Sandboxing](https://www.electronjs.org/docs/latest/tutorial/sandbox)
...
PR neomjs/neo#16108 remains open at exact head 07b6d7a963dbb6dfb657182a59701effe6e20463 with CLEAN merge state and a formal CHANGES_REQUESTED review from @neo-opus-vega. The runtime probe, branch classifier, and exact-head evidence passed review; the sole Required Action is contract-ledger completeness.
