...
- PR neomjs/neo#16110 initially published exact head `cc7090f85d04c7aa6e49609b2b785be039d5bea5` as `fix(memory-core): make summary receipts replayable (#16105)`. The patch adds a durable replay capability, so the operator challenged the type and the branch was corrected to `feat` before review.
...
This is not the general commit-safety problem tracked by neomjs/neo-agent-brain#168, and it is not the syntax-only Conventional Commits RFC from neomjs/neo#3191. The failure is the semantic mapping from delivered delta to type.
...
4. Add regression fixtures for the `#10061` (`chore` → `feat`) and PR neomjs/neo#16110 (`fix` → `feat`) failure shapes, plus valid `fix` and `chore` controls.
...
|---|---|---|---|---|---|
| Agent type-selection decision | `pull-request-workflow.md` §3.1 | One ordered `capability` / `restoration` / `zero delta` decision | Fail the preflight question closed; do not guess from ticket labels | Rewrite the existing conditional payload section | Regression examples from `#10061` and PR neomjs/neo#16110 |
...
- [ ] Regression fixtures cover the exact `#10061` and PR neomjs/neo#16110 misclassification shapes.
...
- Implementing the broader autonomous CommitGate from neomjs/neo-agent-brain#168.
- Further changes to PR neomjs/neo#16110, which is already corrected.

## Avoided Traps

- **Infer from the ticket label.** PR neomjs/neo#16110 is a `bug` ticket whose delivered delta is still a new replay capability.
...
Targeted GitHub, repository, and Knowledge Base sweeps found neomjs/neo#10061, neomjs/neo#3191, and neomjs/neo-agent-brain#168 as adjacent prior art, not active duplicates.

## Related

Related: neomjs/neo#10061
Related: neomjs/neo#3191
Related: neomjs/neo-agent-brain#168
Related: neomjs/neo#16105

PR evidence: neomjs/neo#16110

Origin Session ID: 019fa530-53d6-7271-bf05-51497720b29c

Handoff Retrieval Hints:
- `query_raw_memories("PR neomjs/neo#16110 fix feat commit type correction friction")`
...
| `validateChangeClass({changeClass, commitSubject, prTitle})` | Maps only own keys `capability`, `restoration`, and `zero-delta` to `feat`, `fix`, and `chore`; every supplied subject is checked against the same map result | Returns explicit errors; never inspects GitHub labels, files, or diff size | Exact `#10061` `chore` and PR neomjs/neo#16110 `fix` regression fixtures; valid `feat`/`fix`/`chore` controls |
...
- 2026-08-02T10:31:52Z @neo-gpt cross-referenced by PR #16335
