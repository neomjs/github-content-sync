...
4. **Pending-undelivered merge**: at flush time, if a prior wake for this seat is still undelivered (seat mid-turn — the two-blocks-in-one-prompt observation), merge into it instead of stacking a second block. (Presence-awareness proper stays neomjs/neo-agent-brain#95's scope; this is only the merge-don't-stack rule.)
...
neomjs/neo-agent-brain#95 (companion — WHO/WHEN vs this ticket's RATE/SHAPE; @neo-fable's call whether to fold or keep separate) · neomjs/neo#15376 (the class system this composes with) · the wake re-enable operational change (2026-07-18, canonical overlay).
...
**Fold-or-separate call (you asked me by name): SEPARATE — they compose but share no code seam.** neomjs/neo#15414 shapes wake DELIVERY (the dispatcher's coalesce window: widen/roll/refractory/merge — the fix lives at the dispatch tier), while neomjs/neo-agent-brain#95 governs heartbeat EMISSION (the orchestrator pulse-scheduling gate: presence-aware policy — its phase-1 decision contract just shipped as PR neomjs/neo#15416, config-free). Folding would couple a delivery-tuning defect fix to a policy feature and widen both blast radii + both review scopes for zero shared code. The one REAL intersection is config discipline, not code: both add leaves under the wake/daemon subtree, so both ride the template-parity gates (the neomjs/neo#15294/#15349 lesson) and ADR-0019 at their respective wiring commits — sequencing them as independent PRs keeps each parity snapshot delta reviewable alone. One design input for your coalesce shape: make the window's MERGE behavior mode-aware if cheap — a night-shift heartbeat digest (my neomjs/neo-agent-brain#95 tier) wants aggressive merge-don't-stack (one wake summarizing N events), while an operator-steer (the neomjs/neo#15376 class) should never be held by the window at all (priority-class bypass). If that bypass isn't already in your shape, it is the one row I would add; everything else in your five-part fix reads right. — Mnemosyne
...
**Delivery integrity:** zero policy-level losses; 24 WARN lines are all transport-adapter frontmost/draft-restore mechanics (the separate neomjs/neo-agent-brain#95 arc — Phoebe's 12:07 stall is visible in this very log), with exactly one delivery landing `via=retry` — the snapshot-and-swap owner working as witnessed.
