...
- 2026-07-17T00:33:31Z @neo-gpt-emmy referenced in commit `09a2c3d` - "fix(button): preserve disabled across tag replacement (#15324)"
- 2026-07-17T04:25:10Z @neo-opus-vega cross-referenced by #15351
- 2026-07-17T15:41:17Z @neo-opus-vega cross-referenced by #15365
- 2026-07-17T15:44:44Z @neo-fable-clio referenced in commit `dfcf0a1` - "fix(button): own the UA cascade the native disabled attribute activates (#15324)

Grace's RC on PR #15327 named the risk precisely: dev has zero SCSS for native
:disabled, so projecting the attribute reintroduces the user-agent stylesheet as
a second styling author, and every existing assertion is presence-only.

Her prescribed isolation test settles it empirically. Same node, class constant,
attribute on vs off: backgroundColor, borderTopColor and opacity are equal (the
class declares them and wins on specificity) — and color diverges, exactly the
one axis .neo-disabled never declares on the button element itself. Chromium
paints rgba(16,16,16,0.3) (disabled GrayText) against the rgb(0,0,0) baseline.

So the repair takes her second branch: own the state. button/Base.scss pins
`&:disabled { color: inherit }` inside the .neo-disabled button rule — the class
stays the sole visual authority, and the witness stays in the suite permanently:
red against the unpinned head, green with the pin, so removing the pin (or
projecting a new attribute axis) reopens the assertion.

The afterSetDisabled JSDoc now points at the pin + witness instead of implying a
continuity the diff never established (RA-2), and #15324 carries the backfilled
visual-continuity AC with the durable rule for the next native-attribute
projection (RA-3).

Takeover note: authored on Emmy's branch under the Jul 17-19 window lift
(token-dead author); her three Chromium witnesses pass unchanged on this tree —
the earlier local reds were a foreign dev-server on the shared port serving a
different checkout, not her evidence. Tooling ticket to follow.

6/6 component (incl. the new witness) + 7/7 unit button specs green locally."
- 2026-07-17T15:48:46Z @neo-fable-clio cross-referenced by #15367
- 2026-07-17T16:24:17Z @tobiu referenced in commit `743d020` - "fix(button): project disabled state to native controls (#15324) (#15327)

* fix(button): project disabled state to native controls (#15324)

* fix(button): preserve disabled across tag replacement (#15324)

* fix(button): own the UA cascade the native disabled attribute activates (#15324)
...
- 2026-07-17T16:24:17Z @tobiu closed this issue
