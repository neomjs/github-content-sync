...
neomjs/neo#15739 owns the required ADR-0027 amendment. neomjs/neo-agent-brain#92 remains provisional for the different, still-deferred `restore-shadow-fill` action.
...
Without an exact action ticket, neomjs/neo-agent-brain#93 can accidentally become a second mutation controller or route into neomjs/neo-agent-brain#92's unresolved in-place repair semantics.
...
- neomjs/neo#15691 owns full provider-free bundle/row admission before mutation.
- neomjs/neo#15692 supplies the landed bounded 250-row Memory Core stored-vector importer.
...
Implement the orchestrator-owned `restore-empty-target` action after neomjs/neo#15739 lands:
...
3. Canonicalize a versioned v1 target-set descriptor containing the ordered destination identities, destination-topology fingerprint, bundle-manifest fingerprint, and neomjs/neo#15691 descriptor fingerprint.
4. Derive:
...
7. Restore all three targets into run-owned isolated destinations, using neomjs/neo#15692 for memories/summaries and the canonical graph importer for graph staging.
...
| Inputs | Versioned target set, neomjs/neo#15691-admitted bundle descriptor, topology and bundle fingerprints, injected clock/bounds. |
...
| Scale | neomjs/neo#15695 must supply 5,000/20,000 vector-plus-graph staging/promotion timing and peak-memory evidence before implementation merge. |
| Versioning | Any target-set member, boot-seed manifest, receipt, or transition-schema change is explicit and fail-closed. |

## Decision Record Impact

**BLOCKED_BY neomjs/neo#15739.** The implementation must conform to the merged ADR-0027 amendment. This ticket creates no second decision record and is not merge-eligible while the amendment is only proposed.
...
- [ ] No `restore-delta-merge` compatibility alias survives in classifier, dispatcher, receipts, tests, docs, or neomjs/neo-agent-brain#93
...
- [ ] neomjs/neo#15691 admission completes before staging and neomjs/neo#15692 batches remain bounded and provider-free
...
- [ ] neomjs/neo#15695 records 5,000/20,000 vector-plus-graph staging/promotion timing and peak memory before this PR may merge
- [ ] neomjs/neo-agent-brain#93 consumes the admitted action only and contains no importer call or restore child spawn
- [ ] Unit and disposable-store integration coverage passes under the repository's custom Playwright harness

## Out of Scope

- Bundle/row compatibility work owned by neomjs/neo#15691
- Bounded vector importer mechanics already supplied by neomjs/neo#15692
- The selector/projection implementation owned by neomjs/neo-agent-brain#93
- `restore-shadow-fill` and neomjs/neo-agent-brain#92
...
- **Selector-owned mutation** — neomjs/neo-agent-brain#93 remains request/projection only.

## Dependencies

Blocked by neomjs/neo#15739, neomjs/neo#15691, and neomjs/neo#15695. neomjs/neo#15692 is available.
...
Live latest-open sweep: checked the latest 20 open issues immediately before creation on 2026-07-22; only the intended ADR blocker neomjs/neo#15739 matched this design space.
