...
2. Add the **Brain-structural sweep dimension** with its evidence recipe and the two-layer rule: (a) mechanical probes — structure-map LOC outliers benchmarked against the Body's ceiling, folder-shape consistency across `ai/daemons/*`, concept-guide↔code-home coherence (every `learn/agentos/*.md` concept guide should map to a legible code home; the Dream Pipeline specimen is the calibration example); (b) routing — findings here cite neomjs/neo-agent-brain#126/D#14302 as the owning canon lane and never self-authorize restructuring pre-SSOT.
...
- [ ] The Brain-structural sweep dimension exists with its mechanical evidence recipe (LOC-vs-Body baseline, daemon folder-shape consistency, concept-guide↔code-home coherence) AND the two-layer rule: measure-and-report now, no prescriptive shape claims pre-SSOT, findings route to the neomjs/neo-agent-brain#126/D#14302 canon lane.
...
- Authoring the intended-architecture SSOT or restructuring `ai/` (owned by the neomjs/neo-agent-brain#126 goal-anchor's deferred gates and the D#14302/D#14305 ideation lane).
...
- Do NOT let the new structural dimension become implementation authority for file moves — neomjs/neo-agent-brain#126's body explicitly withholds that authority until its Lane-0 SSOT gate; the radar reports, the canon lane decides.
- Do NOT bloat the SKILL.md router with the taxonomy (Map/Atlas split per ADR 0008).
- Do NOT pin model names again.

## Related

- ADR 0008 (skill anatomy) · ADR 0007 (compaction taxonomy) · ADR 0019 (the `ai/`-config debt catalog the radar should point at) · neomjs/neo#11422 (router-hardening lineage) · `#15345` (source-text-guard taxonomy — a debt class born after this guide was written).
- **Brain-structural dimension anchors:** neomjs/neo-agent-brain#126 (deferred goal-anchor — Brain domain-mapping + Body-idiom integrity; its Lane-0 SSOT gate is the canon dependency) · D#14302 (Restructure `ai/` to domain-first) · D#14305 (Architecture immune system — keep `ai/` elegant as it evolves) · #13846 (conceptual Body/Brain/Neural-Link authority) · neomjs/neo#14714 (Body-idiom integrity strand) · `learn/agentos/DreamPipeline.md` (the concept↔code-home calibration specimen).

Decision Record impact: aligned-with ADR 0007 / ADR 0008; nothing amended. The structural dimension defers to neomjs/neo-agent-brain#126's future Decision Record rather than pre-empting it.

Release classification: not release-blocking (skill-substrate refresh; boardless).

Live latest-open sweep: checked latest 20 open issues at 2026-07-23T20:17:46Z; no equivalent found. A2A claim sweep (all-status, last 15): no overlapping `[lane-claim]`/`[lane-intent]` — recent claims (#15768→PR neomjs/neo#15769, neomjs/neo#15760, neomjs/neo#15763) are disjoint. (Body updated same-day by the ticket author with the Brain-structural dimension after a second operator directive; no scope collision introduced — D#14302/#14304 own the canon lane, this ticket owns only the radar.)
...
Implementing PR: neomjs/neo#15777 (head `dcf44cd911`; initially cut from the neomjs/neo#15763 feature branch by mistake and rebased onto `origin/dev` same-hour; cycle-1 review fixes at `dcf44cd911` added the authority/observation/hypothesis separation).
...
| §2 pre-flight (conditional payload) | ADR index + pr-review tables (*authority*); `ai:structure-map` (*observation only* — current-tree measurement, never intended-placement policy) | Taxonomy DERIVED at sweep time from named authorities with the authority/observation/hypothesis classes kept explicit; authorities-win conflict rule; freshness trigger names its own revalidation condition | Pattern-canon ADR graduation → revalidate the pointer list | guide §2 | no copied pattern list remains (diff); lint-guides OK; PR neomjs/neo#15777 cycle-1 review falsified the prior "what-belongs-where" framing — corrected here, in the ticket body, and in the guide |
