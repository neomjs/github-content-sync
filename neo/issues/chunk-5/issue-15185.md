...
Epic neomjs/neo-agent-brain#97 needs a standards-compliant local Streamable HTTP ingress that is actually confined to loopback and does not introduce a user-identity system. The approved source contract lives in [D#15173](https://github.com/orgs/neomjs/discussions/15173).
...
neomjs/neo#15188 completed through [PR neomjs/neo#15191](https://github.com/neomjs/neo/pull/15191), merged into `dev` as [`d839f644`](https://github.com/neomjs/neo/commit/d839f644ae45da8cce904488c5691076110582fa).

The live dependency graph now shows this ticket's only recorded prerequisite as closed. neomjs/neo#15185 remains open and unassigned, so the implementation lane is code-ready for normal maintainer self-selection.
...
- **Currency / successor evidence:** the current `dev` source still lacks an actual `mcpListenHost` bind, a local-bearer auth strategy, and the absent-vs-present Origin guard. Exact issue/PR searches for `mcpListenHost` and `local-bearer` found no competing implementation; neomjs/neo#15188's prerequisite landed through PR neomjs/neo#15191.
...
- **Epic review:** [Euclid's pre-work review on neomjs/neo-agent-brain#97](https://github.com/neomjs/neo-agent-brain/issues/97#issuecomment-4974669103) remains the governing parent-level challenge artifact.
...
Implementation boundary clarification: this ticket will provide the generic ephemeral-token generator/launch contract and shared ingress primitive. neomjs/neo#15187 remains the owner of the executable BigData reference journey that consumes that contract; this avoids duplicating its acceptance surface.
