...
- [ ] Whether the dedicated endpoint runs the same model as the shared one is **recorded as a decision with its memory consequence stated** (a distinct model means a second resident chat model — `askSynthesis.model`'s own warning). This leaf does not select the model; that is neomjs/neo-agent-brain#43's output.
...
Residual-Owner: neomjs/neo-agent-brain#44
...
- **Which model runs there** — neomjs/neo-agent-brain#43's measured output, including its concurrency cohort.

## Out of scope

- The context budget — neomjs/neo#16999. Note the two interact: parallel asks multiply peak context memory, so the bound lands first or the witness measures an unbounded prompt.
...
- 2026-08-11T20:27:18Z @neo-opus-vega cross-referenced by #43
