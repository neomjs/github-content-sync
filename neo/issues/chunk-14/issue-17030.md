...
Discovered during `ticket-intake` on neomjs/neo#17009. @neo-opus-vega recorded the first fixed-population witness on that ticket (2026-08-12T12:52Z): three receipt-confirmed `mark_read` calls (12:02, 12:19, 12:25) all visible via `get_message`, all absent from the unread projection 27+ minutes later; her `totalCount` read 664 against single-digit genuine unread.
...
This is the **root divergence** underneath neomjs/neo#17009's wake-preview symptoms. neomjs/neo#17009's PR owns the wake-path admission gate (authored-age filter + surviving-set digest derivation); it deliberately does **not** repair this projection — a fresh-but-read message would still false-render through a stale projection. Two mechanisms, two leaves, explicitly linked so neither close claims the other.

## The Fix

Root-cause the projection consumption gap with the discriminator as the falsifier, then make the unread projection observe a committed `readAt` — strict per-write visibility on the read path. If strict visibility is measured to be too costly, surface the reconciling state **explicitly** in the list response instead (the OR-clause ported from neomjs/neo#17009 AC-4, preserved as a measured fallback, never the silent default). The wake-digest read-state resolver must observe a committed `readAt` within the same discipline, so a read message cannot render unread in a later digest.
...
- [ ] `totalCount` for a fixed mailbox is **stable across consecutive reads absent writes** (ported verbatim from neomjs/neo#17009 AC-4, now backed by a fixed-population/no-write witness).
...
- Wake-path age admission (`isMessageWakeFresh` at Shape-B flush) — neomjs/neo#17009's PR owns it.
- Unread-count magnitude semantics — neomjs/neo#16541, shipped as PR neomjs/neo#16918.
...
- neomjs/neo#17009 — wake-path leaf + discovery site (its AC-4 ports here)
- neomjs/neo#16541 — `COMPLETED`; shipped the digest-count reconciliation this projection feeds
- neomjs/neo#16748 — `NOT_PLANNED`; held the read-state split-brain framing without a reproducer — this ticket supersedes that unowned scope with two live ones
- neomjs/neo#16965 — sibling read-path integrity (index loss masquerading as empty inbox)
- neomjs/neo#14426 — archived; sync/rebuild-pass property-loss hypothesis precedent

Live latest-open sweep at creation: neomjs/neo#17026 (12:20Z), neomjs/neo#17025 (12:17Z), neomjs/neo#17024 (12:17Z) — 20 open issues checked created-descending 2026-08-12T16:47Z; no equivalent. A2A in-flight claim sweep (30 messages, all read-states): no claim on the projection/read-path scope; @neo-kimi-phoebe's 16:33Z neomjs/neo#17009 lane-claim named this successor leaf as intent. KB semantic sweep: neomjs/neo#16541 / neomjs/neo-agent-brain#87 / neomjs/neo#14426 surfaced, none open and equivalent.
