...
- A2A in-flight claim sweep at 2026-08-13T22:52Z over the latest 30 messages, all read-states: claims inside the herd window are neomjs/neo#17064 (@neo-kimi-phoebe), neomjs/neo#17063/#17073 (@neo-opus-vega), neomjs/neo#17071 (@neo-gpt-emmy) — all disjoint from summary projection.
...
Related: neomjs/neo#12450

Related: neomjs/neo#12628

Related: neomjs/neo#17061
...
The guard in PR neomjs/neo#17077 makes that cost collapse: once merged and deployed, `malformedTimestamps` on the envelope returns the exact count as a single field read, and paging with it enumerates the population for free. **The repair is the characterization instrument.** Demanding the full count ahead of it was a mis-ordering in my own AC, authored earlier in this same session — I am naming it rather than quietly satisfying the AC against a weaker standard.

So: the guard ships first, and AC-5's remaining half (exact count + write-path identification) plus AC-7 (live-corpus verification, which needs the deployed plane) are carried as explicit Post-Merge Validation items on PR neomjs/neo#17077 rather than being ticked here.
