...
An external deployment exposed a bounded-attempt chain that stays active for hours. At the 2026-08-11 live read, `tenantRepoSync` had remained `running` since 08:48Z while one Knowledge Base embedding batch exhausted five 1,800,000ms attempts and a later batch continued retrying. PR neomjs/neo#16978 / ticket neomjs/neo#16973 correctly made one provider timeout terminal for the current `VectorService` sweep.
...
Consequently, repository B can leave the queue and dispatch immediately after repository A times out, even though that timeout does not prove A's server-side runner stopped. neomjs/neo#16978 prevents same-repository retry/batch amplification, but it cannot stop the next repository already queued in the same `runTask`.
...
- neomjs/neo#16978 makes a provider timeout throw out of `VectorService.embedChunks()` after one offer while preserving already-upserted batches as the durable resume boundary (`ai/services/knowledge-base/VectorService.mjs:637–661, 738+`).
...
| Durable KB progress | Chroma IDs + neomjs/neo#16978 timeout-terminal sweep | Already-upserted prefix remains the resume boundary | No successful prefix is re-bought | Existing VectorService contract | Second-run resume control |
...
- The broader plane-global admission declaration still open under neomjs/neo#16780 AC-5.
...
Parent incident: neomjs/neo-agent-brain#54  
Predecessor repair: neomjs/neo#16973 / PR neomjs/neo#16978  
Provider cancellation contract: neomjs/neo-agent-brain#48  
Plane-global admission/observability: neomjs/neo#16780  
Dead-generation activity rows: neomjs/neo-agent-brain#45  
Adaptive batch policy: neomjs/neo#16972
...
- 2026-08-14T11:45:49Z @neo-gpt cross-referenced by PR #17106
