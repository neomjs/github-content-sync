...
The first executable leg of neomjs/neo#17130 (C2 cutover — browser-direct cockpit wake consumer), per the leg map at claim time (issue comment IC_kwDODSospM8AAAABPBZxYg). Builds ONLY on frozen wire (PRs neomjs/neo#17103/#17116) + landed C1 (PR neomjs/neo#17176): zero dependency on the D#16764 fold or any S-chain state. The relay sibling `ai/services/fleet/fleetWakeSseConsumer.mjs` is the shape authority; the realm boundary carries no imports, so the parser twin is bound by a parity spec (the endpoint-normalizer precedent).
...
Store-bound rendering, honest-degraded rows, and the whitebox-e2e journey (leg 2 of neomjs/neo#17130); relay-side consumption (#17101, shipped); C3 render states (#16744).

## Lineage

The browser twin of the relay consumer shipped in PR neomjs/neo#17116 (slice 3), consuming the C1 custody contract shipped in PR neomjs/neo#17176 — both named, neither replaced.

Origin: neomjs/neo#17130 leg map · Retrieval Hint: "browser wake stream consumer twin two mints closure custody parity parser"

Live latest-open sweep: latest 6 checked 2026-08-15T16:08Z (nearest: neomjs/neo#17188/#17186/#17184 — test-isolation surfaces, different domain); no equivalent. A2A claim sweep: current-hour window clean (my own neomjs/neo#17130 claim broadcast is the parent's).
