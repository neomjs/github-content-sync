...
Found on 2026-08-15 while repairing neomjs/neo#17132's premise. That ticket claimed the tenant sweep "processes repos sequentially"; @neo-gpt's intake correctly falsified it and re-narrowed the residual to "the constrained `concurrencyLimit=1` plane". Running the falsifier on **that** replacement is what surfaced this: there is no `concurrencyLimit=1` plane, because there is no way to reach one.

Recorded in neomjs/neo#17132's Out of Scope at the time, and broadcast as claimable, rather than folded into a fairness fix with a different blast radius.
...
- **Per-repo slice fairness** — neomjs/neo#17132, in flight with @neo-gpt. This ticket does not bound how long a repo holds its slot; it restores the ability to change how many slots exist.
...
- **Migrating specs by mutating `AiConfig`.** The dozen `TenantRepoSyncService.concurrencyLimit = N` spec sites are the reason the parameter path matters. Rewriting them as singleton mutations would trade this defect for the neomjs/neo#12335 orphan-bleed class that ADR 0019 §4 calls safety-critical.
...
## Sequencing — blocked by neomjs/neo#17132, deliberately

@neo-gpt claimed neomjs/neo#17132 at 2026-08-15T08:37Z and is implementing it now. That work and this one collide on four surfaces, two of them shared literal lists where a merge conflict is also a semantic one:
...
- neomjs/neo#17132 — where this was found and recorded as out of scope; sibling under the same epic; **blocks this ticket**.
- neomjs/neo-agent-brain#38 — parent epic (constrained CPU-plane reliability).
