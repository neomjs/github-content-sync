...
PR neomjs/neo#17077 (closing neomjs/neo#17076) guarded the summaries projections against a per-row timestamp defect that was failing whole `query_summaries` calls. PR neomjs/neo#17083 (closing neomjs/neo#17082) then fixed the identical defect on the memory surfaces, and put the guard in a **new shared module** — `ai/services/memory-core/helpers/resolveRowTimestamp.mjs`.
...
This duplication was deliberate and is recorded as such in PR neomjs/neo#17083's `## Deltas from ticket`: at the time, neomjs/neo#17077 was already **approved at its head**, and amending it to share the helper would have invalidated that approval for a refactor that could land afterwards. That constraint is now gone.
...
- The other Memory Core timestamp projections already surveyed and found safe (`MemoryCoreRecorderService` projects SQLite epoch integers where `NULL` coerces rather than throws — see neomjs/neo#17082).
- Backfilling the malformed corpus rows bracketed on neomjs/neo#17076.
...
Unlike neomjs/neo#17076 and neomjs/neo#17082, this one needs no running Agent OS plane — the specs use in-memory fixtures.
...
Related: neomjs/neo#17076

Related: neomjs/neo#17082
...
## Implementer: @dchaudhari7177 — PR neomjs/neo#17095
...
**@dchaudhari7177 implemented this ticket in PR neomjs/neo#17095** and did it properly. For the record, and for whoever reads this ticket later:
