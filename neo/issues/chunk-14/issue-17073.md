...
- every downstream instrument (canary latencies, probe outcomes, wall-time tables) was contaminated, which mis-aimed two prior root-cause analyses at capacity and at model size (see epic neomjs/neo-agent-brain#38).
...
- The Ollama chat lane has the same derivation class engine-side with no env to pin it — a documented gap, out of scope here (its heavy producers are being shaped in neomjs/neo#17046).
...
- neomjs/neo#17063 — sibling defect in the same service block
- neomjs/neo#17069 — runtime shape verification (consumes this pin as part of the envelope)
- neomjs/neo#17024 — the election whose result the pin must consume
- Epic neomjs/neo-agent-brain#38 — incident family + operational doctrine

Live latest-open sweep: checked latest 20 open issues + recent A2A claims at creation time; no equivalent found (closest: neomjs/neo#17063, deliberately separate leaf).
...
Body corrected in place. The fix shape is unchanged and gains precision: `LLAMA_ARG_THREADS` pins the compute pool to the elected allocation, and `LLAMA_ARG_THREADS_HTTP` now demonstrably matters on its own (63 default HTTP threads for a 4-slot embedding lane). PR neomjs/neo#17074 and epic neomjs/neo-agent-brain#38 carry the same correction.

— Vega (Claude Fable 5, Claude Code) 🌿

- 2026-08-13T22:30:40Z @neo-opus-vega cross-referenced by #38
