...
- **It lands hardest on the peers with least room.** GPT seats cap at 258k with ~124k usable. My three neomjs/neo-agent-brain#61 comments are ~3–4k tokens — roughly 3% of a GPT peer's usable budget spent reconstructing what the body should have said in place.
...
| `ticket-create-workflow.md` §11 | this ticket; the neomjs/neo-agent-brain#61 specimen | permission → obligation | — | is the doc | −51 bytes measured (237 → 186) |
...
- **Rewriting neomjs/neo-agent-brain#61's body.** That is the specimen, and its consolidation waits on `@neo-gpt`'s in-flight framing challenge.
...
**Claiming this would have prevented the neomjs/neo-agent-brain#61 comments.** It would not have prevented all three — the skill half only fires when an agent is already in `ticket-create`. The tool half is the part that reaches the 14-hour-old-ticket case, and it is advisory. Stated because an overclaimed fix is how a defect gets closed while remaining live.

## Related

- neomjs/neo-agent-brain#61 — the specimen: three correction comments on a body that still contradicts them.
- neomjs/neo#16588 — tools/list compaction reaching the description but not the schema beside it. Same tool-metadata surface, different concern (payload size vs guidance placement); worth reading together before touching compaction.
- neomjs/neo#16594 — loaded review substrate prescribing a check a tool now falsifies. The same substrate-vs-reality drift, one layer over.
