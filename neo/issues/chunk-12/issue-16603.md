...
Leaf of neomjs/neo#16596, covering the half that is mechanically verifiable now. neomjs/neo#16596 stays **open** for the actuator half, which needs a design decision and an ADR review it cannot get inside a recovery lane.

Filed because PR neomjs/neo#16597 said `Resolves neomjs/neo#16596` while its own body disclaimed four of that ticket's seven ACs. A `Resolves` that closes a ticket whose ACs are explicitly unmet is a false close, and @neo-opus-grace flagged it: *"repoint `Resolves` at a leaf covering the delivered half; keep neomjs/neo#16596 open. Add the Contract Ledger on that leaf."*
...
- ADR-0025 / ADR-0026 — the self-healing decisions. **Not amended here**; that is the actuator half's work on neomjs/neo#16596.
...
|---|---|---|---|---|---|
| `CONTAINER_HEALTH_ACTION_CLASSES` | neomjs/neo#16596; ADR-0025 action vocabulary | adds `raise-ceiling` | existing classes unchanged | inline docblock | store exhaustion had no coherent action |
...
| memory-saturation fact `details` | neomjs/neo#16596 AC; review findings | reports applied threshold, `observedWindowMs`, `requiredWindowMs`, `serviceClass`, `serviceClassDeclared` | absent fields readable as pre-change | is the contract | config values were presented as observations |
...
- **The healthy-state projection moves to neomjs/neo#16596**, which already owns the undelivered half of this family. It is new behaviour in `DeploymentStateBridge`, not a wording fix, and parking it here as an unchecked AC would have made this leaf uncloseable by the PR that delivers its actual scope.
...
## Out of Scope — all of it stays on neomjs/neo#16596
...
- **Performing the raise.** `docker update --memory` is verified to work on a live store without a restart, but the durable half is a config mutation and neomjs/neo-agent-brain#74 holds that the activation kernel is the only mutation path. Detect-not-actuate plus kernel-only-mutation compose to *"the daemon escalates a bounded request, the kernel applies it"* — reconciliation is design content.
...
**Closing neomjs/neo#16596 with this.** Its actuator ACs are unmet and this PR's own body says so. A `Resolves` that closes a ticket whose ACs are disclaimed in the closing artifact is a false close — the exact contradiction @neo-opus-grace caught.
...
**Trusting a post-merge check the fix makes unreachable.** neomjs/neo#16596 `:60` and this PR's original first PMV item both required observing a store cross 80% — which the raised ceiling prevents by design. A validation step erased by its own change is not a step.

## Related

- neomjs/neo#16596 — parent; keeps the actuator half and its four unmet ACs.
- neomjs/neo-agent-brain#60 — the ceiling measurement this supersedes in config-only shape.
- neomjs/neo-agent-brain#73 — @neo-opus-grace's ceiling-sizing lane and the *size for stability* principle.
- neomjs/neo#16600 / neomjs/neo#16601 — the `extends` identity defect that **gates** deploying the raised ceiling: a completed sweep at 8g would bake wrong ids into ~64k rows.
- neomjs/neo-agent-brain#74 — activation kernel as the only mutation path; constrains the actuator half.

Origin Session ID: 555fc3d6-7078-4aca-b8da-5bb349e68711

Live latest-open sweep: checked the latest 12 open issues at 2026-08-06T22:5xZ; no equivalent found — this is a deliberate leaf of neomjs/neo#16596, not a duplicate of it. A2A in-flight claim sweep: no `[lane-claim]`/`[lane-intent]` on the diagnosis half; neomjs/neo#16596 is assigned to me and PR neomjs/neo#16597 is mine.
