...
2. **No genuinely external repo is registered**, so the lane's actual purpose — pulling content the plane does not already have — is unexercised. `kbSync` already owns the `neo` corpus, and both lanes are now container-plane (`kbSync` moved via neomjs/neo#16556), so the one registered tenant duplicates a corpus rather than adding one.
...
- **The embed-stage failure — neomjs/neo-agent-brain#64.** Registration alone will not make ingestion succeed; `KB_VECTOR_EMBED_FAILED` still blocks it. This removes the *cost* obstacle to proving it, not the defect.
- Whether `neo-shared/neo` should remain a tenant entry at all (#16566 question 1).
- First-ingest round-trip cost on a cold mirror — neomjs/neo-agent-brain#65.

## Why a separate ticket

neomjs/neo-agent-brain#64's proof AC needs a subject that is cheap to ingest repeatedly. Landing that subject is mechanically independent of root-causing the embed failure, so doing it separately means the proof run is ready the moment the embed fix lands.

## Related

- neomjs/neo-agent-brain#64 — the embed-stage blocker; this makes its proof AC affordable.
- neomjs/neo-agent-brain#65 — why tracked-file count dominates first-ingest cost.
- neomjs/neo#16556 — moved `kbSync` to the container plane, which is why both lanes now share one.
