...
`ai/mcp/server/shared/services/AuthService.mjs` ships the forge-PAT contract (`NEO_AUTH_MODE` ∈ `local-bearer`/`gitlab-pat`/`github-pat`, neomjs/neo#12378/#12383): self-validated bearer, validated forge identity as caller identity. The fleet surface takes the same seat. Per `learn/agentos/cloud-deployment/ClientAuthentication.md`, forge auth establishes identity — Fleet authorization is the grant families' job (S5).
...
- **Epic-review prerequisite**: satisfied by the non-author structured review on neomjs/neo-agent-brain#83 — https://github.com/neomjs/neo-agent-brain/issues/83#issuecomment-5227088809 (@neo-gpt-emmy, 2026-08-08).
- **Premise V-B-A (current source, exact head)**: `AuthService.mjs` ships all three modes (`local-bearer`/`gitlab-pat`/`github-pat` — verified at lines 164/178/185); the composed fleet-server ALREADY seats AuthService with `github-pat` via the canonical compose's `local-auth` anchor (S1 neomjs/neo#16735, shipped) — so **AC-1 is substrate-satisfied**; the lane's work is AC-2..AC-5. `resolveViewerStreamKey` already keys streams on the immutable proto-tuple `provider:<authProvider>:<providerUserId>` — but `normalizedProviderBaseUrl` is absent from it, and the wire-verb path (`fleetServerPolicy.mjs` → `dispatchFleetRequest`) does not key authorization-relevant decisions on the tuple. `ClientAuthentication.md` exists (16.7KB, current Aug 2).
- **Currency / successor-risk**: no successor ticket covers the admission subject (D#16720 family swept: neomjs/neo#16743 wire vocabulary + neomjs/neo#16790 identity join are CLOSED siblings, different scopes); the post-creation drift is ADDITIVE (S7 wake lane + policy hardening), not superseding.
- **ADR successor-risk**: ADR 0038 (FM client topology — the four identity facts + credential-class ledger this lane keys on) is accepted and current (applied on PR neomjs/neo#17116 today); no successor. ADR-0019 gate noted: any `ai/` config-leaf touch in this lane re-reads §3 first.
...
**ROI**: strongly positive — root blocker of S3/S4/S5 (#16737, neomjs/neo-agent-brain#52, neomjs/neo-agent-brain#51); the multi-user admission subject is the precondition for every grant-family stream.

**Design fork (Tier 2.5, ping-and-continue — sent to @neo-opus-ada via A2A)**: AC-2's "(— S4's build)" vs neomjs/neo-agent-brain#52's "Blocked by S2": reading = S2 derives + stamps the tuple at admission behind ONE named seam (`deriveOwnerPrincipal`); S4 owns the durable normalization contract + operator-to-agent relation on top, and may reshape the seam's internals without the admission call sites moving.
