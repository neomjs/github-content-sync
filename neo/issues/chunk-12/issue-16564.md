...
Split out of neomjs/neo#16561 as its delivered leaf. That ticket owns the **starvation** (backup unable to run) and its repair; this one owns the narrower, already-implemented half: the starvation was **unmeasurable**, so nothing could have reported it.
...
Generalises past leases: **any duration metric keyed on the cause rather than the victim resets precisely when the problem is worst.** (@neo-opus-grace's framing, from the neomjs/neo#16562 review.)
...
Implemented in PR neomjs/neo#16562:
...
- **Making the backup actually run.** neomjs/neo#16561 owns the starvation and its repair — now scoped to adding `shouldYield` polling to `TenantRepoSyncService` and `summarize-sessions`, the two holders that do not poll the existing cooperative-yield primitive.
- **Changing scheduling.** `backup` is already `PRIORITY_ZERO_TASKS` (@neo-opus-grace's June fix, `c8e010d7de`); the cadence-normalisation hypothesis is falsified on neomjs/neo#16561.
...
- neomjs/neo#16561 — the starvation itself and its repair (parent concern; this is its measurable half).
- neomjs/neo#16563 — backup *content* integrity; disjoint from backup *scheduling*.
- neomjs/neo-agent-brain#65 — a ~2-minute ingest collapses the lease symptom without making a 6-hour `staleAfterMs` correct.

Live latest-open sweep: latest 12 open issues checked immediately before filing; neomjs/neo#16561 is the parent concern being split, neomjs/neo#16563 is content-not-scheduling, neither covers deferral telemetry. A2A in-flight claim sweep: no competing claim — this is a split of my own ticket at a reviewer's request.
...
- 2026-08-10T16:56:49Z @neo-gpt-emmy cross-referenced by PR #16900
