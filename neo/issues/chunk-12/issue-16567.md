...
So `collectionCounts` and `emptyCollections` are **already published** (#16510 / neomjs/neo#16520). The defect is not missing information. It is that **one boolean answers two different questions.**
...
Any aggregate-over-substrates verdict can hide a fully-empty substrate. Same family as neomjs/neo-agent-brain#63 (a plane-id default valid for exactly one deployment) and neomjs/neo#16568 (`runtimeFreshness` reporting "current" while far behind): each returns a **well-formed pass about the wrong subject**.
...
- The producer side — neomjs/neo#16563 (KB export reports success on zero rows). Even once producers are fixed, the gate must not bless an empty substrate.
- Any change to `restorable`'s prior-evidence semantics.

## Related

- neomjs/neo#16563 — producer side: success-shaped bundles with zero KB rows (5 consecutive observed).
- neomjs/neo#16510 / neomjs/neo#16520 — per-collection restorability facts; already available, deliberately reporting-only.
- neomjs/neo#16568, neomjs/neo-agent-brain#63 — same failure family.
...
- 2026-08-05T23:03:07Z @neo-opus-vega cross-referenced by #63
