...
Live latest-open sweep: checked the newest 20 open issues and the latest 30 A2A messages at 2026-08-08T10:23Z. No equivalent ticket or competing claim exists; neomjs/neo#16671 owns deploy-composition admission, not repo-client transport defaults.
...
The parent neomjs/neo-agent-brain#84 ledger names `GH_TOKEN` for these rows. That spelling is obsolete: the canonical remote-plane credential is `NEO_MCP_REMOTE_TOKEN`, while `GH_TOKEN` remains repository workflow authority.
...
|---|---|---|---|---|---|
| `Client#bearerTokenEnvVar` | ADR 0019; existing remote transport contract from neomjs/neo#10900 | Resolve one named Bearer slot just in time for remote transport construction | Missing/empty fails before connect; literal header plus slot is rejected | JSDoc on the config and method | Focused unit tests for precedence, cloning, non-mutation, collision, and failure |
| Built-in `memory-core` row | ADR 0019 §10.7; parent neomjs/neo-agent-brain#84 transport matrix | `streamable-http` to `http://127.0.0.1:3102/mc/mcp` using `REMOTE_MCP_CREDENTIAL_ENV_VAR` | No stdio or `GH_TOKEN` fallback | Client config schema note | Static contract plus authenticated routed health/read probes |
| Built-in `knowledge-base` row | ADR 0019 §10.7; parent neomjs/neo-agent-brain#84 transport matrix | `streamable-http` to `http://127.0.0.1:3102/kb/mcp` using `REMOTE_MCP_CREDENTIAL_ENV_VAR` | No stdio or `GH_TOKEN` fallback | Client config schema note | Static contract plus authenticated routed health/query probes |
...
- Fleet-generated seat adapters and provisioning templates already delivered by neomjs/neo#16181 and neomjs/neo#16205.
- Deploy-pipeline composition admission owned by neomjs/neo#16671.
- Removing stdio from file-system, repository-workflow, or Neural Link families.
- Closing the broader cutover, deletion ledger, roadmap, reboot, or persistence ACs in parent neomjs/neo-agent-brain#84.
...
Related: neomjs/neo#10900

Related: neomjs/neo#16181

Related: neomjs/neo#16205

Related: neomjs/neo#16671
