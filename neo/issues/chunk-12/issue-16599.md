...
The divergence itself has a known cause under investigation on neomjs/neo-agent-brain#66 (an `extends` value absent from one side, and `extends` is a hash input). **That cause is not this ticket.** This ticket is that `--mode merge` cannot see the condition at all, and reports success while creating duplicates.
...
## The writer fence — design record, MOVED to neomjs/neo-agent-brain#72 (built 2026-08-07, falsified same day)
...
### The fence ACs have MOVED to neomjs/neo-agent-brain#72 (2026-08-07) — this ticket is the detector only
...
| restore receipt `imported` field | neomjs/neo#16563 receipt-honesty family | split into `inserted` / `idAlreadyPresent` / `natural-key-divergent` | retain `imported` as the sum | docblock + receipt schema | `"imported": 59754` counted 7,900 pre-existing rows |
...
- **The `extends` / class-hierarchy regression** that produced the observed divergence — that is neomjs/neo-agent-brain#66's finding and needs its own ticket. This ticket is the detector, and it must work for *any* derivation change, not that one.
...
- neomjs/neo-agent-brain#66 — the corpus-loss incident; carries the measurements above and the `extends` derivation regression that exposed this.
- neomjs/neo#16563 — receipts reporting success on empty/failed operations; the receipt-classification AC here is the same family.
- neomjs/neo#16590 — tenant-scoped stale-id gathering; duplicated rows share the stamp it scopes on, which is why duplication resolves as deletion.
- neomjs/neo#16591 — corpus-wipe refusal proven below the surface agents call; sibling fail-closed-on-destructive-path work.
- neomjs/neo-agent-brain#64 — the tenant-ingestion epic whose corpus this restores into.

Origin Session ID: 555fc3d6-7078-4aca-b8da-5bb349e68711

Live latest-open sweep: checked latest 20 open issues at 2026-08-06T19:0xZ; nearest neighbour is neomjs/neo#16563 (receipt honesty on export), which does not cover merge identity semantics. A2A in-flight claim sweep: no `[lane-claim]`/`[lane-intent]` on restore merge semantics in the herd window.
...
- 2026-08-07T06:46:18Z @neo-opus-grace cross-referenced by #66
