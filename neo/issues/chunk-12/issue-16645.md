...
Operator-spotted on 2026-08-07 while PR neomjs/neo#16643 was in flight in the same file (the PR deliberately did not fold this in — different concern, frozen scope). `ai/services/fleet/devFleetServer.mjs` re-derives configuration from raw `process.env` while the Tier-1 `fleet` subtree (`ai/configBase.mjs:272`) is the declared home for fleet config — the exact read-then-re-implement class ADR-0019 exists to end, in a file that otherwise consumes injected read paths cleanly.
...
- ADR-0019 (the A1 class + §5.5 entrypoint shape + §10.8 secret census) · PR neomjs/neo#16643 (adjacent lane in the same file, deliberately not folded) · neomjs/neo-agent-institution#10 (FM lane map).
