...
**Inferred cause of the timing:** neomjs/neo#16556 moved kbSync from host-edge to the container plane on 2026-08-05, between the Aug-3 bundle (96.42% populated) and today's rebuild (0.00%). On host-edge the generated file was present in the working tree. This is not a criticism of neomjs/neo#16556, which resolved a real two-sided ownership contradiction — the latent defect is that the hierarchy was a *gitignored build output* rather than a declared plane-present artifact, so no plane move could have known it was carrying it.
...
| `aiConfig.hierarchyPath` (`configBase.mjs:266`) | ADR-0019 provider SSOT; neomjs/neo#16556 plane ownership | resolve to an artifact present on the plane that runs kbSync | none | `configBase.mjs` leaf docblock | file present on host, absent in container (`docker exec`) |
...
- **The chroma memory ceiling** (#16595) and the diagnosis/routing half (#16596 / neomjs/neo#16597). Independent cause; this ticket's sequencing note only records that the ceiling fix must not be used to rebuild before this lands.
...
- **Reverting neomjs/neo#16556.** The plane move was correct; the artifact declaration was missing.
...
- neomjs/neo-agent-brain#66 — the corpus-loss incident; carries the full measurement chain and the git-churn analysis that isolated this.
- neomjs/neo#16599 — merge keyed on a content digest; would have *detected* this divergence at restore time.
- neomjs/neo-agent-brain#60 / neomjs/neo#16597 — the chroma ceiling; the rebuild this blocks.
- neomjs/neo#16556 — moved kbSync host-edge → container, the timing correlate.
- neomjs/neo#16590 — tenant-scoped stale-id gathering; the mechanism by which a re-derived id becomes a deletion.
- neomjs/neo#16563 — receipts reporting success on a degraded operation; same family as the swallowed warn.
...
- 2026-08-06T22:15:08Z @neo-opus-vega cross-referenced by #61
...
- 2026-09-17T12:49:34Z @neo-opus-grace cross-referenced by PR #18822
