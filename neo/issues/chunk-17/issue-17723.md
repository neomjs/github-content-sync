...
Implementation slice of neomjs/neo-agent-brain#30 (the osascript focus-race defect): the receiver-side transport that makes Claude-seat delivery structurally focus-free. The parent ticket measured 14 receiver-log failures across 5 subscriptions, 12 of them Claude seats, including a **silent misroute class that reports success** — keystrokes landing in whichever window held frontmost. The parent also verified the replacement capability exists: Claude Code's cross-session messaging (registry + per-session socket, `peerProtocol: 1`), confirmed live on this deployment.

This leaf owns the *receiver and protocol code* for that transport so it can land and be reviewed independently of the host-side legs (live courier fire, route migration, log sampling), which remain owned by neomjs/neo-agent-brain#30.

Live latest-open sweep: checked latest 20 open issues at 2026-08-24T18:0xZ; no equivalent transport slice found (#17227 is the parent, neomjs/neo-agent-brain#50 the ingress complement). A2A in-flight claim sweep (all read-states, last ~60 min): claims on neomjs/neo#17719/#17044/#17718 only — zero scope overlap.
...
- Live courier-session fire and receipt reconciliation into receiver state — remains on neomjs/neo-agent-brain#30
...
neomjs/neo-agent-brain#30 — parent defect; retains live-fire, migration, sampling, and disposition ACs
neomjs/neo-agent-brain#50 — ingress transport complement (remote topology half)
neomjs/neo-agent-brain#146 — sibling-delivery validation lane
