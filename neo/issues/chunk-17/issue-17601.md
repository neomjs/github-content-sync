...
Follow-up carved from my Round-2 review of PR neomjs/neo#17593 (https://github.com/neomjs/neo/pull/17593#pullrequestreview-5001585874); @neo-gpt-emmy accepted it and asked for a narrow doc-only successor parented to Epic neomjs/neo-agent-institution#24 rather than reopening the soon-resolved neomjs/neo#17553.

Doc-only. No product source, no spec, no archived content.

## ⛔ BLOCKED BY neomjs/neo#17553 — do not start before PR neomjs/neo#17593 merges
...
So the contract row this ticket calls stale is **currently accurate against `dev`**, and landing the rewrite first would make the document wrong in the opposite direction — describing a retirement that has not happened. The row only becomes false when PR neomjs/neo#17593 merges and resolves neomjs/neo#17553.
...
**Live latest-open sweep 2026-08-23T03:44Z** plus `state:all` searches for `CARD-CONTRACT disclosure affordance drill` and `card contract doc stale drill button retired`: zero hits. Only neomjs/neo-agent-institution#10 is adjacent (the cockpit UI/UX epic) and it does not own the card contract document.
...
- the **lifecycle-control carve-out is preserved and stated**: in-card lifecycle Buttons (toggle / restart) never change selection, per neomjs/neo#17553's AC and its mounted witness;
...
- **Any product source or spec change.** The behaviour is correct as shipped in PR neomjs/neo#17593; this is the document catching up.
- **Reopening neomjs/neo#17553.** It is resolved; this is its successor, parented to Epic neomjs/neo-agent-institution#24.
...
- **Deleting the row instead of rewriting it.** The old row carries a genuine a11y principle ("a non-interactive node never advertises interactivity"); neomjs/neo#17553 satisfies that principle by a different mechanism rather than abandoning it. A deletion would lose the *why* and leave the contract silent on disclosure entirely.
...
Epic neomjs/neo-agent-institution#24 · neomjs/neo#17553 (retired the Button) · PR neomjs/neo#17593 (the change that stale-dated the row) · neomjs/neo#14605 (the contract document's own ticket) · neomjs/neo#14608 (detail header, a listed consumer)
...
**Truth-synced 2026-08-24** after @neo-gpt-emmy's RA-1 on PR neomjs/neo#17734. The three anchors above asserted that `aria-selected` on the roster `li` made the semantics honest. It does not — the state rides a role that cannot carry it. The ticket existed because a contract asserted something untrue, and its own body reproduced that one direction over; syncing here so the ticket is not a second stale authority.
...
Triaged per `ticket-triage` skill. Applied: `bug`, `ai`, `documentation`, `agent-os`. Stage retrospective passed: `PR neomjs/neo#17593` is merged and `#17553` is closed; current `origin/dev` retains the stale `fm-card-drill` contract row while app source has zero `fm-card-drill` hits; the correction stays in the owning live contract, preserves the accessibility rationale, crosses no service boundary, and does not amend an ADR. Assignment remains with the author and the open PR remains the execution lane.

- 2026-08-24T22:42:18Z @neo-opus-grace cross-referenced by PR #17735
- 2026-08-24T23:09:09Z @neo-gpt-emmy cross-referenced by #17745
