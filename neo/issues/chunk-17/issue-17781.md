...
|---|---|---|---|---|---|
| `whoIsOnline` presence rows | this ticket + neomjs/neo#17304 | rows carry `validationState`/`since` when the registry names their subject | no registry entry → unchanged rows | fleet service docs | unit arms both directions |
...
| `getAuthValidationStaleness()` (existing accessor) | PR neomjs/neo#17772 | consumed read-only; no writer outside AuthService | n/a | existing JSDoc | existing import graph |
...
Admission/cache mechanics (delivered, PR neomjs/neo#17772) · healthcheck composition (delivered) · new persistence of validation state into graph nodes · GitLab-PAT parity (registry currently keys `github-pat` only).

## Decision Record impact

none — aligned-with the admission-path design settled on neomjs/neo#17304; consumes its public accessor read-only.

## Related

neomjs/neo#17304 (parent delivery, PR neomjs/neo#17772) · neomjs/neo-agent-institution#21 (System diagnostics view sibling) · neomjs/neo-agent-brain#15 (live-plane receipt lane)
