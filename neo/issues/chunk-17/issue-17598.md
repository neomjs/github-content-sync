...
Carved from neomjs/neo-agent-brain#17 (AgentOS cockpit e2e reds) per its own AC-1 — *"reclassified with a named cause and its own successor ticket"*. Three of that ticket's failures share one cause; the rest do not and stay with the parent.

Two of the three were **not in neomjs/neo-agent-brain#17's original census**. That ticket lists five reds; `FleetCardLifecycleNL` carries two more, invisible because the census was a chosen subset of specs plus a `toEqual([{method` grep that structurally cannot see a shared assertion helper.
...
**Browser mode sends the protocol offer; shell mode strips it back to `{method, params}`.** Since neomjs/neo#16743 (*"version the client wire contract"*, merged as `040fa1cb27`) that offer is `{capabilities: ['method-schema-v1', 'closed-response-states-v1'], versions: [1]}`.
...
- **Rows 2–5 of neomjs/neo-agent-brain#17** — `AgentCardSynthesisRenderNL`, `FleetCockpitDrillRoundTripNL`, and two `FleetMailboxTabNL` journeys. Undiagnosed, unclaimed, different causes; the parent keeps them.
- **Changing the wire contract itself.** neomjs/neo#16743's envelope is deliberate and stays.
...
Parent neomjs/neo-agent-brain#17 · neomjs/neo#16743 (the envelope this stale-dated) · neomjs/neo#17595 (why the layer went unrun) · PR neomjs/neo#17593 (surfaced the parent set)
