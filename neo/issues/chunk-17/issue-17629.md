...
neomjs/neo#17231 fixed the spatial half of this class (the adapter types into the process it verified, never a fresh frontmost read). This is the **temporal sibling**: verified process, wrong moment.

## The Architectural Reality

- The wake daemon (`ai/daemons/wake/`) resolves routes and the host-side adapter injects into the seat's terminal session; neomjs/neo#17231's invariant binds the target process at verification time.
...
| Harness-side safe-point ingestion (file-drop/IPC consumed between turns instead of keystrokes) | removes the class permanently for harnesses that support it | not available on all seats; a per-harness split reintroduces the generic-adapter debt neomjs/neo-agent-brain#135 tracks |
...
- [ ] The neomjs/neo#17231 invariant (verified-process binding) is untouched and its existing coverage stays green.

## Out of Scope

- Wake route/envelope addressing defects (#17586, neomjs/neo#17619, neomjs/neo#17615 own that family).
...
neomjs/neo#17231 (spatial sibling — verified-process typing, closed), neomjs/neo-agent-brain#19 / neomjs/neo#17619 / neomjs/neo#17615 (wake route family, distinct), neomjs/neo-agent-brain#135 (generic harness adapter surface), neomjs/neo-agent-brain#136 (wake/heartbeat mechanical-enforcement epic).
