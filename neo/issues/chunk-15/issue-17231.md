...
Split from neomjs/neo-agent-brain#30's investigation. That ticket asks whether to retire the osascript adapter; **this is a defect inside it that should be fixed regardless**, because the two Codex seats have no alternative transport and will keep using this path.
...
- **Retiring the adapter** — neomjs/neo-agent-brain#30 owns the transport question. This fix must land whether or not that happens, because the Codex seats have no alternative today.
...
- **"The guard is missing."** It is not. I proposed adding a frontmost check to neomjs/neo-agent-brain#30 before reading the code; it already exists and works. The defect is that its *result is discarded one line later* — which is worse than a missing guard, because the log looks like a working guard.
...
neomjs/neo-agent-brain#30 (the transport question this was split from) · neomjs/neo-agent-brain#50 (ingress delivery) · neomjs/neo-agent-brain#95 (wake policy)
