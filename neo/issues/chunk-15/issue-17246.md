...
Two verified findings from the same investigation that this ticket deliberately does **not** fix — both belong to neomjs/neo-agent-brain#152, and bundling them would make this un-reviewable:

- **`contentTrust.signals` has zero consumers.** A grep across `ai/` finds no reader of the signals array outside the sanitizer and its own specs. The three `STEALTH_SIGNALS` patterns in `astroturfSanitizer.mjs:49-72` fire into a field nothing acts on. neomjs/neo#17236 produced `signals: []` — a true negative for all three encoded incidents and a false all-clear for this one.
...
- **Adding a fourth regex to `STEALTH_SIGNALS`.** Tempting, and wrong: the signals have no consumer, so a new pattern would be pure diagnostics — it would have changed nothing about neomjs/neo#17236's ingestion. Detector work belongs behind a consumer, in neomjs/neo-agent-brain#152.
...
- Parent Epic: neomjs/neo-agent-brain#154 (organism self-defense)
- Sibling: neomjs/neo-agent-brain#152 (P8 link quarantine + stealth-intent detection) — owns the two Out-of-Scope findings
- Precedent: neomjs/neo#12995 (KB tier taint + sync denylist) — shipped the discussion-side lever this extends
- Incident evidence: neomjs/neo#17236 (redacted + locked), `resources/content/issues/chunk-16/issue-17236.md`
- Playbook: `.agents/skills/hostile-content-quarantine/`

Origin Session ID: 3f264a19-c7d4-481e-bc80-5c288bca177f

Retrieval Hint: `query_raw_memories("sync containment denylist issues discussions author-level astroturf neomjs/neo#17236")`
