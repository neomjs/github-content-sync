...
First live PoC of the Operator compose surface (2026-08-17 ~18:05Z, neomjs/neo#17271 session): the operator sent "operator a2a test" / "ping" from the cockpit's Operator pane — **it arrived attributed to `@neo-fable-clio`** (`from: @neo-fable-clio`, `senderPrincipalClass: 'agent'`), woke that seat correctly, and now exists in the identity graph as a message the agent never wrote. Sending works (a first!); attribution is wrong. Operator verbatim: "showing you as the sender => new bug. but a first time PoC that sending itself works."
...
- The operator-principal modeling gap is already tracked: neomjs/neo-agent-brain#52 (ownerPrincipal + the operator-to-agent derived relation). `senderPrincipalClass` exists on the wire and currently asserts `'agent'` for these sends.
...
3. **The real model (#16738's scope, not duplicated here):** operator principal as first-class sender (`senderPrincipalClass: 'operator'`), with the operator-to-agent derived relation for seats they steer. This ticket stops at detection + honesty + docs; the principal model stays neomjs/neo-agent-brain#52's.
...
neomjs/neo-agent-brain#52 (ownerPrincipal contract — the real model) · neomjs/neo-agent-brain#28 (operator-authority sibling: bench write path, same principal question) · neomjs/neo#17271 (origin session receipts) · Epic neomjs/neo-agent-institution#10 (parent)
...
- 2026-08-17T19:49:49Z @neo-fable-clio cross-referenced by #28
- 2026-08-18T11:37:32Z @neo-fable-clio cross-referenced by PR #17348
- 2026-08-18T15:52:20Z @neo-fable-clio cross-referenced by #17328
