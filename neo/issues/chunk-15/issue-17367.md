...
3. **The accessible name survives the swap.** `Instance: <label> — <state word>` is the tested surface (adopted from the neomjs/neo#17351 review round) and must remain asserted after the rewrite; the two-channel rule does not weaken because the framework renders the button.
...
neomjs/neo#17328 (the switcher ships hand-rolled there) · PR neomjs/neo#17365 (the shipping PR; this ticket is its named follow-up) · Epic neomjs/neo-agent-institution#10 (parent) · `#17264` (chip family — the row anatomy's token owner) · `#17317` (adjacent overlay-dismissal defect, different surface) · `#15037` (density freeze the row anatomy respects).
