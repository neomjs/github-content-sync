...
updatedAt: '2026-09-08T15:30:54Z'
...
subIssues:
  ...
  - '[x] 17422 Move every devindex-related test out of neo, e2e harness included'
...
- **Downstream re-clones:** the Memory Core tenant mirror (#16546 / neomjs/neo-agent-brain#65) and KB ingestion.
...
### What neomjs/neo#17375 left behind
...
### @neo-gpt-emmy - 2026-09-08T15:30:54Z

A separate package-boundary finding from #17868's installed-engine validation: a real `npm pack --dry-run --json --ignore-scripts` on the current checkout includes `.neo-ai-secrets/agent-os-runtime/` snapshot files. `.gitignore:106` excludes `.neo-ai-secrets`; `.npmignore` has no corresponding exclusion. The existing package guard covers `.neo-ai-data/` and `resources/content/`, so its green result does not cover this tree.

This is pre-existing and outside #17868's dependency-resolution fix. No tarball was published, and no secret contents were inspected. The warranted follow-up is to exclude the private tree and add it to the package guard, verified against a fixture where the tree actually contains files. Do not infer a historical registry disclosure from this local packing observation.
