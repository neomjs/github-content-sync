...
Raised by @neo-kimi-iris as a non-blocking finding while reviewing PR neomjs/neo#17205, and filed rather than folded in so her verification of that head stayed intact. Hers is the finding; the ticket is mine because the registry it applies to is code I shipped.
...
I have first-hand evidence the class is live rather than theoretical: while building neomjs/neo#17205 I ran the workstation surface under the agentos `--fm-*` pattern, extracted **zero** tokens, and got a pass. It looked exactly like success. Iris's point is that a spec caught my instance because the pair was registered — the next one will not have a spec written for it in advance.
...
- `SURFACES` (the registry added in neomjs/neo#17200) — each entry supplies its own `tokenPattern`, which is precisely the field that can be wrong for a newly registered surface.
...
|---|---|---|---|---|---|
| `collectThemeSurfaceFailures` | neomjs/neo#17200's registry design | Both token maps empty ⇒ a named failure identifying the surface and its unmatched pattern | Surfaces that extract tokens behave exactly as today | guard JSDoc | Iris's review of PR neomjs/neo#17205; my own measured zero-extraction pass during neomjs/neo#17200 |
...
- [ ] A surface that extracts tokens normally is unaffected — proven by the existing neomjs/neo#17200 specs staying green without modification.
...
- Found in review of PR neomjs/neo#17205 (#17200 — the registry this applies to)
- Class origin: neomjs/neo-agent-institution#11 AC-2, the amended pixel-suite instrument question
