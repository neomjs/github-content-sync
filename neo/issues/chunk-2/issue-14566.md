...
Leaf 1 of Epic neomjs/neo-agent-brain#117 (direction-weighted Golden Path), graduated from Discussion #14453. GPT's §5.2 STEP_BACK ask D made the Decision-Record path a graduation blocker: *"any SUMMARY_* direction field must cite ADR-0028 and any new graph node/edge class must cite/update ADR-0024"* (DC…17528043). This leaf IS that path, and it sequences FIRST — every other leaf's schema claims derive authority from it.
...
- `learn/agentos/decisions/0024-*.md` — node/edge vocabulary + `PROTECTED_EDGE_TYPES` + decay semantics + §2.9 honesty boundary (the non-blocking-dependency precedent this epic reuses for the neomjs/neo#14503 cold-start sibling).
...
- neomjs/neo#14433's armed direction-slot (nullable, representation-agnostic) is the consumption point the DR must specify, not re-invent.
...
| `#14433` direction-slot | armed design note (Clio) | DR specifies the slot's key grammar; slot stays nullable | null = unattributed, conserved | the new ADR | neomjs/neo#14433 comments |
| `EVOLUTION_GOAL` schema | neomjs/neo-agent-brain#123 Leaf-2 home (`BUSINESS_GOAL` generalization) | shared schema, per #14453/#14548 OQ5 cross-verdict (composable, not merged) | — | the new ADR | neomjs/neo-agent-brain#123; #14548 |
...
Parent: Epic neomjs/neo-agent-brain#117. Source: #14453 (closed). ADR-0023/0024/0028 · neomjs/neo#14433 · neomjs/neo-agent-brain#123 · #14548 · neomjs/neo-agent-brain#122.
