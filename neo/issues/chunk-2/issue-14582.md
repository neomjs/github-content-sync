...
- Tool registration + schemas: the Memory Core MCP server tool registry (`ai/mcp/server/memory-core/`), sibling to the tools shipped by neomjs/neo#13499 (turn-presence writer substrate) / neomjs/neo#13498 (who_is_online liveness projection) — both CLOSED; this is a post-ship regression or a schema/handler drift introduced with a later projection change (#14198 tier-projection touched memory-core tool metadata in the same era — candidate origin, unverified).
...
Wake-daemon policy (that is neomjs/neo-agent-brain#118/#14576); any new presence features; heartbeat cadence tuning.

## Related

neomjs/neo#13499 · neomjs/neo#13498 (the shipped substrate this regressed on) · neomjs/neo#14198 (candidate drift origin, unverified) · neomjs/neo-agent-brain#118/#14576 (the consumers whose accuracy this gates) · neomjs/neo#14426 (sibling infra-forensics class).
...
- ⏳ **AC5** — `who_is_online` freshness reflects active turns again (downstream consumer, neomjs/neo#13498).
