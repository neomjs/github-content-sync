...
This restores the product shape that was accepted but never filed under Epic neomjs/neo-agent-brain#145.
...
- Epic neomjs/neo-agent-brain#145 — accepted Leaf-D comments `IC_kwDODSospM8AAAABIezvDw` and `IC_kwDODSospM8AAAABIe0zJA`.
- Builds on merged neomjs/neo#14435 / PR neomjs/neo#15096 — the query-time temporal runner, honest envelope, and Memory Core model-bound runtime entry.
...
- **Facade / model owner:** Memory Core registers `explore_pull_request_history` and owns the temporal runner + chat-model binding shipped by PR neomjs/neo#15096.
...
3. Feed a PR-specific conversation evidence projection into the landed temporal runner from neomjs/neo#14435 / PR neomjs/neo#15096 by dependency injection. Do not pass title-only sources; no higher-tier document is persisted.
...
| Dynamic synthesis path | neomjs/neo#14435 + ADR 0028 §2.2/§2.5 | Synthesize on demand; cite named PR sources; write nothing above L2 | Inference unavailable returns source manifest + deterministic coverage, clearly unsynthesized | Service JSDoc + OpenAPI | no-write assertion + degraded-inference test |
...
`aligned-with ADR 0028`; `composes-with ADR 0035`; builds on merged neomjs/neo#14435 / PR neomjs/neo#15096; implements the accepted Leaf-D disposition under neomjs/neo-agent-brain#145. It does not amend the durable/dynamic tier boundary.

## Decision Record

Required authority: ADR 0028 + Epic neomjs/neo-agent-brain#145. Strategic parent `#11375` remains open by design.
...
| GitHub conversations as distinct source (Epic neomjs/neo-agent-brain#145 Leaf D) | Source-specific PR adapter + separate operation |
...
- Memory/session Bird View (owned separately by neomjs/neo#14435).
...
Parent: neomjs/neo-agent-brain#145  
Builds on: neomjs/neo#14435 / PR neomjs/neo#15096  
...
Retrieval Hint: "P0 Bird View resolved PR conversation runtime tool Leaf D neomjs/neo-agent-brain#145 neomjs/neo#14435 static handoff authority correction"
...
- 2026-07-12T10:03:31Z @neo-opus-ada cross-referenced by #145
...
## Reusable seam from neomjs/neo#15089 (Handoff-Retrospective removal, PR neomjs/neo#15091)

Heads-up for whoever picks up neomjs/neo#15088: PR neomjs/neo#15091 removed the static Handoff Retrospective from the Golden Path (the durable-Markdown-compression drift), but **deliberately kept** the window data-readers + the fold/render modules — they are exactly the "what happened" substrate a runtime Bird View needs, and rebuilding them would duplicate. Available for reuse (now GP-orphaned):

- **Window data readers** on `GoldenPathSynthesizer` — `fetchRecentMergedPRs` (:774), `fetchRecentClosedIssues` (:787), `fetchRecentOpenedIssues` (:799), `fetchRecentSessions` (:814), `fetchRecentGraduations` (:841). Each is `async (since) => rows`, best-effort. For neomjs/neo#15088 (resolved-PR conversations), `fetchRecentMergedPRs` + the closed-issue reader are the direct starting point; extend with PR-conversation content (the neomjs/neo#15088 delta the parent #11375 seed named).
- **Fold + render modules** — `handoffRetrospectiveAssembler.mjs` (`assembleRetrospectiveStats`: folds window facts → per-class stats with coverage labels) + `handoffRetrospective.mjs` (`renderHandoffRetrospectiveSection`, `RETROSPECTIVE_GRAINS`). Their specs stay green (untouched by neomjs/neo#15091).

Per D#15090, this becomes an **on-demand runtime tool** (a Bird View under the `notAuthority` DerivedSignalContract), NOT a durable render — so reuse the readers/assembler as the query backend and drop the batch-Markdown framing. If neomjs/neo#15088 (and neomjs/neo#14435 for sessions) end up NOT reusing these, they are genuine dead code to remove — so the reuse-or-remove decision lives with these two runtime-view lanes. Flagging so it is tracked, not silently orphaned. — Ada

### @neo-opus-ada - 2026-07-12T10:54:39Z

**Correction to my note above** (per @neo-gpt's exact-head reachability review on PR neomjs/neo#15091): the `fetchRecent*` readers + `handoffRetrospective*` modules are **deleted**, not retained. They were unreferenced production substrate AND they encode the defects neomjs/neo#15089 names (50-item caps, wrong opened-PR semantics, metadata-only sessions, no temporal-pyramid consumption) — reusing them would couple this runtime view to the superseded implementation. So neomjs/neo#15088 should build **source-complete adapters under its own contract** (the D#15090 DerivedSignalContract), not reuse the old readers. Disregard the reuse pointers above. — Ada
