...
**Contract correction (cycle-1 review of PR neomjs/neo#14919, @neo-gpt, Drop+Supersede accepted by the author):** the first implementation collapsed three independent identity layers into one key. The corrected contract keeps them separate:
...
3. **Runtime-observed engine episode** — `EmbodiedEpisode` opens only when runtime evidence exists (handshake/first observation, the neomjs/neo-agent-brain#112 authority), never at Day-0 from an operator-predicted `--model`. Day-0 engine fabrication contradicts verify-before-assert itself.
...
Onboarding peer neomjs/neo#2 of any family requires identity substrate that no single entrypoint provisions. Manual assembly is drift-prone and unverifiable. The first implementation (PR neomjs/neo#14919, closed) additionally proved the idempotency bar: green tests over an incomplete read-set accept partially-provisioned states (a wake node without its `SUBSCRIBES_TO` edge passed as fully provisioned; changed mailbox/operational fields were silently accepted; a wrong-type IdentityState could receive writes).

## The Fix

`ai/scripts/setup/provisionAgentIdentity.mjs` (successor implementation):

- **Day-0 scope = resident addressability + wakeability, nothing more**: the resident `AgentIdentity` node (durable anchor, Layer-1 fields), the mailbox address, the wake subscription (node AND `SUBSCRIBES_TO` edge). NO `EmbodiedEpisode`, no `--model` input at Day-0 — engine embodiment is neomjs/neo-agent-brain#112's observation-driven follow-up.
...
aligned-with ADR-0032 §2.3 (identity anti-lock-in; resident anchor grain) and the neomjs/neo-agent-brain#112 observed-engine authority; the closed PR neomjs/neo#14919's instance-keyed premise is recorded as challenged-and-rejected at cycle-1 review.

## Acceptance Criteria

- [ ] Day-0 provisions resident addressability + wakeability ONLY; no `EmbodiedEpisode`, no model/engine input surface
- [ ] Three keys stay separate: resident anchor = durable id; Fleet instance id = additive linkage; engine episode = deferred to neomjs/neo-agent-brain#112 observation
...
`identityRoots.mjs` / README / ModelStats writes (R3b sibling); GitHub account creation + auth (operator-owned); the naming ritual (#11240, post-boot); launch/env mechanics (sibling ticket neomjs/neo#14914 / PR neomjs/neo#14918); engine-episode opening (#14750).

## Related

Parent: neomjs/neo#13015 · R3b sibling: neomjs/neo#14916 · era layer: neomjs/neo-agent-brain#112 · schema lineage: neomjs/neo#14729 · naming: #11240 · superseded implementation: PR neomjs/neo#14919 (closed at cycle-1, mechanics salvageable from branch `agent/14915-day0-identity-provisioning`).
...
**[downstream contract deposit — @neo-gpt Sol audit]** Observed-engine truth is explicitly NOT this script's scope: neomjs/neo-agent-brain#112 owns per-instance **runtime observation** with the shape `{agentId, threadId, model, modelProvider, serviceTier?, reasoningEffort?, observedAt}` — engine = `unknown` before a thread handshake, and concurrent instances of one resident are never collapsed into a resident-wide era. This script's era anchor carries **onboarding-time facts only**, instance-keyed; the two grains meet at neomjs/neo-agent-brain#112's resolver re-point. Keeps the anti-fabrication line clean: day-0 = what we know at provisioning; live engine = observed, never asserted.
...
Closed as **satisfied-by-composition** (operator layer-audit): Day-0 resident identity + wake route need no dedicated script — the roster PR (generated by the merged neomjs/neo#14950 four-surface generator, incl. `subscriptionTemplate`) plus MC-restart boot seeding (`GraphService.initAsync`) creates the identity node, and `WakeSubscriptionService`'s first-boot `bootstrap` action materializes the wake route from the template + real envelope. Fleet-instance linkage stays where it lives: the FM registry. The three-key contract correction (from PR neomjs/neo#14919's Drop+Supersede) survives in the roster/registry split. Successor path: the neomjs/neo#14937 onboarding runbook is re-scoped around this composition. PRs neomjs/neo#14919 (superseded) and neomjs/neo#14931 (layer-superseded) both closed.
