...
Vega's B4 work on the Fleet cockpit controls is now blocked on a concrete back-signal contract. The accepted contract on neomjs/neo#14611 says the control surface emits `lifecycleIntent {action, agentId}` / `{action, scope: 'fleet'}`, and the C2 adapter writes honest round-trip state onto the per-card provider fields that B4 renders.

This ticket exists because the consumed Lane-C proofs are already closed (`#14563` and `#14595`), while neomjs/neo#14611 is assigned to Vega for the UI/control-cluster half. The adapter side needs its own close target before Euclid edits tracked code.
...
- neomjs/neo#14611 owns the B4 UI/control-cluster rendering: verb buttons, pending/rejected/unauthorized/timeout presentation, and unit/NL proof of the rendered state machine.
...
- The accepted B4/C2 contract is recorded on neomjs/neo#14611: `pendingAction: String|null` and `controlReason: {action, kind, reason}|null`, with C2 clearing stale `controlReason` when a new accepted intent enters pending.
...
|---|---|---|---|---|---|
| `lifecycleIntent` adapter | neomjs/neo#14611 contract comment | consumes `{action, agentId}` and maps to existing registry bridge method | unsupported action -> `controlReason.kind='rejected'`, no bridge call | JSDoc / PR body | unit covers action mapping and unsupported action |
| `pendingAction` provider field | neomjs/neo#14611 B4/C2 contract | set when intent is accepted; clear on settle/reject/deny/timeout | no accepted intent -> remains `null` | JSDoc / provider field summary | unit covers pending lifecycle |
| `controlReason` provider field | neomjs/neo#14611 B4/C2 contract | set terminal non-success reason; clear on new accepted pending and successful settle | no failure -> `null` | JSDoc / provider field summary | unit covers rejected/unauthorized/timeout and stale-clear |
...
- B4 visual rendering, buttons, SCSS, and per-card control-cluster layout; those remain neomjs/neo#14611.
...
- Full Neural-Link round-trip proof unless it naturally composes from neomjs/neo#14611 after B4 lands.
...
Parent neomjs/neo-agent-institution#10 · consumes neomjs/neo#14611 contract · follows closed neomjs/neo#14563 / neomjs/neo#14595 · related neomjs/neo-agent-brain#118.
...
A2A in-flight sweep: checked latest 30 all-state messages immediately before creation; Vega owns B4 neomjs/neo#14611, and no competing C2 adapter claim exists.
Local exact sweep: searched issue/discussion resources plus `apps`, `src`, `ai`, and `test` for `pendingAction`, `controlReason`, `lifecycleIntent`, and provider-field terms; no implementation ticket or code seam found beyond neomjs/neo#14611's B4 contract.
...
Retrieval Hint: "Fleet cockpit lifecycleIntent pendingAction controlReason provider field C2 adapter neomjs/neo#14611"
