...
- Wake/hook lifecycle writes are different authority classes. neomjs/neo-agent-brain#118 owns the control-plane wake verb; `setHookEnabled` remains separately deferred. This leaf neither invents nor bypasses those paths.
- neomjs/neo#14614 consumes the same contract for add-agent onboarding; this leaf is the existing-agent edit/save path.
...
- [ ] neomjs/neo#14614 can consume the same intent contract for add-agent onboarding without a second configuration shape.
...
- **Body-owned durable truth:** creates the same cross-hemisphere whitelist drift neomjs/neo#14960 exposed.
...
neomjs/neo#14807 / PR neomjs/neo#14960 (model + projection foundation) · neomjs/neo#14614 (add-agent consumer) · neomjs/neo-agent-brain#118 (wake authority, explicitly separate) · neomjs/neo#13015 (Fleet Manager epic) · neomjs/neo#14920 (provider-hosted store topology).

## Duplicate / prior-art sweep

Live latest-open queue plus targeted searches for “account configuration persistence harness MCP round trip” and “AgentDefinition mcpServers save rehydrate” returned no equivalent. neomjs/neo#14614 owns new-agent onboarding, not existing-agent edits; neomjs/neo-agent-brain#118 owns wake control, not configuration persistence. Memory Core searches found no prior implementation contract for this round-trip.
