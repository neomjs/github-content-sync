...
- 2026-08-04T20:57:10Z @neo-opus-ada added the `bug` label
- 2026-08-04T20:57:10Z @neo-opus-ada added the `ai` label
- 2026-08-04T20:57:10Z @neo-opus-ada added the `architecture` label
- 2026-08-04T20:57:23Z @neo-opus-ada assigned to @neo-opus-ada
- 2026-08-04T21:01:22Z @neo-opus-ada referenced in commit `ab5e7d8` - "fix(ai): every export path states what it captured, and the plane gets backups again (#16516)

The Memory Core healthcheck reports `backup.lastSuccessful: null, count: 0`. The
plane has no backups at all, and the corpus @neo-opus-grace restored today was
unprotected the moment it came back.

Three export paths own completeness accounting. All three mis-stated what they
captured, in two opposite directions.

OVER-STRICT — a collection that GREW was destroyed as a partial export:

  PARTIAL_COLLECTION_EXPORT: neo-agent-memory exported 32272/32271 records

One row MORE than expected. `expected` is a snapshot taken before the streaming
pass, and the guard used strict inequality in both directions, so one live agent
writing one memory aborted the whole bundle. The collection is still growing —
the same healthcheck reads 32,358.

UNDER-STRICT — the Knowledge Base export could not detect loss at all. It never
counted what it wrote: it returned `count`, the pre-pass snapshot, and dropped
corrupted vectors in its per-id rescue path with a log and no tally. That number
feeds the backup orchestrator's `verifyBundleIntegrity` KB row-count parity, so
the verifier was comparing the snapshot against itself — an export that silently
lost rows verified clean. That is the instrument that certified the backup set
the restore depended on.

The comparison has three outcomes, not two, now single-sourced in
`helpers/exportCompleteness.mjs`:

  exported <  expected   genuine loss     -> throw, unchanged
  exported >  expected   source grew      -> keep the bundle, record the caveat
  exported == expected   clean            -> unchanged

plus `indeterminate` for a non-finite count, treated as fail-loud at both
callers: an absent measurement must never certify a bundle.

A grown export is NOT recorded as clean. Neither path holds a single-instant
read, so the caveat stays at the call site where the mechanism differs — the
vector paths page by offset (an insert in an already-walked page shifts later
rows, so a concurrent write can skip one row while duplicating another and still
finish high); the graph path reads Nodes and Edges as separate statements, so the
two tables can come from different instants.

Each site was found a different way, and the failure name found only the first.
The graph site turned up by grepping the SHAPE rather than the error that fired.
The Knowledge Base site turned up by asking who consumes the returned count —
fixing only the path that threw would have left the corpus-protecting one broken.

The graph window does not need a second process: between the counts and the scans
the method awaits two dynamic imports and `fs.ensureDir`. better-sqlite3 blocks
writes only on the same connection during iteration, so another daemon lands in
that gap freely. Reproduced against a real WAL database, one concurrent insert
from a second connection: expected=100 exported=101, shipped guard destroys the
backup, repaired guard does not, and a genuine 98/101 loss still aborts.

The spec's mechanical sunset was proven red by re-introducing the two-way
predicate, which failed naming the exact line, then restored."
- 2026-08-04T21:03:05Z @neo-opus-ada cross-referenced by PR #16519
- 2026-08-05T09:26:58Z @tobiu referenced in commit `5365a29` - "fix(ai): every export path states what it captured, and the plane gets backups again (#16516) (#16519)
...
The spec's mechanical sunset was proven red by re-introducing the two-way
predicate, which failed naming the exact line, then restored."
- 2026-08-05T09:26:58Z @tobiu closed this issue
