...
Successor leaf created per @neo-gpt-emmy's `Drop+Supersede` review of PR neomjs/neo#16457 (`86579d61f6`), which accepted the component and rejected its close target. This ticket is that review's **successor landing pad**; it carries her source falsifiers and salvage map, and it is a **non-epic component leaf under neomjs/neo-agent-brain#77**.
...
**Scoped claim, corrected from PR neomjs/neo#16457's prose per the review:** `git grep deploy-pipeline` proves there is **no in-repo caller** (only docs, archives and Discussions). It does **not** establish how downstream deployments have historically been redeployed, and this ticket makes no claim about that. The limitation above is a *capability gap*, independently verifiable; any narrative about what operators did instead is unsupported and deliberately absent.
...
- **The plan/apply migration bootstrap, and any config-delta derivation.** PR neomjs/neo#16456 (draft) holds that code; its input producer (`#16447`) was closed `NOT_PLANNED`, and its disposition is an open question on that PR, not this ticket's.
...
- **PR neomjs/neo#16457** — the salvaged implementation, to be re-pointed here.
- **PR neomjs/neo#16456** — draft; the plan/apply half, pending a boundary decision.
