...
- `package.json` — `dependencies: {}` (empty, correct); `devDependencies`: 45 at filing; **41 after sibling neomjs/neo#16363 shipped (PR neomjs/neo#16369)**. The Brain set per the census: `better-sqlite3` + `chromadb` (+ `@chroma-core/default-embed`, the runtime-resolved default embedder with no direct repo importer).
...
4. **Docs:** the two paths land in the contributor-facing docs (deployment guide — neomjs/neo-agent-brain#86 is a natural consumer).
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| `package.json › devDependencies.{better-sqlite3, chromadb, @chroma-core/default-embed}` | neomjs/neo#16204 census (re-run live at intake) | Removed from the base manifest; identical pins move to `package.brain.json` | n/a (removal) | GETTING_STARTED | Tier-guard spec (absence assertion); fresh base-install log (AC1) |
...
| Root `prepare` Windows portability (pre-existing, out of scope here) | neomjs/neo#16393 | Routed to successor — it blocks native Windows on dev today, independent of the tier | — | — | The successor ticket |

## Out of Scope

- The five zero-usage removals — sibling ticket neomjs/neo#16363 (shipped as PR neomjs/neo#16369: 4 of 5 removed; `mermaid` retained on the portal-asset evidence).
- The two host-edge importer removals — owned by neomjs/neo#16180 (wake path) and neomjs/neo#16202 (NL recorder); this ticket's mechanism must not depend on their landing order (either order works; note it in the PR).
- `chromadb` client removal — it is a Brain-tier keep (container-side managers use the sanctioned JS client).

## Related

- neomjs/neo#16204 — the census (evidence + the accepted boundary; PR neomjs/neo#16354)
- neomjs/neo#16363 — the removal sibling
- neomjs/neo#16180 / neomjs/neo#16202 — host-edge importer owners
- neomjs/neo-agent-brain#86 — deployment-guide consumer
