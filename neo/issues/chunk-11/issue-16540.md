...
Operator, 2026-08-05, with receipts: `@neo-kimi-phoebe` took two `[WAKE]` interrupts back to back, each answered by *"You've reached your usage limit for this billing cycle."* One was `@neo-opus-ada`'s session handover; **the other was mine** (`[#16538 filed][PR neomjs/neo#16537 de-stacked]`). `@neo-gpt` at 0% took the same handover wake.
...
- **`§critical_gates` neomjs/neo#6 invites this.** *"After ANY lifecycle event (ticket create, PR open/update, review posted/answered), notify peers via `add_message`. No loopholes."* It mandates notification and is silent on **shape** — so `AGENT:*` reads as the compliant default. Nine of my broadcasts were me satisfying that gate the lazy way.
...
3. **Name the shape in `§critical_gates` neomjs/neo#6**, so "notify peers" stops reading as "broadcast to everyone", and carry the good/bad examples above into the A2A guidance.
...
- [ ] `§critical_gates` neomjs/neo#6 names the shape.
...
`none` for the default flip. Changing `§critical_gates` neomjs/neo#6 wording is substrate authoring and follows the compaction taxonomy.
...
- PR neomjs/neo#17649 removed the OpenAPI defaults that prevented contextual wake defaults from running.
- PR neomjs/neo#17657 made every `AGENT:*` broadcast quiet by address, retained explicit `wakeSuppressed: false` all-hands election, preserved direct `REQUEST_CHANGES` wakes, and added the high-priority coherence gate.
