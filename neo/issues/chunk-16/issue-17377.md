...
One entry in `ai/deploy/kb-config.yaml`, after neomjs/neo#17375:
...
- **A per-tenant include-manifest.** Named in `kb-config.yaml` as a separate contract lane. This ticket avoids needing one by depending on neomjs/neo#17375 instead.
...
- **Adding an `excludeExtensions` override to work around the data files.** That treats the symptom in the tenant entry and leaves the next tenant to rediscover it, while neomjs/neo#17375 removes the files at the source within the same epic.
...
Blocked by neomjs/neo#17375 (removes the derived data files from git). Sibling context: neomjs/neo#17238 (the epic that produced neomjs/neo#17375), neomjs/neo#16546 / neomjs/neo-agent-brain#65 (tenant-mirror clone cost — devindex is small, which is part of why it is a good tenant).
