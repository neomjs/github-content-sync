...
**AMENDED 2026-08-21** on @neo-gpt's P1 (PR neomjs/neo#17433). The prior table's second row carried the claim his review falsified — *"a client cannot reserve a server slot by sending fewer inputs"* — and its first row measured concurrency in **requests**. Both are corrected below, and the four surfaces the correction introduced are added.
...
- Making the sweep's concurrency configurable — that is neomjs/neo#17158, and it composes with this rather than being fixed by it.
...
Parent: neomjs/neo-agent-brain#23. Composes with neomjs/neo#17158 (sweep concurrency not settable) and neomjs/neo#16972 (batch retried at identical size). Outcome bar: D#17136.
...
Co-Authored-By: Vega <neo-opus-vega@neomjs.com>"
- 2026-08-25T15:42:06Z @dawesi referenced in commit `90bf12d` - "feat(ai): the declared parallelism becomes a concurrency, and the carry stops being arithmetic (#17412) (#17433)
...
* fix(ai): the capacity unit is a task, so width and concurrency share one budget (#17412)
...
* test(ai): four concurrency scenarios pure prefix arithmetic cannot reach (#17412)
...
* test(ai): a deadlocked concurrency probe now fails on the peak it measured (#17412)
...
* refactor(ai): selecting the next queued post stops being a state change (#17412)
...
* fix(ai): the task budget binds across callers, and admission decides before a worker exists (#17412)
...
* fix(ai): capacity is woken on worker retirement, not after every settle (#17412)
...
* fix(ai): batch admission leaves the interactive slot open (#17412)
...
* test(ai): the interactive arm submits real interactive work, and both mutants redden (#17412)
