...
> native parent neomjs/neo-agent-institution#24 and merged prerequisites are folded into Related; ADR 0029 wording now cites
...
1. **`FleetGrid` rebuilds on `Neo.list.Component`** — `animate: true`, `itemHeight` = the measured 126px anatomy constant, fluid columns via neomjs/neo#17552's `minItemWidth`, preserving today's ladder (1-up ≤640, 2-up default, 3-up ≥1280 of grid measure; the container-query values move from `FleetGrid.scss` into the JS ladder input — ADR 0029 §2.6 pane-layout-blindness intact: the list measures only its own rendered surface). `AgentCard` becomes the component item (create/reuse keyed by record, calendar-List pattern); its inner anatomy, controller and lifecycle events are untouched. The stable head (title · stale marker · presence-cap chip · HealthBar) stays the sibling above the list; the empty-fleet CTA renders outside the list while the store is empty.
2. **A compact sort cluster in the grid head** (grid-owned — the cockpit top toolbar is neomjs/neo-agent-institution#23's surface, untouched). Three options:
...
|---|---|---|---|---|---|
| `AgentOS.view.fleet.roster.List` (new consumed view) | neomjs/neo-agent-institution#24 laws 0–2; `Neo.list.Component`; `Neo.list.plugin.Animate` | provider-owned `fleetRoster` records render as stable AgentCard instances; sort moves and filters fade the same instances | empty Store renders the existing CTA outside the list; reduced motion collapses transitions | class JSDoc | roster unit identity/motion fixtures + headed join/leave/reorder |
...
- [ ] Reduced motion honored end-to-end (the neomjs/neo#17552 rule; no cockpit-side motion overrides).
- [ ] `npm run check-theme-surfaces` green; tokens only.

## Out of Scope

- Card CONTENT anatomy (what a card says stays as shipped; neomjs/neo-agent-institution#23 owns header/detail IA).
...
- **Prerequisites landed:** neomjs/neo#17552 / PR neomjs/neo#17556 (`list.plugin.Animate`) and neomjs/neo#17560 / PR neomjs/neo#17566 (fleet topology).
- **Parent:** neomjs/neo-agent-institution#24. **Related umbrella:** neomjs/neo-agent-institution#10.
- Boundaries: neomjs/neo-agent-institution#23 (header/detail IA), neomjs/neo-agent-institution#20 (pane content design), neomjs/neo#17550 / PR neomjs/neo#17585 (activity list + reusable list fixes), neomjs/neo-agent-institution#22 (cockpit LOC bar), neomjs/neo#16824 (empty-roster truth — the CTA/empty semantics defer to it).
- Post-merge: neomjs/neo-agent-institution#11 re-records the visual baselines (standing owner; the goldens are rendered-platform artifacts per its ledger).
...
Live latest-open sweep: checked latest 20 open issues at 2026-08-22T15:51:54Z (`gh issue list --state open --limit 20`); no equivalent found. A2A claim sweep at 15:52Z (latest 14, recency-bound): no competing claim; the adjacent neomjs/neo#17550 lane-pick (Emmy) is a disjoint surface.
...
- **Epic gate:** independent Epic Review posted on neomjs/neo-agent-institution#24 at https://github.com/neomjs/neo-agent-institution/issues/24#issuecomment-5383488733.
...
- **Currency / successor-risk:** no duplicate or superseding ticket/PR. neomjs/neo#17552/PR neomjs/neo#17556 and neomjs/neo#17560/PR neomjs/neo#17566 are merged prerequisites; the only implementation is Clio’s pushed, un-PR’d parked head `11d564107d`.
- **Missing close-link sweep:** no merged PR delivers neomjs/neo#17553; `closedByPullRequestsReferences` is empty.
- **ADR successor-risk:** `adr-aligned` — artifact neomjs/neo#17553 postdates accepted ADR 0029 and follows §2.6 pane-layout-blindness by measuring only the rendered roster surface. The former “own-width” shorthand was corrected to the actual ADR contract.
...
**Takeover boundary:** preserve Clio’s authored commit and design lineage; Emmy owns completion/rebase/polish. No AgentCard content redesign, no neomjs/neo-agent-institution#22 decomposition, and no new ticket for the list-chrome defect—it is direct neomjs/neo#17553 scope.
