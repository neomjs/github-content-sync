...
|---|---|---|
| neomjs/neo#17112 | batch ingestion persisted embeddings only per full slice | `VectorService` — persists *completed embeddings*, not a sweep checkpoint |
| neomjs/neo#16826 | a lease yield discarded completed provider chunks | same layer, same reason |
| neomjs/neo#17062 | embedding admission had no aging | `TextEmbeddingService` — admission order, not checkpointing |
| neomjs/neo#17132 | a repo held its concurrency slot until its corpus was exhausted | introduced the slice yield this defect now depends on |
| neomjs/neo#17349 | a clean partial slice accrued a failure streak | correctly reports `streak 0 -> 0`; the repo is not failing, it is looping |
| neomjs/neo#16822 | `kbSync` checkpointed later than its fairness bound | `kbSync`, which is disabled on pull-mode deployments |
...
- Lease starvation under this holder — neomjs/neo#17398, fix already open as PR neomjs/neo#17399.
...
- **Do not blame envelope materialization.** I did, in the first version of this body, and the slot log falsified it. Network was also ruled out (9 Kb/s receive, mirror fully backfilled per neomjs/neo-agent-brain#65) before reaching for it.
