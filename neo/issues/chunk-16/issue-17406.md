...
updatedAt: '2026-09-03T21:50:18Z'
...
Split from neomjs/neo-agent-brain#24, which keeps the wiring. This leaf is the measurement itself: two axes over a commit's added lines, plus the bars they are judged against.
...
- neomjs/neo-agent-brain#24 — parent; keeps the hook wiring.
- neomjs/neo#16217 — the artifact-surface sibling; same operator observation, different reader.
- neomjs/neo#17396 — checkers whose granularity is finer than the structure they judge.
...
- 2026-08-26T14:57:23Z @tobiu added parent issue #24
- 2026-08-28T15:37:38Z @neo-opus-vega unassigned from @neo-opus-vega
### @neo-opus-grace - 2026-09-03T21:50:17Z

## The required substrate points at a path that left the repo

"Required substrate for the successor" names `buildScripts/util/check-aiconfig-test-mutation.mjs:160-200` as the sanctioned Acorn precedent. That file is no longer in `neomjs/neo` — it moved out in the split and now lives at `ai/scripts/lint/check-aiconfig-test-mutation.mjs` in `neomjs/neo-agent-brain`.

The precedent itself is intact and is a better fit than the line reference suggested: it blanks comment, string, template-quasi and regex spans via `onComment` + `onToken` (`acorn.parse`, not `acorn.tokenizer`), and **fails closed on a file this Acorn cannot parse** — which is this ticket's own `not-measured ≠ clean` requirement, already implemented.

What this changes for a successor working in `neo`: `grep -R onComment buildScripts/` returns nothing. The five Acorn users under `buildScripts/` (`check-fixed-sleeps`, `check-engine-brain-boundary`, `check-derived-domain`, `esmDistTransforms`, `astTemplateProcessor`) all parse for AST nodes, not comment ranges. **There is no in-tree comment-range precedent to lift here** — the successor either crosses to the brain repo for the pattern or re-derives it, and that choice is worth making deliberately rather than discovering at implementation time.

Surfaced from #18234, which deletes the prose this ticket would have measured.

*Grace (Claude Opus 5), nightshift heartbeat. Not claiming this lane.*


- 2026-09-03T21:50:17Z @neo-opus-grace cross-referenced by #18234
