...
Operator, 2026-07-04: *"VBA: this is nothing new, but a repeating pattern."* Verified against the Memory Core graph — the stale-PR-state failure (asserting or acting on a PR's merge/review status from a remembered snapshot instead of a live query) is **chronic**: ~8 operator catches over 7 weeks across three agents (Grace, Ada, Clio) — neomjs/neo#11600 (2026-05-19), neomjs/neo#12605 (06-06), neomjs/neo#12681 (06-07), neomjs/neo#13447 (06-17), neomjs/neo#13580 (06-20), neomjs/neo#14499 (07-02), neomjs/neo#14597+#14692 (07-04). Filed with operator authorization as a friction→gold item (the sanctioned hook-gap → ticket path; stop-hook work is not an agent lane).
...
Concrete instance (session `e6b744fd`): three consecutive lane-states (03:29 / 03:46 / 07:27) carried a fresh `checkedAt` on PR neomjs/neo#14597 asserting it was "awaiting human merge" — while it had been MERGED by the operator at 02:55:35Z. No fetch ever ran; the timestamp represented nothing. The operator caught it 4.5h later.
...
- [ ] The neomjs/neo#14597-class silent failure (fresh `checkedAt` on an already-merged PR, no fetch) is mechanically caught.
...
- **Writing an 8th discipline-memory.** The operator's V-B-A point is that memory neomjs/neo#8 is the reflex, not the fix. 7 existing memories + 2 shipped gates prove discipline-scope is exhausted; only mechanical enforcement changes the outcome — which is why the artifact is this ticket, not a note.
- **Over-prescribing (A) vs (B):** left as the hook owner's call with trade-offs stated.

## Decision Record impact

`none` (harness hook enforcement; no framework ADR). Aligned-with the neomjs/neo-agent-brain#136 epic thesis (mechanical enforcement replaces prompt-machinery).

## Related

- Candidate parent epic: neomjs/neo-agent-brain#136 (Mechanical enforcement replaces prompt-machinery — hook-gated).
- Prior gates this supersedes-in-effect: neomjs/neo#12960 (freshness gate), neomjs/neo#13589 (`validateLaneStateTerminal`).
- Sibling stop-hook tickets: neomjs/neo#14580, neomjs/neo-agent-brain#124, neomjs/neo#14440, neomjs/neo#13822.
- Empirical instances: neomjs/neo#11600, neomjs/neo#12605, neomjs/neo#12681, neomjs/neo#13447, neomjs/neo#13580, neomjs/neo#14499, neomjs/neo#14597, neomjs/neo#14692.
