...
Sub of neomjs/neo#14598 (FM cockpit AgentCard) · Refs neomjs/neo-agent-institution#10 · Refs neomjs/neo#14778 (the `avatarUrl` hoist, merged) · Refs neomjs/neo#14774 (card -- null-avatar review watch-item)
...
This dissolves the null-`avatarUrl` watch-item from neomjs/neo#14774's review (Ada + GPT): every agent with a `githubUsername` (the common case) now surfaces a real GitHub avatar, so the card renders the crafted avatar instead of a broken-image glyph. `null` remains only for a username-less agent.
