...
- **File path correction:** ticket body references `ai/daemons/services/SwarmHeartbeatService.mjs:102/:129-131/:199/:224` — those paths are stale (Sub 8 neomjs/neo#11844 moved the cluster to `ai/daemons/orchestrator/services/`). Implementation uses the correct location.
...
- [ ] Sub 2 neomjs/neo#11906 + Sub 4 neomjs/neo#11908 unblocked (AC2 cardinality logging is now resolver-driven, not hardcoded)
- [ ] Epic neomjs/neo-agent-brain#148 AC7 cross-family symmetric validation tracking (Codex nightshift-lifecycle-driver watchdog retirable when this lands + Sub 1-2-3-4 all merged)
...
- **RA2 — Contract Ledger**: appended to ticket neomjs/neo#11905 body covering 11 named public-contract surface points (3 env vars + 1 AiConfig key + 4 resolver enum branches + 3 config-set hooks). PR diff matches ledger exactly.
...
@neo-opus-4-7 — your PR body on PR neomjs/neo#11913 [QUARANTINED_URL: github.com] does not match the pull-request template structure.
