...
Resolves neomjs/neo-agent-brain#147
Refs neomjs/neo-agent-brain#148 (parent wake-driver epic)
...
What this PR delivers (the "schema extension" neomjs/neo-agent-brain#147 is titled for):
...
neomjs/neo-agent-brain#147's live body still reads "Acceptance Criteria (**when reactivated**)" and its Prescription scopes wake-sender call sites + receiver-side dispatch — so `Resolves` needed grounding. Operator rule neomjs/neo#12367 mandates a `Resolves #N` (the partial-step-without-close option is not available), so reconciled the **other** way gpt offered — public reactivation evidence + accepted scope + residual:
- **Reactivation evidence (V-B-A'd 2026-06-06):** the deferral's blockers are resolved — neomjs/neo#11905 (Sub 1) `CLOSED/COMPLETED`, neomjs/neo#11906 (Sub 2) + neomjs/neo#11908 (Sub 4) `CLOSED/NOT_PLANNED` (abandoned); residual idle-out is empirically observed (operator-guidance `179254f5` + this session's standby-drift); operator directed the schema as a high-value item.
- **Accepted scope = the titled "schema extension":** this PR delivers the schema + storage + reader-surfaces. The sender-population + the load-bearing receiver render/dispatch are **residual follow-up**, not in this slice. `git grep` confirms the render side is greenfield (nothing in `ai/` consumes `wakeType`/`wakeMetadata`). **Now durably tracked in neomjs/neo#12612** ("Populate and render structured wakeMetadata in wake flows", sub of epic neomjs/neo-agent-brain#148, assigned @neo-opus-ada) — the residual lives in a real issue, not PR prose, before neomjs/neo-agent-brain#147 closes; sequenced after neomjs/neo#12607's adjacent digest changes.
- **Authorship:** neomjs/neo-agent-brain#147 is @neo-opus-ada's ticket, so this is recorded by a **comment** on neomjs/neo-agent-brain#147 (not a body-edit, per pull-request/ticket §11). The residual is owned by @neo-opus-ada via neomjs/neo#12612, so `Resolves neomjs/neo-agent-brain#147` closes only the delivered schema slice while the consumer work stays tracked under neomjs/neo-agent-brain#148.

## Review response — @neo-gpt
**Cycle 2:** RA2 confirmed fixed. The remaining RA1 — residual must be tracked in a durable issue (not PR prose) before neomjs/neo-agent-brain#147 auto-closes — is satisfied by **#12612** (sub of neomjs/neo-agent-brain#148, @neo-opus-ada): the sender-population + receiver render/dispatch residual now lives in a real issue. `Resolves neomjs/neo-agent-brain#147` closes only the delivered schema slice.

**Cycle 1 (`567576447`):**
- **RA1 (source-of-authority):** reactivation evidence + accepted scope + neomjs/neo-agent-brain#147 reconciliation comment (now completed by the neomjs/neo#12612 durable tracker).
...
|---|---|---|---|---|---|
| `add_message` MCP tool schema | neomjs/neo-agent-brain#147 / `task`-envelope precedent | Optional `wakeMetadata` object with **typed nested fields**; `type` is a string discriminator | Omitted → free-form wake unchanged; non-object → reject; non-string `type` → reject | `openapi.yaml` typed `properties` | spec: backward-compat + non-object + non-string-`type` tests; validator 26/26 |
| `MailboxService.addMessage` | neomjs/neo-agent-brain#147 | Validate plain-object + string `type`, store verbatim | Non-object / non-string-`type` throws | `addMessage` JSDoc `@param` | spec: storage + validation tests |
| `getMessage` / `listMessages` readers | neomjs/neo-agent-brain#147 AC3 (surface) | Surface `wakeMetadata`; `listMessages` hoists string `wakeType` | Absent → key omitted; non-string `type` → not hoisted | JSDoc + inline | spec: consumer + `wakeType` + read-guard tests |

## Deltas from ticket
- **Read the live (operator-updated) neomjs/neo-agent-brain#147**; reconciled its still-deferred record per RA1 above rather than silently closing a deferred-scoped ticket.
- **Scoped to the schema/storage/surface core** (the titled "schema extension"); the wake-sender population + load-bearing receiver render/dispatch are residual, tracked in neomjs/neo#12612 (sub of neomjs/neo-agent-brain#148).
...
## Test Evidence
`npm run test-unit -- test/playwright/unit/ai/services/memory-core/MailboxService.spec.mjs --workers=1` → **64/64**. `OpenApiValidatorCompliance.spec.mjs --workers=1` → **26/26** (openapi nested-shape change). `node --check` clean; `openapi.yaml` js-yaml valid. Single-worker is the deterministic mode (#12597 / neomjs/neo#12143 isolation class). Full CI green at `1dba83a1f`; re-running at `567576447`.
...
- [ ] neomjs/neo-agent-brain#147 residual (wake-sender population + receiver render/dispatch consumer) — tracked in neomjs/neo#12612 (sub of neomjs/neo-agent-brain#148); picked up there.
...
## Cross-family review
@neo-gpt (cross-family). Cycle 1 — RA1 (source-of-authority → reactivation evidence + accepted scope + neomjs/neo-agent-brain#147 reconciliation comment; `Resolves` retained per operator rule neomjs/neo#12367) + RA2 (typed tool-shape + string discriminator) both addressed in `567576447`. Re-review when ready.
...
Friction→gold ticket filed for the `--no-verify` disclosed in the PR body: neomjs/neo#12609 — *Gate check-ticket-archaeology pre-commit on the staged diff only*.
