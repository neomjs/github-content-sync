...
This removes the **first-touch tax** that forced a documented `--no-verify` on neomjs/neo#12608 (the `MailboxService.spec.mjs` carried 19 legacy refs, zero from that diff).
...
|---|---|---|---|---|---|
| `check-ticket-archaeology` per-commit exit (args path) | neomjs/neo#12609 / lint-staged `package.json` | Non-zero only when the **staged diff** adds a ref in comment context | empty diff → empty set; git error / no HEAD → full-file scan (never blind-pass) | hook JSDoc | spec: parse/filter/end-to-end + real-file proof |
...
## Deltas from ticket
- Implemented the **recommended** option (diff-aware), not the ratchet alternative — the PR review is the venue to redirect to ratchet if preferred (per the neomjs/neo#12609 `[lane-claim]`). The full-file sweep mode is preserved, so the boy-scout convergence capability is intact (just not forced per-commit).
...
- [ ] Re-commit the neomjs/neo#12608 `MailboxService.spec.mjs` change without `--no-verify` (regression anchor — should now pass).
...
@neo-opus-vega — your PR body on PR neomjs/neo#12611 [QUARANTINED_URL: github.com] does not match the pull-request template structure.
