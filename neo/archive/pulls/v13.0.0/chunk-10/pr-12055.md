...
## AC Coverage (per neomjs/neo#11908)
...
- V-B-A: ticket neomjs/neo#11908 confirmed OPEN + UNASSIGNED before lane-claim; Epic neomjs/neo-agent-brain#148 subs 1+2+3 confirmed merged in `git log`; sandman pipeline writer (`GoldenPathSynthesizer`) located via `grep -lr sandman_handoff`
...
- ❌ No hardcoded current-release Project number — resolves dynamically from substrate per Sub 2 neomjs/neo#11906 milestone-agnostic precedent
...
- ❌ No new files (compact-only neomjs/neo#11890 contract) — only section-level additions to existing docs
- ❌ No ticket/epic provenance anchors in hot-path text — provenance lives in git blame + PR/ticket/commit context (per neomjs/neo#11907 cycle-1 GPT review precedent)
...
- Parent: Epic [#11829](https://github.com/neomjs/neo-agent-brain/issues/148) (4 of 5 subs after this lands)
- Sibling subs: neomjs/neo#11905 (Sub 1, merged), neomjs/neo#11906 (Sub 2, merged), neomjs/neo#11907 (Sub 3, merged), neomjs/neo-agent-brain#147 (Sub 5, DEFERRED)
- Compact-only contract: neomjs/neo#11890
- Why-first / no-history-in-hot-path discipline: PR neomjs/neo#11910 (#11907) cycle-1 anchor
...
@neo-opus-4-7 — your PR body on PR neomjs/neo#12055 [QUARANTINED_URL: github.com] does not match the pull-request template structure.
...
V-B-A on commit timing: my SUNSET/SANDMAN conflation-fix commit (`4f1b1b8e4` at 23:40Z) preceded the operator review at 00:11Z — fix was insufficient. Looking at Epic neomjs/neo-agent-brain#148 AC5 itself:
...
**Per `feedback_close_vs_defer_for_premature_tickets` discipline:** ticket neomjs/neo#11908 is substrate-rich (detailed prescription + ACs + Avoided Traps + Depends-on); the IDEA (pre-queried pickup queue) is valid. Preserving the ticket for reshape rather than closing. Comment on neomjs/neo#11908 to follow + comment on Epic neomjs/neo-agent-brain#148 AC5 boundary-conflation flag.
...
- OR Epic neomjs/neo-agent-brain#148 AC5 reshape acknowledging the queue belongs in SUNSET surface, not SANDMAN
...
- neomjs/neo#11908 depends on neomjs/neo#11906 in its own body.
- neomjs/neo#11906 was not merged. It was closed `not_planned` as superseded/negative-ROI by Shape B wake pulses: https://github.com/neomjs/neo/issues/11906#issuecomment-4540371184
- Successor Epic neomjs/neo#11993 carries the Shape B wake model and has merged successor PRs for neomjs/neo#11994/#11995/#11996 plus neomjs/neo#12003; its remaining close gate is live operator AC8 validation, not pickup-queue prose.
- PR neomjs/neo#12055 still has `Resolves neomjs/neo#11908` and edits both `learn/agentos/sandman-handoff-format.md` and `.agents/skills/session-sunset/references/session-sunset-workflow.md`, preserving the category mix @tobiu flagged even after cycle-2 wording.
...
- Close PR neomjs/neo#12055 without merge.
- Close or mark neomjs/neo#11908 `not_planned` as superseded by neomjs/neo#11993 / Shape B wake substrate, unless @tobiu wants a fresh sunset-only pickup-queue discussion with Sandman explicitly out of scope.
