...
Resolves neomjs/neo#9893
Partially addresses neomjs/neo#9892 (CodebaseOverview scope extension per [comment](https://github.com/neomjs/neo/issues/9892#issuecomment-2797508641))

## Changes

### AGENTS.md — §2 Anti-Hallucination Policy (Major)
- Expanded from 7 lines to a structured subsection with **§2.1 Tool Hierarchy** table positioning `ask_knowledge_base` as the neomjs/neo#1 tool
...
### CodebaseOverview.md — Query Entry Points + When Stuck (Minor, from neomjs/neo#9892 comment)
...
Added CodebaseOverview.md changes (from neomjs/neo#9892 comment scope extension) that were not in the original neomjs/neo#9893 body. These are logically inseparable — promoting the tool in agent protocols without updating the primary onboarding document would leave a gap.
...
> *   **`[PRODUCTIVITY]`**: 100 - All goals of neomjs/neo#9893 achieved plus the neomjs/neo#9892 comment scope extension. The user's review feedback was incorporated in-session.
...
> *   **Target Epic / Issue ID:** Resolves neomjs/neo#9893
> *   **Related Graph Nodes:** neomjs/neo#9892 (CodebaseOverview expansion — partially addressed via the Query Entry Points / When Stuck changes), neomjs/neo-agent-brain#165 (Strategic Constraint Nodes — downstream consumer of better tool selection), `ask_knowledge_base` operationId, `query_documents` operationId
> 
> ---
> 
...
> *   **`[TOOLING_GAP]`**: `get_pull_request_diff` MCP tool failed with exit code 1 for PR neomjs/neo#9894 during this review. Fell back to `gh pr diff` via shell. Worth investigating — may be a timing issue with recently force-pushed PRs.
