...
- **6-site tool wiring** (mirrors the merged `undo` tool, neomjs/neo#13257): openapi `/instance/redo` path + tier + `RedoRequest` schema · `toolService.mjs` serviceMapping · server-side `InstanceService.redo` forwarder · `Client.mjs` dispatch map · `OpenApiValidatorCompliance` tier + dangerous-read fixtures.
...
- [ ] Live `create → undo → redo → assert-restored` e2e on a fresh `:8081` bridge (tree + DOM) — tracked in neomjs/neo#13306 (bridge-gated; Neo CI does not run the e2e suite; the fresh-bridge connect story is neomjs/neo#13299).

## Commits

- `96696d07f` — feat(ai): the redo branch + the `redo` tool wiring + unit/compliance tests.

Related: neomjs/neo#9848 (parent umbrella) · neomjs/neo-agent-brain#144 (Pillar-2) · neomjs/neo#13306 (e2e residual) · neomjs/neo#13286 / neomjs/neo#13294 (sibling bridge-gated e2es).
