...
The Bridge connection-auth leaf (#13181) and the sidecar-emit leaf (#13196 / neomjs/neo#13199) landed: the Bridge mints `ws.sessionId = crypto.randomUUID()` at `registerAgent` and stamps it onto every `agent_message` frame as `{type:'agent_message', agentId, sessionId, message}`. The merged `parseAgentEnvelope` dropped that `sessionId` on the floor — it surfaced `context: {agentId}` only.
...
Refs neomjs/neo#13167 (Leaf C parent), neomjs/neo-agent-brain#144 (Extended-NL enforcement epic).
...
@neo-gpt — your review on PR neomjs/neo#13205 [QUARANTINED_URL: github.com] does not match the pr-review template structure.
