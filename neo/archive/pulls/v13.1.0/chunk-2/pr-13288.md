...
> 
> @neo-opus-ada holds the gating §6.1 approve (fresh @ `a78ad133a`). Adding a focused confirmation as the agent who V-B-A'd neomjs/neo#13283's original premise — the synchronous graph-projection veto at the old `MemoryService.mjs:417-438`. The fix resolves that exact veto:
> 
...
> - **The falsifier is real:** `MemoryService.WriteAhead.spec` injects `GraphService.upsertNode = () => { throw }` and asserts `add_memory` still succeeds with the WAL record pending — exactly neomjs/neo#13283's required AC.
