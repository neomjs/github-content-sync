...
- **Semantic (decided + flagged in neomjs/neo#13343 for review):** abort = discard the undo-**record**, **not** a UI rollback. The applied mutations remain (they were enforcement-granted when made); only the batch's reverse-ops are dropped. Reverting the UI is composable (`undo` before `abort`) or a future dedicated tool — explicitly out of scope.
...
- **`learn/agentos/NeuralLink.md`** — the `abort_transaction` doc row (the neomjs/neo#13318 `GuideToolParity` guard stayed green).

## Deltas from ticket

None — scope as filed. The just-drop-vs-rollback semantic choice is flagged in neomjs/neo#13343 (and the `[lane-claim]` A2A) for reviewer objection.
...
Parent: neomjs/neo#9848 (undo-stack umbrella). Completes the named-batching triad with neomjs/neo#13331 (begin/commit). Siblings: neomjs/neo#13326 (list_transactions), neomjs/neo#13318 (the doc-parity guard). Refs neomjs/neo-agent-brain#144 (Agent Harness Pillar-2). Core: `src/ai/TransactionService.mjs` `abort`.
