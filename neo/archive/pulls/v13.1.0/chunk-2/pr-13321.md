...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#13318

Adds a unit guard asserting agent-consumed tool guides stay in **tool-coverage parity** with their OpenAPI SSOT — the friction→gold from neomjs/neo#13309's doc-drift (a guide claiming "33 tools" while the live surface exposed 41, the entire mutation surface undocumented, caught only by a manual diff after it shipped and costing 3 review cycles on neomjs/neo#13310). v1 covers `learn/agentos/NeuralLink.md` ↔ the neural-link openapi; the guard fails-loud on either-direction drift, naming the offending tools.
...
- **`learn/agentos/NeuralLink.md`** — adds the `redo` row (the deferred neomjs/neo#13310 follow-up; now that neomjs/neo#13307 merged, the openapi exposes `redo`), restoring doc==openapi=41 so the guard is green. This discharges the post-#13307-merge redo-doc-row I committed to on neomjs/neo#13310.

## Deltas from ticket

Folded the deferred `redo` doc-row into this PR rather than a separate tiny one — it is the parity-fix that makes the new guard green, so the two belong in one change. neomjs/neo#13318 was filed for the guard alone; the redo-row is the prerequisite that neomjs/neo#13307's merge made due.
...
Friction source: neomjs/neo#13309 / neomjs/neo#13310 (the doc-drift + its 3-cycle fix). Sibling guard: `OpenApiValidatorCompliance.spec`. The folded redo-row closes the neomjs/neo#13310 → neomjs/neo#13307 follow-up. Refs neomjs/neo-agent-brain#144 (the tool surface IS the agent's body-interface — its guide must not lie to the agent reading it).
