...
Resolves neomjs/neo#13302

Adds one compact routing note to the Agent Harness entry workflow: Electron shell/build-root work routes through neomjs/neo#13033, and new Electron-support tickets should not be filed unless live state shows disjoint scope, supersession, or explicit handoff.
...
No scope additions. The implementation keeps the note inside `.agents/workflows/agent-harness.md` and does not modify AGENTS.md, skills, ADRs, neomjs/neo#13033 ownership, or Electron runtime code.

## Substrate Slot Rationale

Modified surface: `.agents/workflows/agent-harness.md`, an on-demand mission-entry workflow. Disposition delta: rewrite/clarify one existing step. Trigger-frequency: harness-entry only. Failure-severity: duplicate-lane coordination friction. Enforceability: discipline-only via workflow read. Decay mitigation: the note includes live-state escape hatches for disjoint scope, supersession, and explicit handoff, so it can be removed or updated when neomjs/neo#13033 changes.

## Test Evidence

- `git diff --check` -> passed.
- `nl -ba .agents/workflows/agent-harness.md` -> verified the workflow remains a compact 17-line entry document.
- Pre-create duplicate sweep found neomjs/neo#13033 as the existing Electron implementation lane and no open workflow-routing equivalent.

## Post-Merge Validation

- [ ] Fresh Agent Harness entrant sees neomjs/neo#13033 before filing or claiming Electron-support work.
