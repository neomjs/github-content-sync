...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#13372

A live store's `model` is the serialized `Neo.data.Model` (an object — `{className, fields, keyProperty, …}`), but the neural-link OpenAPI declared `model: type: string` on both `inspect_store` and `list_stores`. Strict neomjs/neo#10043 output validation rejected the object with `-32602` ("data/model must be string"), so **any object-model store was unreadable through the agent-facing `inspect_store` tool** — exactly the verification path neomjs/neo#13362 AC4 needs.
...
- The ticket's AC3 covers both ops; `list_stores`' top-level array response is wrapped by the validator as `{result: [...]}` (the `buildOutputZodSchema` non-object path), so the round-trip asserts that wrapping — not a bare array. (The `{stores:[…]}` / `isLoaded` envelope divergence stays neomjs/neo#10072's, per the ticket's Out of Scope.)
...
- [ ] An agent completes neomjs/neo#13362 AC4's store verification through the **real** `inspect_store` MCP tool against an object-model store — no `-32602` (the live-tool path the in-process fixture in PR neomjs/neo#13363 bypasses).
