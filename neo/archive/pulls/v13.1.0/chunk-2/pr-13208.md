...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#13207

## Summary

Extended-NL **Leaf C (part 2a)** — the pure write-enforcement **decision core**, advancing neomjs/neo#13167's Contract Ledger rows 4–6 (write-service enforcement / subtree-path source / no-agent fail-closed).

The in-heap primitives are merged on `dev` (`LockRegistry` neomjs/neo#13125, `WriteGuard` neomjs/neo#13134, `deriveSubtreePath` neomjs/neo#13138) but nothing decides **whether** a given write is enforced and **with what lock — or denied fail-closed**. New `src/ai/resolveWriteLock.mjs` — `resolveWriteLock(context, componentId, parentOf)` — is that decision, returned as a value (never a side effect), so the four security-critical outcomes are unit-provable with no live heap, socket, or `WriteGuard`:
...
Pure (`parentOf` injected exactly as `deriveSubtreePath` takes it; composes it). This is the same pure-core-ahead-of-wiring decomposition @neo-gpt endorsed on neomjs/neo#13205 (`parseAgentEnvelope`). It deliberately does **not** mutate or wire anything yet, so it is dev-safe and merges independently of neomjs/neo#13205.

Evidence: L1 (the focused unit spec) — 8/8 green via the custom unit config; `node --check` clean.

## Deltas from ticket (if any)

None — exactly the ticket's scope (the pure decision function + exhaustive spec + Anchor & Echo JSDoc). The `InstanceService` wiring that consumes the decision and calls `WriteGuard` is explicitly the next slice (it reads `context.sessionId` at runtime → gated on neomjs/neo#13205 merging; landing it before would deny every agent write on current `dev`).
...
Refs neomjs/neo#13167 (Leaf B/C contract-of-record), neomjs/neo-agent-brain#143 (Extended-NL epic).
