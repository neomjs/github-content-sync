...
Evidence: L2 (focused Playwright unit coverage + adjacent merge-import rollback parity spec + static syntax/whitespace checks) -> L4 required (operator-run live quarantine/export and any destructive cleanup on production Memory Core). Residual: operator-gated live cleanup validation remains post-merge under neomjs/neo#13093.
...
- [ ] Run the exporter against the operator-approved neomjs/neo#13093 candidate id file and preserve the local `.neo-ai-data/backups/quarantine-*` bundle path.
