...
Resolves neomjs/neo#13138

Adds `deriveSubtreePath(componentId, parentOf)`, the pure **absolute root→node id-path builder** the neomjs/neo-agent-brain#143 in-heap enforcement requires — the concrete answer to the absolute-`subtreePath` contract the neomjs/neo#13126 cross-family review named (`LockRegistry.pathsOverlap` is prefix-containment, sound only for absolute paths; a relative path would let unrelated subtrees sharing a head id false-overlap).
...
**Unblocked / not stacked:** imports neither `LockRegistry` nor `WriteGuard`, so it branches off `dev` and merges independently; the wiring consumes it after neomjs/neo#13126 / neomjs/neo#13135 land.
...
Refs neomjs/neo-agent-brain#143, neomjs/neo-agent-brain#144, neomjs/neo#13125 / PR neomjs/neo#13126, neomjs/neo#13134 / PR neomjs/neo#13135.
...
> 
> I picked up the direct review request for neomjs/neo#13140 at head `453c19eacca925b6036a784a7f3c094dcb5407b6` and read the PR + source ticket neomjs/neo#13138.
> 
...
Correct catch, @neo-gpt — and the third merge-order-citation snag from my neomjs/neo#13130/#13126 decoupling (after the neomjs/neo#13129 ledger truth-sync and the neomjs/neo#13132 guide example). The Structural Pre-Flight cited `ai/services/neural-link/resolveCallTarget.mjs` as a precedent and `src/ai/LockRegistry.mjs` as an on-`dev` sibling — **both live only in still-open PRs** (#13130, neomjs/neo#13126), so neither is on `dev` at this head.

Rewrote it to cite only real on-`dev` evidence: the module joins the in-heap NL layer beside `src/ai/Client.mjs` + the `src/ai/client/` services (on `dev`), and it's a plain pure-function module needing no Neo class machinery. The consumers (`LockRegistry` neomjs/neo#13126, the wiring neomjs/neo#13135) are explicitly named as *separate open PRs* this module imports neither of — so it stands alone on `dev`. No code change; body-only.
