...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#13121

Authored by Claude Opus 4.8 (Claude Code, @neo-claude-opus / Grace). Session e7f14a36-5096-4570-932b-82f860d7a537. Greenlit as a neomjs/neo#13015 sub by @neo-opus-ada (steward).
...
Evidence: L2 (real env-injection through the recording `spawnFn` stub + real resolution-precedence assertions; **71 green** locally) → L2 required (in-process spawn-env + launch-arg resolution; the cross-process embedded-agent enforcement is the post-merge / neomjs/neo-agent-brain#143 surface). No residuals in this leaf.
...
The FM spawns the agent's *harness* (`metadata.launch.{command,args}`), NOT the NL server — the harness launches its NL server as a subprocess inheriting the harness env. So the clean cross-process channel is an env var the inheriting NL subprocess reads. The env-fallback is a launch-parameter source (like the CLI flag / `bridgeCwd`), **not** an AiConfig leaf — no ADR 0019 concern, consistent with neomjs/neo#13106's deliberate CLI/instance-field choice. CLI precedence preserved.

## Deltas from ticket
- The forced-mode reaches the NL server via the `NEO_NL_TOOL_PROJECTION_MODE` env-fallback (a small extension to neomjs/neo#13106's CLI-only knob), since the FM can't append CLI args to a server it doesn't directly spawn. Documented in the ticket's Architectural Reality + Contract Ledger.
...
> Contract refinement vs neomjs/neo#13121 ledger: the ledger named `toolProjectionModeEnvVar` (configurable). The credential-class **intent** is unchanged; the var name is now a fixed contract constant rather than a settable field. Ticket Contract Ledger reconciled in place.
...
Related: neomjs/neo#13015 (parent), neomjs/neo#13049, neomjs/neo#13065, neomjs/neo#13106 (wired); neomjs/neo-agent-brain#143 (consumes the token); aligned ADR 0020.
