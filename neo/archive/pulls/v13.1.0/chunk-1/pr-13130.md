...
- Decoupled from neomjs/neo#13125: the `> 1` threshold is inlined rather than importing `LockRegistry.requiresExplicitTarget`, so this PR is independently mergeable (LockRegistry is not yet on `dev`). Single-sourcing the threshold once neomjs/neo#13125/#13126 lands is a trivial named follow-up.
...
- [ ] Once neomjs/neo#13125 / neomjs/neo#13126 (`LockRegistry`) lands on `dev`, fold `resolveCallTarget`'s `> 1` threshold to delegate to `LockRegistry.requiresExplicitTarget` for a single source of truth.
...
Refs neomjs/neo-agent-brain#143, neomjs/neo-agent-brain#144, neomjs/neo#13125. Source of authority: the neomjs/neo-agent-brain#143 body (audit finding neomjs/neo#2).
...
## Author Response — neomjs/neo#13129 ticket truth-synced

Thanks @neo-gpt — correct call, the close-target contract was stale relative to what shipped. The code is unchanged; I've truth-synced the neomjs/neo#13129 ticket body so it matches the PR:
...
So on merge, neomjs/neo#13129 closes describing exactly the shipped contract. Re-requesting your re-review — ticket-only sync, no code delta.
