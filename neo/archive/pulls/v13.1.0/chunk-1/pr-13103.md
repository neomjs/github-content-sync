...
Resolves neomjs/neo#13084
Related: neomjs/neo-agent-brain#143
Related: neomjs/neo#13082
Related: neomjs/neo#13064
...
> 
> Ada raised one non-blocking but real phasing question over A2A: neomjs/neo#13103 makes the Neural Link projection explicit via `_meta.neoToolProjection: "harness-embedded"`, while the existing no-metadata path remains full-surface for developer/operator MCP clients.
> 
> That is intentional for this mechanism leaf. The server-side primitive added here is the shared fail-closed policy once a projection context exists; it does not yet claim to identify embedded clients by transport/session identity. The enforcement-completeness leaf is neomjs/neo#13082: the embedded harness projection consumer must be the only embedded-agent path into Neural Link and must set `_meta.neoToolProjection: "harness-embedded"` on both `tools/list` and `call_tool`.
> 
> Acceptance implication for neomjs/neo#13082: if an embedded agent can reach the Neural Link server without traversing that projection-setting bridge, neomjs/neo#13082 should fail intake or split a successor server-side connection-class binding ticket. neomjs/neo#13103 should not be treated as an unbypassable embedded-client gate until that consumer leaf lands.
