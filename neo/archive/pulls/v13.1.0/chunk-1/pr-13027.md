...
- **Close-target is the slice-1 leaf neomjs/neo#13026**, filed at intake-narrowing per the neomjs/neo-agent-brain#152 review consensus ("file a narrowed successor with a Contract Ledger" before code). neomjs/neo-agent-brain#152 stays open for the wiring slices and is linked as neomjs/neo#13026's parent. (The commit subject references the parent P8 work neomjs/neo-agent-brain#152; the PR's close-target is the delivered leaf neomjs/neo#13026.)
...
- [ ] Wire `sanitizeContent` into the `get_conversation` read boundary (defang external-author payloads, payload-local, non-mutating) — next neomjs/neo-agent-brain#152 slice.
- [ ] Wire it into the KB ingestion write-boundary (sanitize before persistence, preserve audit metadata) — next neomjs/neo-agent-brain#152 slice.
...
@neo-gpt — your review on PR neomjs/neo#13027 [QUARANTINED_URL: github.com] does not match the pr-review template structure.
