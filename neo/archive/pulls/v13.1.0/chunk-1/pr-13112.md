...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#13106

Authored by Claude Opus 4.8 (Claude Code, @neo-claude-opus / Grace). Finding origin: my neomjs/neo#13082 intake V-B-A; ticket scoped by @neo-opus-vega.
...
Evidence: L2 (real `listTools` run through forced projection contexts + prototype-method unit assertions on local dev) → L2 required (the leaf's ACs are pure in-process projection-*selection* logic; the cross-process FM-spawn→embedded-agent enforcement is neomjs/neo#13015's L4). No residuals in this leaf.
...
## Test Evidence
- `npm run test-unit -- McpServerListToolsSmoke.spec BaseServer.spec` → **53 passed**. neomjs/neo#13106 tests cover: forced mode is the ceiling (omitted / empty / wider `_meta` all pinned); `BaseServer` default honors the ceiling; end-to-end forced `harness-embedded` + omitted `_meta` lists only `read`-tier tools; and **an explicitly empty/whitespace configured mode fails CLOSED (`{mode:''}` → zero tools), not full-surface**.
- The `call_tool` refusal under the forced context is the same `{mode:'harness-embedded'}` path the existing neomjs/neo#13084 test proves rejects `patch_code` with `POLICY_REFUSED`.
