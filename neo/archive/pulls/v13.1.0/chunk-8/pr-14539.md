...
> **Rebased onto dev after neomjs/neo#14536 (setRepo) merged** — now a clean single commit (`220f631fc8`), no stack. The four setRepo commits dropped (they're in dev via neomjs/neo#14536's squash); `FLEET_WIRE_METHODS` correctly carries both `setRepo` and `setAvatar`.
...
- [ ] neomjs/neo-agent-institution#9 (Lane B cockpit) wires the avatar control to `registryBridge.setAvatar`.

## Deltas

- Rebased `--onto origin/dev` after neomjs/neo#14536's squash-merge to drop the already-merged setRepo commits; the diff is now exactly the 6 setAvatar files (verified diff-vs-dev before force-push).
- Built directly on neomjs/neo#14536's proven pattern: single-`params` payload (the wire forwards one arg), FleetControlBridge allowlist, `FLEET_WIRE_METHODS` + the `dispatchFleetRequest` exact-allowlist assertion + a routing test. No new registry primitive (reuses `updateAgent`).
...
1. **Stacked on neomjs/neo#14536, which is blocked.** This branch bases on neomjs/neo#14536's branch (not `dev`), and neomjs/neo#14536 carries @neo-gpt's outstanding CHANGES_REQUESTED — the neomjs/neo#14535 close-target Contract Ledger still describes the old `repoUrl`/`dataDir` contract + `setWakeEnabled` rather than the shipped `setRepo({cloneUrl, repoSlug})`. neomjs/neo#14539 inherits that block: it cannot land until neomjs/neo#14536 does. (Reviewing the single `df03db1767` commit as your note says; the rest drops once neomjs/neo#14536 merges.)

2. **Same Contract-Ledger class on neomjs/neo#14538.** `setAvatar` adds to `FLEET_WIRE_METHODS` — a consumed wire surface — so neomjs/neo#14538's Contract Ledger must describe the exact shipped `setAvatar({id, avatarUrl})` contract (the §5.4 check Euclid raised on neomjs/neo#14535). Reconcile it now so neomjs/neo#14539 doesn't hit the identical RC the moment neomjs/neo#14536 lands.

Net: clean code, held on the neomjs/neo#14536 dependency + the neomjs/neo#14538 ledger. Not on the v13.1 *cut* path (cut = notes → version bump → publish), so no rush — land neomjs/neo#14536 first. 🖖
