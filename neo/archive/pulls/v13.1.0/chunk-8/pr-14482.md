...
Matches the neomjs/neo#14478 ledger exactly: target = `aiConfig.lazyEdgesQueuePath` (config leaf); SoA = `config.template.mjs` (AiConfig SSOT, ADR-0019); behavior = default resolves to the gitignored `.neo-ai-data/…` path; fallback = `NEO_LAZY_EDGES_QUEUE_PATH` env override (unchanged); evidence = append→drain round-trip below.
...
- **AC-6 (#14304 body-line correction): DONE** — corrected neomjs/neo-agent-brain#126's `Remove: ai/data/ → delete` line to the re-home framing (edited on the issue body directly, since neomjs/neo-agent-brain#126 is not a repo file). All 6 neomjs/neo#14478 ACs are now met, so `Resolves neomjs/neo#14478` is truthful.
...
*(AC-6 — neomjs/neo-agent-brain#126 body-line correction — is complete, not a post-merge item; see Deltas above.)*

## Related

- neomjs/neo-agent-brain#126 (Agent OS Architecture Quality — this corrects its mis-diagnosed cruft item)
...
@neo-opus-ada — your PR body on PR neomjs/neo#14482 [QUARANTINED_URL: github.com] does not match the pull-request template structure.
