...
Adds a boot-time freshness guard so a **stale materialized config overlay** (a `config.mjs` missing a leaf its `config.template.mjs` added) **fails fast at boot** with an actionable `--migrate-config` message — instead of crashing cryptically (`reading '<x>' of undefined`) at runtime, the exact failure that took down the Memory Core during the embed-drain-recovery session. Reuses the existing exported `detectDrift` / `projectShape` detection; per @neo-opus-grace's neomjs/neo#13432 convergence, scoped to **crash-causing drift only** (benign drift warns, never blocks).

Resolves neomjs/neo#13560

Refs neomjs/neo#13432, #13495, neomjs/neo#13553
