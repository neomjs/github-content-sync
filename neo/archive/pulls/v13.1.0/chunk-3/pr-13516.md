...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#13491
Refs neomjs/neo-agent-institution#9
Refs #13512
Refs neomjs/neo#13521

Extracts the **Accounts keeper-view** out of the conflated `FleetSettingsPanel`, executing the graduated neomjs/neo-agent-institution#9 cockpit decomposition (separate Accounts + Fleet keeper-views). This is my corrective per the #13512 watch-the-shipper flag — a real authored substrate PR, executing graduated work (graduation = build-authorization; this PR is the merge-gate's to check, not a pre-merge read I manufactured).
...
`FleetSettingsPanel` conflated two of the epic's keeper-views (the neomjs/neo#13491 pre-build finding). This PR separates them along the graduated boundary:
...
- neomjs/neo#13491 was scope-corrected last cycle from "greenfield Accounts view" to "extract from the conflated `FleetSettingsPanel`" (the view already existed). This PR delivers the **extract**, not a duplicate.
- **Close-target reconciled (per Euclid's cross-family review):** neomjs/neo#13491's body now marks ACs 4/5/6 — AiConfig provider-login, basic NL-MCP entry, v14 `IdentityState` slot — as **deferred to follow-up neomjs/neo#13521**. So `Resolves neomjs/neo#13491` honestly closes the extraction scope (ACs 1/2/3/7/8 delivered; the redacted *listing* renders in the Fleet view per the split), not the enhancements. No code change — close-target honesty only.
...
- Scope held to the **Accounts** keeper-view + the enabling split. The Fleet-view rename (`FleetSettingsPanel` → `Fleet`), the structure/nav left-rail sub, and the who_is_online live-status consumer remain separate neomjs/neo-agent-institution#9 subs.
