...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#13413
Refs neomjs/neo#13355
...
**Close-target re-scope (per the neomjs/neo#13409 cross-family review):** neomjs/neo#13355 was re-scoped onto the neomjs/neo#13362/#13363 Neural-Link-created-grid seam. This PR delivers the deterministic half (evidence-pane UI + a local blueprint-driven grid render-together), **not** the NL-create-path provenance integration — so it now **Resolves the narrower fully-delivered leaf neomjs/neo#13413** and **Refs neomjs/neo#13355**, which stays open for the residual NL-created-grid provenance (the grid arriving via the `create_component` seam, with the evidence pane tied to it). Thanks to @neo-gpt for catching the source-of-authority mismatch — it's exactly the foundation question flagged in the original lane-claim.

Evidence: L2 unit (6 tests green, runnable) + L3 e2e render smoke (green locally + CI). This is the deterministic slice's ceiling **by design** — the NL-create-path provenance is the neomjs/neo#13355 residual, not a sandbox ceiling.

## Decision Record impact
Aligned with ADR 0020. Resolves the deterministic-slice leaf neomjs/neo#13413 (split from neomjs/neo#13355). No ADR amendment.

## Deltas from ticket
All 5 ACs of the deterministic leaf neomjs/neo#13413 delivered (evidence pane + accepted-blueprint metadata + fail-closed + render-together grid + unit/e2e coverage). neomjs/neo#13355's broader re-scoped intent (NL-created-grid provenance) is explicitly **not** in this PR — it remains a residual on neomjs/neo#13355 (see Post-Merge Validation).
...
- [ ] **#13355 residual (NL-created-grid provenance):** tie the evidence pane to a grid created via the neomjs/neo#13362/#13363 `create_component` seam (not a deterministic config). Tracked on neomjs/neo#13355 (left open) — `[L3-deferred — needs the AgentOSWidget NL-create seam]`; a properly-designed leaf, not delivered here.
