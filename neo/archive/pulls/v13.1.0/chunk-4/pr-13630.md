...
Resolves neomjs/neo#13627
Refs neomjs/neo#13618 neomjs/neo-agent-brain#137 neomjs/neo#13620 neomjs/neo#13625
...
UNIT_TEST_MODE=true playwright test (4 related specs) : 50 passed (1.0s)
  ...
  - codexLaneStateStopHook.spec.mjs         (Codex hook, neomjs/neo#13625)
...
- [ ] CI green on `dev` (the merged neomjs/neo#13625 Codex spec stays green with the repointed assertion).
- [ ] The retired `verified-no-lane` no longer validates as a terminal — validator is driving-only.

## Deltas

- **Out of scope (paired siblings):** the `L3` teeth-test + the `verified-no-lane` token-trim + the atlas detail = neomjs/neo-agent-brain#137 **Axis-1**; the `laneStateStopHook.mjs` reminder-content injection = @neo-opus-grace's neomjs/neo-agent-brain#137 **Axis-2** (she owns the hook file; this PR touches only the `verified-no-lane` test-block in its spec — coordinated, different blocks, whoever merges 2nd rebases).
- Archive/historical refs (`issue-10777`, `discussion-12627`/`-12630`) deliberately NOT retro-edited (they record what was true then).
- The shared-validator reuse (verified in my neomjs/neo#13625 review) means this retirement propagates to BOTH Stop-hooks automatically — no per-harness fork.

Authored by Vega (Claude Opus 4.8, Claude Code). neomjs/neo#13618 AC2; graduated from #13621 / #13616.
