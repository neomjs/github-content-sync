...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#13641

The Claude Stop-hook (`laneStateStopHook.mjs`, neomjs/neo#13589/#13629) was built and merged but never active in any clone: unlike Codex's tracked `.codex/hooks.json`, Claude's `settings.json`/`settings.local.json` are gitignored and `settings.template.json` carried no hooks block, so activation was an uncompleted per-repo manual step. This adds the Stop-hook block to the tracked template plus an `initClaudeSettings()` materializer wired into `npm prepare`, so every clone self-wires on pull (`--migrate-config`) — Codex-parity, no manual per-repo management.
...
## Deltas from ticket
Rollout mode is enforce per operator directive (vs the hook's documented dry-run-first ramp). The transition cost — prose-only turns block to the 8-block override until the neomjs/neo#13644 emission contract + its always-loaded AGENTS.md pointer land — is the accepted forcing-function tradeoff.
...
- Contract Ledger backfilled on neomjs/neo#13641 (review RA#4).
...
- [ ] Transition: until neomjs/neo#13644 (emission contract) + its AGENTS.md pointer land, prose-only turns block to the 8-block override — the accepted forcing-function cost.
...
Refs neomjs/neo-agent-brain#137, neomjs/neo#13624, neomjs/neo#13628.
...
@neo-opus-grace — your PR body on PR neomjs/neo#13642 [QUARANTINED_URL: github.com] does not match the pull-request template structure.
...
- **`[ADDRESSED]` RA#4 (contract ledger):** backfilled on neomjs/neo#13641.
