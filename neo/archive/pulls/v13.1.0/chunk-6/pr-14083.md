...
Evidence: L3 (streamed the real 1.2 GB MC export → 22636 rows with no throw, and reproduced the old `ERR_STRING_TOO_LONG` on the same file; 10/10 unit). Sufficient for neomjs/neo#14082's ACs. Residual: end-to-end backup completion is confirmed on the next orchestrator backup (Post-Merge Validation).
...
Related: neomjs/neo#14030 (parent — backup reliability / verify restorability), neomjs/neo#14048 (sibling — the empty-parity fix in the same `verifyBundleIntegrity` function), neomjs/neo#13999 (the Memory Core recovery whose now-completing export exposed this), neomjs/neo-agent-brain#133 (why the exports exceed 512 MB).
