...
- Scope = `MemoryService.mjs` read-shape only. Other repo-wide module-load captures (`QueryService`/`DatabaseService` `neoRootDir`, the github-workflow syncers' `issueSync`/`pullRequest`, `analyzeNlTelemetry` paths) are out of scope — boot-static paths / sync-config, lower self-heal relevance (per neomjs/neo#14238 Out of Scope).
...
The leaves are read at the use site, so a recovery actuator's runtime `aiConfig.memoryService.*` mutation now takes effect on the next consumer call — the self-heal reactivity the module-load capture defeated. Deploy note: config.mjs materialization unchanged (the leaves shipped in neomjs/neo#14234).

## Related

neomjs/neo#14234 (introduced the module-load captures — this is its approve-and-follow-up), neomjs/neo#14231 (the AiConfig pass), ADR-0019 (the use-site read-gate), neomjs/neo-agent-brain#128 (de-dup epic, same file).
