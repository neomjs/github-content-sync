...
The full AiConfig-compliance pass on the Memory-Core path — @tobiu's neomjs/neo#14207 flag ("AiConfig. no exceptions"). Two pre-existing ADR-0019 violations (not the de-dup diff):
...
- Both halves of neomjs/neo#14231 are in this PR. Standalone tests/examples/demos reading `process.env.GEMINI_API_KEY` are out of scope (not AiConfig-governed services).
...
neomjs/neo#14207 (where @tobiu flagged it), neomjs/neo-agent-brain#128 (de-dup epic, same files), neomjs/neo#14212 (the `--fix` over-reach — committed `--no-verify` to avoid the whole-file churn), ADR-0019.
