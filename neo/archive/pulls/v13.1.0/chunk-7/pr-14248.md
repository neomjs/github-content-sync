...
`TextEmbeddingService.#postOpenAiCompatible`'s retry guard recognized only HTTP-400 model-load shapes, so an HTTP-404 (the model not resident at the provider — the live neomjs/neo-agent-brain#131 trigger) skipped the model-load-wait retry and blind-aborted the embedding batch. This adds **Shape C** (HTTP-404 → the existing bounded model-load-wait retry) — the Neo-side **self-heal-not-abort** mitigation. The env-root-cause (LM Studio VRAM co-eviction) remains neomjs/neo-agent-brain#131's separate infra investigation.

Resolves neomjs/neo#14247
...
A 404 on `/v1/embeddings` means the model is not found/resident — the model-load-wait retry is exactly the right response (same class as a JIT-unload). Pre-fix, a 404 fell through to the remaining blind POSTs then aborted the batch (which, pre-#14146, discarded all KB-sync progress → the neomjs/neo-agent-brain#131 catastrophic re-embed loop). Identified as **sub-fix neomjs/neo#3 in neomjs/neo#14146** (the checkpoint/resume half landed; this retry-guard half did not).

## Deltas from ticket (if any)

None — implemented exactly per neomjs/neo#14247's AC. The `HTTP 404` substring is the simplest robust match (a 404 on `/v1/embeddings` is model-not-resident by construction; the error body already carries endpoint + model for diagnosability), so no separate body-keyword match was needed. No scope deviation.
...
neomjs/neo-agent-brain#131 (the incident + env-root-cause), neomjs/neo#14146 (sub-fix neomjs/neo#3 origin; closed), neomjs/neo#14124 (warm-provider / embed-write-canary class).
...
@neo-opus-ada — your PR body on PR neomjs/neo#14248 [QUARANTINED_URL: github.com] does not match the pull-request template structure.
