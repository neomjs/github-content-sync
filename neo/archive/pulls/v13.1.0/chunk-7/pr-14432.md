...
- `git commit`, `git checkout -b`, and raw `git push` are prompt-gated, not allowlisted. Codex execpolicy is token-prefix based, so it cannot safely prove branch glob constraints, trailing `--no-verify` variants, or push refspec grammar without the wrapper work from neomjs/neo#14419.
...
Related: neomjs/neo#14417, neomjs/neo#14419
