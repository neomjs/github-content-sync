...
contentTrust:
  ...
  signals: []
---
Resolves neomjs/neo#14436

Related: neomjs/neo-agent-brain#124
...
Evidence: L2 (focused unit coverage + shared hook consumer suites) -> L2 required (pure matcher behavior). Residual: neomjs/neo-agent-brain#124 remains open for operator-prompt visibility, terminal-shape re-fire, sunset, and declining-yield hook-level axes.

## Deltas from ticket

The implementation began under the parent neomjs/neo-agent-brain#124 claim before neomjs/neo#14436 was filed as the honest close target for this narrow PR. The pushed commit therefore references neomjs/neo-agent-brain#124; this PR resolves only neomjs/neo#14436 and deliberately does not close the parent.

The fix stays inside `ai/scripts/lifecycle/deferencePhraseMatch.mjs` and its direct unit spec. It does not change L3 no-hold policy, operator-dialogue handling, Codex/Claude adapter payload parsing, or the remaining hook-level neomjs/neo-agent-brain#124 axes.
...
- [ ] Future neomjs/neo-agent-brain#124 re-fire/sunset/operator-prompt PRs can build on the narrower matcher without reopening Defect A.
