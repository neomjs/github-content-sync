...
Evidence: L2 local static + unit evidence covers the WAL-first acceptance contract; L3 restart/process replay remains residual because replay/idempotency belongs to sibling neomjs/neo#13892. Residual: real cloud drain topology belongs to sibling neomjs/neo#13890.

Related: neomjs/neo#13889
Related: neomjs/neo#13890
Related: neomjs/neo#13892
...
- Stores message WAL records under the existing resolved `memoryWal.dir` (`messages/`) for this acceptance-boundary leaf. Dedicated message drain host/config parity remains scoped to neomjs/neo#13890.
- Keeps current graph projection in `addMessage()` as best-effort derived work after the WAL append. Full replay/idempotency and pending-WAL read visibility remain scoped to neomjs/neo#13892.
- Message vector/search population is deliberately deferred to neomjs/neo-agent-brain#157 / later drain work; this PR does not add model-dependent work to the request path.
...
- [ ] Confirm sibling neomjs/neo#13892 consumes the message WAL record shape for replay/idempotent graph projection.
- [ ] Confirm sibling neomjs/neo#13890 either preserves the `memoryWal.dir/messages` placement or promotes it into explicit message-drain config without splitting local/cloud WAL reachability.
