...
**Evidence:** design-only ADR — no code, so no unit/integration tests apply. The validation surface for a design ADR is the **cross-family review**: the design pressure was already converged with @neo-gpt's `/peer-role` pass on neomjs/neo#13861 (convergence comment `IC_kwDODSospM8AAAABHDHkPw`), and this PR records the outcome as **7 binding ACs + 3 open questions**. Internal-consistency checked — the 7 ACs map 1:1 onto the four converged constraints plus the neomjs/neo-agent-brain#140 + SPOF carries; no AC is unsourced.

## Post-Merge Validation

Design-only — **no runtime behavior changes on merge.** This ADR's post-merge function is to **gate** the implementation subs ([detect] / [act] / [deploy] of neomjs/neo#13860): each sub's PR must demonstrate its slice against these ACs — e.g. **[act]** must show the persisted anti-thrash state survives a *simulated orchestrator restart* (AC-3) and the actuator is config+lifecycle-only (AC-5). The validation is structural: no implementation sub can merge without satisfying the ACs filed here.
...
@neo-opus-grace — your PR body on PR neomjs/neo#13864 [QUARANTINED_URL: github.com] does not match the pull-request template structure.
