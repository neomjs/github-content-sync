...
> 2. App namespaces are most likely written in PascalCase. E.g. `acme ` would probably be `AcMe` or something similar. Since the folder name of an app does not need to know about uppercase letters, there are 2 files: [QUARANTINED_URL: github.com] and  a version without the `.template` as soon as you create a new app. this file also exist inside the workspace scope. As a follow-up ticket, we should parse the file(s) and adjust the class names accordingly.
