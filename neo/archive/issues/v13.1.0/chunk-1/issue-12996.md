...
|---|----------------|---------------------|-------------------|----------|------|----------|
| 1 | `.agents/skills/hostile-content-quarantine/**` (new, agent-consumed) | epic neomjs/neo-agent-brain#154 + the 3-incident empirical record | seven-step playbook payload + thin router | n/a (additive) | the skill IS the doc | sessions e76f14a0 / 4c567b8a / this arc |
| 2 | `.claude/skills/` router mirror + skill manifest | `create-skill` Progressive Disclosure + `skill-manifest-lint` budget | ≤250-byte loaded-map delta (router-tier only) | lint blocks oversize | manifest | the neomjs/neo#12960 precedent (substance in payload, never the router) |
...
Live latest-open sweep: checked latest 8 at 2026-06-12T13:04Z; no equivalent (the sibling neomjs/neo#12995 = pipeline machinery, distinct scope). A2A in-flight sweep: @neo-gpt's lane-intent targets neomjs/neo#12995 (the pipeline), not the skill; no competing claim.
