...
- 2026-06-15T17:34:02Z @neo-opus-vega cross-referenced by #144
...
- neomjs/neo#9559 is still open, unassigned, and unchanged since `2026-03-26T13:39:22Z`. Its body is a broad research+implementation prompt for Neural Link authorization, not a current contract ledger.
- The live successor authority is neomjs/neo-agent-brain#143, **Extended-NL coordination: identity, locking, curated tool surface**. It decomposes the same risk into transport identity, write coordination, and tool-surface curation after the harness/product-surface shift.
- The underlying Bridge-auth half has already landed under neomjs/neo#13172 / PR neomjs/neo#13181:
...
- Spawn/projection provisioning also moved into the Extended-NL family:
  ...
  - `ai/mcp/server/neural-link/openapi.yaml` carries the `x-neo-harness-tool-projection` contract and per-operation `x-neo-tool-tier` metadata from the neomjs/neo#13064/#13082/#13106 line.
- `ask_knowledge_base` still reports neomjs/neo#9559 as open but identifies neomjs/neo#13121/#13065/#13064-class mechanisms as the later closed implementation family. Memory Core had no additional prior-session mapping for this exact neomjs/neo#9559 framing.

I am closing neomjs/neo#9559 as superseded / not planned rather than claiming it. Any remaining Neural Link authorization work should continue through neomjs/neo-agent-brain#143 and its leaf tickets, where the current topology and contract boundaries are already explicit.
