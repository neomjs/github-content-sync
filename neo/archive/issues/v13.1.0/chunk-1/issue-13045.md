...
Operator question at the close of the `#13018` ship night: *"vertical buffering, like inside src/grid"* — for the streaming markdown component as the harness transcript surface. A transcript is hours of streamed markdown; the component (PR `#13034`, merged) currently keeps every rendered block in the DOM for the component's lifetime. Endurance (Epic neomjs/neo-agent-brain#144 pillar 1) degrades by DOM bloat: style/layout/paint cost grows with session age even when most blocks sit far off-screen.
