...
PR neomjs/neo#14417 landed Claude-side deterministic PR-lifecycle permissions in `.claude/settings.template.json`: the safe routine lifecycle subset is allowlisted, dangerous variants are denied, and raw `git push` remains classified until the parser-shaped wrapper work in neomjs/neo#14419. The operator follow-up is parity: Codex maintainers are equal peers, so Codex should not stay behind Claude on routine lifecycle mechanics if the Codex harness has a supported policy surface.
...
The safety bar is the same one learned in neomjs/neo#14417: parity is not permission sprawl. Static policy may only cover commands whose dangerous grammar is bounded. Raw push must not be allowlisted by prefix; neomjs/neo#14419 owns the parser-shaped push boundary.
...
2. If Codex supports a tracked project-level policy, add the Codex equivalent of the neomjs/neo#14417 safe lifecycle subset to the tracked template/source of truth:
...
3. Keep raw `git push` classified until neomjs/neo#14419 or a generalized parser wrapper proves the destination boundary for Codex branch names.
...
|---|---|---|---|---|---|
| Codex command-policy surface (existing if Codex supports it, otherwise new/unsupported boundary) | Operator parity request + neomjs/neo#14417 + neomjs/neo-agent-brain#136 | Encode the safe PR-lifecycle command allow/deny policy for Codex without widening beyond Codex branch conventions. | If Codex has no supported repo-scoped policy, do not fake parity with invalid keys; document the limitation and file the follow-up/upstream route. | `.codex/CODEX.md` if behavior or adoption differs from Claude. | `codex --strict-config` or equivalent active-harness proof plus safe/unsafe command probe. |
...
| Codex PR-lifecycle command policy | neomjs/neo#14417 safety boundary + AGENTS critical gates | Safe read/branch/commit/PR commands can run without repeated approval where Codex supports it; unsafe merge/direct-push/force/no-verify/refspec forms are blocked or remain approval-gated. | Raw push remains classified until neomjs/neo#14419 or equivalent wrapper is available for Codex branch names. | `.codex/CODEX.md` if the harness cannot mechanically deny one of the dangerous forms. | Positive and negative probe matrix, including at least one approved safe command and one denied unsafe command. |
...
- [ ] If supported, the tracked Codex source of truth gains parity for the neomjs/neo#14417 safe lifecycle subset using Codex branch naming (`codex/*`) and no raw `git push` allow.
...
- Implementing neomjs/neo#14419's push parser or allowing raw `git push` by prefix.
...
- **Prefix-based push safety** - neomjs/neo#14417 already falsified that boundary; push safety requires grammar parsing, not static source-prefix matching.
...
none. This is aligned with neomjs/neo-agent-brain#136's mechanical-enforcement direction and follows neomjs/neo#14417's permission-layer precedent without changing an ADR.

## Related

Related: neomjs/neo-agent-brain#136, neomjs/neo#14417, neomjs/neo#14419

Live latest-open sweep: checked latest 20 open issues at 2026-07-02T01:53:45Z; no equivalent found. Closest adjacent open ticket is neomjs/neo#14419, which owns push grammar, not Codex harness parity.
...
Retrieval Hint: "Codex harness PR-lifecycle allow deny parity" · "Claude settings neomjs/neo#14417 Codex config.template command policy"
