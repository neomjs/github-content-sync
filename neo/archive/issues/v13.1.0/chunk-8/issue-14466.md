...
Grace surfaced the missing producer while reviewing the neomjs/neo#14463 / neomjs/neo#13751 hook-read consumer: `lifecycle-state.json` now has multiple readers or near-term consumers, but no writer exists. The current neomjs/neo-agent-brain#137 parent remains `not-code-ready` for AC4 ratio/re-tread/operator-scope observability because those fields still lack source/window semantics. Phase 1 does not need to wait for that: the sources for `{openPRs, unreadCount, goldenPathDirection}` are already live and can unblock the existing hook board consumers.

Live latest-open sweep: checked the latest 20 open issues on 2026-07-02T10:39Z; no equivalent standalone shared-writer ticket found. A2A in-flight sweep: checked latest 30 all-status messages on 2026-07-02T10:39Z; Grace's neomjs/neo-agent-brain#137 co-scope ask exists, but no competing ticket-creation claim exists.
...
- neomjs/neo#13678's live-board fields: `openPRs` and `unreadCount`.
- neomjs/neo#14463 / neomjs/neo#13751's hook-read consumer: `goldenPathDirection`.
- neomjs/neo-agent-brain#137 AC4's future observability fields, which should be additive later but are not source-ready today.
...
- `goldenPathDirection`: compact top-route / degraded-route directive for neomjs/neo#14463's consumer, sourced from the same Golden Path pass that writes `sandman_handoff.md`.
...
| `goldenPathDirection` | Golden Path synthesis result / neomjs/neo#13751 consumer contract | Emit compact directional data from the routed top node or degraded-route state. | If Golden Path route is degraded, emit an explicit degraded/null state rather than a fake recommendation. | JSDoc field contract; no new ADR unless direction semantics change. | Unit fixture for routed, empty, contradiction, and degraded route states. |
...
- [ ] The implementation does not add neomjs/neo-agent-brain#137 AC4 ratio/re-tread/operator-scope fields yet; it only leaves an additive schema path for them.
- [ ] The PR body explicitly names post-merge validation for a real hook block reading the generated file.

## Out of Scope

- neomjs/neo-agent-brain#137 AC4 ratio-over-window, re-tread, or `forcedContinuationNoCompliantLane` observability fields.
- COP rendering under neomjs/neo-agent-institution#8.
...
- **Do not keep this inside neomjs/neo-agent-brain#137 AC4.** Phase 1 has resolved sources and unblocks existing consumers; AC4 Phase 2 remains source-unresolved and should not hold the writer hostage.
...
Related: neomjs/neo-agent-brain#137
Related: neomjs/neo#13678
Related: neomjs/neo#13751
Related: neomjs/neo#14463
Related: neomjs/neo-agent-brain#136

Origin Session ID: 8facbc96-c346-4633-9141-79a968ca1c5d

Retrieval Hint: `lifecycle-state.json writer GoldenPathSynthesizer openPRs unreadCount goldenPathDirection neomjs/neo-agent-brain#137 IC-4864771245`
...
- 2026-07-02T10:42:03Z @neo-gpt cross-referenced by #137
...
Superseded by neomjs/neo#14473 after the neomjs/neo#14469 team review resolved the stale path premise.

Reason: this ticket specified .claude/logs/lifecycle-state.json, but current Claude and Codex hook readers resolve lifecycle state through .neo-ai-data. neomjs/neo#14473 preserves the useful writer work while making the shared resolver, per-agent collision-proof topology, and reader/writer path agreement explicit.
