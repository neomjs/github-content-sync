...
PR neomjs/neo#14469 implemented the writer requested by neomjs/neo#14466, but team review found that the ticket described the wrong consumer path. The implementation is useful salvage, yet the default write target is dead-on-arrival against current hook readers: it writes the stale .claude/logs/lifecycle-state.json contract from neomjs/neo#14466 while the live readers resolve state under .neo-ai-data.

This ticket supersedes neomjs/neo#14466 instead of trying to patch PR neomjs/neo#14469 in place. Tobias challenged the PR to drop/supersede, @neo-gpt accepted the author-side disposition, @neo-opus-vega and @neo-fable-clio posted peer-role concurrence with falsifying evidence, and @neo-opus-grace released her redundant peer-role claim after Vega resolved the constant evidence.

Live latest-open sweep: checked latest 20 open issues at 2026-07-02T14:24:34Z; neomjs/neo#14466 is the stale superseded source, and no separate replacement-path contract ticket was present. A2A in-flight sweep: checked latest 30 all-status messages at 2026-07-02T14:24:34Z; saw Vega and Clio peer-role posts, Grace claim release, and Vega topology correction, but no competing ticket-creation claim. KB ticket sweep for lifecycle-state hook live board .neo-ai-data harness-state duplicate returned unrelated historical board material and no equivalent ticket.

## The Problem

neomjs/neo#14466 transcribed the hook consumer as .claude/logs/lifecycle-state.json. That was stale against the current hook source. Implementing it faithfully creates a writer and consumer that do not meet:
...
- neomjs/neo#14466 still says the writer should produce .claude/logs/lifecycle-state.json.
...
Replace neomjs/neo#14466 with a resolver-first implementation:
...
3. Port the useful PR neomjs/neo#14469 writer pieces: payload builder, generated timestamp, own-PR board, unread-count honesty, Golden Path direction projection, same-directory temp + rename write, and degrade-by-omission behavior.
...
6. Close PR neomjs/neo#14469 unmerged after the replacement ticket exists; the code should be ported, not merged from that branch as-is.
...
- [ ] The writer ports the useful neomjs/neo#14469 behavior but writes only through the shared resolver.
- [ ] Reader/writer path agreement is pinned by a regression test that would have failed PR neomjs/neo#14469 .claude/logs default.
...
- [ ] PR body explicitly states how PR neomjs/neo#14469 code was ported or discarded and why neomjs/neo#14466 was superseded.
...
- Adding neomjs/neo-agent-brain#137 AC4 ratio/re-tread/operator-scope fields.
...
Supersedes neomjs/neo#14466
Related: neomjs/neo#14469
...
Closed not-planned with neomjs/neo#14481. The hook-board direction is dropped — stop-hook machinery on a planning failure. The one real bug (writer to a dead path) is moot without the board; a minimal board would be a one-line path fix if ever wanted. Related ledger removal: neomjs/neo#14518. — Grace 🖖
