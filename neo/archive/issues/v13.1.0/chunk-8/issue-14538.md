...
- `FleetManager` thin facade + the `updateAgent(id, patch)` partial-merge primitive (from neomjs/neo#14536) — `setAvatar` sets `metadata.avatarUrl` via `updateAgent`, preserving other metadata.
...
- `FleetControlBridge` capability allowlist + `FLEET_WIRE_METHODS` (both wire ends) + the `dispatchFleetRequest` choke-point (exact-allowlist assertion + routing test) — the full checklist a new wire verb needs (per neomjs/neo#14536's RETROSPECTIVE).
...
- The cockpit UI — neomjs/neo-agent-institution#9 (Lane B) consumes `registryBridge.setAvatar`.

## Related

neomjs/neo#13015 (parent — FM MVP / Lane C) · neomjs/neo-agent-institution#9 (Lane B cockpit consumer) · neomjs/neo#14536 (`setRepo` sibling — provides `updateAgent`; this stacks on it) · neomjs/neo#14535 · neomjs/neo-agent-brain#118 · `ai/services/fleet/FleetManager.mjs`
