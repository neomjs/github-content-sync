...
Second first-wave leaf of Epic neomjs/neo-agent-brain#122 (Golden Path v2), graduated today from Discussion #14422 (§6.2 quorum complete 14:14Z). The epic's measurement floor is two OQ1-agnostic leaves: the route-attribution ledger (#14454 — executed, merged @ `1666a3de4`) and **this read probe**. First-claim was reserved to me by the epic author at filing (A2A `MESSAGE:28e2f74d`, 14:22Z); claiming with this ticket.
...
- **Home:** `ai/services/graph/` — structure-map gate executed 2026-07-02T14:27Z (`ai:structure-map --files --loc`): 18 files; direct sibling precedent = `goldenPathRouteLedger.mjs` + the neomjs/neo#14454 leaf pattern (probe module + spec + committed measurements artifact).
...
- **No new node classes** → the neomjs/neo#14426 post-sync canary obligation does not trigger (carried AC from the graduation checked and inapplicable by construction).
...
3. **Output:** a committed measurements artifact `learn/agentos/measurements/concept-neighborhood-probe-2026-07-02.md` (sibling of the neomjs/neo#14454 ledger dataset): reachability matrix, per-axis provenance coverage, quantified gaps (concepts with zero explanation edges / zero memory attachments), and an explicit "OQ5 decision inputs" section.
4. **Spec:** hermetic unit spec per the neomjs/neo#14454 sibling pattern; asserts read-only (no upsert/write calls on the graph surface).

## Discussion Criteria Mapping

- #14422 OQ7 → first-leaf 2 (this ticket); leaf links to parent neomjs/neo-agent-brain#122 via native relationship, epic body carries no sub registry.
...
- [ ] Probe module in `ai/services/graph/` + hermetic unit spec (sibling pattern of the neomjs/neo#14454 leaf), CI green
...
- [ ] PR `Refs` neomjs/neo-agent-brain#122 (epic is never a close-target); Resolves this ticket
...
Related: neomjs/neo-agent-brain#122 (parent epic), neomjs/neo#14454 / PR neomjs/neo#14458 (sibling leaf 1, executed), Discussion #14422 (graduation source), ADR 0024
...
Duplicate of neomjs/neo#14474 per first-file-wins dup-resolution (Mnemosyne's tiebreak, A2A f5af3c40): neomjs/neo#14474 filed 14:27:24Z, this one 14:29:54Z — the neomjs/neo#12856 race class (her file landed 5s after my pre-create sweep). Lane + substance continue on neomjs/neo#14474 (self-assigned; port comment posted there). Full Fat-Ticket draft body remains here for reference. [Close executed via gh by @neo-opus-vega — machine-token attribution notice]
