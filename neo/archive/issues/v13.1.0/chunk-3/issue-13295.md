...
The live Codex adapter evidence on neomjs/neo#13287 still matters, but the latest mailbox V-B-A surfaced an upstream condition that does not require an adapter split: multiple targeted direct messages in the recent `@neo-gpt` inbox were persisted with `wakeSuppressed: true`, including subjects that are lifecycle-actionable by the current contract.

Examples observed in `list_messages({status:'all', limit:30})` at 2026-06-15T03:44:09Z:

- `[review][REQUEST_CHANGES] neomjs/neo#13290 — clean work, one close-target gate` from `@neo-opus-grace` to `@neo-gpt`, `wakeSuppressed: true`.
- `[review][APPROVED] neomjs/neo#13290 — close-target clean, merge-eligible (human-gate)` from `@neo-opus-grace` to `@neo-gpt`, `wakeSuppressed: true`.
- `[review][APPROVED] neomjs/neo#13293 — clean docs capture, merge-eligible` from `@neo-opus-grace` to `@neo-gpt`, `wakeSuppressed: true`.
...
5. Keep neomjs/neo#13287 separate: it remains the live Codex submit/start-turn validation lane for wake-eligible A2A messages.
...
| neomjs/neo#13287 live Codex submit matrix | neomjs/neo#13287 / neomjs/neo#13292 / `ai/docs/wake-prompt-landing-matrix.md` | Matrix validation must distinguish wake-eligible direct messages from mailbox-only suppressed messages. | A suppressed message is not valid evidence that Codex failed to submit a wake; it never emitted a wake by contract. | Add a note only if needed while implementing neomjs/neo#13287. | neomjs/neo#13287 live matrix artifact. |
...
- [ ] neomjs/neo#13287 receives a comment or evidence note clarifying that suppressed messages are upstream-invalid evidence for Codex submit/start-turn failure.

## Out of Scope

- Changing Codex app-server, `osascript`, or any harness adapter.
- Proving live Codex prompt submission/start-turn behavior for wake-eligible messages; that remains neomjs/neo#13287.
- Removing `wakeSuppressed` or reversing the neomjs/neo#12635 FYI/awareness relaxation.
- Re-delivering already-read messages or changing neomjs/neo#13281/#13285 retry/read-state behavior.
...
- **Wake-noise regression:** reverting to "all peer messages wake" would undo the intended neomjs/neo#12635 awareness/FYI noise reduction.

## Related

- Related: neomjs/neo#13287 (live Codex submit/start-turn validation for wake-eligible direct messages).
- Related: neomjs/neo#13291 and neomjs/neo#13292 (boundary pin + matrix docs for the current wake-submit investigation).
- Related historical context: `#10545`, `#12635`.
- Parent context: neomjs/neo-agent-brain#144 (Agent Harness).

## Duplicate / Claim Sweep

- Live latest-open sweep: checked latest 20 open issues at 2026-06-15T03:44:09Z. Adjacent open tickets found: neomjs/neo#13287, neomjs/neo#13291, neomjs/neo#13292; none is scoped to guarding/auditing `wakeSuppressed:true` misuse on actionable direct A2A messages.
