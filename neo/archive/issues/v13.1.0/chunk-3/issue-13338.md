...
Sub-leaf of neomjs/neo#13015 (Fleet Manager MVP). The `FleetManager` facade now exposes start / stop / restart (#13336) + `fleetRepoStatus` — but the operator cannot **remove** an agent from the fleet. That is the missing CRUD-delete in the MVP's "full agent CRUD". `FleetRegistryService.removeAgent(id)` (definition + PAT cleanup) already exists (`:174`); the gap is the turnkey facade verb that stops a running agent first (so removal never orphans a live process) then deregisters.

## The Problem

Removing an agent must (a) not leave an orphaned running process — a deregistered-but-running harness is unmanageable — and (b) not silently orphan the agent's checkout-path-keyed auto-memory. (a) is solved by stop-then-deregister. (b) is why this leaf is **deliberately non-destructive to the on-disk checkout**: deleting the checkout orphans the path-keyed auto-memory, and that reconciliation is the blocked neomjs/neo-agent-brain#142 (a Memory-Core delete/archive/tombstone policy). Deletion and reconciliation cannot be safely decoupled, so the destructive repo cleanup stays coupled in neomjs/neo-agent-brain#142; this leaf ships the safe registry-removal half.
...
- The on-disk checkout + its path-keyed auto-memory are deliberately NOT removed (the coupled destructive cleanup is neomjs/neo-agent-brain#142).
...
| `FleetLifecycleService.stop(id)` | existing neomjs/neo#13049 (`:217`) | consumed via `stopAgent` | — | existing | read `FleetLifecycleService.mjs:217` |
...
- [ ] The on-disk checkout + auto-memory are NOT touched (non-destructive); documented in JSDoc with the neomjs/neo-agent-brain#142 coupling noted.
- [ ] Unit tests mirror `FleetManager.spec.mjs`; full unit suite green. JSDoc `@summary` on the method.

## Out of Scope

- Destructive checkout removal (`removeAgentRepo`) + the auto-memory reconciliation — the coupled, blocked neomjs/neo-agent-brain#142 (Memory-Core policy). This leaf is deliberately non-destructive to disk.
- A `removeRepo` opt-in flag — deferred: it would open the orphaning window neomjs/neo-agent-brain#142 closes; the destructive path is better added coupled with the reconciliation in neomjs/neo-agent-brain#142.

## Avoided Traps

- **Deregister without stopping** — rejected: orphans a live process (unmanageable). Stop-first.
- **Deleting the checkout here (or a `removeRepo` flag now)** — rejected: orphans path-keyed auto-memory (the neomjs/neo-agent-brain#142 problem); deletion can't be safely decoupled from reconciliation.

## Related

Parent: neomjs/neo#13015. Builds on the facade (#13192, neomjs/neo#13336). Registry removal: existing `FleetRegistryService.removeAgent`. Coupled destructive follow-up: neomjs/neo-agent-brain#142.

Release classification: post-release (inherits neomjs/neo#13015).
