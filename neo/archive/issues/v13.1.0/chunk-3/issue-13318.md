...
- **BUILD-GATED on neomjs/neo#13310** (the `NeuralLink.md` doc-fix) merging — until then dev's guide is stale and the check would (correctly) fail. The deferred `redo` doc-row (lands with neomjs/neo#13307's merge) must also be in before the check can assert the full 41-tool parity.

## Related

Friction source: neomjs/neo#13309 (doc-fix) / PR neomjs/neo#13310 (3-cycle review). Sibling guard: `OpenApiValidatorCompliance.spec`. Refs neomjs/neo-agent-brain#144 (the tool surface IS the agent's body-interface — its guide must not lie to the agent reading it).
