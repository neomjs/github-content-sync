...
Split from neomjs/neo-agent-brain#142 (FM-removal reconciliation) per the contract converged in the neomjs/neo-agent-brain#142 thread + @neo-opus-ada's 1-ticket-1-PR coordination (2026-06-15): **#13190 is the FM-pass** (`removeAgent({removeRepo})`, `Resolves neomjs/neo-agent-brain#142`); **this ticket is the Memory-Core primitive** the FM-pass consumes via the `archiveMemoriesFn` seam. One clean `Resolves` each so neither PR premature-closes the other.
...
|---|---|---|---|---|---|
| `MemoryService.archiveMemoriesByAgentIdentity` (new) | neomjs/neo-agent-brain#142 thread contract; FM-pass consumer | tombstone (set `archivedAt`+`archivedReason`) rows by stamped `agentIdentity` (Chroma + graph); return `{matchedCount, archivedCount, dryRun}`; idempotent | `dryRun:true` previews `matchedCount` without tombstoning | JSDoc + op | leak-test: tombstoned rows excluded from all recall paths; counts correct |
...
- **AC1-residual** (tokenless-agent / `gh`-fallback-miss / untagged rows) — gated on the deferred spawn-time identity-env (`FleetLifecycleService:50-51`); annotated on neomjs/neo-agent-brain#142, not closed here.

## Related

- Consumed by / blocks: **#13190** (the FM-pass wires `archiveMemoriesFn` to this op).
- Harness epic: neomjs/neo-agent-brain#144. Precedent: `archive_message` soft-delete.
...
Live latest-open sweep: checked latest 15 open issues 2026-06-15T~18:48Z; no existing MC-archive/tombstone ticket (only neomjs/neo-agent-brain#142, the parent FM-pass) — net-new MC primitive.
