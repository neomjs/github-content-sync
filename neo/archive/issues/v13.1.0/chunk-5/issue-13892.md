...
|---|---|---|---|---|---|
| Message WAL graph replay | Parent epic neomjs/neo#13889 and memory graph projection precedent | Pending message WAL records project to `MESSAGE` nodes plus mailbox edges | Failed projection remains pending/retryable | JSDoc / PR body | L2 replay tests |
...
- WAL-first `add_message` acceptance; that is neomjs/neo#13891.
- Local/cloud drain host topology and deployment wiring; that is neomjs/neo#13890.
...
Parent: neomjs/neo#13889

Related: neomjs/neo#13890, neomjs/neo#13891, neomjs/neo-agent-brain#157
...
## Intake addendum — neomjs/neo#13288 precedent / neomjs/neo#13898 review carry-forward

Grace's neomjs/neo#13898 review surfaced the memory-side neomjs/neo#13288 precedent: WAL-first acceptance is only half the durability story. The follow-up replay leaf must carry both pieces of that precedent:
...
- **Pending-read overlay:** accepted WAL records that are not graph-projected yet must not become invisible to mailbox reads. Mirror the neomjs/neo#13288 memory pending-overlay shape with the message equivalent, or document an equivalent mailbox-read overlay that preserves direct/broadcast delivery semantics while projection is pending.

Code implementation should wait for neomjs/neo#13891 / PR neomjs/neo#13898 to land on `dev`; this ticket is now linked as blocked by neomjs/neo#13891. When implementation starts, update the PR body/evidence against this addendum and the existing Contract Ledger so the closeout proof covers both replay and read visibility.
