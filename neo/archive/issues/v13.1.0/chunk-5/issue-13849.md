...
Fresh neomjs/neo#13750 V-B-A on 2026-06-22 showed that the original handoff freeze is no longer the only failure shape:

- `resources/content/sandman_handoff.md` was fresh (`Jun 22 04:08:22 2026` local).
- `## Current Release / Incident Focus` was fresh (`2026-06-22T02:08:08.187Z`) and correctly ranked neomjs/neo#13750 first.
...
- GitHub exact search for `Golden Path current focus computed blog neomjs/neo#13750 neomjs/neo#10074` returned neomjs/neo#13750 and neomjs/neo#10074 only; no narrow duplicate leaf.
- A2A latest-50 sweep showed no lane-claim or lane-intent for this scope.
- Local exact sweep found prior related work (#13758, neomjs/neo#13800, neomjs/neo#13828, neomjs/neo#13844), but none owns the current contradiction contract: Current Focus is right while Computed Golden Path silently routes to unrelated blog work.

Release classification: PRIO-zero release/orchestrator steering defect under neomjs/neo#13750. Project attachment is intentionally left to the operator/current release board owner because the local ticket-create guide still names the prior v13 project number, while this leaf is a post-v13.0 Agent OS drain follow-up.

## Problem

The Computed Golden Path is the execution steering surface consumed by the orchestrator. If Current Release / Incident Focus says the PRIO-zero lane is neomjs/neo#13750 but Computed Golden Path tells agents to work neomjs/neo#10074, the swarm can still derail into valid-but-wrong meta/content work while the system-drain lane remains unresolved.
...
- neomjs/neo#13758 added the current-focus visibility section and computed recommendation filtering.
- neomjs/neo#13800 repaired the frontier baseline recency source.
- neomjs/neo#13828 covers stale frontier guide-edge cleanup when no computed section renders.
- neomjs/neo#13844 covers candidate-pool population, not policy when a candidate is computed but operationally wrong.
- neomjs/neo-agent-brain#158 is opposite-direction context: it asks for external-facing/blog visibility, not incident-work protection from blog over-routing.
...
- [ ] A focused unit fixture reproduces: Current Focus ranks neomjs/neo#13750 / PRIO-zero work first while the computed candidate is unrelated `Blog Post` / neomjs/neo#10074-style narrative work.
...
- [ ] The implementation does not use a raw current-focus boost, recency multiplier, or blanket Blog Post exclusion; neomjs/neo#10074 remains valid work under neomjs/neo#13383 when the release/incident steering context permits it.
...
- [ ] The PR body cites the neomjs/neo#13750 evidence comment and explains why neomjs/neo#13758/#13800/#13828/#13844 did not already close this contract.

## Out of Scope

- Writing or closing neomjs/neo#10074.
- Reworking the whole Golden Path scoring model.
- Closing neomjs/neo#13750.
- Reopening the neomjs/neo-agent-brain#158 external-facing visibility objective.
