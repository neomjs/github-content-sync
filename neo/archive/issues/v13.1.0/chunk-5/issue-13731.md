...
The active neomjs/neo-agent-brain#137 lane needs a PR-closeable leaf for the hook-only schema-display slice. The parent ticket remains open for AC4 ratio-over-window / COP observability, so the runtime reminder schema display must not magic-close neomjs/neo-agent-brain#137.

This successor is deliberately narrower than neomjs/neo#13643. neomjs/neo#13643 attempted to solve the emission contract with permanent skill / always-loaded substrate and was closed `not_planned` after the operator rejected that shape as context bloat. The sanctioned shape is hook-only: display the machine-readable `lane-state` block schema when the Stop hook actually blocks, with zero AGENTS.md pointer and no new skill payload.
...
|---|---|---|---|---|---|
| Claude Stop-hook reminder | `parseLaneState.mjs`, `validateLaneStateTerminal.mjs`, neomjs/neo-agent-brain#137 runtime friction | On block, display the fenced `lane-state` JSON shape plus validator gotchas after the existing no-hold directive. | Missing/invalid lane-state still blocks; schema text is guidance only, not a stop permission. Hook-internal failures still fail open. | JSDoc remains aligned with runtime reminder behavior. | Focused hook unit tests assert the schema label, required keys, and gotcha text. |
...
None. This is aligned with neomjs/neo-agent-brain#137 AC1/AC5 and the existing parser/validator seam. It explicitly avoids resurrecting the rejected neomjs/neo#13643 always-loaded / skill-substrate shape.
...
- neomjs/neo-agent-brain#137 AC4 ratio-over-window observability.
- COP rendering.
...
- Reopening neomjs/neo#13643's rejected permanent-substrate approach.

## Avoided Traps

- **Permanent skill / AGENTS.md substrate:** rejected because neomjs/neo#13643 already proved that shape is context bloat for a machine-format detail the hook can convey on block.
...
Parent: neomjs/neo-agent-brain#137
Related: neomjs/neo#13643

Live latest-open sweep: checked latest 20 open issues on 2026-06-21T08:15Z; no equivalent active leaf found. neomjs/neo#13643 found as prior rejected shape, not an active close target.
A2A in-flight claim sweep: checked latest 30 inbox messages across read states on 2026-06-21T08:15Z; no competing claim. Ada confirmed hook-only display as the neomjs/neo#13643 boundary; Vega requested the validator gotcha text.
KB duplicate sweep: queried hook-only runtime reminder / fenced lane-state schema / validator gotchas; surfaced closed historical parser/validator work (#12633 / neomjs/neo#13575) and no equivalent active leaf.
...
Retrieval Hint: `hook-only lane-state schema display`, `closed neomjs/neo#13643 substrate bloat boundary`, `parseLaneState fenced lane-state block`, `validateLaneStateTerminal gotchas`

## Timeline

- 2026-06-21T08:21:48Z @neo-gpt cross-referenced by PR #13732
- 2026-06-21T09:48:23Z @neo-gpt cross-referenced by #13742
- 2026-06-25T05:43:23Z @neo-gpt cross-referenced by #137
