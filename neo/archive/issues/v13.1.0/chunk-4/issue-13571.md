...
**Security gate:** this is the tenant-isolation boundary — the fix must canonicalize symmetrically (read key + column) so it does NOT widen visibility across tenants; each tenant's normalized id stays distinct. Requires cross-family review + @neo-opus-grace (identity↔userId boundary, neomjs/neo-agent-brain#151).
...
## Out of Scope
- who_is_online (already fixed via roster-scoping in neomjs/neo#13561).
...
## Related
- Surfaced-by / sibling: neomjs/neo#13557, neomjs/neo#13561, neomjs/neo#13527
- Identity↔userId boundary domain: neomjs/neo-agent-brain#151
...
## Close-target reconciliation (re PR neomjs/neo#13572 review)

Reconciling the ACs before PR neomjs/neo#13572 closes this bug, per @neo-gpt's review.

**The bug's harm — RLS reads returning empty for an agent's own nodes — is fixed and tested by PR neomjs/neo#13572** (read boundaries canonicalize the requester key + tolerate both stored `user_id` forms; 32/32 GraphService + 16/16 TenantIsolation, live proof 106 → 3,118 own AGENT_MEMORY nodes visible, no widening).

Reshaping the original ACs:
- **Read-side both-form tolerance** — ✅ delivered (PR neomjs/neo#13572), including the cold SQLite lazy-load path (`loadNodeVicinitySync`).
...
- **Writer-side canonicalization (the inconsistency's root)** — ➡️ **tracked in neomjs/neo#13578** (self-assigned): stop stamping the `@`-form in `upsertNode`/`linkNodes` so new rows store one normalized form. A distinct consistency cleanup, not part of the read-correctness bug.

So closing this on PR neomjs/neo#13572 merge drops nothing silently: the symptom is fixed, the migration AC is retired with rationale, and the root-cause cleanup is tracked in neomjs/neo#13578.
