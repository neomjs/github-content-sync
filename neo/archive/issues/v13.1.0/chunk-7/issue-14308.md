...
- neomjs/neo-agent-brain#135 is the broad generic-adapter design surface. This ticket is the narrow bug leaf needed now: operator-dialogue stop fallback + live Codex transcript-schema coverage.
...
| Shared hook substrate direction | neomjs/neo-agent-brain#135 and v13.2 neomjs/neo-agent-brain#126 planning | Keep the immediate fix narrow, but do not add more duplicated semantics to harness-local adapters | Broad extraction into `ai/agent/hooks` or equivalent stays a follow-up unless this fix needs a tiny shared helper | Related ticket/comment | Regression tests prove Codex/Claude parity at the contract boundary |

## Decision Record impact

Aligned with neomjs/neo-agent-brain#135's generic-by-default harness adapter direction and neomjs/neo-agent-brain#126's v13.2 architecture-quality work. No ADR change required for the narrow bug fix; a later shared `ai/agent/hooks` extraction may need a Decision Record if it establishes a new cross-harness substrate boundary.
...
- Rewriting the full harness adapter surface from neomjs/neo-agent-brain#135.
...
- **Broad architecture rewrite first:** rejected. Fix the live parity bug, then feed any shared-substrate extraction into neomjs/neo-agent-brain#135 / neomjs/neo-agent-brain#126.

## Related

- neomjs/neo#14034 — prior operator-dialogue wording/hint work; closed, but did not cover the live Codex transcript-schema/fallback failure.
- neomjs/neo-agent-brain#132 — adjacent operator-gated closeout tension; related but not equivalent.
- neomjs/neo-agent-brain#135 — broad generic harness adapter surface.
- neomjs/neo-agent-brain#137 / neomjs/neo#13822 — no-hold and value-floor design surfaces.
- neomjs/neo-agent-brain#126 — v13.2 Agent OS architecture-quality umbrella.

Live latest-open sweep: checked latest 20 open issues at 2026-06-28T21:51:14Z; no equivalent found. Nearby non-duplicates checked: neomjs/neo-agent-brain#132, neomjs/neo-agent-brain#135, neomjs/neo-agent-brain#137, neomjs/neo#13822. Exact GitHub searches for `Codex Stop hook operator dialogue` and `promptSource transcript_path operatorInLoop` found prior closed/adjacent work but no open equivalent.
