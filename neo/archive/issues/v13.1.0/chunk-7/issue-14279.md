...
The `check-ticket-archaeology.mjs` guard (built via neomjs/neo#12294) catches decay-prone ticket/Epic/Discussion/ADR refs in durable `.mjs` comments/JSDoc. Two gaps make it leak:
...
2. **It runs only in `.husky/pre-commit`** — `git commit --no-verify` fully bypasses it (empirically: PR neomjs/neo#14273 shipped 5+ `#14186` anchors that way; 6th family-wide recurrence).
...
1. **Whole-touched-file** — remove the staged-added-lines filter; scan each touched in-scope file in full (revert the neomjs/neo#13717 scoping for this guard; it matches `check-block-alignment`). Reuse the existing comment-context + `ESCAPE_MARKER` + `TICKET_PATTERNS`.
...
`none` — tooling/CI. Aligns with the §15.6 Source-comment intent filter (`learn/agentos/AGENTS_ATLAS.md`). Note: supersedes neomjs/neo#13717's added-lines-scoping *for this guard* per the operator's boy-scout directive (the churn neomjs/neo#13717 avoided is now the intended debt-reduction mechanism).
...
neomjs/neo#12294 (guard), neomjs/neo#13717/#13718/#12609 (added-lines scoping — superseded here for this guard), neomjs/neo#13816 (error msg), neomjs/neo-agent-brain#168 (autonomous CommitGate — different), PR neomjs/neo#14273 (6th-recurrence anchor).
...
- 2026-09-18T15:38:39Z @neo-opus-vega cross-referenced by PR #18905
