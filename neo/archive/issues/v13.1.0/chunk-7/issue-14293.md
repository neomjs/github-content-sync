...
**Likely regression trigger:** the heavy-maintenance-lease fairness epic landed 2026-06-27/28 (#14205, neomjs/neo#14273). kbSync now **releases** the lease at batch boundaries and resumes (so a starved heavy task interleaves), widening kbSync's wall-clock window. The un-coordinated chroma recycle/defrag now has room to fire mid-sync where it previously would not. The two gaps above are pre-existing; the lease-yield surfaced/widened the collision (this is not a defect to revert in neomjs/neo#14273).
...
- `ai/scripts/maintenance/syncKnowledgeBase.mjs` (~L120): acquires `heavyMaintenanceLease` for the sync; neomjs/neo#14273 threads the lease-yield predicate so kbSync yields + resumes.
...
- neomjs/neo#14273 / neomjs/neo#14205 — the heavy-maintenance-lease fairness epic that widened the kbSync window.
- neomjs/neo-agent-brain#131 — sibling "KB-sync fails mid-sync", different layer (embedder model eviction vs chroma connection).
- neomjs/neo-agent-brain#133 — Chroma store bloat (the defrag's purpose).

Live latest-open sweep: checked latest 20 open issues at 2026-06-28; no equivalent (closest neomjs/neo-agent-brain#131 is a different mid-sync failure layer).
...
- 2026-06-28T19:13:28Z @neo-opus-grace cross-referenced by PR #14298
