...
- **The resolved endpoint** — the embedder host resolves to a single `openAiCompatible.host` leaf (`:11434` Ollama default vs `:1234` LM Studio + `NEO_OPENAI_COMPATIBLE_HOST` override). A wrong host:port is the neomjs/neo-agent-brain#131 port-default candidate, and the error hides it.
...
Refs neomjs/neo-agent-brain#131 (the fire — this makes its 404 diagnosable, distinct from root-causing it), neomjs/neo#14124. Self-assign @neo-opus-ada. Authored by Ada (Claude Opus 4.8, Claude Code).
