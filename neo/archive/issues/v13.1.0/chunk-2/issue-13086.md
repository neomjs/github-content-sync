...
Epic neomjs/neo-agent-brain#144 names the Agent Harness / infinite-canvas product line, and neomjs/neo#13030 captured the QT-grade docking strategy scoping anchor for dock zones, nested splits, tabbed slots, preview feedback, and persistence.

The first decomposition leaf, neomjs/neo#13061, defined the dock-zone model contract and PR neomjs/neo#13062 is merged. That unlocks the next leaf: preview state for drag-to-dock interactions. This ticket deliberately stays before split/tab rendering and before layout persistence.
...
If preview state is bundled with split/tab rendering or persistence, reviewers would have to decide interaction semantics, visual affordances, layout mutation, and serialization in one PR. That repeats the broad-ticket risk neomjs/neo#13030 explicitly avoided.
...
- neomjs/neo#13030 is open as the scoping anchor, and its comments record neomjs/neo#13061 / PR neomjs/neo#13062 as the completed dock-zone model-contract leaf.
- PR neomjs/neo#13062 is merged, so the model leaf is no longer a blocker for preview-state decomposition.
- Latest-open sweep checked the latest 20 open issues at 2026-06-13T10:12Z; no equivalent preview-state leaf exists. Closest issues: neomjs/neo#13030 scoping anchor and neomjs/neo#13063 stale componentRef restore behavior.
- Scope-specific GitHub searches for `drag-to-dock preview` and `dock preview state` returned only neomjs/neo#13030 and neomjs/neo#13063, not a preview-state contract leaf.
...
- A2A last-30 sweep at 2026-06-13T10:12Z had no overlapping lane-claim or lane-intent; the active neomjs/neo#13077 claim is wake-delivery retry-store work and non-overlapping.
...
|---|---|---|---|---|
| Drag-to-dock preview-state contract | neomjs/neo#13030 + neomjs/neo#13061 model contract | Define a transient preview payload for edge/split/tab drop intent | Do not implement split/tab rendering or persistence in this leaf | Contract doc, schema-like table, or JSDoc-backed code |
...
- Cross-window choreography; neomjs/neo#13025 and neomjs/neo#13028 own that surface.
...
Prior model leaf: neomjs/neo#13061
Prior model PR: neomjs/neo#13062
Adjacent choreography: neomjs/neo#13025, neomjs/neo#13028
