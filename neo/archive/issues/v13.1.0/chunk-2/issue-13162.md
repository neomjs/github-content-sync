...
Sub-leaf of neomjs/neo#13015 (FM MVP) — the capstone of the repo-provisioning trio. neomjs/neo#13145 / PR neomjs/neo#13146 (derive the path), neomjs/neo#13148 / PR neomjs/neo#13149 (inspect the state → action), and neomjs/neo#13155 / PR neomjs/neo#13156 (provision the action) are all merged on dev. This composes them into the single "ensure an agent's repo exists" operation the Fleet Manager lifecycle calls.
...
- **Single-writer** (Scenario-C-zero per neomjs/neo#13015): the inspect→provision sequence isn't TOCTOU-guarded — and doesn't need to be (git clone fails safe if the dir changed, and the FM is single-writer per the epic guardrail).
...
- Parent neomjs/neo#13015; grandparent neomjs/neo-agent-brain#144. Composes neomjs/neo#13145 / PR neomjs/neo#13146, neomjs/neo#13148 / PR neomjs/neo#13149, neomjs/neo#13155 / PR neomjs/neo#13156 (the now-merged trio).

Live latest-open sweep: checked open issues + an ensure / orchestrate / compose-repo scan at 2026-06-14T00:49Z; no equivalent found (the one "orchestrator" hit is neomjs/neo#12065, the unrelated REM pipeline epic).
