...
V-B-A finding while verifying neomjs/neo#13082 against live `dev` (HEAD `8e545f48b`, post neomjs/neo#13103/#13084 merge): the Neural Link harness tool-projection mechanism is fully implemented and enforced (#13084 shipped `ToolService.isToolAllowedForProjection` + the OpenAPI tier contract + `POLICY_REFUSED` on `call_tool`), but the **projection mode is selected solely from client-supplied request metadata**. An embedded agent that simply omits the flag receives the full developer/operator surface — including `write-locked` and `admin` tools. This is the "server-side connection-class binding leaf" @neo-gpt flagged as a split-out in the 2026-06-13 team-sunset handoff on neomjs/neo-agent-brain#144.
...
| `_meta.neoToolProjection` (existing client hint, `Server.mjs:85`) | neomjs/neo#13084 | Demoted to a *narrowing-only* hint; cannot escalate past the server-instance ceiling | Server forced mode is the ceiling | Test: client sending no `_meta` under forced mode still gets the restricted surface |
...
- Parent: neomjs/neo-agent-brain#143 (Extended-NL coordination: curated tool surface)
- Builds on: neomjs/neo#13084 / PR neomjs/neo#13103 (projection boundary + mechanism), neomjs/neo#13064 / PR neomjs/neo#13078 (tier contract)
- Consumer/injector: neomjs/neo#13015 (Fleet Manager MVP — spawn-time provisioning)
- Sibling (closed; verified done by neomjs/neo#13103): neomjs/neo#13082
- Concept: ADR 0020 (`learn/agentos/decisions/0020-agent-harness-concept.md`)

Live latest-open sweep: checked latest 25 open issues at 2026-06-13 (post-#13103 merge); only neomjs/neo-agent-brain#143 (parent) overlaps — no equivalent leaf. A2A in-flight sweep: turn-start `list_messages` (50, recency-bound) shows no `[lane-claim]`/`[lane-intent]` on this scope; neomjs/neo#13082/#13084 released/merged.
...
- 2026-06-15T18:05:07Z @neo-opus-grace cross-referenced by #141
...
- 2026-07-16T08:32:07Z @neo-opus-vega cross-referenced by PR #15224
