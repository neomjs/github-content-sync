...
`#13030` is the docking strategy scoping anchor under Epic neomjs/neo-agent-brain#144. Its first contract leaves are now merged: neomjs/neo#13061 / PR neomjs/neo#13062 defined the dock-zone model, neomjs/neo#13086 / PR neomjs/neo#13087 defined the transient `dockPreview` state, and neomjs/neo#13089 / PR neomjs/neo#13090 defined the split/tab adapter boundary.

All open PRs were verified empty at the start of this nightshift cycle. Ada has taken neomjs/neo#13104 for the preview-renderer leaf. This ticket creates the next non-overlapping one-PR leaf: turn the accepted split/tab adapter contract into concrete renderable Neo layout/tab structures, without bundling preview rendering or persistence.
...
- Live issue state at 2026-06-13T16:23:03Z: neomjs/neo#13030 remains open as the scoping anchor; neomjs/neo#13104 is open and assigned to Ada for preview rendering; no open split/tab rendering implementation ticket exists in the latest-open queue.
- Scope-specific GitHub sweep for `split/tab rendering`, `split tab rendering`, `split/tab adapter`, and `docking renderable structures` found only neomjs/neo#13030, neomjs/neo#13104, and the closed contract leaves neomjs/neo#13089 / neomjs/neo#13086 / neomjs/neo#13061.
...
- A2A last-30 sweep at 2026-06-13T16:23:03Z showed the recent neomjs/neo#13104 lane claim and prior lifecycle broadcasts, but no competing `[lane-claim]` or `[lane-intent]` for split/tab layout rendering.
- Knowledge Base semantic check found adjacent historical primitives (#3519 tab drag/drop, neomjs/neo#2192 splitter, neomjs/neo#8155 dashboard detach/reintegration), but no existing issue for implementing this exact dock-zone-to-split/tab rendering leaf.
...
|---|---|---|---|---|
| Split/tab rendering adapter | neomjs/neo#13089 / PR neomjs/neo#13090 and `HarnessDockZoneModel.md` | Consume dock-zone model nodes and return renderable Neo configs/components | Do not create a generic core `Dock` primitive without structural evidence | Unit/static tests cover split and tab projection |
...
| Component identity handoff | neomjs/neo#13063 and model stable item identity | Resolve live `componentRef` or blueprint into child configs while keeping serialized ids stable | Missing refs render recoverable placeholder/fail-safe | Tests prove stale refs do not mutate/drop model records |
| Preview boundary | neomjs/neo#13104 and neomjs/neo#13086 | Adapter consumes committed layout model, not transient hover state | No `dockPreview`, DOMRect, pointer, or window geometry dependency in this leaf | Source/tests reject preview-only fields in adapter input |
| Persistence boundary | neomjs/neo#13030 blueprint compatibility | Output remains persistence-compatible | No save/restore storage implementation in this PR | PR body names the later persistence leaf boundary |

## Decision Record Impact

Aligned-with ADR 0020 and Epic neomjs/neo-agent-brain#144. This ticket implements a harness/dashboard docking leaf and should not amend an ADR unless implementation evidence proves the adapter must become a general core layout primitive.
...
Depends on: neomjs/neo#13061 / PR neomjs/neo#13062 and neomjs/neo#13089 / PR neomjs/neo#13090
Adjacent: neomjs/neo#13086 / PR neomjs/neo#13087, neomjs/neo#13104, neomjs/neo#13063, neomjs/neo#13025, neomjs/neo#13028
