...
- Live latest-open GitHub sweep checked the latest 20 open issues at 2026-06-13T09:49Z. No equivalent boundary ticket exists. Closest issues: neomjs/neo-agent-brain#143 parent epic, neomjs/neo#13064 tier-contract leaf, neomjs/neo#13082 consumer leaf.
...
|---|---|---|---|---|---|
| Harness Neural Link projection boundary | neomjs/neo-agent-brain#143 tool-surface design row + ADR 0020 co-habitation guardrails | Embedded harness agents have an explicit boundary for receiving a curated Neural Link tool surface. | Fail closed to no write/admin projection when authority context is absent. | JSDoc on the boundary plus PR body Contract Ledger. | Unit/static test proves embedded projection is narrower than the full developer surface. |
...
| `#13082` consumer interface | neomjs/neo#13064 tier contract, after PR `#13078` lands | The consumer leaf receives a stable interface instead of inventing filter/list/gate semantics. | `#13082` remains blocked until this boundary and neomjs/neo#13064 are present on `dev`. | Comment on `#13082` links this dependency. | Native `blocked_by` relationship from `#13082` to this ticket. |
...
Aligned-with ADR 0020. This ticket implements one of the neomjs/neo-agent-brain#143 curated tool-surface prerequisites and does not amend the harness concept. It should not redefine Topological Locking, Bridge-token auth, or admin authority; those remain separate leaves under neomjs/neo-agent-brain#143.
...
- [ ] Tests or static validation prove the selected boundary is where `#13082` can consume `x-neo-harness-tool-projection` / `x-neo-tool-tier` after neomjs/neo#13064 lands.
...
- Implementing the tier-map consumer itself; that remains neomjs/neo#13082.
- Changing the tier taxonomy from neomjs/neo#13064 / PR `#13078`.
...
Blocks neomjs/neo#13082
Blocked by neomjs/neo#13064
...
> - `#13082` correctly depends on this boundary; attempts to add `#13082 blocked_by neomjs/neo#13084` and `#13084 blocked_by neomjs/neo#13064` returned GitHub's duplicate-target validation, so the issue graph already carries those blocker targets.
> - PR `#13078` is approved/clean but unmerged; its OpenAPI tier contract lives in `ai/mcp/server/neural-link/openapi.yaml` once neomjs/neo#13064 lands.
> - Current checkout still has no `x-neo-tool-tier` / `x-neo-harness-tool-projection` fields, so this implementation must either wait for neomjs/neo#13078 on `dev` or explicitly base on that PR.
...
> - embedded projection lists `read` tools only while neomjs/neo-agent-brain#143 locking/auth leaves are absent;
...
- 2026-06-21T18:20:02Z @neo-opus-grace cross-referenced by PR #13795
- 2026-06-29T16:09:49Z @neo-gpt cross-referenced by #14333
