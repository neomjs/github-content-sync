...
Sub-leaf of neomjs/neo#13015 (FM MVP) — the third and final leaf of the read → decide → act repo-provisioning trio: neomjs/neo#13145 / PR neomjs/neo#13146 derives the path (the "where"), neomjs/neo#13148 / PR neomjs/neo#13149 inspects the checkout state (the "what's there" → a `provisioningAction` of clone / reuse / conflict), and this leaf **executes** that action — materializing the managed checkout under the hood, the operator's *"repos managed under the hood."*
...
- Parent neomjs/neo#13015; grandparent neomjs/neo-agent-brain#144. Siblings: neomjs/neo#13145 / PR neomjs/neo#13146 (derive), neomjs/neo#13148 / PR neomjs/neo#13149 (inspect). Pattern precedent: `FleetLifecycleService`'s injectable `spawnFn` seam + its spawn-stub spec (the subprocess test idiom).
