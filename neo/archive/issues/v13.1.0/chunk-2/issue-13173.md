...
Leaf B of neomjs/neo#13167 (Extended-NL identity-transport — the contract umbrella). The first landable + **backward-compatible** half of the Body-side enforcement wiring: the Client learns to unwrap the Bridge's `{type:'agent_message', agentId, message}` sidecar (per @neo-opus-grace's locked contract) and thread the verified `agentId` as a request context to the service dispatch — **before** the Bridge starts emitting the sidecar. That ordering is the cross-leaf finding @neo-opus-grace surfaced: a Client that already understands `agent_message` never breaks the live agent→app channel (Leaf A neomjs/neo#13172 = the Bridge auth; the `(1b)` emit is split off as the final flip, sequenced last).
...
- The Bridge auth (Leaf A / neomjs/neo#13172) + the `(1b)` emit-flip (@neo-opus-grace).

## Decision Record impact

`aligned-with` neomjs/neo#13167 — implements its ledger row 3 + the consume side of row 2.

## Related

- Parent neomjs/neo#13167 (umbrella + Contract Ledger); grandparent neomjs/neo-agent-brain#143. Sequenced **before** the `(1b)` emit (the ordering @neo-opus-grace found). Leaf A = neomjs/neo#13172.

Live latest-open sweep: this leaf was just split off neomjs/neo#13167 with @neo-opus-grace (2026-06-14T01:43Z); no equivalent open ticket.
