...
`#13030` is the scoping anchor for QT-grade docking strategies on Neo's infinite canvas: dock zones, nested splits, tabbed dock slots, and blueprint-compatible save/restore. The first leaf, neomjs/neo#13061 / PR neomjs/neo#13062, has landed the dock-zone model contract. The next leaf, neomjs/neo#13086 / PR neomjs/neo#13087, defines transient drag-to-dock preview state and is approved but not merged as of this filing.
...
- how stable dock item identity maps to component references and stale restore behavior already handled by neomjs/neo#13063;
- where drag-preview producers hand off to real layout mutation after neomjs/neo#13086 lands;
- whether blueprint persistence fields are consumed now or deferred to a persistence leaf.

That would recreate the neomjs/neo#13030 broad-ticket problem inside a PR. The missing piece is a narrow adapter contract: model in, Neo layout/tab primitives out, no save/restore implementation yet.

## The Architectural Reality

V-B-A before filing:

- neomjs/neo#13030 remains open as the scoping anchor and explicitly lists remaining leaves after neomjs/neo#13061: drag-to-dock preview, split/tab rendering adapter, blueprint persistence, and demand validation.
- neomjs/neo#13061 is closed and PR neomjs/neo#13062 is merged, so the dock-zone model contract is the source of authority for this adapter leaf.
- neomjs/neo#13086 / PR neomjs/neo#13087 is approved and green but still open, so preview-state producer details should be treated as adjacent context, not a hard implementation dependency.
- Live latest-open sweep at 2026-06-13T10:56Z checked the latest 20 open issues. Adjacent issues found: neomjs/neo#13030, neomjs/neo#13063, neomjs/neo#13086. No equivalent split/tab adapter boundary leaf exists.
- Scope-specific GitHub sweep for `dock split tab rendering adapter` found neomjs/neo#13030 and neomjs/neo#13063 only; neither defines the adapter boundary this ticket covers.
- A2A last-30 sweep at 2026-06-13T10:56Z showed no competing `[lane-claim]` / `[lane-intent]` for this split/tab adapter scope.
- Local source sweep found existing primitives that should be considered before adding anything new: `src/tab/Container.mjs`, `src/component/Splitter.mjs`, layout-family usage around `vbox` / `hbox` / card semantics, and the existing dashboard drag lineage named by neomjs/neo#13030.
...
- Memory Core semantic search surfaced the active neomjs/neo#13030 decomposition trail (#13061, neomjs/neo#13086, neomjs/neo#13087) and no prior split/tab adapter leaf.
...
1. Read the merged dock-zone model contract from neomjs/neo#13061 / PR neomjs/neo#13062 and identify the minimal adapter input it exposes.
...
|---|---|---|---|---|
| Split/tab adapter boundary | neomjs/neo#13061 dock-zone model contract + neomjs/neo#13030 decomposition | One named boundary consumes dock-zone model nodes and produces Neo layout/tab structures | If ownership is uncertain, document rejected placements and keep implementation docs-only | PR body names chosen owner and cites source reads |
...
| Component identity handoff | neomjs/neo#13063 stale componentRef restore behavior + dock model stable IDs | Preserve stable dock item identity while allowing stale runtime refs to fail closed | Do not make runtime component refs part of serializable blueprint state | Contract distinguishes serializable IDs from runtime refs |
| Preview producer boundary | neomjs/neo#13086 preview-state contract when merged | Adapter consumes committed drop/layout mutations, not live hover/preview state | If neomjs/neo#13086 is not on `dev`, name the pending dependency and keep preview coupling out | PR body distinguishes producer vs committed adapter boundary |
| Blueprint persistence seam | neomjs/neo#13030 blueprint compatibility requirement | Keep emitted structure persistence-compatible | Actual save/restore implementation is a later leaf | JSON examples remain persistence-ready without writing storage code |

## Decision Record Impact

Aligned-with ADR 0020 and Epic neomjs/neo-agent-brain#144. This ticket refines the harness docking product-line decomposition and should not amend an ADR unless implementation proves the adapter must become a general core layout primitive.
...
- [ ] Runtime component references remain outside the serializable layout state and align with neomjs/neo#13063's stale-restore behavior.
- [ ] Drag-preview/hover state from neomjs/neo#13086 is not bundled; the boundary is for committed layout structure.
...
Scoping source: neomjs/neo#13030
Depends on: neomjs/neo#13061 / PR neomjs/neo#13062
Adjacent: neomjs/neo#13063, neomjs/neo#13086 / PR neomjs/neo#13087, neomjs/neo#13025, neomjs/neo#13028
