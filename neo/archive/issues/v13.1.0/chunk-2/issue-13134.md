...
Leaf of neomjs/neo-agent-brain#143 (Extended-NL coordination), building the **in-heap authoritative layer** the epic names ("in-heap (App-Worker) enforcement as the authoritative layer — the heap is the shared truth; Bridge-level checks are advisory"). It is the consumer of neomjs/neo#13125's `LockRegistry` (the stateless conflict math): `WriteGuard` *holds* the live lock state and exposes the write-lifecycle API the in-app Neural Link client calls before applying a write-class operation.

`Release classification:` boardless — harness coordination line, post-v13.

`Depends on:` neomjs/neo#13126 (LockRegistry must be on `dev`). The implementing PR stacks on the neomjs/neo#13126 branch and retargets to `dev` once it merges.

## The Problem

`LockRegistry` decides conflict over a *caller-held* table but holds no state. Enforcement needs a stateful **authority** that owns the live lock table for an App-Worker heap, so a write-class op acquires-and-holds a subtree lock and a *different* agent's overlapping write is denied while it is held (guardrail neomjs/neo#1: Topological Locking before any multi-writer slice).
...
| Lock lifetime | neomjs/neo-agent-brain#143 epic + convergence A2A | held-until-release + disconnect-sweep | per-op rejected; lease = additive follow-up | unit: held lock blocks a 2nd writer until released |

## Decision Record impact

aligned-with ADR 0020 (guardrail neomjs/neo#1); consumes neomjs/neo#13125. No ADR amendment.
...
Parent: neomjs/neo-agent-brain#143. Refs neomjs/neo-agent-brain#144 (guardrail neomjs/neo#1), neomjs/neo#13125 / PR neomjs/neo#13126 (LockRegistry), neomjs/neo#13130 (sibling auto-targeting slice).
...
- 2026-07-22T02:24:43Z @neo-gpt cross-referenced by #15681
