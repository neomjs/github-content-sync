...
Sub-leaf of neomjs/neo#13015 (Fleet Manager MVP) — the repo-provisioning surface the epic names as a *future, in-scope* leaf: *"per-agent clone/worktree management under the hood (create, locate, health-check; path stability honored because downstream memory is path-keyed)."* The MVP operator loop is *define agents → start/stop → repos managed under the hood*. The registry (#13031), lifecycle (#13049), settings pane (#13058), and bridge-token (#13065 / neomjs/neo#13121) leaves have all landed — but **nothing yet derives where an agent's managed checkout lives**.
...
- Parent: neomjs/neo#13015 (Fleet Manager MVP); grandparent neomjs/neo-agent-brain#144.
- Sibling leaves: neomjs/neo#13031 (registry / credentials), neomjs/neo#13049 (lifecycle), neomjs/neo#13058 (settings pane), neomjs/neo#13065 / neomjs/neo#13121 (bridge-token).
