...
Sub-leaf of neomjs/neo-agent-brain#143 (Extended-NL coordination). The multi-writer write-enforcement subsystem now spans five composable components — `LockRegistry` (#13125), `WriteGuard` (#13134), `deriveSubtreePath` (#13138), `parseAgentEnvelope` (#13174), `resolveWriteLock` (#13207/PR neomjs/neo#13208) — plus the planned `InstanceService` write-path wiring (#13167 Leaf B/C). V-B-A (2026-06-14): `grep -rl 'WriteGuard|LockRegistry' learn/` → **zero** — the architecture exists only as scattered per-file JSDoc + the neomjs/neo#13167 ticket (ephemeral). There is **no consolidated, KB-queryable architecture doc**.
...
A focused `learn/agentos/` architecture doc (sibling to `HarnessDockZoneModel.md`): the problem (co-habited-heap write corruption), the model (held-until-release subtree locks keyed on the `(agentId, sessionId)` writer pair), the per-layer roles, the data flow, the fail-closed discipline, and an explicit **STATUS** section (primitives merged on `dev`; `resolveWriteLock` approved in neomjs/neo#13208; write-path wiring + disconnect-sweep pending under neomjs/neo#13167). Add to `learn/tree.json` (mirrors-folder). Status-marking keeps it honest while the subsystem is ~80% built.
...
- Parent neomjs/neo-agent-brain#143; consumes the neomjs/neo#13167 contract-of-record. Sibling doc pattern: `learn/agentos/HarnessDockZoneModel.md`.
