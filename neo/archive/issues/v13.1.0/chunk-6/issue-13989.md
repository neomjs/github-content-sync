...
Live latest-open sweep: checked the latest 20 open issues on 2026-06-25; no equivalent default-enabled deployment-state bridge ticket found. Search sweep for `deployment-state bridge enabled default opt-out snapshot-missing` found neomjs/neo-agent-brain#134 only, which is the smoke/closeout gate rather than the implementation fix. A2A in-flight sweep: `list_messages({status: 'all', limit: 30})` found no competing lane claim for this scope.
...
- The KB/MC tools then correctly report `snapshot-missing`, but neomjs/neo-agent-brain#134 cannot close because there is no snapshot to inspect.
...
|---|---|---|---|---|---|
| `AiConfig.orchestrator.deploymentStateBridge.enabled` | ADR-0019 config SSOT + neomjs/neo-agent-brain#134 smoke evidence | Defaults to enabled, with explicit env opt-out | `NEO_DEPLOYMENT_STATE_BRIDGE_ENABLED=false` disables writer | config comments | config/unit tests |
| KB/MC deployment inspection tools | neomjs/neo-agent-brain#134 public-surface smoke gate | Read a fresh bounded snapshot when orchestrator is running | Return explicit unavailable/stale reasons | MCP OpenAPI + tool summaries | KB/MC smoke after restart |
...
- [ ] Post-merge validation: restart orchestrator + KB/MC, then neomjs/neo-agent-brain#134 KB/MC-only smoke observes `status: available` or a meaningful stale/degraded status instead of `snapshot-missing`.

## Out of Scope

- Adding shell, Docker socket, restart, exec, or actuator authority to KB/MC.
- Changing the recovery actuator task policy.
- Closing neomjs/neo-agent-brain#134 without a live KB/MC-only smoke.
...
Related: neomjs/neo-agent-brain#134, neomjs/neo#13926, neomjs/neo#13914, ADR-0019, ADR-0025, ADR-0026.

Origin Session ID: 1c4b42c3-289a-4196-bec0-36a3a9f16fa6

Retrieval Hint: `deployment-state bridge enabled default opt-out snapshot-missing neomjs/neo-agent-brain#134`
