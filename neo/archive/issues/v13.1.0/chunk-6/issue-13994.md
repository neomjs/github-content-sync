...
During adversarial self-review of PR neomjs/neo#13986 / issue neomjs/neo#13984, the operator reported that the chat model stayed locked overnight while LM Studio showed 400k+ generated tokens. The identified source session was not large enough to explain that counter by input size alone, and neomjs/neo#13986 now explicitly treats that symptom as generated-output runaway rather than as proof that input chunking alone explains the failure.
...
Live latest-open sweep: checked the latest 20 open issues at 2026-06-25T08:05:46Z. neomjs/neo#13984 is related and covers bounding a single REM parser call, but no equivalent ticket covered 400k+ counter overflow exceeding the source session size or possible multi-session/context contamination.

A2A in-flight sweep: checked recent all-status A2A traffic at 2026-06-25T08:05:46Z. Recent claims/updates were my own neomjs/neo#13984/#13986 lifecycle messages plus unrelated neomjs/neo#6777/#13936 traffic; no competing claim for this follow-up scope was present.

KB/local prior-art sweep: `ask_knowledge_base("Existing issue or documentation about REM 400k token LM Studio context reuse multiple sessions SemanticGraphExtractor DreamService", type="ticket")` surfaced related prior art (#12074 context reuse benchmark, neomjs/neo#13918 parser repair overflow, neomjs/neo#12073 chunking, neomjs/neo#13984 current cap work), but no explicit 400k overflow / multi-session contamination ticket. Local search across `resources/content/issues`, `resources/content/discussions`, and `learn/agentos` found REM/context prior art but no exact equivalent.
...
- PR neomjs/neo#13986 caps provider completion for a single Tri-Vector call, records active call diagnostics, and marks provider-size parser failures terminal for cadence.
...
`SemanticGraphExtractor.createTriVectorChunks()` plans prompt chunks from `session.turnDocuments` or `session.document`; after neomjs/neo#13986 it clamps the effective prompt budget by `graphChunkLimitTokens`, `safeProcessingLimitTokens`, and `contextLimitTokens - graphOutputLimitTokens` before subtracting envelope tokens (`ai/services/graph/SemanticGraphExtractor.mjs:293`).
...
| Active REM diagnostics | `remRunStateStore` / PR neomjs/neo#13986 diagnostics | Diagnostics let an operator correlate one in-flight chunk with provider-visible counters | If the provider cannot expose per-call counters, record that explicitly | `learn/agentos/rem-state-model.md` if fields change | Script/probe comparing diagnostics to provider counter |
...
Handoff Retrieval Hint: `REM 400k token counter SemanticGraphExtractor LM Studio context reuse multiple sessions PR neomjs/neo#13986 graphChunkLimitTokens`
