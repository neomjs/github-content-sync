...
@tobiu clarified the freshness model on 2026-05-03 while reviewing neomjs/neo#10646:
...
- The live latest-20 GitHub issue sweep in neomjs/neo#10646 is intended to bridge exactly that freshness gap.
...
- Local resource search over `resources/content/issues`, `resources/content/issue-archive`, `resources/content/discussions`, and `.agents/skills/memory-mining` found adjacent but non-duplicate items:
  - neomjs/neo#10586 — closed documentation bug correcting the false `AGENTS_STARTUP.md` auto-summarization claim.
  - neomjs/neo#9997 — closed implementation improvement prioritizing latest active sessions inside the summarization pipeline.
  - neomjs/neo#10075 — closed ticket that created `memory-mining` and codified the current summary-first query strategy.
  - neomjs/neo#10556 / neomjs/neo#10569 — broader Memory Core visibility and summarization freshness issues, not the skill-level query-order rule.
- Live latest-20 open GitHub issue sweep on 2026-05-03 read neomjs/neo#10650, neomjs/neo#10649, neomjs/neo#10648, neomjs/neo#10647, neomjs/neo#10646, neomjs/neo#10645, neomjs/neo#10644, neomjs/neo#10643, neomjs/neo#10635, neomjs/neo#10633, neomjs/neo#10627, neomjs/neo#10605, neomjs/neo#10604, neomjs/neo#10601, neomjs/neo#10537, neomjs/neo#10517, neomjs/neo#10494, neomjs/neo#10484, neomjs/neo-agent-brain#152, neomjs/neo#10462. None own this `memory-mining` freshness-lane correction.
...
This is the Memory Core analogue of neomjs/neo#10646. In both cases, a semantically convenient cached/indexed layer is useful but not fresh enough to be the only source of truth during active swarm work.
...
- neomjs/neo#10075 created the skill and deliberately prescribed `query_summaries` before `query_raw_memories` for breadth-first lookup.
- neomjs/neo#10586 corrected boot documentation to say auto-summarization is not automatic by default.
- neomjs/neo#9997 improved internal summarization ordering but does not guarantee that active-session summaries exist at query time.
...
- [ ] At least one concrete example covers the wake/A2A regression class from neomjs/neo#10647 and shows raw-memory-first retrieval.
- [ ] Related docs or references mention neomjs/neo#10646 as the analogous GitHub/KB freshness pattern only if doing so does not create cross-skill bloat.
...
- **Trap: solve this by forcing auto-summarization.** Rejected. neomjs/neo#10586 documents why auto-summarization is not automatic by default and why the default is load-bearing.
- **Trap: conclude no prior context from one query surface.** Rejected. Freshness-sensitive work requires raw-memory verification.

## Related

- neomjs/neo#10646 — live GitHub latest-open sweep for ticket-create freshness; same structural freshness gap on the GitHub/KB side.
- neomjs/neo#10075 — created the `memory-mining` skill and current summary-first rule.
- neomjs/neo#10586 — corrected false boot-summarization assumptions.
- neomjs/neo#9997 — prior summarization ordering improvement.
- neomjs/neo#10647 — current wake/A2A incident where raw memories and same-day context are especially important.
