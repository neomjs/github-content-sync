...
3. **Team vs Private Retrieval Policy**
   - neomjs/neo#10010 owns retrieval-side `memorySharing` behavior for `query_raw_memories` and `query_summaries`.
...
4. **Native Edge Graph Tenant Isolation**
   - neomjs/neo#10011 owns SQLite / Native Edge Graph RLS and tenant isolation semantics.
...
5. **Concept Ontology Replaces Macro DB**
   ...
   - neomjs/neo#10030 Concept Ontology is the active replacement: deterministic, version-controlled concept nodes plus traversal-based gap inference.
...
- neomjs/neo#10016: Multi-Tenant Identity & Data Privacy parent.
- neomjs/neo#10010: Memory Core team/private retrieval policy (`memorySharing`).
- neomjs/neo#10011: Native Edge Graph RLS / tenant isolation.
- neomjs/neo#10030: Concept Ontology and semantic gap inference replacement for Macro DB.
...
- Do not close neomjs/neo#9999 while native sub-issues remain open; prior reopen workflow churn proved parent-state semantics are load-bearing.
...
> ✦ **Architecture Update:**\nThe originally planned Sub-Epic neomjs/neo#10014 (Macro Knowledge Base) has been closed due to semantic dilution problems found during execution.\n\nIt is superseded by **Epic neomjs/neo#10030** (Concept Ontology Layer). We are pivoting the Gap Inference Engine from relying on heavy full-file vector comparisons to utilizing a deterministic, version-controlled JSONL Concept Graph and traversal engine via `ai/services.mjs`.
...
**Updated 2026-04-21:** Phase 1 complete. neomjs/neo#10161 merged; neomjs/neo#10151 shipped. Phase 2 now unlocked — see below.
...
| PR | Resolves | Status |
|---|---|---|
| [#10162](https://github.com/neomjs/neo/pull/10162) | neomjs/neo#10144 AgentIdentity node + seed script + IdentitySchema.md | **Merged** |
| [#10161](https://github.com/neomjs/neo/pull/10161) | neomjs/neo#10151 MemorySessionIngestor deterministic ingestion phase | **Merged** |

Phase 1 is closed. Both Opus-track and Gemini-track deliverables shipped; downstream subs (#10152, neomjs/neo#10153, neomjs/neo#10158, neomjs/neo#10145) are now unblocked.

### Tickets filed

| Ticket | Purpose |
|---|---|
| [#10164](https://github.com/neomjs/neo/issues/10164) | Pre-commit cross-PR file-collision check — empirically anchored on this session's three-way neomjs/neo#10163 / neomjs/neo#10142 / neomjs/neo#10160 collision + neomjs/neo#10162 post-merge conflict |
...
### Phase 4 (opens after neomjs/neo#10147 merges)
...
| [#10150](https://github.com/neomjs/neo-agent-brain/issues/157) Optional Chroma semantic layer | **Either** |
...
**Rule:** whichever identity picks up the first sub from neomjs/neo#10016 or neomjs/neo#10139 runs `epic-review` before ticket-intake on that sub. If both identities pick up subs, both run independent reviews (cross-model asymmetry is the feature per `epic-review-workflow.md` §8).

---

## Outstanding Follow-ups

- **[#10164](https://github.com/neomjs/neo/issues/10164)** (unassigned) — pre-commit cross-PR file-collision check. Empirical cases neomjs/neo#1 + neomjs/neo#2 = neomjs/neo#10163/#10142/#10160 + neomjs/neo#10162 post-merge collision. Consolidation candidate with a stale-local-ticket mechanical guard if the Stale Prevention rule fails in practice (not filed separately; file combined later if either/both discipline rules fail empirically).

- **v1.1 skill-refinement candidates** (track only; file if empirical 3+ instance):
  - `[BLOCKED_BY]` fourth tag for `pull-request` §7 (blocked-on-upstream-merge case — observed once with Gemini's neomjs/neo#10163 `[DEFERRED]` stretch)
...
- `a38d25d1-3003-4d57-b0ac-0effe2c3507e` (prior session — neomjs/neo#10139 mailbox original filing)

Gemini:
- `7a73e53f-801a-490f-b693-b431189aa1a9` (this session — workflow infrastructure + neomjs/neo#10144 + epic-review on neomjs/neo#10143)
- `30e93319-06e2-44d2-adf2-99168a997d08` (prior session — neomjs/neo#10141 / neomjs/neo#10142 Self-Identification origin)
...
1. Begin [#10152](https://github.com/neomjs/neo/issues/10152) Gemma4 extractor provenance edges — attaches extracted entities to Memory/Session nodes now that neomjs/neo#10151 has shipped.
2. Phase 4 entry: [#10148](https://github.com/neomjs/neo/issues/10148) Lifecycle and [#10149](https://github.com/neomjs/neo/issues/10149) Healthcheck preview after neomjs/neo#10147 ships.
...
1. Run `neo-mjs-github-workflow:sync_all` (or `gh issue view neomjs/neo#9999`) to fetch latest state of this comment thread — per `ticket-intake-workflow.md` §1 Stale Local Ticket Prevention.
...
4. Before first sub-pickup from neomjs/neo#10016 or neomjs/neo#10139: run `epic-review` skill (per §5 per-agent-per-epic one-shot).
...
**Updated 2026-04-22T09:15Z** — corrected neomjs/neo#10139 epic-review status (Gemini did post, multiple times; original comment mis-stated "Not formally reviewed").
...
|---|---|---|---|
| [#10165](https://github.com/neomjs/neo/pull/10165) | neomjs/neo#10152 Gemma4 extractor provenance edges + lazy-queue producer | Gemini | **Merged** |
| [#10166](https://github.com/neomjs/neo/pull/10166) | neomjs/neo#10145 stdio identity + anti-spoof guard | Opus | **Merged** |
| [#10167](https://github.com/neomjs/neo/pull/10167) | neomjs/neo#10146 + neomjs/neo#10147 bundled: permissions + mailbox primitives | Gemini | **Merged** |
| [#10170](https://github.com/neomjs/neo/pull/10170) | neomjs/neo#10168 test coverage + role-based addressing + serial fixture isolation | Gemini | **Merged** |
| [#10171](https://github.com/neomjs/neo/pull/10171) | neomjs/neo#10153 lazy back-fill consumer + priorityBackfill CLI | Opus | **Merged** |

**Actuals shifted from the 17:03 handover's proposed delegation:** Gemini picked up the Opus-track Phase 3 (#10147 + neomjs/neo#10146 bundled as option A); Opus picked up the Gemini-track-candidate neomjs/neo#10153 (lazy back-fill) since Gemini was in-context on the mailbox work. Net: same substrate coverage, different track attribution. The bundling + scope-split decisions were negotiated in the planning exchanges visible on this ticket.

### Tickets filed this cycle

| Ticket | Purpose | Parent |
|---|---|---|
| [#10168](https://github.com/neomjs/neo/issues/10168) | Mailbox test-coverage expansion + role-based addressing | neomjs/neo#10139 |
| [#10169](https://github.com/neomjs/neo/issues/10169) | `TAGGED_CONCEPT` auto-emit via `SemanticGraphExtractor` on message bodies | neomjs/neo#10143 |
| [#10172](https://github.com/neomjs/neo/issues/10172) | Graph node-ID case canonicalization (lowercase `memory:` vs uppercase `MEMORY:`) | neomjs/neo#10143 |

---

## Remaining Open Work

### Phase 4 Mailbox subs (now unblocked by neomjs/neo#10167 merge)

| Ticket | Proposed owner | Notes |
|---|---|---|
| [#10148](https://github.com/neomjs/neo/issues/10148) Lifecycle (archive/delete + retracted-marker) | **Gemini** | Gemini in-context on mailbox substrate from neomjs/neo#10167 / neomjs/neo#10170. |
| [#10149](https://github.com/neomjs/neo/issues/10149) Healthcheck extension (inbox/outbox preview) | **Gemini** | Pairs naturally with neomjs/neo#10158 telemetry surface. |
| [#10150](https://github.com/neomjs/neo-agent-brain/issues/157) Optional Chroma semantic layer for "find related messages" | **Either** | Middle-ground; extractor-adjacent (Gemini context) vs retrieval-layer (either). |

### Phase 3 follow-ups from this cycle

| Ticket | Proposed owner | Rationale |
|---|---|---|
| [#10169](https://github.com/neomjs/neo/issues/10169) `TAGGED_CONCEPT` auto-emit | **Gemini** | `SemanticGraphExtractor` territory; Gemini deep context from neomjs/neo#10152 / neomjs/neo#10165. |
...
| [#10158](https://github.com/neomjs/neo/issues/10158) Post-ship telemetry + retention | **Either** | Surfaced by Gemini's epic-review on neomjs/neo#10143; still unassigned. |
...
|---|---|---|
| Opus → Gemini | neomjs/neo#10165 | 3 polish items (`/tmp` convention, JSDoc gap, partial coverage) → addressed in single follow-up |
| Opus → Gemini | neomjs/neo#10167 | Scope reconciliation (6 of 8 edge types missing) + openapi duplicate blocker → addressed cleanly |
| Opus → Gemini | neomjs/neo#10170 | 5 polish items (RA1–RA5) → addressed in single follow-up |
| Gemini → Opus | neomjs/neo#10171 | 2 Depth Floor challenges (SIGKILL orphan, canonical-case debt) → `[ADDRESSED]` + `[DEFERRED]` to neomjs/neo#10172 |
...
Opus:
- `d69ac7a0-9fe8-4416-b766-cd9edb8bee71` (this session — neomjs/neo#10145 + neomjs/neo#10153 + four reviews)
- `71dc3cd8-d39d-48e1-ac62-e240ca67d1a5` (prior session — workflow infrastructure + Phase 1 Opus track)

Gemini:
- `ada4aecb-60fc-4b25-a76b-cb92600835a2` (this session — neomjs/neo#10152 + neomjs/neo#10146 + neomjs/neo#10147 + neomjs/neo#10168 + epic-review on neomjs/neo#10139)
- `7a73e53f-801a-490f-b693-b431189aa1a9` (prior — neomjs/neo#10144 + epic-review on neomjs/neo#10143)
...
**Gemini track:**
1. [#10148](https://github.com/neomjs/neo/issues/10148) Mailbox lifecycle — continues the neomjs/neo#10167 / neomjs/neo#10170 arc.
...
- [#10150](https://github.com/neomjs/neo-agent-brain/issues/157) Chroma semantic layer
...
- 2026-04-23T12:41:30Z @neo-opus-ada cross-referenced by #156
...
> 
> Priority 0 for neomjs/neo#10139 closed empirically this session. The full 5-layer A2A stack now composes end-to-end with zero manual SQLite queries and zero harness-restart workarounds:
> 
> | Layer | Ticket / PR | Authored |
> |---|---|---|
> | Auth + stdio identity | neomjs/neo#10145 / neomjs/neo#10166 | Claude (prior session) |
> | Identity binding (await unwrap) | neomjs/neo#10249 / neomjs/neo#10250 | Claude |
> | Write-path edges (normalization) | neomjs/neo#10259 / neomjs/neo#10262 | Claude |
> | Cross-process read coherence | neomjs/neo#10257 / neomjs/neo#10258 + neomjs/neo#10260 / neomjs/neo#10261 | Claude + Gemini |
> | Config-gated reply policy | neomjs/neo#10252 / neomjs/neo#10253 | Claude |
> | Per-agent blocking (BLOCKED_BY) | neomjs/neo#10255 / neomjs/neo#10266 | Gemini |
> 
> **Empirical verification**: post-merge of neomjs/neo#10258 + neomjs/neo#10261 + harness restart, direct-DM between `@neo-opus-4-7` ↔ `@neo-gemini-3-1-pro` works bidirectionally. `list_messages` surfaces peer writes on first call. `from` attribution populated correctly. `mailbox.defaultReplyPolicy: 'open'` enables first-contact DM without bootstrap workarounds.
> 
> ## Docs + Pattern Codification (shipped)
> 
> - **PR neomjs/neo#10254** — `.github/AI_QUICK_START.md` three-harness parity (Claude Desktop config added) + `.neo-ai-data` symlink convention + `NEO_AGENT_IDENTITY` placement discipline + `⌘Q` restart gotcha
> - **PR neomjs/neo#10263** — Empirical Isolation-Test-After-Review pattern codified symmetrically in `pr-review §5.1` + `pull-request §7.10`; Fresh MCP Client Isolation Strategy in `debugging-antigravity` + `self-repair`; WAL-lock troubleshooting with two-layer fix (busy_timeout + await unwrap) in `MemoryCoreMcpAuth.md` **[approved, awaiting merge]**
> - **PR neomjs/neo#10265** — `MultiTenantMigrationGuide.md` as design source of truth for neomjs/neo#10017; `healthcheck.migration.untaggedCount` observability surface; `WriteSideInvariant.spec.mjs` regression guard **[open for review]**
> 
> ## neomjs/neo#9999 Remaining Critical Path
> 
> ### Sub-epic neomjs/neo#10016 — Multi-Tenant Identity & Data Privacy (3/7 → 4/7 done post-merge)
> 
> | Ticket | Status | Track | Depends On |
> |---|---|---|---|
> | neomjs/neo#10000 Hardened identity ingestion | ✅ done | — | — |
> | neomjs/neo#10144 AgentIdentity node type | ✅ done | — | — |
> | neomjs/neo#10145 OAuth2 MCP auth | ✅ done | — | — |
> | neomjs/neo#10146 Cross-tenant permission edges | ✅ done | — | — |
> | **#10017 Migration strategy** | 🟡 PR neomjs/neo#10265 open | Claude | awaiting merge |
> | **#10010 Team vs Private flag** | ⏸ queued | Gemini | neomjs/neo#10017 merge (design anchor) |
> | **#10011 SQLite RLS + tenant isolation** | ⏸ queued | Gemini | neomjs/neo#10017 merge (design anchor) |
> 
> ### Sub-epic neomjs/neo#10015 — Dynamic Topology (2/4 done)
> 
> | Ticket | Status | Track | Notes |
> |---|---|---|---|
> | neomjs/neo#10001 Unified vs federated routing | ✅ done | — | — |
> | neomjs/neo#10007 Bypass db init in unified mode | ✅ done | — | — |
...
> 
> ### Standalone neomjs/neo#9999 subs
> 
> | Ticket | Status | Track | Notes |
> |---|---|---|---|
> | **#10127 Healthcheck topology surface** | ⏸ open | Either | Quick Win shape; pure aggregation; already-spec'd |
> | neomjs/neo#10136 CodebaseOverview Two-Hemispheres rewrite | ⏸ open | Either | Orthogonal to deployment-critical path |
> 
> ### Out-of-band tickets filed this session
> 
> | Ticket | Status | Purpose |
> |---|---|---|
> | neomjs/neo#10267 | 🟠 unassigned | Investigate gh-workflow MCP sync-metadata.json no-op-diff pollution (Option C recommended — split timestamps into gitignored sidecar) |
> 
> ## Pending Operational Step (tobi-side)
> 
> The `normalizeGraphIdentities.mjs` migration script (#10259 / PR neomjs/neo#10262, **already merged**) has NOT yet been run with `--apply` on the primary SQLite. The graph still contains the pre-#10144 aliases `@opus` + `@gemini` plus test-fixture leakage `AGENT:alice` + `AGENT:bob`. Next session's operator step:
> 
> ```bash
> # 1. Review the plan
...
> 
> **Claude track** (once neomjs/neo#10265 merges):
> - Support Gemini's neomjs/neo#10010 / neomjs/neo#10011 via `MultiTenantMigrationGuide.md` as design reference
...
> 
> **Gemini track** (blocked on neomjs/neo#10017 merge to pin `memorySharing` enum semantics):
> - **#10011** (SQLite RLS + tenant isolation) — deepest substrate, her lane
> - **#10010** (Team vs Private retrieval flag) — pairs with neomjs/neo#10011 (tag-on-write + filter-on-read feature pair)
> - **#10008 / neomjs/neo#10009** (Playwright topology coverage) — after tobi coordination on `config.template.mjs` scope boundaries
> 
...
> 
> 1. **Cross-family review catches what solo review misses** — every substrate fix in the neomjs/neo#10241 → neomjs/neo#10266 chain had a catch landed by the *other* family. Empirical substrate of the `#10208` / `#10211` mandate. Pattern now codified symmetrically via neomjs/neo#10263 (`pr-review §5.1` + `pull-request §7.10`).
> 
> 2. **Empirical Isolation-Test-After-Review** as architectural-dispute resolution primitive — converts theoretical debate into binary empirical results in ~5 minutes of harness time. Self-demonstrated by Gemini's revision cycle on neomjs/neo#10263 where she adopted the proposed revision verbatim rather than re-debating.
> 
> 3. **Lazy-tag-on-read is the neomjs/neo#10017 prescription**, not back-fill. Full rationale in `MultiTenantMigrationGuide.md §Core Decision` + the neomjs/neo#10017 reshape note. Four concrete arguments against back-fill are the load-bearing precedent for any future "should we back-fill legacy data?" question.
> 
> 4. **"Scope-name intuition was correct, prescription layer needed reshape"** as a recurring pattern — `#10251 → neomjs/neo#10252 → neomjs/neo#10255 → neomjs/neo#10266` (BLOCKED_BY substrate-flip → config-gated + additive primitive) is the canonical example. When a reviewer flags a prescription but the author's core concept is sound, reshape at the right substrate instead of rejecting wholesale.
> 
> 5. **Diagnostic primitives should be shell-invokable CLIs**, not additional MCP tool surface, when the diagnostic's whole purpose is to bypass the MCP tool cache (Fresh MCP Client Primitive per neomjs/neo#10263). Self-referential recursion-avoidance insight.
> 
> 6. **"Ensure isolation test reproduces the full failure condition"** — my neomjs/neo#10253 Phase-2 isolation test ran in single-harness mode and incorrectly confirmed `busy_timeout` alone would close the race; under real 6-process swarm load, neomjs/neo#10258 + neomjs/neo#10261 had to close additional gaps. Future isolation tests: reproduce the full operational load, not simplified variants.
> 
> ## Session IDs for Memory Mining
> 
> **Claude Opus 4.7 / Claude Code** (chronological, most recent last):
> - `d3cf73b4-21b0-419c-9921-feb472e5a693` — neomjs/neo#10241 plan-mode exploration
> - `5f746864-85c4-4b2c-bd79-64c76a8a2be3` — neomjs/neo#10249 await discovery
> - `57b8fba5-2bd4-4446-ab46-6d1c9086a19d` — neomjs/neo#10252 / neomjs/neo#10253 config-gated reply policy
> - `7ad90eb3-2218-4200-977b-fbfe546d64a8` — neomjs/neo#10258 cross-process coherence + neomjs/neo#10259 identity normalization
> - `29f490c0-41ed-4846-a767-287552d5c3c4` — Post-#10258 merge, first direct-DM empirical success
> - `cff20948-2dbb-4ac4-99e2-df2ebe967a4b` — neomjs/neo#10017 intake reshape + implementation + this handover
> 
> **Gemini 3.1 Pro / Antigravity**:
> - `1ca26c19-dd3c-4f74-ae42-51409514d40c` — neomjs/neo#10242 original, busy_timeout work
> - `1c5933cc-a2d0-4296-ae3f-f4e815d385a2` — neomjs/neo#10260 / neomjs/neo#10261 syncCache + neomjs/neo#10263 docs consolidation + neomjs/neo#10264 / neomjs/neo#10266 opportunistic pickups
> 
...
> 
> 3. **Check PR state**: `list_pull_requests` for open PRs. At handover time: PR neomjs/neo#10263 (approved, awaiting merge) + PR neomjs/neo#10265 (open, awaiting review).
> 
> 4. **Cross-family coordination first**: before picking up any neomjs/neo#9999 sub, A2A mailbox handshake (`list_messages`) to see if the other track has posted a coordination message. Direct-DM to the other identity works without workarounds post-#10258.
> 
> 5. **Priority order for next pickup**:
>    - **If you're Gemini**: wait for neomjs/neo#10265 merge, then neomjs/neo#10011 first (substrate-heavy, unblocks neomjs/neo#10010)
>    - **If you're Claude**: neomjs/neo#10127 (Quick Win) OR stand-by for cross-family review coverage on Gemini's neomjs/neo#10011 PR
> 
> 6. **Before first code change**: `ticket-intake` skill. If the ticket's prescription predates a substrate change this session codified (especially neomjs/neo#10144 graph-first identity), reshape rather than accept blindly. The neomjs/neo#10017 reshape note is the canonical example of the pattern.
> 
> ---
> 
> **Session end state**: A2A operational, design anchor for neomjs/neo#10017 in PR review, cross-family review pattern well-exercised, `#9999` remaining critical path scoped and tracked. Fresh session picks up with the operational step + neomjs/neo#10265 merge + Gemini's neomjs/neo#10011 start; I pick up neomjs/neo#10127 on the next Claude-track session.
> 
...
❌ Tickets cannot be reopened. Created new ticket: neomjs/neo#10323
...
> ## Stage 1 — Roadmap Fit: ✅
> 
> neomjs/neo#9999 still fits the current strategic direction. The live sub-issue graph shows it remains the parent for Multi-Tenant Memory Core, Dynamic Topology, A2A graph primitives, Concept Ontology adjacency, and the new neomjs/neo#10587 Sandman observability slice. `get_context_frontier()` is currently empty, but the issue history, #10137 discussion references, and reopened-state cleanup confirm neomjs/neo#9999 is still the active umbrella rather than a stale closed epic.
> 
> Duplicate sweep note: neomjs/neo#10323 is a reopen artifact from the temporary prevent-reopen workflow, not a competing epic. neomjs/neo#10030 is an intentional child / parallel concept layer after the abandoned Macro DB approach, not a replacement for the whole neomjs/neo#9999 arc.
> 
> ## Stage 2 — Approach Elegance: ✅
> 
> The evolved approach reuses existing substrate rather than creating parallel machinery: Dynamic Topology owns Chroma placement, Multi-Tenant Identity owns user/privacy boundaries, Graph-first Memory and A2A own structured agent coordination, and neomjs/neo#10030 replaces semantic-diluting Macro DB vectors with a deterministic Concept Ontology. That pivot is load-bearing and documented in the epic thread.
> 
> For neomjs/neo#10587 specifically, the elegant layer is the explicit `runSandman.mjs` operator path plus existing Memory Core logging / durable substrate. It should not add a new MCP tool surface and must not re-enable boot-time auto-runs.
> 
> ## Stage 3 — Sub-Structure Coherence: ✅ with hygiene note
> 
> Live sub-issues under neomjs/neo#9999 now include closed foundation work (#10013, neomjs/neo#10057, neomjs/neo#10127, neomjs/neo#10129), open substrate epics (#10015, neomjs/neo#10016, neomjs/neo#10030, neomjs/neo#10143), open coordination/documentation work (#10136, neomjs/neo#10139), and neomjs/neo#10587 as a narrow Sandman failure-observability child. The structure is coherent: neomjs/neo#10587 closes a real operational gap between explicit Sandman execution and future-agent diagnosis without overlapping Gemini's active neomjs/neo#10037 Concept Ontology lane.
> 
> Non-blocking hygiene: the top-level epic body still references the superseded Universal Macro DB framing. The thread and child graph correct it, so this does not block neomjs/neo#10587, but a later body-refresh would reduce cold-reader ambiguity.
> 
> ## Stage 4 — Prescription Layer: ✅
> 
> The child graph maps to the right owner layers:
> 
> - neomjs/neo#10015: topology/config routing.
> - neomjs/neo#10016: identity and privacy boundaries.
> - neomjs/neo#10030: concept graph and semantic gap inference.
> - neomjs/neo#10139/#10143: graph-primary memory and A2A primitives.
> - neomjs/neo#10587: explicit Sandman operator observability, ideally through existing Memory Core logger / durable breadcrumb mechanisms.
> 
> neomjs/neo#10587's prescription is narrow and well-placed: preserve nonzero provider-failure exit, add a durable breadcrumb, test without a live provider, and avoid boot-time automation changes.
> 
> ## Stage 5 — Avoided Traps Completeness: ✅ with additions
> 
> The epic thread already names the major historical trap: the Macro KB vector approach was abandoned because full-file embeddings diluted semantic signal, and neomjs/neo#10030 took over with deterministic Concept Ontology.
> 
> Additional traps to keep explicit for neomjs/neo#10587 and nearby children:
> 
...
> - Do not close neomjs/neo#9999 while native sub-issues remain open; prior reopen workflow churn proves that parent-state semantics are load-bearing.
> 
> ## Verdict
> 
> Greenlight for neomjs/neo#10587 pickup under neomjs/neo#9999. Proceeding with the child ticket intake and implementation path.
> 
...
> 
> Refreshed the neomjs/neo#9999 epic body as a cold-reader entry point for v13.
> 
> What changed:
> - Retired the top-body stale framing around Federated Cloud as the active product path.
> - Retired the Universal Macro DB framing in favor of neomjs/neo#10030 Concept Ontology after the documented semantic-dilution pivot.
> - Promoted current Memory Core contract language: unified shared deployment topology, identity/provenance, neomjs/neo#10010 `memorySharing` enum (`legacy`, `private`, `team`), neomjs/neo#10011 graph RLS, A2A/Graph-first memory, and operator observability.
> - Preserved the comment thread as historical handover context.
> 
> Collision check before edit:
> - neomjs/neo#9999 is OPEN, assigned to @tobiu, and still the active v13 umbrella.
> - `gh pr list --state open --search "9999"` returned no open PR collisions.
> - Recent A2A scan showed Gemini owns neomjs/neo#10010 implementation; this body refresh does not touch that write surface.
> 
...
> | Dimension | State |
> |---|---|
> | Sub-issues | **25/25 effectively closed** — live sub-graph: 24/25 completed + the 25th (`#10030`) closed `CLOSE_COMPLETED` today (its own 14/14 done — verdict at neomjs/neo#10030 issuecomment-4639588315) |
> | Open subs | **none** (after neomjs/neo#10030 closes) — the originally-open `#10034` (custom app) was detached as non-v13 |
...
> ### Rationale
> 
> neomjs/neo#9999's live sub-graph shows 24/25 completed; the one open sub was neomjs/neo#10030 (Concept Ontology), now closed `CLOSE_COMPLETED` in its own resolution pass today (14/14, the neomjs/neo#10034 custom-app detached as non-v13). With neomjs/neo#10030 closed, neomjs/neo#9999 has zero open subs.
> 
> This is the v13 umbrella for Cloud-Native Knowledge + Multi-Tenant Memory Core — the multi-month arc (A2A substrate, identity/auth/provenance, team-vs-private retrieval, RLS, Concept Ontology, unified deployment + observability) is delivered. The epic body's explicit "Do not close neomjs/neo#9999 while native sub-issues remain open" caveat is now satisfied.
> 
> §3.5 Source-Discussion gate: **N/A** — neomjs/neo#9999 is the foundational v13 umbrella epic (it references Discussions #10137 / #10119 as positioning context, not as a graduation source).
> 
