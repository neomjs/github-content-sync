...
PR neomjs/neo#12638 delivered neomjs/neo#9906 by adding durable `TEST FILE -> VALIDATES -> CLASS` edges in the Native Edge Graph. The cross-family review approved the PR and treated the precision concern as non-blocking because neomjs/neo#9906 needed the relation contract first, but the concern becomes load-bearing before any reward / gap-downgrade consumer treats `VALIDATES` as stronger than token-heuristic evidence.

Live latest-open sweep: checked the latest 20 open issues on 2026-06-06 before filing; no equivalent found. Exact live duplicate search for `VALIDATES edge token match precision` returned no issues. Local issue/discussion sweep found related historical anchors (#9906, neomjs/neo#9889, neomjs/neo#10035, neomjs/neo-agent-brain#164) but no ticket for this post-#12638 precision bound. Knowledge Base semantic ticket sweep was attempted and unavailable (`collection not found`), so the filing relies on live GitHub + local synced substrate evidence.
...
Current `origin/dev` after neomjs/neo#12638:
...
|---|---|---|---|---|---|
| `GapInferenceEngine` test-file match predicate | neomjs/neo#9906 delivered relation contract + PR neomjs/neo#12638 review follow-up | `VALIDATES` creation requires stronger evidence than one shared token for multi-token structural names | If only broad token evidence exists, keep or emit `[TEST_GAP]` rather than creating overconfident `VALIDATES` | JSDoc on helper | Unit fixture with sibling-token false-positive candidate |
...
- [ ] The positive exact-match `VALIDATES` fixture from neomjs/neo#12638 still passes.
...
- Reopening neomjs/neo#9906; neomjs/neo#9906 delivered the first relation contract and remains complete.
...
- neomjs/neo#12638 — delivered neomjs/neo#9906 and surfaced this follow-up in review.
- neomjs/neo#9906 — original `TEST -> VALIDATES -> CLASS` relation contract.
- neomjs/neo-agent-brain#164 — parent RLAIF / Dream topology context.
- neomjs/neo#10035 — prior GapInferenceEngine move away from fragile regex concept matching.
