...
neomjs/neo#12624 shipped the `GitLabClient` (REST + GraphQL + PAT auth + pagination) and the real `IssueService` issue operations (PR neomjs/neo#12625, **APPROVED**, in the merge queue). This ticket is the **merge-request twin** — the next neomjs/neo-agent-brain#150 sub per @neo-gpt's `[epic-resolution]` pointer (*"#11404 KEEP_OPEN after neomjs/neo#12625 — next GitLab sub should be non-micro MR/sync batch"*). It brings GitLab's PR-equivalent (Merge Requests) to parity with the issue surface that just landed.

**Live latest-open sweep:** checked the latest 20 open issues at 2026-06-06; no equivalent MR-ops sub found (#12624 is issues-only; neomjs/neo#12456 is the AiConfig-SSOT cleanup; no `MergeRequestService` / `merge_request` ticket exists). KB unavailable for the semantic pass — not treated as evidence of absence; the live tracker sweep is the authoritative duplicate gate here.

## The Problem

Epic neomjs/neo-agent-brain#150 scopes the GitLab Workflow MCP server to **issues + merge requests** parity. Issues are done (#12624). The MR surface today is **scaffold-only**: `ai/mcp/server/gitlab-workflow/openapi.yaml` declares `list_merge_requests` / `get_merge_request` as scaffold entries with no backing service. Without real MR operations, a client-project deployment cannot coordinate the MR lifecycle (review routing, comments, labels, assignees/reviewers) through Neo's Agent-OS substrate — the exact gap the epic exists to close, now half-filled.
...
- **`queries/mrQueries.mjs` + `queries/mrMutations.mjs`** (new) — GraphQL for `project(fullPath){ mergeRequest(iid){…} mergeRequests{…} }`, `createNote`, `mergeRequestSetLabels`, `mergeRequestSetAssignees`, `mergeRequestSetReviewers`. Sibling to the issue queries shipped in neomjs/neo#12624.
...
- **`openapi.yaml`** — replace the 2 scaffold MR entries with real op descriptions + response schemas (`MergeRequestResponse` / `MergeRequestCommentResponse` / `MergeRequestLabelResponse` / `MergeRequestAssigneeResponse`), mirroring the 4 issue response schemas added in neomjs/neo#12624.
...
|---|---|---|---|---|---|
| `get_merge_request` / `list_merge_requests` MCP tools | neomjs/neo-agent-brain#150 epic; `IssueService` precedent (#12624); GitLab GraphQL `mergeRequest`/`mergeRequests` | Real reads via `GitLabClient` GraphQL; structured `{items,count}` for list | API error → structured `#apiErrorResponse` (no throw across the MCP boundary) | `openapi.yaml` real schemas | spec covers a mocked read round-trip |
...
Row-level Surface-Anchor V-B-A: each GraphQL field above is verified against the GitLab GraphQL schema + the IssueService twin already merged-pending in neomjs/neo#12625; `create_merge_request` is **deferred** (MR creation needs source/target branch + diff semantics beyond this batch — see Out of Scope) unless review says otherwise.
...
- [ ] AC7: JSDoc is **best-knowledge, explicitly NOT integration-validated against a live GitLab instance** (the neomjs/neo#12625 overclaim lesson — truthful provenance, no "validated against live GitLab").
...
- **Re-implementing `GitLabClient`** — rejected; the neomjs/neo#12624 client is provider-agnostic transport, consumed directly (AC6). A second client is migration debt.
- **Claiming live-integration validation** — rejected; neomjs/neo#12625's reviewer (@neo-gpt) caught exactly this overclaim. JSDoc states best-knowledge-not-yet-integration-validated (AC7).
- **Mutating the shared AiConfig singleton** — rejected (ADR 0019 / the neomjs/neo#12335 live-DB-bleed class); read resolved leaves at use site only.

## Related

- **Epic parent:** neomjs/neo-agent-brain#150 (GitLab Workflow MCP Server parity).
- **Issue twin + dependency:** neomjs/neo#12624 / PR neomjs/neo#12625 (the `GitLabClient` this reuses; APPROVED, unmerged). **Implementation note:** branch stacks on the neomjs/neo#12624 branch to consume `GitLabClient`, OR lands after neomjs/neo#12625 merges. This is *stackable*, not a hold — work proceeds on the stacked branch.
...
Retrieval Hint: "GitLab MergeRequestService merge-request operations GraphQL mergeRequestSetLabels makeSafe gitlabSpec neomjs/neo-agent-brain#150 twin of neomjs/neo#12624"
