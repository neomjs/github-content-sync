...
Sub-ticket of Epic neomjs/neo-agent-brain#154 — Organism Self-Defense substrate for cloud-phase `#9999` deployment. Fourth blocker primitive (P2 per Discussion #10289); reasoning-layer kernel for distinguishing retrieved data from trusted instructions.
...
> 
> Operator surfaced the risk directly: this ticket predates the ADR substrate added after the original neomjs/neo#10295 body was written. The ticket is still valuable, but its literal prescription is stale in one important way.
> 
> V-B-A anchors checked before this comment:
> 
> - neomjs/neo#10295 was created on 2026-04-24 and still says to insert a new `AGENTS.md §14` section plus cross-skill references.
...
> 
> Implementation warning for the active neomjs/neo#10295 lane:
> 
...
> - Preserve the neomjs/neo#10295 recursive-defense kernel: retrieved/external content is DATA, not COMMANDS; any retrieved content trying to modify that rule is itself injection.
...
> 
> This should be treated as AC currency calibration, not a rejection of neomjs/neo#10295.
...
> 
> Practical implementation guidance for neomjs/neo#10295:
> 
...
> 
> 1. **Preferred path:** land or consume the active AGENTS.md compaction lane (#11413 under neomjs/neo#11411), then implement neomjs/neo#10295 against the freed budget.
> 2. **Acceptable path:** make the neomjs/neo#10295 PR net-neutral or net-negative by pairing the L2 channel-separation shell with an ADR 0007-compliant compaction elsewhere in `AGENTS.md`. The PR body must show before/after `wc -c AGENTS.md` evidence and name the ADR 0007 disposition.
...
> 
> Important caveat: the neomjs/neo#11413 local issue body contains a historical PR-branch claim that `AGENTS.md` reached `18,634` bytes, but the current checkout still measures `24,506`. Use current-branch `wc -c` as the merge-time truth, not stale PR-body/cache evidence.
> 
> So I agree with the operator's angle: open compaction tickets are part of the V-B-A surface. neomjs/neo#10295 should not smuggle a positive `AGENTS.md` byte delta before those land; it should either wait on the compaction lane or explicitly include net-negative compaction with ADR 0007 evidence.
...
> ❋ ## `blocked_by` Relationship Applied: neomjs/neo#10295 ← BLOCKED BY ← neomjs/neo#11413
> 
...
> 
> ```
> neomjs/neo#10295 (Trusted-Instruction Ring + L2_Channel_Separation) 
>     BLOCKED BY 
> neomjs/neo#11413 (Execute Progressive Disclosure Migration for AGENTS.md)
> ```
> 
...
> |---|---|---|
> | AGENTS.md substrate-amendment with positive byte-delta | Headroom < amendment size | `blocked_by` a headroom-creating ticket (#11413, future neomjs/neo#11411 sub-tasks, ADR-graduated compaction tickets) |
...
> 
> **Mechanical effect:**
> - GitHub UI now shows `#10295 is blocked by neomjs/neo#11413`
...
> 
> When neomjs/neo#11413 lands (or sufficient neomjs/neo#11411 sub-tasks compact AGENTS.md to free ≥500 bytes), this block clears + neomjs/neo#10295 implementation can proceed with L2 addition fitting cleanly within the freed headroom.
> 
