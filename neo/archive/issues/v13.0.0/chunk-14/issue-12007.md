...
Two specific tests in `test/playwright/unit/ai/daemons/orchestrator/services/DreamService.spec.mjs` are persistently failing on `origin/dev`. The failures predate Epic neomjs/neo#11993 and PR neomjs/neo#12005 (the substrate-cleanup PR that surfaced this consolidation triage) and have been flagged as "unrelated pre-existing failures" in PR evidence sections multiple times now (#11999, neomjs/neo#12005, possibly earlier). Without a tracking ticket, the failures act as anchored noise in the test-suite floor, degrading future-regression signal quality.
...
2. **`should retry extraction on malformed JSON payload up to 3 times to fix neomjs/neo#9913`** ([DreamService.spec.mjs:967](test/playwright/unit/ai/daemons/orchestrator/services/DreamService.spec.mjs#L967)):
   ```
   Expected: 3
   Received: 0
   ```
   Expected 3 retries (per the original neomjs/neo#9913 fix), got 0. The retry logic isn't firing in this test path. Could be (a) the retry threshold semantic changed in DreamService, (b) the mock setup that triggers the retry condition broke, (c) the assertion target moved (e.g., the variable being checked no longer reflects retry count).
...
- **Discovered during:** PR neomjs/neo#11999 (Epic neomjs/neo#11993 Sub-iii) + PR neomjs/neo#12005 (daemon.mjs router cleanup) test-evidence sections that both flagged "324/326 PASS — 2 unrelated pre-existing DreamService failures"
- **Original neomjs/neo#9913** referenced in the malformed-JSON-retry test name — that's the fix-anchor for the retry-count=3 expectation; bisect should reveal whether the contract from neomjs/neo#9913 was unintentionally walked back
- **Sibling DreamService tickets** (not direct dependencies, just nearby substrate context): neomjs/neo#9890 (DreamService 4th REM Vector executeNLActionDigest), neomjs/neo-agent-brain#165 (Strategic Constraint Nodes), neomjs/neo-agent-brain#169 (Quantitative Reward Signal for Golden Path Edge Reinforcement)
...
- 2026-05-26T11:30:13Z @neo-opus-ada cross-referenced by PR #12018
