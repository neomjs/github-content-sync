...
This is an executable v13.2 epic and a native child of neomjs/neo-agent-brain#90. It coordinates one outcome across multiple one-PR leaf tickets. Leaf acceptance criteria live in those linked children; the live parent-child relationship graph, not this body, is the sub-ticket registry.
...
neomjs/neo-agent-brain#83 is a native child of neomjs/neo-agent-brain#90 and directly serves the v13.2 One Reality release gate in `ROADMAP.md`: Fleet control joins the Docker-canonical Agent OS while irreducibly host-bound effects stay local. The live duplicate sweep found adjacent authorities (#13015 services, neomjs/neo-agent-institution#10 cockpit, closed neomjs/neo#16166 authority split), but no equivalent epic; neomjs/neo-agent-brain#83 is the container-boundary successor slice, not a second Fleet product spine.
...
The live graph currently contains one child only. neomjs/neo#16715 is the correct first prerequisite: it creates the plan/apply seam required before a container-owned envelope can be truthful, preserves the compatibility composer, and explicitly excludes RPC/container/ledger work. There is no overlap or dependency cycle.
...
|---|---|---|---|---|---|
| Pure path-free workspace/MCP intent + host-owned apply with compatibility parity | L2 | neomjs/neo#16715 | (pending) | (pending) | (pending) |
...
**Review verdict:** **Greenlight** — proceed with neomjs/neo#16715. The first leaf is executable as written; the Stage-3 pending rows constrain later decomposition and prevent premature epic closeout.
...
From tonight's one-command receipt (`#16694` comment `IC_kwDODSospM8AAAABOKWbkQ`, PR neomjs/neo#16912), the state of the browser client this epic's containerized control plane will meet:
...
3. PR neomjs/neo#16912's armed exact-Origin bearer handshake — scoped deliberately to the LOCAL relay's process-lifetime bearer (the Option-B dev widening, custody documented). It is NOT a PAT carrier and should not become one.
...
- (a) **Browser admission**: a page holding a PAT-class credential is a different custody decision than a process-lifetime bearer; the `ownerPrincipal` derivation here suggests the cockpit authenticates as the OPERATOR's provider tuple — which likely wants its own redemption/session shape rather than the PR neomjs/neo#16912 handshake reused.
