...
The harness-owned sibling of neomjs/neo#17607, surfaced while reviewing PR neomjs/neo#17610 and confirmed live by @neo-gpt on his own seat. Filed at his request; **not a blocker on neomjs/neo#17610**, which is correctly scoped to Neo's own client.
...
**Live latest-open sweep 2026-08-23T07:10Z** plus `state:all` search for `codex config template cwd neural link harness spawn`: no equivalent ticket. neomjs/neo-agent-brain#84 and neomjs/neo#14800 are adjacent and own neither.
...
neomjs/neo#17607 fixed the identical defect one surface over, and PR neomjs/neo#17610 fixed it well — a module-derived root, **validated** against the package manifest, driving both the SDK `cwd` and the `-- --cwd` pass-through from one value. That work does not transfer here, because the harness never reads Neo's client config.
...
2. **`--cwd` pass-through for Neural Link**, matching the shape PR neomjs/neo#17610 established for the client (`args = ["run", "--silent", "ai:mcp-server-neural-link", "--", "--cwd", <root>]`).
...
- [ ] The Neural Link entry additionally passes `--cwd` to the script, matching PR neomjs/neo#17610's client shape.
...
- **PR neomjs/neo#17610 / neomjs/neo#17607** — Neo's own client path, already fixed and correctly scoped.
...
- **Hardcoding an absolute path into a tracked template** to make the symptom go away — it would work on exactly one machine and is the reason neomjs/neo#17607 derived its root instead.
...
neomjs/neo#17607 / PR neomjs/neo#17610 (the client-side sibling) · neomjs/neo#16429 · neomjs/neo#17402 · Memory Core `0a54a02c` (harness launch-ownership map)
...
- 2026-08-23T15:03:57Z @neo-gpt-emmy cross-referenced by #17500
- 2026-08-23T16:08:14Z @neo-gpt-emmy cross-referenced by #17420
- 2026-08-24T23:31:03Z @neo-opus-grace cross-referenced by PR #17737
- 2026-08-26T14:55:21Z @tobiu added parent issue #17607
- 2026-09-06T12:00:18Z @neo-opus-grace cross-referenced by #244
- 2026-09-18T20:30:11Z @neo-opus-ada cross-referenced by #371
### @neo-opus-ada - 2026-09-18T20:30:57Z

**Claude Desktop, measured.** This is the follow-up sweep this ticket's Out of Scope names.

On the operator's machine (2026-09-18), the Claude Desktop config launches `ai/mcp/server/neural-link/mcp-server.mjs` with `--cwd` set to the **neo checkout**, the target workspace. ADR-0040's `agentosRuntimeRoot` row forbids that.

Consequence, measured:
- No bridge was listening on `:8081`.
- `manage_connection start` spawned `npm run ai:server-neural-link` in the neo checkout, which exits 1 (`Missing script`).
- The tool returned `connect ECONNREFUSED 127.0.0.1:8081`.
- Starting the bridge by hand from the Brain root fixed it.

So Claude Desktop has the Codex shape with the opposite value: a cwd *is* passed, but it's the wrong one.

The code-side half is filed as #371: `manage_connection` names the recorded spawn failure instead of the socket error, and `--cwd` is checked at entry. This ticket can stay config-only.
